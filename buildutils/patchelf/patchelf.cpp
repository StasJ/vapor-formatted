...
template <ElfFileParams> string ElfFile<ElfFileParamNames>::getSectionName(const Elf_Shdr &shdr) {
...
template <ElfFileParams>
void ElfFile<ElfFileParamNames>::writeReplacedSections(Elf_Off &curOff, Elf_Addr startAddr,
                                                       Elf_Off startOffset) {
    /* Overwrite the old section contents with 'X's.  Do this
       *before* writing the new section contents (below) to prevent
       clobbering previously written new section contents. */
    for (ReplacedSections::iterator i = replacedSections.begin(); i != replacedSections.end();
         ++i) {
...
template <ElfFileParams> void ElfFile<ElfFileParamNames>::rewriteSectionsLibrary() {
    ...
    if (isExecutable) {
        if (startOffset >= startPage) {
            ...
            fprintf(stderr,
                    "warning: working around a Linux kernel bug by creating a hole of %zu bytes in "
                    "‘%s’\n",
                    hole, fileName.c_str());
...
template <ElfFileParams> void ElfFile<ElfFileParamNames>::rewriteHeaders(Elf_Addr phdrAddress) {
    ...
    if (shdrDynamic) {
        ...
        for (; (d_tag = rdi(dyn->d_tag)) != DT_NULL; dyn++)
            if (d_tag == DT_STRTAB)
                ...
                if (!shdr)
                    error("cannot find .rel.dyn or .rel.got");
                dyn->d_un.d_ptr = shdr->sh_addr;
    ...
    /* Rewrite the .dynsym section.  It contains the indices of the
       ...
       remapped. */
    for (unsigned int i = 1; i < rdi(hdr->e_shnum); ++i) {
        if (rdi(shdrs[i].sh_type) != SHT_SYMTAB && rdi(shdrs[i].sh_type) != SHT_DYNSYM)
            continue;
        debug("rewriting symbol table section %d\n", i);
        for (size_t entry = 0; (entry + 1) * sizeof(Elf_Sym) <= rdi(shdrs[i].sh_size); entry++) {
            Elf_Sym *sym =
                (Elf_Sym *)(contents + rdi(shdrs[i].sh_offset) + entry * sizeof(Elf_Sym));
...
template <ElfFileParams> void ElfFile<ElfFileParamNames>::modifyRPath(RPathOp op, string newRPath) {
    ...
    if (string(rpath ? rpath : "") == newRPath)
        return;
...
static void patchElf() {
    ...
    /* Check the ELF header for basic validity. */
    if (fileSize < (off_t)sizeof(Elf32_Ehdr))
        error("missing ELF header");
    ...
    if (contents[EI_CLASS] == ELFCLASS32 && contents[EI_VERSION] == EV_CURRENT) {
        ElfFile<Elf32_Ehdr, Elf32_Phdr, Elf32_Shdr, Elf32_Addr, Elf32_Off, Elf32_Dyn, Elf32_Sym>
            elfFile;
        patchElf2(elfFile, fileMode);
    } else if (contents[EI_CLASS] == ELFCLASS64 && contents[EI_VERSION] == EV_CURRENT) {
        ElfFile<Elf64_Ehdr, Elf64_Phdr, Elf64_Shdr, Elf64_Addr, Elf64_Off, Elf64_Dyn, Elf64_Sym>
            elfFile;
        patchElf2(elfFile, fileMode);
    } else {
