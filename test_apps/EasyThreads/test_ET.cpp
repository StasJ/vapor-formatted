...
void *my_thread_func(void *arg) {
    ...
    cout << "Thread id, offset, length " << thread_arg._id << " " << thread_arg._offset << " "
         << thread_arg._length << endl;
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < et.GetNumThreads(); i++) {
        ...
        et.Decompose(opt.n, et.GetNumThreads(), i, &(thread_args_ptr[i]._offset),
                     &(thread_args_ptr[i]._length));
