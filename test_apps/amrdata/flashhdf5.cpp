...
int FlashHDFFile::GetNumberOfDimensions() {
    if (numberOfDimensions <= 0) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            numberOfDimensions =
                ((dimsizes[i] > 0) ? (numberOfDimensions + 1) : numberOfDimensions);
        } // End of FOR
#else
...
//    {
    ...
    //    cout << "cX " << cellDimensions[0] << " cY " << cellDimensions[1] << " cZ " <<
    //    cellDimensions[2] << endl; cout << "DX " << dimensions[0] << " DY " << dimensions[1] << "
    //    DZ " << dimensions[2] << endl;
...
int FlashHDFFile::GetRefineLevels(int refine_levels[]) {
    ...
    status =
        H5Dread(dataset, H5T_NATIVE_INT, memspace, dataspace, H5P_DEFAULT, (void *)refine_levels);
...
int FlashHDFFile::GetRefinementLevel(int idx) {
    ...
    dataset = H5Dopen(datasetId, "refine level");
    status =
        H5Dread(dataset, H5T_NATIVE_INT, memspace, dataspace, H5P_DEFAULT, (void *)&refinelevel);
...
int FlashHDFFile::GetScalarVariable(char variableName[5], int dataPointIndex, double *variable) {
    ...
    */
    ...
    double *unknowns =
        new double[cellDimensions[2] * cellDimensions[1] * cellDimensions[0]]; // This is bad
...
int FlashHDFFile::GetScalarVariable(char variableName[5], int dataPointIndex, double bounds[6],
                                    double *variable) {
...
int FlashHDFFile::GetScalarVariable(char variableName[5], int dataPointIndex, int sizeofrun,
                                    float *variable) {
    ...
    */
    ...
    float *unknowns =
        new float[sizeofrun * cellDimensions[2] * cellDimensions[1] * cellDimensions[0]];
...
int FlashHDFFile::GetScalarVariable(char variableName[5], int dataPointIndex, int sizeofrun,
                                    double bounds[6], double *variable) {
    //    cout << "Loading Variable " << variableName << endl;
    int rank;
    hsize_t dimens_4d[4];
    hid_t ierr;
    ...
    */
    ...
    // This is bad why hard coded ...
    double *unknowns =
        new double[sizeofrun * cellDimensions[2] * cellDimensions[1] * cellDimensions[0]];
