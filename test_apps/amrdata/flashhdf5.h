...
class FlashHDFFile {
  public:
    ...
    // Get scalar data -> variable name, index to block, pointer variable, runlength is for grabbing
    // multiple blocks, bounds is spatial
    ...
    int GetScalarVariable(char variableName[5], int index, int runlength, double bounds[6],
                          double *variable);
