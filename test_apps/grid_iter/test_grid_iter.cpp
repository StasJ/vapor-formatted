...
VAPoR::LayeredGrid *make_layered_grid() {
    ...
    RegularGrid *rg =
        new RegularGrid(opt.dims, opt.bs, zblks, vector<double>(3, 0.0), vector<double>(3, 1.0));
...
VAPoR::CurvilinearGrid *make_curvilinear_grid() {
    ...
    RegularGrid *xrg =
        new RegularGrid(dims2d, bs2d, xblks, vector<double>(2, 0.0), vector<double>(2, 1.0));

    vector<float *> yblks = alloc_blocks(bs2d, dims2d);

    RegularGrid *yrg =
        new RegularGrid(dims2d, bs2d, yblks, vector<double>(2, 0.0), vector<double>(2, 1.0));
...
VAPoR::CurvilinearGrid *make_curvilinear_terrain_grid() {
    ...
    RegularGrid *zrg =
        new RegularGrid(dims, bs, zblks, vector<double>(3, 0.0), vector<double>(3, 1.0));
