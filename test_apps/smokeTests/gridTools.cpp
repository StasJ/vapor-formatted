...
bool TestConstNodeIterator(const Grid *g, size_t &count, size_t &expectedCount,
                           size_t &disagreements, double &time) {
    bool rc = true;
    count = 0;
    expectedCount = 1;
    disagreements = 0;
...
bool TestIterator(Grid *g, size_t &count, size_t &expectedCount, size_t &disagreements,
                  double &time) {
    bool rc = true;
    count = 0;
    expectedCount = 1;
...
bool TestConstCoordItr(const Grid *g, size_t &count, size_t &expectedCount, size_t &disagreements,
                       double &time) {
    bool rc = true;
    count = 0;
    expectedCount = 1;
...
void PrintGridIteratorResults(std::string &gridType, std::string itrType, size_t count,
                              size_t expectedCount, size_t disagreements, double time) {
    std::string passFail = " --- PASS";
    if (count != expectedCount || disagreements > 0) {
        passFail = " --- FAIL";
    }
...
VAPoR::CurvilinearGrid *MakeCurvilinearTerrainGrid(const std::vector<size_t> &bs,
...
LayeredGrid *MakeLayeredGrid(const vector<size_t> &dims, const vector<size_t> &bs,
...
VAPoR::StretchedGrid *MakeStretchedGrid(const vector<size_t> &dims, const vector<size_t> &bs,
                                        const std::vector<double> &minu,
                                        const std::vector<double> &maxu) {
