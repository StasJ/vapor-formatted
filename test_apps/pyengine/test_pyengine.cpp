...
void test_calculate() {
    ...
    rc = PyEngine::Calculate(script, inputVarNames, inputVarDims, inputVarArrays, outputVarNames,
...
void test_controlexec_copy(vector<string> files) {
    ...
    rc = ce.AddFunction("Python", dataSetName, "myscript_ce", script, inputVarNames, outputVarNames,
                        outputMeshNames);
    if (rc < 0)
        exit(1);
...
void test_controlexec_coord(vector<string> files) {
    ...
    script += "import numpy as np\n"
              ...
              "		print f, eval(f).shape, 'min,max,mean: ', eval(f).min(), eval(f).max(), "
              "eval(f).mean()\n"
              "\n"
              "\n";
    ...
    rc = ce.AddFunction("Python", dataSetName, scriptName, script, inputVarNames, outputVarNames,
                        outputMeshNames, coordFlag);
    if (rc < 0)
        exit(1);
