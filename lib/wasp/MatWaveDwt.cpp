...
int wextend_1D_center(const T *sigIn, size_t sigInLen, U *sigOut, size_t addLen,
                      MatWaveBase::dwtmode_t leftExtMethod, MatWaveBase::dwtmode_t rightExtMethod) {
    int count = 0;
    ...
    case MatWaveBase::SYMH: {
        for (count = 0; count < addLen; count++) {
            sigOut[count + sigInLen + addLen] = sigIn[sigInLen - count - 1];
        }
        break;
    }
    case MatWaveBase::SYMW: {
        for (count = 0; count < addLen; count++) {
            sigOut[count + sigInLen + addLen] = sigIn[sigInLen - count - 2];
        }
        break;
    }
    case MatWaveBase::ASYMH: {
        for (count = 0; count < addLen; count++) {
            sigOut[count + sigInLen + addLen] = sigIn[sigInLen - count - 1] * (-1);
        }
        break;
    }
    case MatWaveBase::ASYMW: {
        for (count = 0; count < addLen; count++) {
            sigOut[count + sigInLen + addLen] = sigIn[sigInLen - count - 2] * (-1);
        }
        break;
    }
    case MatWaveBase::SP0: {
        for (count = 0; count < addLen; count++) {
            sigOut[count + sigInLen + addLen] = sigIn[sigInLen - 1];
        }
        break;
    }
    case MatWaveBase::SP1: {
        for (count = (addLen - 1); count >= 0; count--) {
            sigOut[sigInLen + 2 * addLen - count - 1] =
                sigIn[sigInLen - 1] -
                (sigIn[sigInLen - 2] - sigIn[sigInLen - 1]) * (addLen - count);
        }
        break;
    }
    case MatWaveBase::PPD: {
...
void inverse_xform_even(const double *cA, const double *cD, size_t sigOutLen,
                        const double *low_filter, const double *high_filter, int filterLen,
                        double *sigOut, bool matlab) {
...
//
void inverse_xform_odd(const double *cA, const double *cD, size_t sigOutLen,
                       const double *low_filter, const double *high_filter, int filterLen,
                       double *sigOut) {
    size_t xi; // input and out signal indecies
    int k;     // filter index
...
void inverse_xform(const double *cA, const double *cD, size_t sigOutLen, const double *low_filter,
                   const double *high_filter, int filterLen, double *sigOut, bool matlab) {
    if (filterLen % 2) {
...
template <class T, class U> void transpose(const T *a, U *b, size_t s1, size_t s2) {
...
int dwt_template(MatWaveDwt *dwt, const T *sigIn, size_t sigInLen, const WaveFiltBase *wf,
                 MatWaveBase::dwtmode_t mode, U *cA, U *cD, size_t L[3], SmartBuf &sbuf, V dummy) {
    if (!wf) {
    ...
    if (wf->issymmetric()) {
        if ((mode == MatWaveBase::SYMW && (filterLen % 2)) ||
    ...
    if (!std::numeric_limits<V>::is_integer) {
        ...
        forward_xform(s, L[0] + L[1], wf->GetLowDecomFilCoef(), wf->GetHighDecomFilCoef(),
...
int MatWaveDwt::dwt(const double *sigIn, size_t sigInLen, double *C, size_t L[3]) {
    ...
    return (dwt_template(this, sigIn, sigInLen, wavelet(), dwtmodeenum(), cA, cD, L, _dwt1dSmartBuf,
                         dummy));
}
...
template <class T, class U, class V>
int idwt_template(MatWaveDwt *dwt, const T *cA, const T *cD, const size_t L[3],
                  const WaveFiltBase *wf, MatWaveBase::dwtmode_t mode, U *sigOut, SmartBuf &sbuf,
                  V dummy) {
    if (!wf) {
        MatWaveDwt::SetErrMsg("Invalid state, no wavelet");
        return (-1);
    ...
    if (do_sym_conv) {
        ...
        if ((L[0] > L[1]) && (mode == MatWaveBase::SYMH))
            cDPadLen = L[0];
    ...
    if (!std::numeric_limits<V>::is_integer) {
        ...
        inverse_xform(cAdbl, cDdbl, L[2], wf->GetLowReconFilCoef(), wf->GetHighReconFilCoef(),
...
template <class T, class U, class V>
int dwt2d_template(MatWaveDwt *dwt, const T *sigIn, size_t sigInX, size_t sigInY,
                   const WaveFiltBase *wf, MatWaveBase::dwtmode_t mode, U *cA, U *cDh, U *cDv,
                   U *cDd, size_t L[10], SmartBuf &sbuf2d, SmartBuf &sbuf1d, V dummy) {
    if (!wf) {
        MatWaveDwt::SetErrMsg("Invalid state, no wavelet");
        return (-1);
    }
...
int MatWaveDwt::dwt2d(const double *sigIn, size_t sigInX, size_t sigInY, double *C, size_t L[10]) {
    ...
    return dwt2d_template(this, sigIn, sigInX, sigInY, wavelet(), dwtmodeenum(), cA, cDh, cDv, cDd,
                          L, _dwt2dSmartBuf, _dwt1dSmartBuf, dummy);
}
...
int MatWaveDwt::dwt2d(const double *sigIn, size_t sigInX, size_t sigInY, double *cA, double *cDh,
                      double *cDv, double *cDd, size_t L[10]) {
    double dummy = 0;
...
int MatWaveDwt::dwt2d(const float *sigIn, size_t sigInX, size_t sigInY, float *cA, float *cDh,
                      float *cDv, float *cDd, size_t L[10]) {
    double dummy = 0;
...
int MatWaveDwt::dwt2d(const long *sigIn, size_t sigInX, size_t sigInY, long *cA, long *cDh,
                      long *cDv, long *cDd, size_t L[10]) {
    long dummy = 0;
...
int MatWaveDwt::dwt2d(const int *sigIn, size_t sigInX, size_t sigInY, int *cA, int *cDh, int *cDv,
                      int *cDd, size_t L[10]) {
    long dummy = 0;
...
template <class T, class U, class V>
int idwt2d_template(MatWaveDwt *dwt, const T *cA, const T *cDh, const T *cDv, const T *cDd,
                    const size_t L[10], const WaveFiltBase *wf, MatWaveBase::dwtmode_t mode,
                    U *sigOut, SmartBuf &sbuf2d, SmartBuf &sbuf1d, V dummy) {
    if (!wf) {
        MatWaveDwt::SetErrMsg("Invalid state, no wavelet");
        return (-1);
    }
...
int MatWaveDwt::idwt2d(const double *C, const size_t L[10], double *sigOut) {
    ...
    return idwt2d_template(this, cA, cDh, cDv, cDd, L, wavelet(), dwtmodeenum(), sigOut,
                           _dwt2dSmartBuf, _dwt1dSmartBuf, dummy);
}
...
int MatWaveDwt::idwt2d(const double *cA, const double *cDh, const double *cDv, const double *cDd,
...
int MatWaveDwt::idwt2d(const float *cA, const float *cDh, const float *cDv, const float *cDd,
...
int MatWaveDwt::idwt2d(const long *cA, const long *cDh, const long *cDv, const long *cDd,
...
int MatWaveDwt::idwt2d(const int *cA, const int *cDh, const int *cDv, const int *cDd,
...
int _dwtz_template(MatWaveDwt *dwt, V *sigIn, size_t sigInX, size_t sigInY, size_t sigInZ,
                   const WaveFiltBase *wf, MatWaveBase::dwtmode_t mode, T *cA, T *cD,
...
template <class T, class V>
int dwt3d_template(MatWaveDwt *dwt, const T *sigIn, size_t sigInX, size_t sigInY, size_t sigInZ,
                   const WaveFiltBase *wf, MatWaveBase::dwtmode_t mode, T *C, size_t L[27],
                   SmartBuf &sbuf3d1, SmartBuf &sbuf3d2, SmartBuf &sbuf2d, SmartBuf &sbuf1d,
                   V dummy) {
    if (!wf) {
        MatWaveDwt::SetErrMsg("Invalid state, no wavelet");
        return (-1);
    ...
    for (size_t z = 0; z < sigInZ; z++) {
        ...
        rc = dwt2d_template(dwt, plane, sigInX, sigInY, wf, mode, cAptr, cDhptr, cDvptr, cDdptr,
                            xyL, sbuf2d, sbuf1d, dummy);
        if (rc < 0)
            return (rc);
    }
    ...
    rc = _dwtz_template(dwt, cAXYbuf, L[0], L[1], sigInZ, wf, mode, cLLL, cLLH, sbuf3d2, sbuf1d,
                        dummy);
    if (rc < 0)
        return (rc);
    printmatrix3d("cLLL", cLLL, L[0], L[1], L[2]);
    printmatrix3d("cLLH", cLLH, L[3], L[4], L[5]);
    rc = _dwtz_template(dwt, cDhXYbuf, L[6], L[7], sigInZ, wf, mode, cLHL, cLHH, sbuf3d2, sbuf1d,
                        dummy);
    if (rc < 0)
        return (rc);
    printmatrix3d("cLHL", cLHL, L[6], L[7], L[8]);
    printmatrix3d("cLHH", cLHH, L[9], L[10], L[11]);
    rc = _dwtz_template(dwt, cDvXYbuf, L[12], L[13], sigInZ, wf, mode, cHLL, cHLH, sbuf3d2, sbuf1d,
                        dummy);
    if (rc < 0)
        return (rc);
    printmatrix3d("cHLL", cHLL, L[12], L[13], L[14]);
    printmatrix3d("cHLH", cHLH, L[15], L[16], L[17]);
    rc = _dwtz_template(dwt, cDdXYbuf, L[18], L[19], sigInZ, wf, mode, cHHL, cHHH, sbuf3d2, sbuf1d,
                        dummy);
    if (rc < 0)
        return (rc);
...
int MatWaveDwt::dwt3d(const double *sigIn, size_t sigInX, size_t sigInY, size_t sigInZ, double *C,
                      size_t L[27]) {
    double dummy = 0.0;

    return dwt3d_template(this, sigIn, sigInX, sigInY, sigInZ, wavelet(), dwtmodeenum(), C, L,
                          _dwt3dSmartBuf1, _dwt3dSmartBuf2, _dwt2dSmartBuf, _dwt1dSmartBuf, dummy);
}

int MatWaveDwt::dwt3d(const float *sigIn, size_t sigInX, size_t sigInY, size_t sigInZ, float *C,
                      size_t L[27]) {
    double dummy = 0.0;
...
int MatWaveDwt::dwt3d(const long *sigIn, size_t sigInX, size_t sigInY, size_t sigInZ, long *C,
                      size_t L[27]) {
    long dummy = 0.0;
...
int MatWaveDwt::dwt3d(const int *sigIn, size_t sigInX, size_t sigInY, size_t sigInZ, int *C,
                      size_t L[27]) {
    long dummy = 0.0;
...
template <class T, class V>
int _idwtz_template(MatWaveDwt *dwt, T *cA, T *cD, size_t sigInX, size_t sigInY, size_t cALen,
                    size_t cDLen, const WaveFiltBase *wf, MatWaveBase::dwtmode_t mode, V *sigOut,
                    size_t sigOutZ, SmartBuf &sbuf3d, SmartBuf &sbuf1d, V dummy) {
    if (!wf) {
        MatWaveDwt::SetErrMsg("Invalid state, no wavelet");
        return (-1);
    }
...
template <class T, class U, class V>
int idwt3d_template(MatWaveDwt *dwt, const T *cLLL, const T *cLLH, const T *cLHL, const T *cLHH,
                    const T *cHLL, const T *cHLH, const T *cHHL, const T *cHHH, const size_t L[27],
                    const WaveFiltBase *wf, MatWaveBase::dwtmode_t mode, U *sigOut,
                    SmartBuf &sbuf3d1, SmartBuf &sbuf3d2, SmartBuf &sbuf2d, SmartBuf &sbuf1d,
                    V dummy) {
    if (!wf) {
        MatWaveDwt::SetErrMsg("Invalid state, no wavelet");
    ...
    //
    int rc;
    rc = _idwtz_template(dwt, cLLL, cLLH, L[0], L[1], L[2], L[5], wf, mode, cAXYbuf, L[26], sbuf3d2,
                         sbuf1d, dummy);
    if (rc < 0)
        return (-1);
    rc = _idwtz_template(dwt, cLHL, cLHH, L[6], L[7], L[8], L[11], wf, mode, cDhXYbuf, L[26],
                         sbuf3d2, sbuf1d, dummy);
    if (rc < 0)
        return (-1);
    rc = _idwtz_template(dwt, cHLL, cHLH, L[12], L[13], L[14], L[17], wf, mode, cDvXYbuf, L[26],
                         sbuf3d2, sbuf1d, dummy);
    if (rc < 0)
        return (-1);
    rc = _idwtz_template(dwt, cHHL, cHHH, L[18], L[19], L[20], L[23], wf, mode, cDdXYbuf, L[26],
                         sbuf3d2, sbuf1d, dummy);
    if (rc < 0)
        return (-1);
    ...
    for (size_t z = 0; z < L[26]; z++) {
        ...
        rc = idwt2d_template(dwt, cAptr, cDhptr, cDvptr, cDdptr, xyL, wf, mode, plane, sbuf2d,
                             sbuf1d, dummy);
        if (rc < 0)
            return (-1);
    }
...
int MatWaveDwt::idwt3d(const double *C, const size_t L[27], double *sigOut) {
    ...
    return idwt3d_template(this, cLLL, cLLH, cLHL, cLHH, cHLL, cHLH, cHHL, cHHH, L, wavelet(),
                           dwtmodeenum(), sigOut, _dwt3dSmartBuf1, _dwt3dSmartBuf2, _dwt2dSmartBuf,
                           _dwt1dSmartBuf, dummy);
}
...
int MatWaveDwt::idwt3d(const double *cLLL, const double *cLLH, const double *cLHL,
                       const double *cLHH, const double *cHLL, const double *cHLH,
                       const double *cHHL, const double *cHHH, const size_t L[27], double *sigOut) {
    double dummy = 0.0;
...
int MatWaveDwt::idwt3d(const float *cLLL, const float *cLLH, const float *cLHL, const float *cLHH,
...
int MatWaveDwt::idwt3d(const long *cLLL, const long *cLLH, const long *cLHL, const long *cLHH,
                       const long *cHLL, const long *cHLH, const long *cHHL, const long *cHHH,
...
int MatWaveDwt::idwt3d(const int *cLLL, const int *cLLH, const int *cLHL, const int *cLHH,
