...
//
void block_align(const vector<size_t> &start, const vector<size_t> &count, const vector<size_t> &bs,
                 vector<size_t> &astart, vector<size_t> &acount) {
    astart = start;
    acount = count;
...
class thread_state {
  public:
    ...
    thread_state(int id, EasyThreads *et, int nthreads, string &varname,
                 const vector<NetCDFCpp *> &ncdfcptrs, const vector<size_t> &start,
                 const vector<size_t> &count, const vector<size_t> &bs, const vector<size_t> &udims,
                 const vector<size_t> &ncoeffs, const vector<size_t> &encoded_dims,
                 const vector<Compressor *> &compressors, void *data, int data_type,
                 unsigned char *mask, void *block, void *coeffs, int block_type, int xtype,
                 unsigned char *maps, int level, bool unblock_flag)
        : _id(id), _et(et), _nthreads(nthreads), _varname(varname), _ncdfcptrs(ncdfcptrs),
          _start(start), _count(count), _bs(bs), _udims(udims), _ncoeffs(ncoeffs),
          _encoded_dims(encoded_dims), _compressors(compressors), _data(data),
          _data_type(data_type), _mask(mask), _block(block), _coeffs(coeffs),
          _block_type(block_type), _xtype(xtype), _maps(maps), _level(level),
...
// Determine POD type
//
template <class T> int NetCDFType(T dummy) {
...
void Block(const T *data, const unsigned char *mask, vector<size_t> dims, vector<size_t> start,
           U *block, vector<size_t> bs, string mode, U &min, U &max) {
    min = 0;
    max = 0;
...
void UnBlock(U *block, vector<size_t> bs, T *data, vector<size_t> dims, vector<size_t> origin,
    ...
    //
    size_t x0 =
        (rank >= 1 && start[rank - 1] > origin[rank - 1]) ? start[rank - 1] - origin[rank - 1] : 0;
    size_t y0 =
        (rank >= 2 && start[rank - 2] > origin[rank - 2]) ? start[rank - 2] - origin[rank - 2] : 0;
    size_t z0 =
        (rank >= 3 && start[rank - 3] > origin[rank - 3]) ? start[rank - 3] - origin[rank - 3] : 0;
...
//
template <class T>
int DecomposeBlock(Compressor *cmp, const T *block, size_t n, T *coeffs, unsigned char *maps,
...
//
template <class T>
int ReconstructBlock(Compressor *cmp, const T *coeffs, const T *datarange,
                     const unsigned char *maps, int xtype, vector<size_t> ncoeffs,
                     vector<size_t> encoded_dims, T *block, size_t n, int level) {
...
//
template <class T>
int StoreBlock(string varname, NetCDFCpp *ncdfcptr, vector<size_t> bcoords, size_t block_size,
               const T *block
...
//
template <class T>
int StoreBlockCompressed(string varname, vector<NetCDFCpp *> ncdfcptrs, vector<size_t> bcoords,
                         vector<size_t> ncoeffs, vector<size_t> encoded_dims, const T *coeffs,
                         const T *datarange, unsigned char *maps, int xtype
...
//
template <class T>
int FetchBlock(string varname, NetCDFCpp *ncdfcptr, vector<size_t> bcoords, size_t block_size,
               T *block) {
...
//
template <class T>
int FetchBlockCompressed(string varname, vector<NetCDFCpp *> ncdfcptrs, vector<size_t> bcoords,
                         vector<size_t> ncoeffs, vector<size_t> encoded_dims, T *coeffs,
                         T *datarange, unsigned char *maps, int xtype
...
template <class T> void *RunWriteThreadTemplate(thread_state &s, T dummy) {
    ...
    for (int i = s._id; i < n; i += s._nthreads) {
        ...
        s._et->MutexLock();
        int rc =
            StoreBlock(s._varname, s._ncdfcptrs[0], bcoords, s._encoded_dims[0], (T *)s._block);
        if (rc < 0) {
            s._status = -1;
        }
        s._et->MutexUnlock();
...
void *RunWriteThreadCompressedTemplate(thread_state &s, T dummy1, U dummy2) {
    ...
    for (int i = s._id; i < n; i += s._nthreads) {
        ...
        //
        U datarange[2];
        Block((T *)s._data, s._mask, s._count, roi_start, (U *)s._block, s._bs,
              s._compressors[s._id]->dwtmode(), datarange[0], datarange[1]);

        //
        // Wavelet transform the current block
        //
        int rc = DecomposeBlock(s._compressors[s._id], (const U *)s._block, vproduct(s._bs),
        ...
        //
        s._et->MutexLock();
        rc = StoreBlockCompressed(s._varname, s._ncdfcptrs, bcoords, s._ncoeffs, s._encoded_dims,
...
void *RunReadThreadCompressedTemplate(thread_state &s, T dummy1, U dummy2) {
    ...
    for (int i = s._id; i < n; i += s._nthreads) {
        ...
        s._et->MutexLock();
        int rc =
            FetchBlockCompressed(s._varname, s._ncdfcptrs, bcoords, s._ncoeffs, s._encoded_dims,
                                 (U *)s._coeffs, datarange, s._maps, s._xtype);
        if (rc < 0)
            s._status = -1;
        s._et->MutexUnlock();
        ...
        //
        rc = ReconstructBlock(s._compressors[s._id], (U *)s._coeffs, datarange, s._maps, s._xtype,
                              s._ncoeffs, s._encoded_dims, blockptr, vproduct(s._bs), s._level);
        if (rc < 0) {
            s._status = -1;
            break;
        }
...
int WASP::DefVar(string name, int xtype, vector<string> dimnames, string wname, vector<size_t> bs,
    ...
    int rc;
    rc = _GetCompressedDims(dimnames, wname, bs, cratios, xtype, cdimnames, cdims,
...
int WASP::DefVar(string name, int xtype, vector<string> dimnames, string wname, vector<size_t> bs,
                 vector<size_t> cratios, double missing_value) {
    if (!_waspFile) {
        SetErrMsg("Not a WASP file");
        return (-1);
    }
...
int WASP::InqVarCompressionParams(string name, string &wname, vector<size_t> &bs,
                                  vector<size_t> &cratios) const {
    wname.clear();
    bs.clear();
    cratios.clear();
...
int WASP::InqVarDimlens(string name, int level, vector<size_t> &dims_at_level,
...
int WASP::InqDimsAtLevel(string wname, int level, vector<size_t> dims, vector<size_t> bs,
...
bool WASP::InqCompressionInfo(vector<size_t> bs, string wname, size_t &nlevels, size_t &maxcratio) {
    return (Compressor::CompressionInfo(compressor_bs(bs), wname, true, nlevels, maxcratio));
}
...
//
void WASP::_dims_at_level(vector<size_t> dims, vector<size_t> bs, int level, string wname,
                          vector<size_t> &dims_at_level, vector<size_t> &bs_at_level) {
    dims_at_level.clear();
    bs_at_level.clear();
...
//
bool WASP::_validate_put_vara_compressed(vector<size_t> start, vector<size_t> count,
...
//
bool WASP::_validate_get_vara_compressed(vector<size_t> start, vector<size_t> count,
                                         vector<size_t> bs, vector<size_t> udims,
...
template <class T, class U>
int WASP::_PutVara(vector<size_t> start, vector<size_t> count, const T *data,
    ...
    vector<size_t> encoded_dims;
    _get_encoding_vectors(_open_wname, _open_bs, _open_cratios, _open_varxtype, ncoeffs,
                          encoded_dims);
    ...
    if (!_open_wname.empty()) {
        ...
        maps = (unsigned char *)_sigbuf.Alloc(maps_size * _nthreads *
                                              NetCDFCpp::SizeOf(_open_varxtype));
    }
    ...
    for (int i = 0; i < _nthreads; i++) {
        ...
        argvec.push_back((void *)new thread_state(
            i, _et, _nthreads, _open_varname, _ncdfcptrs, start, count, _open_bs, _open_udims,
            ncoeffs, encoded_dims, _open_compressors, (void *)data, data_type,
            (unsigned char *)mask, block + i * block_size, coeffs + i * coeffs_size, block_type,
            _open_varxtype, maps + i * maps_size * NetCDFCpp::SizeOf(_open_varxtype), 0, true));
    }
...
int WASP::PutVara(vector<size_t> start, vector<size_t> count, const float *data,
                  const unsigned char *mask) {
...
int WASP::PutVara(vector<size_t> start, vector<size_t> count, const double *data,
                  const unsigned char *mask) {
    return (WASP::_PutVara(start, count, data, mask));
}
...
int WASP::PutVara(vector<size_t> start, vector<size_t> count, const int *data,
                  const unsigned char *mask) {
    return (WASP::_PutVara(start, count, data, mask));
}
...
int WASP::PutVara(vector<size_t> start, vector<size_t> count, const int16_t *data,
                  const unsigned char *mask) {
    return (WASP::_PutVara(start, count, data, mask));
}
...
int WASP::PutVara(vector<size_t> start, vector<size_t> count, const unsigned char *data,
                  const unsigned char *mask) {
    return (WASP::_PutVara(start, count, data, mask));
}
...
template <class T, class U>
int WASP::_GetVara(vector<size_t> start, vector<size_t> count, bool unblock_flag, T *data,
                   U dummy) {
    vector<size_t> ncoeffs;
    vector<size_t> encoded_dims;
    _get_encoding_vectors(_open_wname, _open_bs, _open_cratios, _open_varxtype, ncoeffs,
    ...
    if (!_validate_get_vara_compressed(start, count, bs_at_level, dims_at_level, _open_cratios,
                                       unblock_flag)) {
        SetErrMsg("Invalid parameter");
        return (-1);
    }
    ...
    for (int i = 0; i < _nthreads; i++) {
        ...
        argvec.push_back((void *)new thread_state(
            i, _et, _nthreads, _open_varname, _ncdfcptrs, start, count, bs_at_level, dims_at_level,
            ncoeffs, encoded_dims, _open_compressors, data, data_type, NULL, blkptr,
            coeffs + i * coeffs_size, block_type, _open_varxtype,
            maps + i * maps_size * NetCDFCpp::SizeOf(_open_varxtype), _open_level, unblock_flag));
    }
...
int WASP::GetVara(vector<size_t> start, vector<size_t> count, float *data) {
    return (WASP::_GetVara(start, count, true, data));
}

int WASP::GetVaraBlock(vector<size_t> start, vector<size_t> count, float *data) {
...
int WASP::GetVara(vector<size_t> start, vector<size_t> count, double *data) {
    return (WASP::_GetVara(start, count, true, data));
}

int WASP::GetVaraBlock(vector<size_t> start, vector<size_t> count, double *data) {
    return (WASP::_GetVara(start, count, false, data));
}
...
int WASP::GetVara(vector<size_t> start, vector<size_t> count, int *data) {
    return (WASP::_GetVara(start, count, true, data));
}

int WASP::GetVaraBlock(vector<size_t> start, vector<size_t> count, int *data) {
    return (WASP::_GetVara(start, count, false, data));
}
...
int WASP::GetVara(vector<size_t> start, vector<size_t> count, int16_t *data) {
    return (WASP::_GetVara(start, count, true, data));
}

int WASP::GetVaraBlock(vector<size_t> start, vector<size_t> count, int16_t *data) {
    return (WASP::_GetVara(start, count, false, data));
}
...
int WASP::GetVara(vector<size_t> start, vector<size_t> count, unsigned char *data) {
    return (WASP::_GetVara(start, count, true, data));
}

int WASP::GetVaraBlock(vector<size_t> start, vector<size_t> count, unsigned char *data) {
    return (WASP::_GetVara(start, count, false, data));
}
...
template <class T>
int WASP::_CopyHyperSlice(string varname, NetCDFCpp &src_ncdf, NetCDFCpp &dst_ncdf,
...
// 'dimnames'
//
int WASP::_GetCompressedDims(vector<string> dimnames, string wname, vector<size_t> bs,
                             vector<size_t> cratios, int xtype, vector<string> &cdimnames,
...
//
void WASP::_get_encoding_vectors(string wname, vector<size_t> bs, vector<size_t> cratios, int xtype,
                                 vector<size_t> &ncoeffs, vector<size_t> &encoded_dims) const {
    ncoeffs.clear();
    encoded_dims.clear();
...
//
bool WASP::_validate_compression_params(string wname, vector<size_t> dims, vector<size_t> bs,
                                        vector<size_t> cratios) const {
...
int WASP::_get_compression_params(string name, vector<size_t> &bs, vector<size_t> &cratios,
                                  vector<size_t> &udims, vector<size_t> &dims,
                                  string &wname) const {
    bs.clear();
    cratios.clear();
    udims.clear();
