...
//
SignificanceMap::SignificanceMap(const unsigned char *map, size_t nx, size_t ny, size_t nz,
                                 size_t nt) {
    vector<size_t> dims;
    ...
    _sigMapEncode = NULL;
    _sigMapEncodeSize = 0;
    if (_SignificanceMap(map, dims) < 0)
        return;
}
...
int SignificanceMap::GetCoordinatesXYZT(size_t offset, size_t *x, size_t *y, size_t *z,
                                        size_t *t) const {
    ...
    if (GetCoordinates(offset, &idx) < 0)
        return (-1);
...
void SignificanceMap::GetNextEntryRestart() {
    if (!_sorted)
        SignificanceMap::Sort();
    _idxentry = 0;
}

int SignificanceMap::GetNextEntry(size_t *idx) {

    if (_idxentry >= _sigMapVec.size())
        return (0);
...
void SignificanceMap::GetMap(unsigned char *encodedMap) {
    ...
    for (int i = 0; i < _dimsVec.size(); i++)
        header_data.push_back(_dimsVec[i]);
...
int SignificanceMap::Append(const SignificanceMap &smap) {
    if (_sigMapVec.size() == 0) {
        if (_SignificanceMap(smap._dimsVec) < 0)
            return (-1);
    }
