...
Compressor::Compressor(vector<size_t> dims, const string &wavename, const string &mode)
...
template <class T>
int compress_template(Compressor *cmp, const T *src_arr, T *dst_arr, size_t dst_arr_len, T *C,
                      size_t clen, size_t *L, SignificanceMap *sigmap, const vector<size_t> &dims,
                      size_t nlevels, vector<void *> indexvec,
...
int Compressor::Compress(const float *src_arr, float *dst_arr, size_t dst_arr_len,
                         SignificanceMap *sigmap) {

    return compress_template(this, src_arr, dst_arr, dst_arr_len, (float *)_C, _CLen, _L, sigmap,
                             _dims, _nlevels, _indexvec, my_compare_f);
}

int Compressor::Compress(const double *src_arr, double *dst_arr, size_t dst_arr_len,
                         SignificanceMap *sigmap) {

    return compress_template(this, src_arr, dst_arr, dst_arr_len, (double *)_C, _CLen, _L, sigmap,
                             _dims, _nlevels, _indexvec, my_compare_d);
}

int Compressor::Compress(const int *src_arr, int *dst_arr, size_t dst_arr_len,
                         SignificanceMap *sigmap) {

    return compress_template(this, src_arr, dst_arr, dst_arr_len, (int *)_C, _CLen, _L, sigmap,
                             _dims, _nlevels, _indexvec, my_compare_i);
}

int Compressor::Compress(const long *src_arr, long *dst_arr, size_t dst_arr_len,
                         SignificanceMap *sigmap) {

    return compress_template(this, src_arr, dst_arr, dst_arr_len, (long *)_C, _CLen, _L, sigmap,
                             _dims, _nlevels, _indexvec, my_compare_l);
}

namespace {
template <class T>
int decompress_template(Compressor *cmp, const T *src_arr, T *dst_arr, T *C, size_t clen,
                        const size_t *L, int nlevels, SignificanceMap *sigmap,
...
int Compressor::Decompress(const float *src_arr, float *dst_arr, SignificanceMap *sigmap) {

    return decompress_template(this, src_arr, dst_arr, (float *)_C, _CLen, _L, _nlevels, sigmap,
                               _dims);
}

int Compressor::Decompress(const double *src_arr, double *dst_arr, SignificanceMap *sigmap) {

    return decompress_template(this, src_arr, dst_arr, (double *)_C, _CLen, _L, _nlevels, sigmap,
                               _dims);
}

int Compressor::Decompress(const int *src_arr, int *dst_arr, SignificanceMap *sigmap) {

    return decompress_template(this, src_arr, dst_arr, (int *)_C, _CLen, _L, _nlevels, sigmap,
                               _dims);
}

int Compressor::Decompress(const long *src_arr, long *dst_arr, SignificanceMap *sigmap) {

    return decompress_template(this, src_arr, dst_arr, (long *)_C, _CLen, _L, _nlevels, sigmap,
                               _dims);
}

namespace {
template <class T>
int decompose_template(Compressor *cmp, const T *src_arr, T *dst_arr,
                       const vector<size_t> &dst_arr_lens, T *C, size_t clen, size_t *L,
                       vector<SignificanceMap> &sigmaps, const vector<size_t> &dims, size_t nlevels,
...
template <class T>
int reconstruct_template(Compressor *cmp, const T *src_arr, T *dst_arr, T *C, size_t clen,
                         const size_t *L, int nlevels, int l, vector<SignificanceMap> &sigmaps,
                         const vector<size_t> &dims) {
    if (!C) {
        Compressor::SetErrMsg("Invalid state");
        return (-1);
...
int Compressor::Decompose(const float *src_arr, float *dst_arr, const vector<size_t> &dst_arr_lens,
                          vector<SignificanceMap> &sigmaps) {
    return decompose_template(this, src_arr, dst_arr, dst_arr_lens, (float *)_C, _CLen, _L, sigmaps,
                              _dims, _nlevels, _indexvec, my_compare_f);
}

int Compressor::Decompose(const double *src_arr, double *dst_arr,
                          const vector<size_t> &dst_arr_lens, vector<SignificanceMap> &sigmaps) {
    return decompose_template(this, src_arr, dst_arr, dst_arr_lens, (double *)_C, _CLen, _L,
                              sigmaps, _dims, _nlevels, _indexvec, my_compare_d);
}

int Compressor::Decompose(const int *src_arr, int *dst_arr, const vector<size_t> &dst_arr_lens,
                          vector<SignificanceMap> &sigmaps) {
    return decompose_template(this, src_arr, dst_arr, dst_arr_lens, (int *)_C, _CLen, _L, sigmaps,
                              _dims, _nlevels, _indexvec, my_compare_i);
}

int Compressor::Decompose(const long *src_arr, long *dst_arr, const vector<size_t> &dst_arr_lens,
                          vector<SignificanceMap> &sigmaps) {
    return decompose_template(this, src_arr, dst_arr, dst_arr_lens, (long *)_C, _CLen, _L, sigmaps,
                              _dims, _nlevels, _indexvec, my_compare_l);
}

int Compressor::Reconstruct(const float *src_arr, float *dst_arr, vector<SignificanceMap> &sigmaps,
                            int l) {
    if (l == -1)
        l = GetNumLevels();
    return reconstruct_template(this, src_arr, dst_arr, (float *)_C, _CLen, _L, _nlevels, l,
                                sigmaps, _dims);
}

int Compressor::Reconstruct(const double *src_arr, double *dst_arr,
                            vector<SignificanceMap> &sigmaps, int l) {
    if (l == -1)
        l = GetNumLevels();
    return reconstruct_template(this, src_arr, dst_arr, (double *)_C, _CLen, _L, _nlevels, l,
                                sigmaps, _dims);
}

int Compressor::Reconstruct(const int *src_arr, int *dst_arr, vector<SignificanceMap> &sigmaps,
                            int l) {
    if (l == -1)
        l = GetNumLevels();
    return reconstruct_template(this, src_arr, dst_arr, (int *)_C, _CLen, _L, _nlevels, l, sigmaps,
                                _dims);
}

int Compressor::Reconstruct(const long *src_arr, long *dst_arr, vector<SignificanceMap> &sigmaps,
                            int l) {
    if (l == -1)
        l = GetNumLevels();
    return reconstruct_template(this, src_arr, dst_arr, (long *)_C, _CLen, _L, _nlevels, l, sigmaps,
                                _dims);
}
...
bool Compressor::CompressionInfo(vector<size_t> dims, const string wavename, bool keepapp,
                                 size_t &nlevels, size_t &maxcratio) {
    nlevels = 0;
    maxcratio = 0;
