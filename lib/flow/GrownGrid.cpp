...
GrownGrid::GrownGrid(const VAPoR::Grid *gp, VAPoR::DataMgr *mp, float z)
    : _grid2d(gp), _dataMgr(mp), _defaultZ(z), _minZ(z > 0.0f ? z * 0.99999f : z * 1.00001f),
      _maxZ(z > 0.0f ? z * 1.00001f : z * 0.99999f) {}
...
bool GrownGrid::InsideGrid(const std::vector<double> &coords) const {
    if (coords.size() < 3)
        return false;
    if (coords[2] < _minZ || coords[2] > _maxZ)
        return false;
    if (!_grid2d->InsideGrid(coords))
        return false;
