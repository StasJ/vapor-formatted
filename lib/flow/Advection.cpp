...
int Advection::AdvectTillTime(Field *velocity, float deltaT, float targetT,
        ...
        while (p0.time < targetT) {
            ...
            if (!velocity->InsideVolumeVelocity(p0.time, p0.location)) {
                ...
                for (int i = 0; i < 3; i++) {
                    if (_isPeriodic[i]) {
                        loc[i] =
                            _applyPeriodic(loc[i], _periodicBounds[i][0], _periodicBounds[i][1]);
                        locChanged = true;
                    }
                }
                if (!locChanged) // no dimension is periodic
