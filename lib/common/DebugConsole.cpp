...
int Redirect(MHD_Connection *connection, const char *path) {
    struct MHD_Response *response;
    response = MHD_create_response_from_buffer(strlen("REDIRECT"), (void *)"REDIRECT",
                                               MHD_RESPMEM_PERSISTENT);
...
int answer_to_connection(void *cls, struct MHD_Connection *connection, const char *url,
    ...
    response =
        MHD_create_response_from_buffer(data.length(), (void *)data.c_str(), MHD_RESPMEM_MUST_COPY);
