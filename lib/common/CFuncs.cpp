...
string clean_separators(string path) {
    ...
    string unixsep = "/";
    string winsep = "\\";
    while ((pos = path.find(unixsep)) != string::npos) {
        path.replace(pos, 1, winsep);
    }
    return (path);
#endif
}
...
void Wasp::Splitpath(string path, string &volume, string &dir, string &file, bool nofile) {
    ...
    if (nofile || (path.substr(path.size() - 1, 1) == "/") ||
        (path.substr(path.size() - 2, 2) == "/.") || (path.substr(path.size() - 3, 3) == "/..")) {
