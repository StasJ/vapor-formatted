...
#else // WIN32
    // In the windows implementation, the first thread to arrive locks all of the barrier mutices,
    // then lets the other threads in. The other threads then release their notifier mutices to
    // notify the first thread that they've arrived. Once the first thread owns all the notifier
    // mutices, it releases the blocker and notifier mutices, and the other threads now each own a
    // blocker mutex. When the other threads own a blocker mutex, they release it and relock their
    // notifier mutices.
    int status = 0;
    if (!initialized_c)
        return -1;
