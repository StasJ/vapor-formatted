...
int Wasp::CvtToDimension3D(const char *from, /* the string	*/
    ...
    } else if (!((sscanf(from, "%dx%dx%d", &(dptr->nx), &(dptr->ny), &(dptr->nz)) == 3) ||
...
void OptionParser::PrintOptionHelp(FILE *fp, int linelimit, bool docopyright) {
    ...
    for (int i = 0; i < (int)_optTbl.size(); i++) {
        ...
        if (default_value.size() > f3)
            f3 = default_value.size();
    }
    f1 += 4;
    f2 += 4;
    f3 += 4;
    ...
    for (int i = 0; i < (int)_optTbl.size(); i++) {
        ...
        fprintf(fp, format.str().c_str(), option.c_str(), arg_count_str.str().c_str(),
                defvalstr.c_str());
