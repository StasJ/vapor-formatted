...
ColorMap::Color::Color(double h, double s, double v)
    : _hue((float)h), _sat((float)s), _val((float)v) {}
...
ColorMap::Color ColorMap::getDivergingColor(float ratio, float index) const {
    ...
    float hsv2[3] = {(float)cps[4 * index + 4], (float)cps[4 * index + 5],
                     (float)cps[4 * index + 6]};
    float rgb1[3], rgb2[3];
...
ColorMap::Color ColorMap::colorNormalized(float nv) const {
    ...
    if (ratio > 0.f && ratio < 1.f) {
        ...
        if (itype == TFInterpolator::diverging) {
            ...
            float rgb[3];
            for (int i = 0; i < 3; i++)
                rgb[i] = aRGB[i] + (bRGB[i] - aRGB[i]) * ratio;
            ...
            for (int i = 0; i < 3; i++)
                lab[i] = aLAB[i] + (bLAB[i] - aLAB[i]) * ratio;
            ...
            for (int i = 0; i < 2; i++)
                lch[i] = aLCH[i] + (bLCH[i] - aLCH[i]) * ratio;
...
void ColorMap::SetInterpType(TFInterpolator::type t) {
    SetValueLong(_interpTypeTag, "Set Color Interpolation", (long)t);
}
