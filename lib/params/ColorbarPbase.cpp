...
ColorbarPbase::ColorbarPbase(ParamsBase::StateSave *ssave)
    : ParamsBase(ssave, ColorbarPbase::GetClassType()) {
...
ColorbarPbase::ColorbarPbase(ParamsBase::StateSave *ssave, XmlNode *node)
    : ParamsBase(ssave, node) {}

ColorbarPbase::~ColorbarPbase() {
    MyBase::SetDiagMsg("ColorbarPbase::~ColorbarPbase() this=%p", this);
}
...
void ColorbarPbase::SetTextColor(vector<double> color) {
    ...
    SetValueDoubleVec(_colorbarTextColorTag, "set colorbar background color", color);
}
