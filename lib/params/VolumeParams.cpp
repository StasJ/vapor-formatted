...
const std::string VolumeParams::_algorithmWasManuallySetByUserTag =
    "AlgorithmWasManuallySetByUserTag";
...
long VolumeParams::GetSamplingMultiplier() const {
    return GetValueLong(SamplingRateMultiplierTag, 1);
}

void VolumeParams::SetSamplingMultiplier(long d) {
...
void VolumeParams::SetLightingEnabled(bool v) {
    SetValueLong(LightingEnabledTag, "Lighting enabled", v);
}
bool VolumeParams::GetLightingEnabled() const {
    return GetValueLong(LightingEnabledTag, GetDefaultLightingEnabled());
}
void VolumeParams::SetPhongAmbient(float v) {
    SetValueDouble(PhongAmbientTag, "Phong ambient", v);
};
float VolumeParams::GetPhongAmbient() const {
    return GetValueDouble(PhongAmbientTag, GetDefaultPhongAmbient());
};
void VolumeParams::SetPhongDiffuse(float v) {
    SetValueDouble(PhongDiffuseTag, "Phong diffuse", v);
};
float VolumeParams::GetPhongDiffuse() const {
    return GetValueDouble(PhongDiffuseTag, GetDefaultPhongDiffuse());
};
void VolumeParams::SetPhongSpecular(float v) {
    SetValueDouble(PhongSpecularTag, "Phong specular", v);
};
float VolumeParams::GetPhongSpecular() const {
    return GetValueDouble(PhongSpecularTag, GetDefaultPhongSpecular());
};
void VolumeParams::SetPhongShininess(float v) {
    SetValueDouble(PhongShininessTag, "Phong shininess", v);
};
float VolumeParams::GetPhongShininess() const {
    return GetValueDouble(PhongShininessTag, GetDefaultPhongShininess());
};
