...
static ParamsRegistrar<DatasetParams::ScriptParams>
    registrar2(DatasetParams::ScriptParams::GetClassType());
...
DatasetsParams::DatasetsParams(ParamsBase::StateSave *ssave)
    : ParamsBase(ssave, DatasetsParams::GetClassType()) {
...
DatasetsParams::DatasetsParams(ParamsBase::StateSave *ssave, XmlNode *node)
...
DatasetsParams::DatasetsParams(const DatasetsParams &rhs) : ParamsBase(rhs) {
...
void DatasetsParams::SetScript(string datasetName, string name, string script,
...
bool DatasetsParams::GetScript(string datasetName, string name, string &script,
                               vector<string> &inputVarNames, vector<string> &outputVarNames,
                               vector<string> &outputVarMeshes, bool &coordFlag) const {
    script.clear();
    inputVarNames.clear();
...
//
DatasetParams::DatasetParams(ParamsBase::StateSave *ssave)
    : ParamsBase(ssave, DatasetParams::GetClassType()) {
...
DatasetParams::DatasetParams(ParamsBase::StateSave *ssave, XmlNode *node)
    : ParamsBase(ssave, node) {
    SetDiagMsg("DatasetParams::DatasetParams() this=%p", this);
    ...
    // from scratch;
    //
    if (node->GetTag() != DatasetParams::GetClassType()) {
        node->SetTag(DatasetParams::GetClassType());
    }
...
DatasetParams::DatasetParams(const DatasetParams &rhs) : ParamsBase(rhs) {
...
void DatasetParams::SetScript(string name, string script, const vector<string> &inputVarNames,
                              const vector<string> &outputVarNames,
                              const vector<string> &outputVarMeshes, bool coordFlag) {
...
bool DatasetParams::GetScript(string name, string &script, vector<string> &inputVarNames,
