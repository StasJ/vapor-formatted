...
OSPRayParams::OSPRayParams(ParamsBase::StateSave *ssave) : ParamsBase(ssave, _classType) {

    _init();
}

OSPRayParams::OSPRayParams(ParamsBase::StateSave *ssave, XmlNode *node) : ParamsBase(ssave, node) {
...
OSPRayParams::OSPRayParams(const OSPRayParams &rhs)
...
void OSPRayParams::SetAOSamples(int n) {
    SetValueLong(_aoSamplesTag, "Number of ambient occlusion samples", n);
}

// Reset settings settings to initial state
void OSPRayParams::_init() { SetValueLong(_samplesPerPixelTag, "Samples Per Pixel", 1); }
