...
double VAPoR::ViewpointParams::_defaultLightDirection[3][4] = {
    {0.f, 0.f, 1.f, 0.f}, {0.f, 1.f, 0.f, 0.f}, {1.f, 0.f, 0.f, 0.f}};
...
// Constructor
//----------------------------------------------------------------------------
ViewpointParams::ViewpointParams(ParamsBase::StateSave *ssave)
...
double ViewpointParams::getDiffuseCoeff(int lightNum) const {
    ...
    vector<double> defaultv;
    for (int i = 0; i < 3; i++)
        defaultv.push_back(_defaultDiffuseCoeff[i]);
...
double ViewpointParams::getSpecularCoeff(int lightNum) const {
    ...
    vector<double> defaultv;
    for (int i = 0; i < 3; i++)
        defaultv.push_back(_defaultSpecularCoeff[i]);
...
void ViewpointParams::SetNearClip(float v) {
    SetValueDouble(m_nearClip, "Near clipping distance", v);
}
