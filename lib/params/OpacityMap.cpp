...
OpacityMap::OpacityMap(ParamsBase::StateSave *ssave)
    : ParamsBase(ssave, OpacityMap::GetClassType()), _minSSq(0.0001), _maxSSq(1.0), _minFreq(1.0),
      _maxFreq(30.0), _minPhase(0.0), _maxPhase(2 * M_PI) {
...
OpacityMap::OpacityMap(ParamsBase::StateSave *ssave, XmlNode *node)
    : ParamsBase(ssave, node), _minSSq(0.0001), _maxSSq(1.0), _minFreq(1.0), _maxFreq(30.0),
      _minPhase(0.0), _maxPhase(2 * M_PI) {}
...
float OpacityMap::opacityDataAtNorm(float nv) const {
    ...
    case CONTROL_POINT: {
        ...
        if (ratio > 0. && ratio < 1.) {
            ...
            float o = TFInterpolator::interpolate(GetInterpType(), cps[2 * index],
                                                  cps[2 * index + 2], ratio);
            return o;
        }
