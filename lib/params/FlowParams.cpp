...
float FlowParams::GetRakeBiasStrength() const {
    return float(GetValueDouble(_rakeBiasStrength, 0.0f));
}

void FlowParams::SetRakeBiasStrength(float strength) {
...
void FlowParams::SetPastNumOfTimeSteps(int val) {
...
void FlowParams::SetSeedInjInterval(int val) {
    SetValueLong(_seedInjInterval,
                 "What's the interval of injecting seeds into an unsteady flow advection", val);
}
