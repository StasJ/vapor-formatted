...
void DataStatus::_getExtents(size_t ts, const map<string, vector<var_info_t>> &varMap,
    ...
    for (itr = varMap.begin(); itr != varMap.end(); ++itr) {
        ...
        for (auto it = variables.begin(); it != variables.end(); ++it) {
            ...
            vector<int> axes;
            bool status = DataMgrUtils::GetExtents(dataMgr, local_ts, var.varnames, var.refLevel,
                                                   var.compLevel, minVExts, maxVExts, axes);
            if (!status)
                continue;
            ...
            if (minVExts.size() == 2) {
                ...
                if (has3D) {
                    double z = DataMgrUtils::Get2DRendererDefaultZ(dataMgr, ts, var.refLevel,
                                                                   var.compLevel);
...
map<string, vector<DataStatus::var_info_t>> DataStatus::_getFirstVar(string dataSetName,
                                                                     size_t &ts) const {
...
void DataStatus::GetActiveExtents(const ParamsMgr *paramsMgr, string winName, string dataSetName,
                                  size_t ts, vector<double> &minExts,
