...
void DataStatus::GetExtents(size_t ts, const map<string, vector<string>> &varMap,
...
void DataStatus::GetActiveExtents(const ParamsMgr *paramsMgr, string winName, size_t ts,
                                  vector<double> &minExts, vector<double> &maxExts) const {
...
void DataStatus::GetActiveExtents(const ParamsMgr *paramsMgr, size_t ts, vector<double> &minExts,
...
void DataStatus::reset() {
    ...
    if (domainVars.size() > 0) {
        // see if a variable exists at the first time.  If so use the domainVars.  Otherwise find
        // another
        getDomainVars = false;
    ...
    // Now use the domain vars to calculate extents. They will be the union of all the extents of
    // the domain-defining variables:
...
void DataStatus::mapBoxToVox(Box *box, string varname, int refLevel, int lod, int timestep,
                             size_t voxExts[6]) {
    double userExts[6];
...
bool DataStatus::GetExtents(size_t timestep, vector<double> &minExts, vector<double> &maxExts) {
    ...
    for (int j = 0; j < _domainVars.size(); j++) {
        int rc = dataMgr->GetVariableExtents(
            timestep, _domainVars[j], maxXFormPresent(_domainVars[j], timestep), tempMin, tempMax);
        if (rc != 0)
            continue;
        if (!varfound) {
            minExts = tempMin;
...
int DataStatus::mapVoxToUser(size_t timestep, string varname, const size_t vcoords[3],
    ...
    int rc = rGrid->GetUserCoordinates(wcoords[0], wcoords[1], wcoords[2], uCoords, uCoords + 1,
                                       uCoords + 2);
    return rc;
}
void DataStatus::mapUserToVox(size_t timestep, string varname, const double uCoords[3],
...
//
int DataStatus::getGrids(size_t ts, const vector<string> &varnames,
    ...
    for (int i = 0; i < varnames.size(); i++) {
        ...
        StructuredGrid *rGrid =
            dataMgr->GetVariable(ts, varnames[i], *refLevel, *lod, tminExts, tmaxExts, true);
...
int DataStatus::getGrids(size_t ts, const vector<string> &varnames, int *refLevel, int *lod,
...
int DataStatus::getGrids(size_t ts, const vector<string> &varnames, const double extents[6],
                         int *refLevel, int *lod, StructuredGrid **grids) const {
    DataMgr *dataMgr = GetActiveDataMgr();
    if (!dataMgr) {
        return (-1);
