...
MapperFunction::MapperFunction(ParamsBase::StateSave *ssave)
    : ParamsBase(ssave, MapperFunction::GetClassType()), _numEntries(256) {
...
MapperFunction::MapperFunction(ParamsBase::StateSave *ssave, XmlNode *node)
    : ParamsBase(ssave, node), _numEntries(256) {
...
int MapperFunction::LoadFromFile(string path, vector<double> defaultDataBounds) {
    ...
    if (newTF->getMinMapValue() == 0 && newTF->getMaxMapValue() == 0) {
        newTF->setMinMaxMapValue(defaultDataBounds[0], defaultDataBounds[1]);
    }
...
void MapperFunction::makeLut(float *clut) const {
    ...
    cout << "num entries " << _numEntries << endl;
    cout << "minMapValue " << getMinMapValue() << endl;
...
int MapperFunction::mapPosition(float x, float minValue, float maxValue, int hSize) {
    double psn =
        (0.5 + ((((double)x - (double)minValue) * hSize) / ((double)maxValue - (double)minValue)));
