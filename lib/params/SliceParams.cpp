...
SliceParams::SliceParams(DataMgr *dataMgr, ParamsBase::StateSave *ssave)
...
SliceParams::SliceParams(DataMgr *dataMgr, ParamsBase::StateSave *ssave, XmlNode *node)
...
std::vector<int> SliceParams::GetSampleRates() const {
    ...
    for (int i = 0; i < tmpVec.size(); i++)
        returnVec.push_back((int)tmpVec[i]);
