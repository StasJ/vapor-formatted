...
void RenderParams::_init() {
    ...
    if (varnames.size()) {
        varname = varnames[0];
        for (int i = 0; i < varnames.size() && i < fieldVarNames.size(); i++) {
            fieldVarNames[i] = varnames[i];
        }
    }
...
void RenderParams::_initBox() {
    ...
    if (!_dataMgr->VariableExists(0, varname, 0, 0))
        return;
...
RenderParams::RenderParams(DataMgr *dataMgr, ParamsBase::StateSave *ssave, const string &classname,
                           int maxdim)
...
RenderParams::RenderParams(DataMgr *dataMgr, ParamsBase::StateSave *ssave, XmlNode *node,
                           int maxdim)
...
RenderParams *RenParamsFactory::CreateInstance(string className, DataMgr *dataMgr,
    ...
    if (it != _factoryFunctionRegistry.end())
        instance = it->second(dataMgr, ssave, node);
...
vector<string> RenParamsFactory::GetFactoryNames() const {
    vector<string> names;
    map<string,
        function<RenderParams *(DataMgr *, ParamsBase::StateSave *, XmlNode *)>>::const_iterator
        itr;
...
RenParamsContainer &RenParamsContainer::operator=(const RenParamsContainer &rhs) {
    ...
    for (int i = 0; i < names.size(); i++) {
        ...
        RenderParams *mypb =
            RenParamsFactory::Instance()->CreateInstance(classname, _dataMgr, _ssave, eleNode);
