    ...
    for (int i = 0; i < winSepNode->GetNumChildren(); i++) {
        ...
        if (winNode->HasChild(_renderersTag)) {
            ...
            for (int j = 0; j < renderersNode->GetNumChildren(); j++) {
                (void)make_ren_container(winName, renderersNode->GetChild(j)->GetTag());
            }
        }
    }
...
RenderParams *ParamsMgr::CreateRenderParamsInstance(string winName, string className,
                                                    string instName) {
    if (!_dataStatus || !_dataStatus->GetDataMgr()) {
        SetErrMsg("Invalid state : no DataStatus");
        return (NULL);
...
RenderParams *ParamsMgr::CreateRenderParamsInstance(string winName, string instName,
