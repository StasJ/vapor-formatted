        ...
        if (winNode->HasChild(_renderersTag)) {
            ...
            for (int j = 0; j < renderersNode->GetNumChildren(); j++) {
                ...
                for (int k = 0; k < dataSepNode->GetNumChildren(); k++) {
                    (void)make_ren_container(winName, dataSetName,
                                             dataSepNode->GetChild(k)->GetTag());
                }
            }
        }
    }
...
RenParamsContainer *ParamsMgr::createRenderParamsHelper(string winName, string dataSetName,
    ...
    if (find(instNames.begin(), instNames.end(), instName) != instNames.end()) {
        ...
        if (!(myWinName == winName) && (myDataSetName == dataSetName) &&
...
void ParamsMgr::RemoveRenderParamsInstance(string winName, string dataSetName, string className,
                                           string instName) {
...
RenderParams *ParamsMgr::GetRenderParams(string winName, string dataSetName, string className,
...
void ParamsMgr::GetRenderParams(string winName, string dataSetName,
                                vector<RenderParams *> &rParams) const {
    rParams.clear();
...
ParamsMgr::getWinMap3(const map<string, map<string, map<string, RenParamsContainer *>>> &m3,
                      string key) const {
    // map[a][b][c]
    //
    map<string, map<string, map<string, RenParamsContainer *>>>::const_iterator itr;
...
//
const map<string, RenParamsContainer *> *
ParamsMgr::getWinMap2(const map<string, map<string, RenParamsContainer *>> &m2, string key) const {
    // map[b][c]
    //
    map<string, map<string, RenParamsContainer *>>::const_iterator itr;
...
ParamsMgr::getWinMap3(map<string, map<string, map<string, RenParamsContainer *>>> &m3,
                      string key) const {
    // map[a][b][c]
    //
...
ParamsMgr::getWinMap3(map<string, map<string, map<string, RenParamsContainer *>>> &m3, string key1,
                      string key2) const {
...
//
map<string, RenParamsContainer *> *
ParamsMgr::getWinMap2(map<string, map<string, RenParamsContainer *>> &m2, string key) const {
    // map[b][c]
    //
...
vector<string> ParamsMgr::GetRenderParamInstances(string winName, string dataSetName,
                                                  string className) const {
...
RenParamsContainer *ParamsMgr::get_ren_container(string winName, string dataSetName,
...
RenParamsContainer *ParamsMgr::make_ren_container(string winName, string dataSetName,
                                                  string renderName) {
    ParamsSeparator *windowsSep = new ParamsSeparator(_rootSeparator, _windowsTag);
...
ViewpointParams *ParamsMgr::get_vp_params(string winName) const {
    ...
    if (itr == _viewpointParamsMap.end())
        return (NULL);
...
void ParamsMgr::GetAppRenderParams(string dataSetName,
                                   vector<RenderParams *> &appRenderParams) const {
    appRenderParams.clear();
    std::map<string, RenParamsContainer *>::const_iterator itr;
    itr = _otherRenParams.find(dataSetName);
    if (itr == _otherRenParams.cend())
        return;
...
//#define DEBUG
#ifdef DEBUG
    cout << "ParamsMgr::PMgrStateSave::Save() : saving node " << node->GetTag() << " : "
         << description << endl;
#endif
