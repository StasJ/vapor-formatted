...
#ifdef VAPOR3_0_0_ALPHA
void Box::buildLocalCoordTransform(double transformMatrix[12], double extraThickness, int timestep,
...
void Box::convertThetaPhiPsi(double *newTheta, double *newPhi, double *newPsi, int axis,
                             double rotation) const {

    // First, get original rotation matrix R0(theta, phi, psi)
    double origMatrix[9], axisRotate[9], newMatrix[9];
    vector<double> angles = GetAngles();
    getRotationMatrix(angles[0] * M_PI / 180., angles[1] * M_PI / 180., angles[2] * M_PI / 180.,
                      origMatrix);
...
// stretching.
void Box::calcLocalBoxCorners(double corners[8][3], float extraThickness, int timestep,
                              double rotation, int axis) const {
...
void Box::calcRotatedStretchedBoxExtents(vector<double> stretchFactors,
                                         double *bigBoxExtents) const {
...
bool Box::cropToBox(const double bxExts[6]) {
    ...
    // box intersection is after corner.
    ...
    getRotationMatrix((float)(angles[0] * M_PI / 180.), (float)(angles[1] * M_PI / 180.),
                      (float)(angles[2] * M_PI / 180.), rotMatrix);
    ...
    if (!pointInBox) {
        ...
        // probeCoords values are along the dir[0] and dir[1] directions, with a value of +1
        // indicating the probe x-width Thus the startPoint in world coords is
...
// Find a point that lies in the probe plane and in a box, if the probe intersects a face of the
// box. Return false if there is no such intersection
bool Box::intersectRotatedBox(double boxExts[6], double intersectPoint[3], double probeCoords[2]) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int k = 0; k < 3; k++) {
            // Classify each corner as to whether it is inside or outside the half-space defined by
            // each face cornerInFace[i][j] is true if the cor[i][j] is inside the half-space
    ...
    for (int face = 0; face < 6; face++) {
        ...
        // A side of probe is determined by line (1-t)*cor[k] + t*cor[(k+1)%4], going from cor[k] to
        // cor[k+1] for each pair of corners, equation is
        // (1-t)*cor[k][faceDim] + t*cor[k+1][faceDim] = boxExts[faceDim+faceDir*3]
        // t*(cor[(k+1)%4][faceDim] - cor[k][faceDim]) = boxExts[faceDim+faceDir*3] -
        // cor[k][faceDim] i.e.: t = (boxExts[faceDim+faceDir*3] -
        // cor[k][faceDim])/(cor[(k+1)%4][faceDim] - cor[k][faceDim]);
        int interNum = 0;
        double interPoint[2][3];
        double interT[2];
        int interSide[2];
        // a. determine either 2 or 0 intersection points between probe boundary and box face, by
        // intersecting all sides of probe with face
        for (int k = 0; k < 4; k++) {
        ...
        if (interNum == 2) {
            ...
            if (interSide[1] - interSide[0] == 2) {
                ...
                if (interSide[0] == 0) {
                    // is vertex 0 in this half-space? If so use min t-coordinate to cut the probe
                    // max x-extents
                    if (cornerInFace[interSide[0]][face]) {
                        double mint = Min(interT[0], interT[1]);
                        if (maxx > mint)
                            maxx = mint;
                    } else { // must be vertex 0 is outside half-space, so use maxt to trim probe
                             // min x-extents
                        double maxt = Max(interT[0], interT[1]);
                        if (minx < maxt)
                            minx = maxt;
                    }
                } else { // It must intersect the two vertical edges, check if vertex 0 is inside
                         // half-space
                ...
                if (interSide[0] == 0 && interSide[1] == 1) {
                    ...
                    if (cornerInFace[interSide[1]][face]) {
                        // if vertex 1 is inside then minx must be at least as large as newcorx,
                        // maxy as small as newcory
                    ...
                    double newcory = 0.5 * (interT[1] - 1.);
                    if (cornerInFace[interSide[0]][face]) {
                        // if vertex 0 is inside then maxx must be at least as small as newcorx,
                        // maxy as small as newcory
                        maxx = Min(maxx, newcorx);
                        maxy = Min(maxy, newcory);
                    ...
                    if (cornerInFace[interSide[1]][face]) {
                        // if vertex 2 is inside then minx must be at least as large as newcorx,
                        // miny as large as newcory
                        minx = Max(minx, newcorx);
                        miny = Max(miny, newcory);
                    ...
                    double newcory = 0.5 * (interT[1] + 1.);
                    if (cornerInFace[interSide[1]][face]) {
                        // if vertex 3 is inside then maxx must be at least as small as newcorx,
                        // miny as large as newcory
                        maxx = Min(maxx, newcorx);
            ...
            // If no intersections, check if the probe is completely outside slab determined by the
            // face If any corner is outside, entire probe is outside, so check first corner
    ...
    minx = 0.5 * (minx + 1.);
    miny = 0.5 * (miny + 1.);
    // Determine center of intersection (in world coords) by bilinearly interpolating from corners
    // using minx, maxx, miny,maxy, then taking average.
...
int Box::interceptBox(const double boxExts[6], double intercept[6][3]) {
    ...
    for (int edge = 0; edge < 12; edge++) {
        ...
        int coord1 = (edge + 1) % 3;
        int coord2 = (edge + 2) % 3;
        double rhs = dst - boxExts[coord1 + 3 * ((edge / 3) % 2)] * probeNormal[coord1] -
                     boxExts[coord2 + 3 * (edge / 6)] * probeNormal[coord2];
...
void Box::getRotatedVoxelExtents(string varname, float voxdims[2], int numRefinements) {
    ...
    StructuredGrid *rGrid =
        GetDataMgr()->GetVariable(_dataStatus->getMinTimestep(), varname, numRefinements, 0);
    VAssert(rGrid);
