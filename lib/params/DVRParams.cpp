...
DVRParams::DVRParams(DataMgr *dataManager, ParamsBase::StateSave *stateSave)
    : RenderParams(dataManager, stateSave, DVRParams::GetClassType(), 3 /* max dim */) {
    SetDiagMsg("DVRParams::DVRParams() this=%p", this);
}

// Constructor
DVRParams::DVRParams(DataMgr *dataManager, ParamsBase::StateSave *stateSave, XmlNode *node)
