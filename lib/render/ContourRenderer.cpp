...
ContourRenderer::~ContourRenderer() {
    ...
    for (auto it = _lineCache.begin(); it != _lineCache.end(); it++)
        if (std::find(timestepsToDelete.begin(), timestepsToDelete.end(), it->first.first) ==
            timestepsToDelete.end())
            timestepsToDelete.push_back(it->first.first);
...
int ContourRenderer::performRendering(size_t timestep, DataMgr *dataMgr) {
    ...
    for (int iso = 0; iso < cParams->GetNumContours(); iso++) {
        ...
        for (int linenum = 0; linenum < lines.size(); linenum++) {
            ...
            if (mapToTerrain) {
                if (std::isnan(pointa[0]) || std::isnan(pointa[1]) || std::isnan(pointb[0]) ||
                    std::isnan(pointb[1])) {
                    dontDraw = true;
                } else {
...
bool ContourRenderer::pointWithinBounds(double x, double y, double z, vector<double> varMin,
...
void ContourRenderer::buildEdges(int iso, float *dataVals, float mv) {
    ...
    for (int j = 0; j < _sampleSize - 1; j++) {
        for (int i = 0; i < _sampleSize - 1; i++) {
            ...
            if ((dataVals[i + j * _sampleSize] == mv) ||
