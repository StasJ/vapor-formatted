...
bool ContourRenderer::_isCacheDirty() const {
    ContourParams *p = (ContourParams *)GetActiveParams();
    if (cacheParams.varName != p->GetVariableName())
        return true;
    if (cacheParams.heightVarName != p->GetHeightVariableName())
        return true;
    if (cacheParams.ts != p->GetCurrentTimestep())
        return true;
    if (cacheParams.level != p->GetRefinementLevel())
        return true;
    if (cacheParams.lod != p->GetCompressionLevel())
        return true;
    if (cacheParams.useSingleColor != p->UseSingleColor())
        return true;
    if (cacheParams.lineThickness != p->GetLineThickness())
        return true;
    ...
    if (cacheParams.boxMin != min)
        return true;
    if (cacheParams.boxMax != max)
        return true;
    if (cacheParams.contourValues != contourValues)
        return true;

    float constantColor[3];
    p->GetConstantColor(constantColor);
    if (memcmp(cacheParams.constantColor, constantColor, sizeof(constantColor)))
        return true;
    ...
    if (cacheParams.contourColors != contourColors)
        return true;
    if (cacheParams.opacity != tf->getOpacityScale())
        return true;
...
/*
inline void contourVertex(const double contour, const vector<double> &ac, const vector<double> bc,
...
void ContourRenderer::_buildCache() {
    ...
    Grid *grid = _dataMgr->GetVariable(cacheParams.ts, cacheParams.varName, cacheParams.level,
                                       cacheParams.lod, cacheParams.boxMin, cacheParams.boxMax);
    Grid *heightGrid = NULL;
    if (!cacheParams.heightVarName.empty())
        heightGrid =
            _dataMgr->GetVariable(cacheParams.ts, cacheParams.heightVarName, cacheParams.level,
                                  cacheParams.lod, cacheParams.boxMin, cacheParams.boxMax);
