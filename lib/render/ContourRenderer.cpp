...
int ContourRenderer::performRendering(size_t timestep, DataMgr *dataMgr) {
    ...
    if (mapToTerrain) {
        heightGrid =
            (StructuredGrid *)dataMgr->GetVariable(ts, hgtVar, level, lod, boxMinExt, boxMaxExt);
    }
...
int ContourRenderer::edgeCode(int i, int j, float isoval, float *dataVals) {
    ...
    if ((dataVals[i + _sampleSize * j] < isoval && dataVals[i + 1 + _sampleSize * j] >= isoval) ||
        (dataVals[i + _sampleSize * j] >= isoval && dataVals[i + 1 + _sampleSize * j] < isoval))
        intersectionCode += 1;
    // check for crossing between (i+1,j+1) and (i+1,j)
    if ((dataVals[i + 1 + _sampleSize * j] < isoval &&
         dataVals[i + 1 + _sampleSize * (j + 1)] >= isoval) ||
        (dataVals[i + 1 + _sampleSize * j] >= isoval &&
         dataVals[i + 1 + _sampleSize * (j + 1)] < isoval))
        intersectionCode += 2;
    // check for crossing between (i,j+1) and (i+1,j+1)
    if ((dataVals[i + _sampleSize * (j + 1)] < isoval &&
         dataVals[i + 1 + _sampleSize * (j + 1)] >= isoval) ||
        (dataVals[i + _sampleSize * (j + 1)] >= isoval &&
         dataVals[i + 1 + _sampleSize * (j + 1)] < isoval))
        intersectionCode += 4;
    // check for crossing between (i,j+1) and (i,j)
    if ((dataVals[i + _sampleSize * j] < isoval && dataVals[i + _sampleSize * (j + 1)] >= isoval) ||
        (dataVals[i + _sampleSize * j] >= isoval && dataVals[i + _sampleSize * (j + 1)] < isoval))
        intersectionCode += 8;
    ...
    case (15): // disambiguate 7 and 8, based on whether or not average is on same side of isovalue
        ...
        avgvalue =
            0.25 * (dataVals[i + _sampleSize * j] + dataVals[i + _sampleSize * (j + 1)] +
                    dataVals[i + 1 + _sampleSize * (j + 1)] + dataVals[i + 1 + _sampleSize * j]);
...
bool ContourRenderer::cacheIsValid(int timestep) {
    ...
    for (int i = 0; i < minExt.size(); i++) {
        if (thisKey->extents[i] != minExt[i])
            return false;
        if (thisKey->extents[i + minExt.size()] != maxExt[i])
            return false;
