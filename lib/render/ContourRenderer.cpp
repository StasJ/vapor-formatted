...
int ContourRenderer::performRendering(size_t timestep, DataMgr *dataMgr) {
    ...
    if (mapToTerrain) {
        ...
        // Note that the height variable may be retrieved when building the cache, however it will
        // not actually be used until render time, when it will be re-retrieved.
    ...
    for (int iso = 0; iso < cParams->GetNumIsovalues(); iso++) {
        ...
        for (int linenum = 0; linenum < lines.size(); linenum++) {
            ...
            if (mapToTerrain) {
                ...
                float z1 =
                    heightGrid->GetValue(point1[0] + userExts[0], point1[1] + userExts[1], 0.);
                float z2 =
                    heightGrid->GetValue(point2[0] + userExts[0], point2[1] + userExts[1], 0.);
...
int ContourRenderer::buildLineCache(DataMgr *dataMgr) {
    ...
    cout << "box  " << boxMax[0] << " " << boxMin[0] << " " << boxMax[1] << " " << boxMin[1]
         << endl;
    ...
    for (int i = 0; i < _gridSize; i++) {
        ...
        for (int j = 0; j < _gridSize; j++) {
            ...
            // if (j/500==0)
            // cout << var << " " << dataCoords[0] << " " << dataCoords[1] << " " << dataCoords[2]
            // << " " << val << endl;
            // find the coords that the texture maps to
            //			bool dataOK = true;
            //			for (int k = 0; k< 3; k++){
            //				if (dataCoords[k] < extExtents[k] || dataCoords[k] > extExtents[k+3]) dataOK =
            //false; 				dataCoords[k] += minExts[k]; //Convert to user coordinates.
            //			}
            //			if(dataOK) { //find the coordinate in the data array
            //				dataVals[i+j*_gridSize] =
            //isolineGrid[0]->GetValue(dataCoords[0],dataCoords[1],dataCoords[2]);
            //			}
            // cout << "coords " << dataCoords[0] << " " << dataCoords[1] << " " << dataCoords[2] <<
            // endl;
        }
    }
    ...
    // Loop over each isovalue and cell, and classify the cell as to which edges are crossed by the
    // isoline. when there is an isoline crossing, a line segment is saved in the cache, defined by
    // the two endpoints.
...
int ContourRenderer::edgeCode(int i, int j, float isoval, float *dataVals) {
    ...
    if ((dataVals[i + 1 + _gridSize * j] < isoval &&
         dataVals[i + 1 + _gridSize * (j + 1)] >= isoval) ||
        (dataVals[i + 1 + _gridSize * j] >= isoval &&
         dataVals[i + 1 + _gridSize * (j + 1)] < isoval))
        intersectionCode += 2;
    // check for crossing between (i,j+1) and (i+1,j+1)
    if ((dataVals[i + _gridSize * (j + 1)] < isoval &&
         dataVals[i + 1 + _gridSize * (j + 1)] >= isoval) ||
        (dataVals[i + _gridSize * (j + 1)] >= isoval &&
         dataVals[i + 1 + _gridSize * (j + 1)] < isoval))
        intersectionCode += 4;
    ...
    case (12):
        ecode = 4;
        break;
    case (15): // disambiguate 7 and 8, based on whether or not average is on same side of isovalue
               // as (i,j) vertex. average is used as best approximation of value at center of cell.
        avgvalue = 0.25 * (dataVals[i + _gridSize * j] + dataVals[i + _gridSize * (j + 1)] +
                           dataVals[i + 1 + _gridSize * (j + 1)] + dataVals[i + 1 + _gridSize * j]);
        if (((dataVals[i + _gridSize * j] < isoval) && (avgvalue < isoval)) ||
            ((dataVals[i + _gridSize * j] > isoval) && (avgvalue > isoval))) {
            // average agrees with (i,j), so use segments that connect 0-1 and 1-2 edge [case 2] as
            // well as 2-3 and 3-0 [case 4]
            ecode = 7;
        } else // use segments that connect edges 1-2 and 2-3 as well as 3-0 and 0-1 [case 1 and
               // case 3]
            ecode = 8;
        break;
    default:
...
int ContourRenderer::addLineSegment(int timestep, int isoIndex, float x1, float y1, float x2,
                                    float y2) {
...
// std::map<pair<int,int>, bool> _markerBit;  //indicate whether or not an edge has been visited
// during traversal
...
void ContourRenderer::traverseCurves(int iso) {
    ...
    string isoText;
    doubleToString((cParams->GetIsovalues()[iso]), isoText, cParams->GetNumDigits());
    ...
    // Iterate looking for unmarked edge in first mapping.  Note that all edges must appear in both
    // mappings
        ...
        while (1) {
            ...
            if (edgeEdgeTestIter == _edgeEdge1.end()) {
                ...
                if (edgeEdgeTestIter == _edgeEdge2.end()) {
                    ...
                    // See if we can go the other direction from the start (note this code is
                    // replicated below...)
                    if (firstDirection) {
                        firstDirection = false;
            ...
            if (!_markerBit[nextEdge]) {
                ...
                if ((edgeEdgeTestIter == _edgeEdge2.end()) ||
                    (_markerBit[edgeEdgeTestIter->second])) {
                    // Both ends are marked (or there is no other end) so
                    // we are at (one) end of isoline
                    // See if we can go the other direction from the start (this is replication of
                    // above code!)
                    if (firstDirection) {
                        firstDirection = false;
                        // Try other direction at start
...
void ContourRenderer::attachAnnotation(int numComponents, int iso) {
    ...
    // there should be about A annotations in crossing the domain; i.e. annotation interval should
    // be about 1/A times the grid size when textDensity is .5, and A is a normalization constant. So
    // define
    // annotSpace = (2-g/A) + (g/A-1)/density where g is grid length or 2*A, whichever is larger
    // If the interval is shorter than the component and larger than 0.1 times the component, then
    // just one annotation is generated
    float A = 3.;
    ...
    for (int comp = 0; comp < numComponents; comp++) {
        ...
        while (1) {
            ...
            if (edgeEdgeTestIter == _edgeEdge1.end()) {
                edgeEdgeTestIter = _edgeEdge2.find(currentEdge);
                if (edgeEdgeTestIter == _edgeEdge2.end()) {
                    // this means currentEdge goes nowhere (in either mapping); must be at end
                    // already
...
void ContourRenderer::buildEdges(int iso, float *dataVals, float mv) {
    ...
    ContourParams *cParams = (ContourParams *)GetActiveParams();
    const vector<double> &isovals = cParams->GetIsovalues();
    if (cParams->GetTextDensity() > 0. &&
        cParams->GetTextEnabled()) { // create a textObject to hold annotation of this isovalue
        ...
        string isoText;
        doubleToString((cParams->GetIsovalues()[iso]), isoText, cParams->GetNumDigits());
        //		int objNum = TextObject::addTextObject(this,
        //GetAppPath("VAPOR","share",vec).c_str(),(int)cParams->GetTextSize(),lineColor,
        //bgc,(int)cParams->GetTextLabelType(), isoText); 		_objectNums[iso] = objNum;
    }
...
bool ContourRenderer::cacheIsValid(int timestep) {
    ...
    if (thisKey->textEnabled != cParams->GetTextEnabled())
        return false;
    vector<double> ivals = cParams->GetIsovalues();
...
void ContourRenderer::updateCacheKey(int timestep) {
    ...
    thisKey->isovals = cParams->GetIsovalues();
