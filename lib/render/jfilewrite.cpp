...
void resampleImage(unsigned char *image1, int xin, int yin, unsigned char *image2, int xout,
                   int yout) {
...
int VAPoR::write_JPEG_file(FILE *outfile, int image_width, int image_height,
...
int VAPoR::read_JPEG_file(const char *filename, unsigned char **imageBuffer, int *width,
                          int *height) {
    ...
    while (cinfo.output_scanline < cinfo.output_height) {
        ...
        memcpy((void *)(firstImage +
                        (cinfo.output_height - cinfo.output_scanline) * (cinfo.output_width) * 3),
               buffer[0], (size_t)cinfo.output_width * 3);
    ...
    *height = newYDim;
    if (newXDim == cinfo.output_width && newYDim == cinfo.output_height) {
        ...
        *imageBuffer = (unsigned char *)malloc((size_t)(3 * newXDim * newYDim));
        resampleImage(firstImage, cinfo.output_width, cinfo.output_height, *imageBuffer, newXDim,
                      newYDim);
        free(firstImage);
    }
