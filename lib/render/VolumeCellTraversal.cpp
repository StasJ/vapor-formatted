...
static void GetFaceCoordinateIndices(const ivec3 &cell, const ivec3 &face, ivec3 &i0, ivec3 &i1,
                                     ivec3 &i2, ivec3 &i3) {
    // CCW
    if (face == F_DOWN) {
        i0 = cell + ivec3(0, 0, 0);
...
static void GetFaceVertices(const ivec3 &cellIndex, const ivec3 &face, const float *data,
...
static bool ComputeSideBBoxes(ivec3 side, int fastDim, int slowDim, vec3 *boxMins, vec3 *boxMaxs,
...
int VolumeCellTraversal::LoadData(const Grid *grid) {
    ...
    for (int level = 1; level < levels; level++) {
        ...
        for (int z = 0; z < 6; z++) {
            ...
            // If the upper level is odd, the top and right border pixels map to 6 pixels in the
            // previous level which are accounted for here.
            if (mUpW % 2 == 1) {
                for (int y = 0; y < mH; y++) {
            ...
            // If the both upper dims are odd, the top-right pixel maps to 9 pixels in the previous
            // level. 8 of these were already accounted for. This accounts for the last pixel in the
            // corner
...
std::string VolumeCellTraversal::_addDefinitionsToShader(std::string shaderName) const {
    ...
    if (vendor == GLManager::Vendor::Nvidia || vendor == GLManager::Vendor::AMD ||
