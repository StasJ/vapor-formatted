...
void GetFaceCoordinateIndices(const ivec3 &cell, const ivec3 &face, ivec3 &i0, ivec3 &i1, ivec3 &i2,
                              ivec3 &i3) {
    // CCW
    if (face == F_DOWN) {
        i0 = cell + ivec3(0, 0, 0);
...
vec3 GetCoordAtIndex(const ivec3 &index, const float *data, const ivec3 &dims) {
    ...
    return vec3(data[3 * (z * w * h + y * w + x)], data[3 * (z * w * h + y * w + x) + 1],
                data[3 * (z * w * h + y * w + x) + 2]);
}

void GetFaceVertices(const ivec3 &cellIndex, const ivec3 &face, const float *data,
                     const ivec3 &dims, vec3 &v0, vec3 &v1, vec3 &v2, vec3 &v3) {
    ivec3 i0, i1, i2, i3;
...
bool ComputeSideBBoxes(ivec3 side, int fastDim, int slowDim, vec3 *boxMins, vec3 *boxMaxs,
                       float *coordData, const ivec3 &cellDims, const ivec3 &coordDims,
                       const int bd, const int sd) {
    ...
    for (index[slowDim] = 0; index[slowDim] < cellDims[slowDim]; index[slowDim]++) {
        for (index[fastDim] = 0; index[fastDim] < cellDims[fastDim]; index[fastDim]++) {
            ...
            boxMins[sideID * bd * sd + index[slowDim] * sd + index[fastDim]] =
                glm::min(v0, glm::min(v1, glm::min(v2, v3)));
            boxMaxs[sideID * bd * sd + index[slowDim] * sd + index[fastDim]] =
                glm::max(v0, glm::max(v1, glm::max(v2, v3)));
        }
    }
    return false;
}
...
VolumeCellTraversal::~VolumeCellTraversal() {
    ...
    if (maxTexture)
        glDeleteTextures(1, &maxTexture);
    if (BBLevelDimTexture)
        glDeleteTextures(1, &BBLevelDimTexture);
    if (VAO)
...
int VolumeCellTraversal::LoadData(const Grid *grid) {
    ...
    glBindTexture(GL_TEXTURE_3D, coordTexture);
    glTexImage3D(GL_TEXTURE_3D, 0, GL_R32F, 0, 0, 0, 0, GL_RED, GL_FLOAT,
    ...
    ComputeSideBBoxes(F_LEFT, 1, 2, boxMins, boxMaxs, data, ivec3(cw, ch, cd), ivec3(w, h, d), bd,
                      sd);
    ComputeSideBBoxes(F_RIGHT, 1, 2, boxMins, boxMaxs, data, ivec3(cw, ch, cd), ivec3(w, h, d), bd,
                      sd);
    ComputeSideBBoxes(F_UP, 0, 1, boxMins, boxMaxs, data, ivec3(cw, ch, cd), ivec3(w, h, d), bd,
                      sd);
    ComputeSideBBoxes(F_DOWN, 0, 1, boxMins, boxMaxs, data, ivec3(cw, ch, cd), ivec3(w, h, d), bd,
                      sd);
    ComputeSideBBoxes(F_FRONT, 0, 2, boxMins, boxMaxs, data, ivec3(cw, ch, cd), ivec3(w, h, d), bd,
                      sd);
    ComputeSideBBoxes(F_BACK, 0, 2, boxMins, boxMaxs, data, ivec3(cw, ch, cd), ivec3(w, h, d), bd,
                      sd);
    ...
    glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGB16F, 0, 0, 0, 0, GL_RGB, GL_FLOAT,
                 NULL); // Fix driver bug with re-uploading large textures
    ...
    for (int level = 1; level < levels; level++) {
        ...
        // minMip[level]);
        glTexImage3D(GL_TEXTURE_2D_ARRAY, level, GL_RGB16F, ms, ms, 6, 0, GL_RGB, GL_FLOAT,
                     minMip[level]);
        glBindTexture(GL_TEXTURE_2D_ARRAY, maxTexture);
        // glTexSubImage3D(GL_TEXTURE_2D_ARRAY, level, 0, 0, 0, ms, ms, 6, GL_RGB, GL_FLOAT,
        // maxMip[level]);
...
ShaderProgram *VolumeCellTraversal::GetShader() const {
    return _glManager->shaderManager->GetShader("rayCellTraversal:BB_LEVELS " +
                                                std::to_string(BBLevels));
}
