...
GeoTIFWriter::GeoTIFWriter(const std::string &path)
    : TIFWriter(path), gtif(nullptr), _hasTiePoint(false), _hasPixelScale(false),
...
int GeoTIFWriter::Write(const unsigned char *buffer, const unsigned int width,
...
int GeoTIFWriter::ConfigureFromProj4(const std::string proj4String) {
    ...
    if (proj.GetString("proj") == "lcc" || proj.GetString("proj") == "utm") {
        if (GTIFSetFromProj4(gtif, proj4String.c_str()) == 0) {
            SetErrMsg("Unable to configure GeoTIFF using GTIFSetFromProj4(%s)",
                      proj4String.c_str());
            return -1;
        }
    } else {
        ...
        int ellipse =
            Proj4StringParser::Proj4EllipseStringToGeoTIFEnum(proj.GetString("ellps", "WGS84"));
        if (ellipse < 0)
            return -1;
