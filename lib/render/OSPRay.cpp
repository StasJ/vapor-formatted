...
bool OSPRayInitialized() { return OSPInitStatus == OSP_NO_ERROR; }

// Adapted from
// https://github.com/ospray/ospray/blob/release-1.2.x/apps/volumeViewer/glUtil/util.cpp#L42-L62

OSPTexture OSPRayGetDepthTextureFromOpenGLPerspective(const double &fovy, const double &aspect,
                                                      const double &zNear, const double &zFar,
                                                      const glm::vec3 &cameraDir,
                                                      const glm::vec3 &cameraUp,
                                                      const float *glDepthBuffer, const int &width,
                                                      const int &height) {
    ...
    for (size_t j = 0; j < height; j++) {
        for (size_t i = 0; i < width; i++) {
            const glm::vec3 dir_ij = normalize(dir_00 + float(i) / float(width - 1) * dir_du +
                                               float(j) / float(height - 1) * dir_dv);
    ...
    //    OSPTexture2D depthTexture = ospNewTexture2D((osp::vec2i&)texSize, OSP_TEXTURE_R32F,
