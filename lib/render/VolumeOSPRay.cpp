...
static bool IsXcode() {
    ...
    for (; s; i++) {
        ...
        if (l.find("__XCODE_BUILT_PRODUCTS_DIR_PATHS") != string::npos) {
            result = true;
            return true;
        }
...
static void PrintPercent(const char *label, long i, long t) {
    if (t > 100 && i % (t / 100) != 0 && i != 0)
        return;
    if (!IsXcode())
        printf("\r[%li%%] %s", i * 100 / t, label);
    else {
        if (i == 0)
            printf("%s", label);
    }
...
VolumeOSPRay::VolumeOSPRay(GLManager *gl, VolumeRenderer *renderer)
...
VolumeOSPRay::~VolumeOSPRay() {
    ...
    if (_ospWorld)
        ospRelease(_ospWorld);
    if (_ospCamera)
        ospRelease(_ospCamera);
    if (_ospTF)
        ospRelease(_ospTF);
    if (_ospInstance)
...
int VolumeOSPRay::Render(bool fast) {
    ...
    OSPFrameBuffer framebuffer = ospNewFrameBuffer(fbSize.x, fbSize.y, OSP_FB_SRGBA,
    ...
    ospSetFloat(_ospVolumeModel, "densityScale",
                GetParams()->GetValueDouble(VolumeParams::OSPDensity, 1));
    ospCommit(_ospVolumeModel);

    //    static int c = 0;
    //    printf("[%i] Render OSP...", c++); flush(cout);
    ospRenderFrameBlocking(framebuffer, _ospRenderer, _ospCamera, _ospWorld);
    //    printf("Done\r"); flush(cout);

    const uint32_t *fb = (uint32_t *)ospMapFrameBuffer(framebuffer, OSP_FB_COLOR);
    //    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, windowSize.x, windowSize.y, 0, GL_RGBA,
    //    GL_UNSIGNED_BYTE, fb);
...
int VolumeOSPRay::LoadData(const Grid *grid) {
    ...
    ospRelease(data);
    ospSetVec2f(_ospTF, "valueRange", -FLT_MAX,
                0); // These initial values are necessary to work around bugs.
    ospCommit(_ospTF);
...
#if 0
    OSPGeometry clip = ospNewGeometry("sphere");
    vec3 pos(0.f);
    data = VOSP::NewCopiedData(&pos, OSP_VEC3F, 1);
    ospSetObject(clip, "sphere.position", data);
    ospRelease(data);
    ospSetFloat(clip, "radius", 2);
    ospCommit(clip);
#else
    OSPGeometry clip = ospNewGeometry("box");
    //    float box[] = {-2, -2, -2, 2, 2, 2};
    ...
    if (p->GetValueLong("osp_enable_clipping", false))
        ospSetObjectAsData(group, "clippingGeometry", OSP_GEOMETRIC_MODEL, clipModel);
...
ShaderProgram *VolumeOSPRay::GetShader() const {
...
void VolumeOSPRay::_setupRenderer(bool fast) {
    ...
    ospSetVec4f(_ospRenderer, "backgroundColor", bgColor[0], bgColor[1], bgColor[2],
                opaqueBG ? 1 : 0);
    ospSetFloat(_ospRenderer, "volumeSamplingRate",
                (fast ? 0.1 : 1) * p->GetValueDouble(VolumeParams::OSPSampleRateScalar, 1));
    ...
    if (fast) {
        ...
        vec3 dir(ddir[0], ddir[1], ddir[2]);
        //        printf("OSPDepthFromGLPerspective(%f, %f, %f, %f, (%f, %f, %f), (%f, %f, %f), %p,
        //        %i, %i)\n", glm::degrees(mm->FOV), mm->Aspect, mm->Near, mm->Far,
        //        dir.x,dir.y,dir.z, up.x,up.y,up.z, _depthData.data(), fbSize.x, fbSize.y);
        OSPTexture depthMap =
            VOSP::OSPDepthFromGLPerspective(glm::degrees(mm->FOV), mm->Aspect, mm->Near, mm->Far,
                                            dir, up, _depthData.data(), fbSize.x, fbSize.y);
...
void VolumeOSPRay::_setupCamera() {
    ...
    float intensity = GetParams()->GetValueDouble("osp_light_intensity", 4);
    ospSetFloat(_ospLightDistant, "intensity", intensity);
...
OSPVolume VolumeOSPRay::_loadVolumeRegular(const Grid *grid) {
    ...
    //    printf("Dims = %li, %li, %li\n", dims[0], dims[1], dims[2]);
    //    PrintVec3(dataMinExt);
    //    PrintVec3(gridSpacing);
...
static WindingOrder GetWindingOrderRespectToZ(const vec3 &a, const vec3 &b, const vec3 &c) {
    return glm::cross(b - a, c - b).z > 0 ? CCW : CW;
}

static WindingOrder GetWindingOrderTetra(const vec3 &a, const vec3 &b, const vec3 &c,
                                         const vec3 &d) {
...
static const char *to_string(WindingOrder o) {
    return o == CCW ? "CCW" : o == CW ? "CW" : "INVALID";
}

static bool isQuadCoPlanar(const vec3 &a, const vec3 &b, const vec3 &c, const vec3 &d) {
...
#undef I
    ...
    //        printf("cells[%i][%i] = (%f, %f, %f)\n", nCells-1, i, coords[ci].x, coords[ci].y,
    //        coords[ci].z);
    //    }
    ...
    for (int i = 0; i < nCells; i++) {
        ...
        if (decompose && (!isQuadCoPlanar(coords[indices[i].i0], coords[indices[i].i1],
                                          coords[indices[i].i2], coords[indices[i].i3]) ||
                          !isQuadCoPlanar(coords[indices[i].i4], coords[indices[i].i5],
                                          coords[indices[i].i6], coords[indices[i].i7]))) {
            Cell c = indices[i];
...
OSPVolume VolumeOSPRay::_loadVolumeUnstructured(const Grid *grid) {
    ...
    //    int maxCells = std::min((int)nCells, (int)GetActiveParams()->GetValueLong("osp_max_cells",
    //    1));
...
#undef add
    ...
    for (unsigned i = 0; i < cellStarts.size(); i++) {
        ...
        if (type == OSP_WEDGE) {
            if (CW == GetWindingOrderRespectToZ(coords[cellIndices[start + 0]],
                                                coords[cellIndices[start + 1]],
                                                coords[cellIndices[start + 2]])) {
                swap(cellIndices[start + 1], cellIndices[start + 2]);
                swap(cellIndices[start + 1 + 3], cellIndices[start + 2 + 3]);
            }
        } else if (type == OSP_TETRAHEDRON) {
            WindingOrder w = GetWindingOrderTetra(
                coords[cellIndices[start + 0]], coords[cellIndices[start + 1]],
                coords[cellIndices[start + 2]], coords[cellIndices[start + 3]]);
            if (w == CW)
                swap(cellIndices[start + 1], cellIndices[start + 2]);
            if (w == INVALID) {
                erase[i] = true;
            ...
            VAssert(CCW == GetWindingOrderTetra(
                               coords[cellIndices[start + 0]], coords[cellIndices[start + 1]],
                               coords[cellIndices[start + 2]], coords[cellIndices[start + 3]]));
            //            if (CCW != GetWindingOrderTetra(coords[cellIndices[start+0]],
            //            coords[cellIndices[start+1]], coords[cellIndices[start+2]],
            //            coords[cellIndices[start+3]])) {
            ...
            //                printf("Winding = %s\n",
            //                to_string(GetWindingOrderTetra(coords[cellIndices[start+0]],
            //                coords[cellIndices[start+1]], coords[cellIndices[start+2]],
    ...
    //    int testCell = std::min(cellStarts.size()-1,
    //    (size_t)GetActiveParams()->GetValueLong("osp_test_cells", 0));
    int testCell = 0;
    if (testCell >= 0) {
        ...
        for (int i = 0; i < testCellNodes; i++) {
            ...
            printf("\tCells[%i].vert[%i] = coords[%i] = (%f, %f, %f)\n", testCell, i, idx,
                   coords[idx].x, coords[idx].y, coords[idx].z);
        }
        printf("\tWinding bottom = %s\n",
               to_string(GetWindingOrderRespectToZ(testCellCoords[0], testCellCoords[1],
                                                   testCellCoords[2])));
        if (testCellNodes == 6)
            printf("\tWinding top = %s\n",
                   to_string(GetWindingOrderRespectToZ(testCellCoords[3], testCellCoords[4],
                                                       testCellCoords[5])));
    }
    ...
    for (auto i : cellIndices)
        VAssert(i < nVerts);
    Progress::Update(1);
    for (auto i : cellStarts)
        VAssert(i < cellIndices.size());
    Progress::Update(2);
    for (auto i : cellTypes)
        VAssert(i == OSP_WEDGE || i == OSP_TETRAHEDRON);
    Progress::Update(3);
    VAssert(cellStarts[cellStarts.size() - 1] +
                (cellTypes[cellTypes.size() - 1] == OSP_WEDGE ? 6 : 4) <=
            cellIndices.size());
    Progress::Update(4);
...
OSPVolume VolumeOSPRay::_loadVolumeTest(const Grid *grid) {
    ...
    for (int i = 0; i < 8; i++)
        coords[i] = coords[i] * (h - l) + l;

    /*
    coords[0] = vec3(1017575.312500, -1062690.250000, -3735.462891);
    coords[1] = vec3(1075161.750000, -1038896.250000, -3019.209961);
    coords[2] = vec3(1049646.250000, -988149.125000, -3870.693848);
    coords[3] = vec3(991829.750000, -1012094.562500, -3956.602295);
    coords[4] = vec3(1017575.312500, -1062690.250000, -3566.794434);
    coords[5] = vec3(1075161.750000, -1038896.250000, -2883.992676);
    coords[6] = vec3(1049646.250000, -988149.125000, -3695.706299);
    coords[7] = vec3(991829.750000, -1012094.562500, -3777.599609);
    ...
    for (int i = 0; i < 8; i++) {
        ...
        c += vec3(-1.07, 1, 0.27);
        c *= 80;
        printf("test[%i] = (%.1f, %.1f, %.1f)\n", i, coords[i].x, coords[i].y, coords[i].z);
    }

    coords[0] = vec3(-4, -5, 1);
    coords[1] = vec3(0.5, -3, 5);
    coords[2] = vec3(-1.5, 1, 0.5);
    coords[3] = vec3(-6, -1, 0);
    coords[4] = vec3(-4, -5, 2);
    coords[5] = vec3(0.5, -3, 6);
    coords[6] = vec3(-1.5, 1, 1.5);
    coords[7] = vec3(-6, -1, 1);
...
//    memcpy(testCell, coords, sizeof(vec3)*nVerts);
     */
...
#define FACE(a, b, c) { vec3 n = glm::normalize(glm::cross(testCell[b]-testCell[a],
testCell[c]-testCell[a])); lgl->Normal3fv((float*)&n); lgl->Vertex(testCell[a]);
lgl->Vertex(testCell[b]); lgl->Vertex(testCell[c]); } #define QUAD(a, b, c, d) { FACE(a, b, d);
FACE(d, b, c); } lgl->Begin(GL_TRIANGLES); QUAD(0, 1, 2, 3); QUAD(4, 5, 6, 7); QUAD(0, 1, 5, 4);
    QUAD(1, 2, 6, 5);
    QUAD(2, 3, 7, 6);
    QUAD(3, 0, 4, 7);
    lgl->End();
