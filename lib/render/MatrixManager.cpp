...
mat4 MatrixManager::GetProjectionMatrix() { return _projectionStack.top(); }

mat4 MatrixManager::GetModelViewMatrix() { return _modelviewStack.top(); }
...
void MatrixManager::PushMatrix() {
    ...
    printf("Push mode=%s depth=%li glMode=%s glDepth=%i\n", GetMatrixModeStr(),
           _currentStack->size(), GetGLMatrixModeStr(), GetGLCurrentStackDepth());
...
void MatrixManager::Ortho(float left, float right, float bottom, float top, float zNear,
                          float zFar) {
...
void MatrixManager::Test() {
    ...
    if (gl_modelview_stack_depth != _modelviewStack.size())
        printf("ModelView stack size not equal gl=%i, my=%i\n", gl_modelview_stack_depth,
               (int)_modelviewStack.size());
    if (gl_projection_stack_depth != _projectionStack.size())
        printf("Projection stack size not equal gl=%i, my=%i\n", gl_projection_stack_depth,
               (int)_projectionStack.size());

    bool equal = true;
    for (int i = 0; i < 16; i++)
        if (gl_modelview_matrix[i] != glm::value_ptr(_modelviewStack.top())[i])
            equal = false;
    if (!equal) {
        printf("ModelView matrix not equal\n");
        std::cout << "gl: " << glm::to_string(glm::make_mat4(gl_modelview_matrix)) << std::endl
                  << "my: " << glm::to_string(_modelviewStack.top()) << std::endl;
    }
    equal = true;
    for (int i = 0; i < 16; i++)
        if (gl_projection_matrix[i] != glm::value_ptr(_projectionStack.top())[i])
            equal = false;
    if (!equal) {
        printf("Projection matrix not equal\n");
        std::cout << "gl: " << glm::to_string(glm::make_mat4(gl_projection_matrix)) << std::endl
