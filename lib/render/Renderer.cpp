...
void Renderer::renderColorbarText(ColorbarPbase *cbpb, float fbWidth, float fbHeight, float llx,
    ...
    for (int tic = 0; tic < numtics; tic++) {
        ...
        _textObject->Initialize(_fontFile, textString, fontSize, dummy, TextObject::LABEL, txtColor,
                                bgColor);
    ...
    if (title != "") {
        ...
        txtColor[3] = 1.f;
        _textObject = new TextObject();
        _textObject->Initialize(_fontFile, title, 20, dummy, TextObject::LABEL, txtColor, bgColor);
