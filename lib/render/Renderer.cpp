...
Renderer::Renderer(const ParamsMgr *pm, string winName, string dataSetName, string paramsType,
...
RendererBase::RendererBase(const ParamsMgr *pm, string winName, string dataSetName,
...
void Renderer::EnableClipToBox(ShaderProgram *shader, float haloFrac) const {
    ...
    if (minExts.size() == 3 || orientation != Box::YZ) { // YZ
        x0Plane[3] = -minExts[0];
        x1Plane[3] = maxExts[0];
    ...
    if (minExts.size() == 3 || orientation != Box::XZ) { // XZ
        y0Plane[3] = -minExts[1];
        y1Plane[3] = maxExts[1];
    ...
    if (minExts.size() == 3 || orientation != Box::XY) { // XY
        z0Plane[3] = -minExts[2];
        z1Plane[3] = maxExts[2];
...
bool Renderer::VariableExists(size_t ts, std::vector<string> &varnames, int level, int lod,
                              bool zeroOK) const {
...
void Renderer::getLocalContainingRegion(float regMin[3], float regMax[3]) {
    // Determine the smallest axis-aligned cube that contains the rotated box local coordinates.
    // This is obtained by mapping all 8 corners into the space.
...
void Renderer::renderColorbar() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, _imgWid, _imgHgt, 0, GL_RGB, GL_UNSIGNED_BYTE,
                 _colorbarTexture);
...
Renderer *RendererFactory::CreateInstance(const ParamsMgr *pm, string winName, string dataSetName,
    ...
    if (it != _factoryFunctionRegistry.end()) {
        instance = it->second(pm, winName, dataSetName, classType, instName, dataMgr);
    }
...
vector<string> RendererFactory::GetFactoryNames() const {
    ...
    map<string, function<Renderer *(const ParamsMgr *, string, string, string, string,
                                    DataMgr *)>>::const_iterator itr;
