...
void Renderer::enableClippingPlanes(vector<double> minExts, vector<double> maxExts,
...
void Renderer::renderColorbar() {
    ...
    */
    ...
    for (int tic = 0; tic < numtics; tic++) {
        ...
        float dummy[] = {0., 0., 0.};
        _textObject = new TextObject();
        _textObject->Initialize("/Users/pearse/Downloads/pacifico/Pacifico.ttf", "My ugly font", 20,
                                dummy, 0, txtColor, bgColor);
        ...
        // float ticOffset = (ury-lly)/2 * (float)(tic)/(float)numtics * fbHeight;// * (Tuy -
        // Tly);//(float)_imgHgt;//(float)fbHeight;
        //			Ty += ticOffset;
