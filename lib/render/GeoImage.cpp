...
int GeoImage::TiffOpen(string path) {
    ...
    if (ok) {
        if (compr != COMPRESSION_NONE && compr != COMPRESSION_LZW && compr != COMPRESSION_JPEG &&
        ...
        if (config == PLANARCONFIG_CONTIG) {
            ...
            for (row = 0; row < h; row++) {
                ...
                for (int k = 0; k < scanlength; k++) {
                    ...
                    texuint32[revrow * w + k] =
                        alphaval << 24 | greyval << 16 | greyval << 8 | greyval;
                }
            }
                ...
                for (row = 0; row < h; row++) {
                    ...
                    if (!(s % 2)) {           // color
                        ...
                        for (int k = 0; k < h; k++) {
                            ...
                            texuint32[revrow * w + k] =
                                alphaval << 24 | (texuint32[revrow * w + k] & 0xffffff);
                        }
                    }
                }
            }
