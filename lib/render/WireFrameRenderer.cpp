...
void WireFrameRenderer::_drawCell(const vector<vector<size_t>> &nodes, const vector<size_t> &dims,
                                  bool layered) const {

    GLuint *indices1 = new GLuint[nodes.size()];
    for (int i = 0; i < nodes.size(); i++) {
        indices1[i] = Wasp::LinearizeCoords(nodes[i], dims);
    }
...
int WireFrameRenderer::_buildCache() {
    ...
    for (; itr != end_itr && coord_itr != end_coord_itr; ++itr, ++coord_itr, ++index) {
        ...
        if (dataValue == mv) {
            ...
            int lut = (dataValue - _cacheParams.tf_minmax[0]) /
