...
static RendererRegistrar<WireFrameRenderer> registrar(WireFrameRenderer::GetClassType(),
                                                      WireFrameParams::GetClassType());

WireFrameRenderer::WireFrameRenderer(const ParamsMgr *pm, string winName, string dataSetName,
                                     string instName, DataMgr *dataMgr)
    : Renderer(pm, winName, dataSetName, WireFrameParams::GetClassType(),
...
bool WireFrameRenderer::_isCacheDirty() const {
    ...
    if (_cacheParams.useSingleColor != p->UseSingleColor())
        return true;
    if (_cacheParams.constantColor != p->GetConstantColor())
        return true;
    if (_cacheParams.constantOpacity != p->GetConstantOpacity())
        return true;
...
// by multiple cells
//
void WireFrameRenderer::_drawCell(const GLuint *cellNodeIndices, int n, bool layered,
                                  const vector<GLuint> &nodeMap, GLuint invalidIndex,
                                  vector<unsigned int> &indices, DrawList &drawList) const {
...
// list of vertices.
//
void WireFrameRenderer::_buildCacheVertices(const Grid *grid, const Grid *heightGrid,
    ...
    for (; nodeItr != nodeEnd; ++nodeItr, ++coordItr) {
        ...
        if (dataValue == mv) {
            ...
            int index = (dataValue - _cacheParams.tf_minmax[0]) /
                        (_cacheParams.tf_minmax[1] - _cacheParams.tf_minmax[0]) * (n - 1);
            if (index < 0) {
                index = 0;
            }
            if (index >= n) {
        ...
        vertices.push_back({(float)coord[0], (float)coord[1], (float)coord[2], color[0], color[1],
                            color[2], color[3]});
    }
...
//
size_t WireFrameRenderer::_buildCacheConnectivity(const Grid *grid,
...

    {
        ...
        for (; cellItr != cellEnd; ++cellItr) {
            ...
            for (int i = 0; i < numNodes; i++) {
                ...
                size_t idx = Wasp::LinearizeCoords(cellNodeIndices.data() + (i * ndim),
                                                   grid->GetDimensions().data(), ndim);
            ...
            _drawCell(cellNodeIndicesLinear.data(), numNodes, layered, nodeMap, invalidIndex,
                      indices, drawList);
        }
    }
