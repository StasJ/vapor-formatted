...
void WireFrameRenderer::_buildCacheVertices(const Grid *grid, const Grid *heightGrid,
    ...
    for (; nodeItr != nodeEnd; ++nodeItr, ++coordItr, ++done) {
        ...
        vertices.push_back({(float)coord[0], (float)coord[1], (float)coord[2], dataValue,
                            mv == dataValue ? 1.f : 0.f});
    }
...
//
size_t WireFrameRenderer::_buildCacheConnectivity(const Grid *grid, const vector<GLuint> &nodeMap,
...
int WireFrameRenderer::_initializeGL() {
    ...
    glVertexAttribPointer(2, 1, GL_FLOAT, GL_FALSE, sizeof(VertexData),
                          (void *)offsetof(struct VertexData, missing));
