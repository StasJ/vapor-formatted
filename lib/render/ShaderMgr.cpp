...
void ShaderMgr::SetShaderSourceDir(string directory) { _sourceDir = directory; }
...
void ShaderMgr::SetGLSLVersion(int version) { _glsl_version = version; }
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
int ShaderMgr::DefineEffect(std::string baseName, std::string defines, std::string instanceName) {
    SetDiagMsg("ShaderMgr::defineEffect(%s,%s,%s)", baseName.c_str(), defines.c_str(),
               instanceName.c_str());
    ...
    for (int i = 0; i < _baseEffects[baseName][0].size(); i++) {
        ...
        _effects[instanceName]->LoadVertexSource(defined, _baseEffects[baseName][2][i]);
    }
    ...
    for (int i = 0; i < _baseEffects[baseName][1].size(); i++) {
        ...
        _effects[instanceName]->LoadFragmentSource(defined, _baseEffects[baseName][3][i]);
    }
...
//----------------------------------------------------------------------------
int ShaderMgr::_loadVertShader(std::string path, std::string fileName, std::string effectName) {
    SetDiagMsg("ShaderMgr::loadVertShader(%s,,%s)", path.c_str(), fileName.c_str());
...
//----------------------------------------------------------------------------
int ShaderMgr::_loadFragShader(std::string path, std::string fileName, std::string effectName) {
    SetDiagMsg("ShaderMgr::loadFragShader(%s,,%s)", path.c_str(), fileName.c_str());
...
std::vector<std::string> ShaderMgr::_preprocess(const string &source, std::string &processed) {
...
//----------------------------------------------------------------------------
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, int value) {
...
// Uploads data to the selected effect's uniform
//----------------------------------------------------------------------------
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, int value1, int value2) {
...
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, int value1, int value2,
    ...
    if (current == _effects[effect]->GetProgram()) {
        // dont enable, just upload data
        if (GLEW_VERSION_2_0) {
            glUniform4i(_effects[effect]->UniformLocation(variable), value1, value2, value3,
                        value4);
        } else {
            glUniform4iARB(_effects[effect]->UniformLocation(variable), value1, value2, value3,
                           value4);
        }
...
// Uploads data to the selected effect's uniform
//----------------------------------------------------------------------------
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, float value) {
...
// Uploads data to the selected effect's uniform
//----------------------------------------------------------------------------
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, float value1,
                                float value2) {
...
int ShaderMgr::UploadEffectData(std::string effect, std::string variable, float value1,
    ...
    if (current == _effects[effect]->GetProgram()) {
        // dont enable, just upload data
        if (GLEW_VERSION_2_0) {
            glUniform4f(_effects[effect]->UniformLocation(variable), value1, value2, value3,
                        value4);
        } else {
            glUniform4fARB(_effects[effect]->UniformLocation(variable), value1, value2, value3,
                           value4);
        }
...
int ShaderMgr::GetUniformValuei(std::string effect, std::string variable, GLint *result) {
...
int ShaderMgr::GetUniformValuef(std::string effect, std::string variable, GLfloat *result) {
    if (!EffectExists(effect)) {
        SetErrMsg("Uniform %s not found", variable.c_str());
        return (-1);
    }
...
bool ShaderMgr::CheckFramebufferStatus() {
    ...
    case GL_FRAMEBUFFER_UNSUPPORTED:
        std::cout << "[ERROR] Framebuffer incomplete: Unsupported by FBO implementation."
                  << std::endl;
        return false;
...
int ShaderMgr::_getEfcFiles(string dirname, vector<string> &efcFiles) {
    efcFiles.clear();
