...
int RayCaster::_initializeGL() {
    ...
    if (_initializeFramebufferTextures() != 0) {
        ...
        int RayCaster::_paintGL(bool fast) {
            ...
            if (upToDate < 0) {
                ...
                glTexImage3D(GL_TEXTURE_3D, 0, GL_R32F, _userCoordinates.dims[0],
                             _userCoordinates.dims[1], _userCoordinates.dims[2], 0, GL_RED,
                             GL_FLOAT, _userCoordinates.dataField);
                ...
                if (_userCoordinates.missingValueMask) // There is missing value.
                    glTexImage3D(GL_TEXTURE_3D, 0, GL_R8UI, _userCoordinates.dims[0],
                                 _userCoordinates.dims[1], _userCoordinates.dims[2], 0,
                                 GL_RED_INTEGER, GL_UNSIGNED_BYTE,
                                 _userCoordinates.missingValueMask);
                else
                    glTexImage3D(GL_TEXTURE_3D, 0, GL_R8UI, 2, 2, 2, 0, GL_RED_INTEGER,
                                 GL_UNSIGNED_BYTE, dummyMask);
        ...
        void RayCaster::_load3rdPassUniforms(long castingMode, const glm::mat4 &inversedMV,
            ...
            float dataRanges[4] = {_userCoordinates.valueRange[0], _userCoordinates.valueRange[1],
                                   _colorMapRange[0], _colorMapRange[1]};
            ...
            int flags[3] = {int(fast), int(lighting),
        ...
        void RayCaster::_renderTriangleStrips(int whichPass, long castingMode) const {
            ...
            for (unsigned int y = 0; y < by - 1; y++) // strip by strip
            {
                ...
                if (attrib1) {
                    ...
                    glBufferData(GL_ARRAY_BUFFER, bx * by * 4 * sizeof(int), attrib1Buffer,
                                 GL_STREAM_READ);
            ...
            for (unsigned int z = 0; z < bz - 1; z++) {
                ...
                if (attrib1) {
                    ...
                    glBufferData(GL_ARRAY_BUFFER, bx * bz * 4 * sizeof(int), attrib1Buffer,
                                 GL_STREAM_READ);
                    glVertexAttribIPointer(1, 4, GL_INT, 0, (void *)0);
                }
                glBufferData(GL_ELEMENT_ARRAY_BUFFER, numOfVertices * sizeof(unsigned int),
            ...
            for (unsigned int z = 0; z < bz - 1; z++) {
                ...
                if (attrib1) {
                    ...
                    glBufferData(GL_ARRAY_BUFFER, by * bz * 4 * sizeof(int), attrib1Buffer,
                                 GL_STREAM_READ);
                    glVertexAttribIPointer(1, 4, GL_INT, 0, (void *)0);
                }
                glBufferData(GL_ELEMENT_ARRAY_BUFFER, numOfVertices * sizeof(unsigned int),
        ...
        double RayCaster::_getElapsedSeconds(const struct timeval *begin, const struct timeval *end)
            const {
