...
RayCaster::RayCaster(const ParamsMgr *pm, std::string &winName, std::string &dataSetName,
    ...
    : Renderer(pm, winName, dataSetName, paramsType, classType, instName, dataMgr),
      _backFaceTexOffset(1), _frontFaceTexOffset(2), _volumeTexOffset(3), _colorMapTexOffset(4),
      _missingValueTexOffset(5), _vertCoordsTexOffset(6), _depthTexOffset(7),
      _2ndVarDataTexOffset(8), _2ndVarMaskTexOffset(9) {
...
int RayCaster::UserCoordinates::GetCurrentGrid(const RayCasterParams *params, DataMgr *dataMgr,
...
void RayCaster::UserCoordinates::CheckUpToDateStatus(const RayCasterParams *params,
                                                     const StructuredGrid *grid, DataMgr *dataMgr,
                                                     bool use2ndVar) {
...
int RayCaster::UserCoordinates::Update2ndVariable(const RayCasterParams *params, DataMgr *dataMgr) {
    ...
    params->GetBox()->GetExtents(extMin, extMax);
    StructuredGrid *grid = dynamic_cast<StructuredGrid *>(dataMgr->GetVariable(
        myCurrentTimeStep, my2ndVarName, myRefinementLevel, myCompressionLevel, extMin, extMax));
    if (grid == nullptr) {
...
void RayCaster::UserCoordinates::IterateAGrid(const StructuredGrid *grid, size_t numOfVert,
                                              float *dataBuf, unsigned char *maskBuf) {
...
void RayCaster::UserCoordinates::FillCoordsXYPlane(const StructuredGrid *grid, size_t planeIdx,
                                                   float *coords) {
    size_t idx = 0;
    double buf[3];
...
void RayCaster::UserCoordinates::FillCoordsYZPlane(const StructuredGrid *grid, size_t planeIdx,
                                                   float *coords) {
    size_t idx = 0;
    double buf[3];
...
void RayCaster::UserCoordinates::FillCoordsXZPlane(const StructuredGrid *grid, size_t planeIdx,
                                                   float *coords) {
    size_t idx = 0;
    double buf[3];
    for (size_t z = 0; z < dims[2]; z++)
...
int RayCaster::_initializeFramebufferTextures() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, _currentViewport[2], _currentViewport[3], 0, GL_RGBA,
                 GL_FLOAT, nullptr);
...
void RayCaster::_drawVolumeFaces(int whichPass, int castingMode,
                                 ...
                                 const glm::mat4 &InversedMV, bool fast) const {
...
void RayCaster::_load3rdPassUniforms(int castingMode, bool fast, bool insideVolume) const {
    ...
    if (lighting) {
        ...
        float coeffsF[4] = {float(coeffsD[0]), float(coeffsD[1]), float(coeffsD[2]),
                            float(coeffsD[3])};
...
void RayCaster::_renderTriangleStrips(int whichPass, int castingMode) const {
    ...
    for (unsigned int y = 0; y < by - 1; y++) // Looping over every TriangleStrip
    {
        ...
        if (attrib1Enabled) // Also specify attrib1 values
        {
            ...
            glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
            glBufferSubData(GL_ARRAY_BUFFER, y * bx * 4 * sizeof(int), 2 * bx * 4 * sizeof(int),
                            (attrib1Buffer + y * bx * 4));
            glVertexAttribIPointer(1, 4, GL_INT, 0, (void *)0);
        }
        // Update indices buffer
        glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, numOfVertices * sizeof(unsigned int),
                        indexBuffer);
    ...
    for (unsigned int z = 0; z < bz - 1; z++) {
        ...
        if (attrib1Enabled) {
            ...
            glEnableVertexAttribArray(1);
            glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
            glBufferSubData(GL_ARRAY_BUFFER, z * bx * 4 * sizeof(int), 2 * bx * 4 * sizeof(int),
                            (attrib1Buffer + z * bx * 4));
            glVertexAttribIPointer(1, 4, GL_INT, 0, (void *)0);
        }
        glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, numOfVertices * sizeof(unsigned int),
                        indexBuffer);
    ...
    for (unsigned int z = 0; z < bz - 1; z++) {
        ...
        if (attrib1Enabled) {
            ...
            glEnableVertexAttribArray(1);
            glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
            glBufferSubData(GL_ARRAY_BUFFER, z * by * 4 * sizeof(int), 2 * by * 4 * sizeof(int),
                            (attrib1Buffer + z * by * 4));
            glVertexAttribIPointer(1, 4, GL_INT, 0, (void *)0);
        }
        glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, numOfVertices * sizeof(unsigned int),
                        indexBuffer);
...
void RayCaster::_updateViewportWhenNecessary(const GLint *viewport) {
    if ((std::memcmp(viewport, _currentViewport, 4 * sizeof(GLint)) != 0)) {
        ...
        glActiveTexture(GL_TEXTURE0 + _backFaceTexOffset);
        glBindTexture(GL_TEXTURE_2D, _backFaceTextureId);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, _currentViewport[2], _currentViewport[3], 0,
...
void RayCaster::_updateDataTextures() {
...
#endif
    glTexImage3D(GL_TEXTURE_3D, 0, GL_R32F, dims[0], dims[1], dims[2], 0, GL_RED, GL_FLOAT,
                 _userCoordinates.dataField);
    ...
    if (_userCoordinates.missingValueMask) {
        glTexImage3D(GL_TEXTURE_3D, 0, GL_R8UI, dims[0], dims[1], dims[2], 0, GL_RED_INTEGER,
                     GL_UNSIGNED_BYTE, _userCoordinates.missingValueMask);
    } else {
        unsigned char dummyMask[8] = {0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u};
        glTexImage3D(GL_TEXTURE_3D, 0, GL_R8UI, 2, 2, 2, 0, GL_RED_INTEGER, GL_UNSIGNED_BYTE,
                     dummyMask);
    }
...
int RayCaster::_updateVertCoordsTexture(const glm::mat4 &MV) {
    ...
    if ((size_t)width == _userCoordinates.dims[0] && (size_t)height == _userCoordinates.dims[1] &&
        (size_t)depth == _userCoordinates.dims[2]) {
        glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, width, height, depth, GL_RGB, GL_FLOAT,
                        coordEye);
    } else {
