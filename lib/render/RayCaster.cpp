...
void RayCaster::_renderTriangleStrips(int whichPass, long castingMode) const {
    ...
    if (attrib1) {
        ...
        glBufferData(GL_ARRAY_BUFFER, bx * by * 3 * sizeof(float), _userCoordinates.frontFaceAttrib,
                     GL_STATIC_READ);
    ...
    if (attrib1) {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, bx * by * 3 * sizeof(float), _userCoordinates.backFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, (void *)0);
    }
    for (unsigned int y = 0; y < by - 1; y++) // strip by strip
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, bx * bz * 3 * sizeof(float), _userCoordinates.topFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, (void *)0);
    }
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, by * bz * 3 * sizeof(float), _userCoordinates.rightFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, (void *)0);
    }
    for (unsigned int z = 0; z < bz - 1; z++) {
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1); // attribute 1 is the logical indices
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, by * bz * 3 * sizeof(float), _userCoordinates.leftFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, (void *)0);
    }
    for (unsigned int z = 0; z < bz - 1; z++) {
