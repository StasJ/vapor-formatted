...
RayCaster::RayCaster(const ParamsMgr *pm, std::string &winName, std::string &dataSetName,
    ...
    : Renderer(pm, winName, dataSetName, paramsType, classType, instName, dataMgr),
      ...
      _missingValueTexOffset(4), _vertCoordsTexOffset(5), _depthTexOffset(6),
      _2ndVarDataTexOffset(7), _2ndVarMaskTexOffset(8) {
...
void RayCaster::_drawVolumeFaces(int whichPass, int castingMode,
                                 ...
                                 const glm::mat4 &InversedMV, bool fast, bool use2ndVar) {
...
void RayCaster::_load3rdPassUniforms(int castingMode, bool fast, bool insideVolume,
...
int RayCaster::_updateVertCoordsTexture(const glm::mat4 &MV) {
    ...
    glTexImage3D(GL_TEXTURE_3D, 0, GL_R32F, 2, 2, 2, 0, GL_RED, GL_FLOAT, dummyVolume);
#endif
    glTexImage3D(GL_TEXTURE_3D, 0, GL_RGB32F, _userCoordinates.dims[0], _userCoordinates.dims[1],
                 _userCoordinates.dims[2], 0, GL_RGB, GL_FLOAT, coordEye);
...
void RayCaster::_update2ndVarTextures() {
...
#endif
    glTexImage3D(GL_TEXTURE_3D, 0, GL_R32F, dims[0], dims[1], dims[2], 0, GL_RED, GL_FLOAT,
                 _userCoordinates.secondVarData);
    ...
    if (_userCoordinates.secondVarMask) {
        glTexImage3D(GL_TEXTURE_3D, 0, GL_R8UI, dims[0], dims[1], dims[2], 0, GL_RED_INTEGER,
                     GL_UNSIGNED_BYTE, _userCoordinates.secondVarMask);
    } else {
        unsigned char dummyMask[8] = {0u, 0u, 0u, 0u, 0u, 0u, 0u, 0u};
        glTexImage3D(GL_TEXTURE_3D, 0, GL_R8UI, 2, 2, 2, 0, GL_RED_INTEGER, GL_UNSIGNED_BYTE,
                     dummyMask);
