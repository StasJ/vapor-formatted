...
RayCaster::UserCoordinates::UserCoordinates() {
    ...
    topFace = nullptr;
    bottomFace = nullptr;
    frontFaceAttrib = nullptr;
    backFaceAttrib = nullptr;
    rightFaceAttrib = nullptr;
    leftFaceAttrib = nullptr;
    topFaceAttrib = nullptr;
    bottomFaceAttrib = nullptr;
    dataField = nullptr;
    xyCoords = nullptr;
    zCoords = nullptr;
...
int RayCaster::_paintGL(bool fast) {
    ...
    if (std::memcmp(newViewport, _currentViewport, 4 * sizeof(GLint)) != 0) {
        ...
        glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, _currentViewport[2],
                              _currentViewport[3]);
    }
    ...
    if (!_userCoordinates.IsMetadataUpToDate(params, _dataMgr)) {
        ...
        glTexImage3D(GL_TEXTURE_3D, 0, GL_R32F, _userCoordinates.dims[0], _userCoordinates.dims[1],
                     _userCoordinates.dims[2], 0, GL_RED, GL_FLOAT, _userCoordinates.dataField);
        ...
        if (_userCoordinates.missingValueMask) // Has missing value!
        {
            ...
            glTexImage3D(GL_TEXTURE_3D, 0, GL_R8UI, _userCoordinates.dims[0],
                         _userCoordinates.dims[1], _userCoordinates.dims[2], 0, GL_RED_INTEGER,
    ...
    _drawVolumeFaces(3, castingMode, insideACell, ModelView, InversedMV,
                     fast); // 3rd pass, perform ray casting
...
void RayCaster::_load3rdPassUniforms(long castingMode, const GLfloat *MVP, const GLfloat *ModelView,
    ...
    int volumeDimensions[3] = {int(_userCoordinates.dims[0]), int(_userCoordinates.dims[1]),
                               int(_userCoordinates.dims[2])};
    ...
    {
        int maxVolumeDim =
            volumeDimensions[0] > volumeDimensions[1] ? volumeDimensions[0] : volumeDimensions[1];
...
void RayCaster::_renderTriangleStrips(int whichPass, long castingMode) const {
    ...
    if (attrib1) {
        ...
        glBufferData(GL_ARRAY_BUFFER, bx * by * 3 * sizeof(int), _userCoordinates.frontFaceAttrib,
                     GL_STATIC_READ);
    ...
    if (attrib1) {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, bx * by * 3 * sizeof(int), _userCoordinates.backFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribIPointer(1, 3, GL_INT, 0, (void *)0);
    }
    for (unsigned int y = 0; y < by - 1; y++) // strip by strip
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, bx * bz * 3 * sizeof(int), _userCoordinates.topFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribIPointer(1, 3, GL_INT, 0, (void *)0);
    }
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, bx * bz * 3 * sizeof(int), _userCoordinates.bottomFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribIPointer(1, 3, GL_INT, 0, (void *)0);
    }
    for (unsigned int z = 0; z < bz - 1; z++) {
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, by * bz * 3 * sizeof(int), _userCoordinates.rightFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribIPointer(1, 3, GL_INT, 0, (void *)0);
    }
    for (unsigned int z = 0; z < bz - 1; z++) {
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, by * bz * 3 * sizeof(int), _userCoordinates.leftFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribIPointer(1, 3, GL_INT, 0, (void *)0);
    }
    for (unsigned int z = 0; z < bz - 1; z++) {
...
#define MAT(m, r, c) (m)[(c)*4 + (r)]
    out[0] = MAT(mat, 0, 0) * in[0] + MAT(mat, 0, 1) * in[1] + MAT(mat, 0, 2) * in[2] +
             MAT(mat, 0, 3) * in[3];
    out[1] = MAT(mat, 1, 0) * in[0] + MAT(mat, 1, 1) * in[1] + MAT(mat, 1, 2) * in[2] +
             MAT(mat, 1, 3) * in[3];
    out[2] = MAT(mat, 2, 0) * in[0] + MAT(mat, 2, 1) * in[1] + MAT(mat, 2, 2) * in[2] +
             MAT(mat, 2, 3) * in[3];
    out[3] = MAT(mat, 3, 0) * in[0] + MAT(mat, 3, 1) * in[1] + MAT(mat, 3, 2) * in[2] +
             MAT(mat, 3, 3) * in[3];
#undef MAT
}
