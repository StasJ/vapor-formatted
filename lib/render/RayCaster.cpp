...
void RayCaster::_renderTriangleStrips(int whichPass, long castingMode) const {
    ...
    if (attrib1) {
        ...
        glBufferData(GL_ARRAY_BUFFER, bx * by * 4 * sizeof(int), _userCoordinates.frontFaceAttrib,
                     GL_STATIC_READ);
    ...
    if (attrib1) {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, bx * by * 4 * sizeof(int), _userCoordinates.backFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribIPointer(1, 4, GL_INT, 0, (void *)0);
    }
    for (unsigned int y = 0; y < by - 1; y++) // strip by strip
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, bx * bz * 4 * sizeof(int), _userCoordinates.topFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribIPointer(1, 4, GL_INT, 0, (void *)0);
    }
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, bx * bz * 4 * sizeof(int), _userCoordinates.bottomFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribIPointer(1, 4, GL_INT, 0, (void *)0);
    }
    for (unsigned int z = 0; z < bz - 1; z++) {
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, by * bz * 4 * sizeof(int), _userCoordinates.rightFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribIPointer(1, 4, GL_INT, 0, (void *)0);
    }
    for (unsigned int z = 0; z < bz - 1; z++) {
    ...
    if (attrib1) {
        glEnableVertexAttribArray(1);
        glBindBuffer(GL_ARRAY_BUFFER, _vertexAttribId);
        glBufferData(GL_ARRAY_BUFFER, by * bz * 4 * sizeof(int), _userCoordinates.leftFaceAttrib,
                     GL_STATIC_READ);
        glVertexAttribIPointer(1, 4, GL_INT, 0, (void *)0);
    }
    for (unsigned int z = 0; z < bz - 1; z++) {
