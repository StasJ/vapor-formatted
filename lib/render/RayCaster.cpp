...
RayCaster::RayCaster(const ParamsMgr *pm, std::string &winName, std::string &dataSetName,
    ...
    : Renderer(pm, winName, dataSetName, paramsType, classType, instName, dataMgr),
      _backFaceTexOffset(0), _frontFaceTexOffset(1), _volumeTexOffset(2), _colorMapTexOffset(3),
      _missingValueTexOffset(4), _xyCoordsTexOffset(5), _zCoordsTexOffset(6), _depthTexOffset(7) {
...
void RayCaster::UserCoordinates::FindBaseStepSize(int mode) {
    if (mode == FixedStep) {
        ...
        float span[3] = {myGridMax[0] - myGridMin[0], myGridMax[1] - myGridMin[1],
                         myGridMax[2] - myGridMin[2]};
        if (numCells < 50.0f) // Make sure at least 100 steps
            baseStepSize =
                std::sqrt(span[0] * span[0] + span[1] * span[1] + span[2] * span[2]) / 100.0f;
...
void RayCaster::_drawVolumeFaces(int whichPass, int castingMode, bool insideACell,
...
void RayCaster::_load3rdPassUniforms(int castingMode, const glm::mat4 &inversedMV,
                                     bool fast) const {
    ...
    glm::ivec3 volumeDims(int(_userCoordinates.dims[0]), int(_userCoordinates.dims[1]),
...
void RayCaster::_updateDataTextures(int castingMode) {
    ...
    if (_userCoordinates.missingValueMask) // There is missing value.
        glTexImage3D(GL_TEXTURE_3D, 0, GL_R8UI, _userCoordinates.dims[0], _userCoordinates.dims[1],
                     _userCoordinates.dims[2], 0, GL_RED_INTEGER, GL_UNSIGNED_BYTE,
...
void RayCaster::_updateNearClippingPlane() {
    ...
    for (int i = 0; i < 4; i++) {
        near[i] /= near[i].w;
        std::memcpy(_userCoordinates.nearCoords + i * 3, glm::value_ptr(near[i]),
                    3 * sizeof(float));
    }
}
