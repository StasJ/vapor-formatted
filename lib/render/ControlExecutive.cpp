...
ControlExec::ControlExec(vector<string> appParamsNames, vector<string> appRenderParamNames,
...
int ControlExec::Paint(string winName, bool fast) {
    ...
    if (rc)
        SetErrMsg("Error performing paint event");
    return rc;
}
...
int ControlExec::ActivateRender(string winName, string dataSetName, string renderType,
    ...
    if (!v->HasRenderer(renderType, renderName)) {
        ...
        //
        RenderParams *rp =
            _paramsMgr->CreateRenderParamsInstance(winName, dataSetName, paramsType, renderName);
        if (!rp) {
...
int ControlExec::ActivateRender(string winName, string dataSetName, const RenderParams *rp,
    ...
    if (!v->HasRenderer(renderType, renderName)) {
        ...
        //
        RenderParams *newRP =
            _paramsMgr->CreateRenderParamsInstance(winName, dataSetName, renderName, rp);
        if (!newRP) {
            SetErrMsg("Invalid renderer of type \"%s\"", renderType.c_str());
            _paramsMgr->EndSaveStateGroup();
            return -1;
...
void ControlExec::_removeRendererHelper(string winName, string dataSetName, string renderType,
    ...
    RenderParams *rParams =
        _paramsMgr->GetRenderParams(winName, dataSetName, paramsType, renderName);
    if (!rParams)
        return;
...
void ControlExec::RemoveRenderer(string winName, string dataSetName, string renderType,
                                 string renderName, bool hasOpenGLContext) {
...
void ControlExec::RemoveAllRenderers(string winName, bool hasOpenGLContext) {
    ...
    for (int k = 0; k < dataSetNames.size(); k++) {
        ...
        vector<string> pClassNames =
            _paramsMgr->GetRenderParamsClassNames(winName, dataSetNames[k]);

        for (int j = 0; j < pClassNames.size(); j++) {
            vector<string> instNames =
                _paramsMgr->GetRenderParamInstances(winName, dataSetNames[k], pClassNames[j]);

            string rClassName =
                RendererFactory::Instance()->GetRenderClassFromParamsClass(pClassNames[j]);

            for (int i = 0; i < instNames.size(); i++) {

                _removeRendererHelper(winName, dataSetNames[k], rClassName, instNames[i], true,
                                      hasOpenGLContext);
            }
        }
    }
...
int ControlExec::OpenData(const std::vector<string> &files, const std::vector<string> &options,
    ...
    if (_dataStatus->GetDataMgr(dataSetName)) {
        ...
        for (int k = 0; k < vizNames.size(); k++) {
            ...
            vector<string> pClassNames =
                _paramsMgr->GetRenderParamsClassNames(vizNames[k], dataSetName);

            for (int j = 0; j < pClassNames.size(); j++) {
                vector<string> instNames =
                    _paramsMgr->GetRenderParamInstances(vizNames[k], dataSetName, pClassNames[j]);
                ...
                for (int i = 0; i < instNames.size(); i++) {
                    ...
                    _removeRendererHelper(vizNames[k], dataSetName, rClassName, instNames[i], false,
                                          false);
                }
            }
        }
    ...
    for (int i = 0; i < appRenderParams.size(); i++) {
        ...
        if (rc < 0) {
            ...
            SetErrMsg("Failure to initialize application renderer \"%s\"",
...
string ControlExec::MakeStringConformant(string s) {
    ...
    for (string::iterator itr = s.begin(); itr != s.end(); ++itr) {
        if (!(isalnum(*itr) || isdigit(*itr) || *itr == '-' || *itr == '_' || *itr == '.')) {
...
void ControlExec::RemoveFunction(string scriptType, string dataSetName, string scriptName) {
    return (_calcEngineMgr->RemoveFunction(scriptType, dataSetName, scriptName));
}
...
std::vector<string> ControlExec::GetFunctionNames(string scriptType, string dataSetName) const {
    ...
    return (_calcEngineMgr->GetFunctionNames(scriptType, dataSetName));
}
