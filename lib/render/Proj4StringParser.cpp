...
Proj4StringParser::Proj4StringParser(const std::string &projString)
    : _string(projString), _tokens(Proj4StringToParameterMap(projString)) {}
...
bool Proj4StringParser::HasKey(const std::string &key) const {
    return _tokens.find(key) != _tokens.end();
}

std::string Proj4StringParser::GetString(const std::string &key,
