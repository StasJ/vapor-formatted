...
int SliceRenderer::_paintGL(bool fast) {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, indexValues.size() * sizeof(unsigned int),
                 &indexValues[0], GL_STATIC_DRAW);
...
void SliceRenderer::_render(int orientation, std::vector<double> min,
...
void SliceRenderer::_setXYVertexPositions(std::vector<double> min, std::vector<double> max) {
    ...
    std::vector<double> temp = {
        min[X], min[Y], zCoord, max[X], min[Y], zCoord,
