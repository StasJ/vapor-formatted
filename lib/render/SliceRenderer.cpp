...
void SliceRenderer::_initTexCoordVBO() {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * sizeof(_texCoords), _texCoords.data(),
                 GL_STATIC_DRAW);
}
...
void SliceRenderer::_createDataTexture(float *dataValues) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RG32F, _textureWidth, _textureHeight, 0, GL_RG, GL_FLOAT,
                 dataValues);
}
