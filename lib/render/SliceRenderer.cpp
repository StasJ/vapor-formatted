...
SliceRenderer::SliceRenderer(const ParamsMgr *pm, string winName, string dataSetName,
    ...
    _vertexCoords = {0.0f, 0.0f, 0.f, 1.0f, 0.0f, 0.f, 0.0f, 1.0f, 0.f,
                     1.0f, 0.0f, 0.f, 1.0f, 1.0f, 0.f, 0.0f, 1.0f, 0.f};
...
void SliceRenderer::_initTexCoordVBO() {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * _texCoords.size(), _texCoords.data(),
                 GL_DYNAMIC_DRAW);
}
...
int SliceRenderer::_resetBoxCache() {
    ...
    int rc = _dataMgr->GetVariableExtents(
        _cacheParams.ts, _cacheParams.varName, _cacheParams.refinementLevel,
        _cacheParams.compressionLevel, _cacheParams.domainMin, _cacheParams.domainMax);
    if (rc < 0) {
...
void SliceRenderer::_createDataTexture(float *dataValues) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RG32F, _textureSideSize, _textureSideSize, 0, GL_RG, GL_FLOAT,
                 dataValues);
}
...
bool SliceRenderer::_isDataCacheDirty() const {
    ...
    if (min != _cacheParams.boxMin)
        return true;
    if (max != _cacheParams.boxMax)
        return true;
