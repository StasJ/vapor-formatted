...
static RendererRegistrar<SliceRenderer> registrar(SliceRenderer::GetClassType(),
                                                  SliceParams::GetClassType());

SliceRenderer::SliceRenderer(const ParamsMgr *pm, string winName, string dataSetName,
...
void SliceRenderer::_initTexCoordVBO() {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * sizeof(_texCoords), &_texCoords[0],
                 GL_STATIC_DRAW);
}
...
int SliceRenderer::_resetDataCache() {
    ...
    int rc;
    rc = _saveTextureData(0);
    rc = _saveTextureData(1);
    rc = _saveTextureData(2);
    if (rc < 0) {
...
void SliceRenderer::_resetColormapCache() {
    ...
    glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA8, _colorMapSize, 0, GL_RGBA, GL_FLOAT,
...
int SliceRenderer::_resetBoxCache() {
    ...
    int rc = _dataMgr->GetVariableExtents(_cacheParams.ts, _cacheParams.varName,
                                          _cacheParams.refinementLevel, _cacheParams.domainMin,
...
void SliceRenderer::_resetTextureCoordinates() {
    ...
    _texCoords.clear();
    _texCoords = {texMinX, texMinY, texMaxX, texMinY, texMinX, texMaxY,
                  texMaxX, texMinY, texMaxX, texMaxY, texMinX, texMaxY};
...
inline void SliceRenderer::_getInlineJSampleCoordinates(std::vector<double> &coords,
...
void SliceRenderer::_getJSampleCoordinates(std::vector<double> &coords,
                                           const std::vector<double> deltas, const int j) const {
    if (_cacheParams.orientation == XY) {
        coords[Y] = _cacheParams.domainMin[Y] + deltas[Y] * j + deltas[Y] / 2.f;
        coords[Z] = _cacheParams.boxMin[Z];
    } else if (_cacheParams.orientation == XZ) {
...
inline void SliceRenderer::_getInlineISampleCoordinates(std::vector<double> &coords,
                                                        const std::vector<double> deltas,
                                                        const int i) const {
    if (_cacheParams.orientation == XY) {
...
void SliceRenderer::_getISampleCoordinates(std::vector<double> &coords,
                                           const std::vector<double> deltas, const int i) const {
    if (_cacheParams.orientation == XY) {
        coords[X] = _cacheParams.domainMin[X] + deltas[X] * i + deltas[X] / 2.f;
    } else if (_cacheParams.orientation == XZ) {
        coords[X] = _cacheParams.domainMin[X] + deltas[X] * i + deltas[X] / 2.f;
...
int SliceRenderer::_saveTextureData(int method = 0) {
    if (method != 0)
        return 0;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, _textureWidth, _textureHeight, 0, GL_RED, GL_FLOAT,
                 dataValues);
    ...
    if (method == 0)
        cout << "New way         " << duration << endl;
    else if (method == 1)
        cout << "New with inline " << duration << endl;
    else if (method == 2)
        cout << "Old way         " << duration << endl;
...
bool SliceRenderer::_isDataCacheDirty() const {
    ...
    if (_cacheParams.textureSampleRate != p->GetSampleRate())
        return true;

    vector<double> min, max;
    Box *box = p->GetBox();
    int orientation = box->GetOrientation();
    if (_cacheParams.orientation != orientation)
        return true;
    ...
    if (min[constantAxis] != _cacheParams.boxMin[constantAxis])
        return true;
...
bool SliceRenderer::_isColormapCacheDirty() const {
    ...
    vector<float> tf_lut;
    tf->makeLut(tf_lut);
    if (_cacheParams.tf_lut != tf_lut)
        return true;
    if (_cacheParams.tf_minMax != tf->getMinMaxMapValue())
        return true;
    return false;
}
...
void SliceRenderer::_setXYVertexPositions(std::vector<double> min, std::vector<double> max) {
    ...
    std::vector<double> temp = {min[X], min[Y], zCoord, max[X], min[Y], zCoord,
                                min[X], max[Y], zCoord, max[X], min[Y], zCoord,
                                max[X], max[Y], zCoord, min[X], max[Y], zCoord};
...
void SliceRenderer::_setXZVertexPositions(std::vector<double> min, std::vector<double> max) {
    double yCoord = min[Y];
    std::vector<double> temp = {min[X], yCoord, min[Z], max[X], yCoord, min[Z],
                                min[X], yCoord, max[Z], max[X], yCoord, min[Z],
                                max[X], yCoord, max[Z], min[X], yCoord, max[Z]};
...
void SliceRenderer::_setYZVertexPositions(std::vector<double> min, std::vector<double> max) {
    double xCoord = min[X];
    std::vector<double> temp = {xCoord, min[Y], min[Z], xCoord, max[Y], min[Z],
                                xCoord, min[Y], max[Z], xCoord, max[Y], min[Z],
                                xCoord, max[Y], max[Z], xCoord, min[Y], max[Z]};
