...
//
int TranslateStretchManip::mouseIsOverHandle(double screenCoords[2], double *boxExtents,
                                             double handleMid[3]) {
    // Determine if the mouse is over any of the six handles.
    // Test first the 3 handles in front, then the object, then the three in back.
    ...
    for (int axis = 0; axis < 3; axis++) {
        if (camPos[axis] > 0.5f * (boxExtents[axis] + boxExtents[axis + 3]))
            octant |= 1 << axis;
    }
...
bool TranslateStretchManip::startHandleSlide(Visualizer *viz, double mouseCoords[2], int handleNum,
                                             Params *manipParams) {
    ...
    for (int i = 0; i < 3; i++) {
        boxCtr[i] = (boxExtents[i] + boxExtents[i + 3]) * 0.5f;
    }
    // project the boxCtr and one more point, to get a direction vector

    if (!viz->projectPointToWin(boxCtr, winCoords))
        return false;
    boxCtr[handleNum] += 0.1f;
    if (!viz->projectPointToWin(boxCtr, dispCoords))
        return false;
...
void TranslateStretchManip::captureMouseDown(int handleNum, const std::vector<double> &camPos,
    ...
    for (int i = 0; i < 3; i++)
        _initialSelectionRay[i] = strHandleMid[i] / stretch[i] - camPos[i];
...
//
TranslateRotateManip::TranslateRotateManip(Visualizer *w, Params *p)
    : TranslateStretchManip(w, p) {}
    ...
    for (int cor = 0; cor < 8; cor++) {
        ...
        if (_selectedHandle >= 0) {
            ...
            if (_isStretching) {
                ...
                // rotate the selected handle by theta, phi, psi to find the side that corresponds
                // to the corners that need to be moved
                int newHandle;
                if (_selectedHandle < 3)
...
void TranslateRotateManip::slideHandle(int handleNum, double movedRay[3], bool constrain) {
    ...
    if (denom != 0.f) {
        ...
        // (_dataStatus->getStretchFactors())[coord]);
    }
