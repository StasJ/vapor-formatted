...
int TranslateStretchManip::mouseIsOverHandle(double screenCoords[2]) {
    ...
    for (int axis = 0; axis < 3; axis++) {
        if (_cameraPosition[axis] > 0.5f * (boxExtents[axis] + boxExtents[axis + 3])) {
            octant |= 1 << axis;
        }
    }
...
// bool TranslateStretchManip::startHandleSlide(Visualizer* viz, double mouseCoords[2], int
// handleNum, Params* manipParams){
...
void TranslateStretchManip::captureMouseDown(int handleNum, const std::vector<double> &camPos,
                                             double *dirVec, int buttonNum,
    ...
    for (int i = 0; i < 3; i++)
        _initialSelectionRay[i] = strHandleMid[i] - camPos[i];
