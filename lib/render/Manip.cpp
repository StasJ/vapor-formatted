...
void TranslateStretchManip::Update(std::vector<double> llc, std::vector<double> urc,
                                   ...
                                   std::vector<double> rotationCenter, double modelViewMatrix[16],
                                   double projectionMatrix[16], std::vector<int> windowSize,
                                   VAPoR::Transform *transform, bool constrain) {
    for (int i = 0; i < 16; i++) {
...
void TranslateStretchManip::mousePress(double screenCoords[2], double handleMidpoint[3],
                                       int buttonNum) {
    double dirVec[3];
...
bool TranslateStretchManip::ReconstructCamera(double position[3], double upVec[3],
...
bool TranslateStretchManip::rayHandleIntersect(double ray[3], const std::vector<double> &cameraPos,
                                               int handleNum, int faceNum, double intersect[3]) {
    double val;
...
void TranslateStretchManip::applyTransform(double extents[6]) {
    if (_transform == NULL)
        return;
    ...
    double origin, scale, translation;
    for (int i = 0; i < 3; i++) {
        origin = origins[i];
        scale = scales[i];
        ...
        extents[i] *= scale;
        extents[i] += origin;
        extents[i] += translation;

        extents[i + 3] -= origin;
        extents[i + 3] *= scale;
        extents[i + 3] += origin;
        extents[i + 3] += translation;
    }
}

void TranslateStretchManip::removeTransform(double extents[6]) {
    if (_transform == NULL)
        return;
    vector<double> translations = _transform->GetTranslations();
    vector<double> rotations = _transform->GetRotations();
...
void TranslateStretchManip::drawHandleConnector(int handleNum, double *handleExtents,
    ...
    if ((handleNum == _selectedHandle) || (handleNum == (5 - _selectedHandle)))
        ...
        glColor4fv(_unselectedFaceColor);
    glBegin(GL_LINES);
    glVertex3f(0.5f * (handleExtents[3] + handleExtents[0]) + handleDisp[0],
               0.5f * (handleExtents[4] + handleExtents[1]) + handleDisp[1],
               0.5f * (handleExtents[5] + handleExtents[2]) + handleDisp[2]);
    glVertex3f(0.5f * (boxExtents[3] + boxExtents[0]) + boxDisp[0],
               0.5f * (boxExtents[4] + boxExtents[1]) + boxDisp[1],
               0.5f * (boxExtents[5] + boxExtents[2]) + boxDisp[2]);
    glEnd();
    glDisable(GL_BLEND);
}
