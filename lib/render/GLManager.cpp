...
GLManager::GLManager()
    : shaderManager(new ShaderManager), fontManager(new FontManager(this)),
      matrixManager(new MatrixManager), legacy(new LegacyGL(this)) {
    _queryVendor();
}
...
bool GLManager::IsCurrentOpenGLVersionSupported() {
    ...
    Wasp::MyBase::SetErrMsg("OpenGL Version \"%s\" is too low and is not supported",
...
void GLManager::ShowDepthBuffer() {
    ...
    // Since this is just for debugging purposes, initialized stays false to support use in multiple
    // contexts
    if (!initialized) {
