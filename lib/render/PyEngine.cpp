...
int alloc_arrays(const vector<vector<size_t>> &inputVarDims,
                 const vector<vector<size_t>> &outputVarDims, vector<float *> &inputVarArrays,
...
void copy_region(const float *src, float *dst, const vector<size_t> &min, const vector<size_t> &max,
...
void get_var_info(DataMgr *dataMgr, const vector<Grid *> &gs, const vector<string> &varNames,
...
int PyEngine::AddFunction(string name, string script, const vector<string> &inputVarNames,
    ...
    //
    if (_checkOutVars(outputVarNames) < 0)
        return (-1);
    ...
    for (int i = 0; i < outputVarNames.size(); i++) {
        ...
        DerivedPythonVar *dvar =
            new DerivedPythonVar(vname, "", DC::XType::FLOAT, outputVarMeshes[i], timeCoordVarName,
                                 true, inputVarNames, script, _dataMgr, coordFlag);
    ...
    _functions[name] =
        func_c(name, script, inputVarNames, outputVarNames, outputVarMeshes, dvars, coordFlag);
...
bool PyEngine::GetFunctionScript(string name, string &script, std::vector<string> &inputVarNames,
                                 std::vector<string> &outputVarNames,
                                 std::vector<string> &outputMeshNames, bool &coordFlag) const {
    script.clear();
    inputVarNames.clear();
...
int PyEngine::Calculate(const string &script, vector<string> inputVarNames,
                        vector<vector<size_t>> inputVarDims, vector<float *> inputVarArrays,
                        vector<string> outputVarNames, vector<vector<size_t>> outputVarDims,
                        vector<float *> outputVarArrays) {
...
int PyEngine::_c2python(PyObject *dict, vector<string> inputVarNames,
                        vector<vector<size_t>> inputVarDims, vector<float *> inputVarArrays) {
...
int PyEngine::_python2c(PyObject *dict, vector<string> outputVarNames,
...
PyEngine::DerivedPythonVar::DerivedPythonVar(string varName, string units, DC::XType type,
                                             string mesh, string time_coord_var, bool hasMissing,
                                             std::vector<string> inNames, string script,
                                             DataMgr *dataMgr, bool coordFlag)
    : DerivedDataVar(varName), _varInfo(varName, units, type, "", std::vector<size_t>(),
...
int PyEngine::DerivedPythonVar::_readRegionAll(int fd, const std::vector<size_t> &min,
    ...
    rc = PyEngine::Calculate(_script, inputNames, inputVarDims, inputVarArrays, outputVarNames,
...
int PyEngine::DerivedPythonVar::_readRegionSubset(int fd, const std::vector<size_t> &min,
                                                  const std::vector<size_t> &max, float *region) {
    ...
    int rc =
        DataMgrUtils::GetGrids(_dataMgr, ts, _inNames, min, max, false, &level, &lod, variables);
    if (rc < 0)
        return (-1);
...
string PyEngine::_getTimeCoordVarName(const vector<string> &varNames) const {
    ...
    if (timeCoordVarName.empty())
        return ("");

    for (int i = 0; i < varNames.size(); i++) {
        if (_dataMgr->IsTimeVarying(varNames[i]))
            return (timeCoordVarName);
    }
