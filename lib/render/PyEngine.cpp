...
int PyEngine::AddFunction(string name, const vector<string> &inputVarNames,
...
int PyEngine::Calculate(const string &script, vector<string> inputVarNames,
                        ...
                        vector<float *> outputVarArrays) const {
...
int PyEngine::_c2python(PyObject *dict, vector<string> inputVarNames,
                        vector<vector<size_t>> inputVarDims, vector<float *> inputVarArrays) const {
    ...
    for (int i = 0; i < inputVarNames.size(); i++) {
        ...
        PyObject *pyArray =
            PyArray_New(&PyArray_Type, dims.size(), pyDims, NPY_FLOAT32, NULL, inputVarArrays[i], 0,
...
DerivedPythonVar::DerivedPythonVar(string varName, string units, XType type string mesh,
                                   string time_coord_var,
                                   bool hasMissing std::vector<string> inNames, string script,
                                   DataMgr *dataMgr)
    : DerivedDataVar(varName), _varInfo(varName, units, type, "", std::vector<size_t>(),
                                        std::vector<bool>(), mesh, time_cord_var, location) {
    _script = script;
    _dataMgr = dataMgr;
    if (hasMissing) {
