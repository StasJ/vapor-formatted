...
IsoSurfaceRenderer::IsoSurfaceRenderer(const ParamsMgr *pm, std::string &winName,
                                       ...
                                       DataMgr *dataMgr)
    : RayCaster(pm, winName, dataSetName, IsoSurfaceParams::GetClassType(),
                IsoSurfaceRenderer::GetClassType(), instName, dataMgr) {}
...
void IsoSurfaceRenderer::_colormapSpecialHandling() {
    ...
    if (_use2ndVariable(params)) {
        VAPoR::MapperFunction *mapperFunc =
            params->RenderParams::GetMapperFunc(params->GetColorMapVariableName());
    ...
    // Note: _colorMapRange[2] keeps the range of a color map, which will later be used in the
    // shader.
