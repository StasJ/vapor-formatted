...
unsigned char *GeoImageGeoTiff::GetImage(size_t ts, const double pcsExtentsReq[4],
                                         string proj4StringReq, size_t maxWidthReq,
                                         size_t maxHeightReq, // not used
                                         double pcsExtentsImg[4], double geoCornersImg[8],
    ...
    //
    int rc = _getGTIFInfo(GeoImage::TiffGetHandle(), width, height, pcsExtentsImg, geoCornersImg,
                          proj4StringImg);
    if (rc < 0)
        return (NULL);
    ...
    // they are unchanged.
    //
    (void)_extractSubtexture(_texture, width, height, pcsExtentsReq, proj4StringReq, pcsExtentsImg,
                             geoCornersImg, proj4StringImg, proj4StringImg, width, height,
                             pcsExtentsImg, geoCornersImg);
...
int GeoImageGeoTiff::_getGTIFInfo(TIFF *tif, size_t width, size_t height, double pcsExtents[4],
                                  double geoCorners[8], string &proj4String) const {
    ...
    // be necessary.
    //
    if (geoCorners[1] < -89.9999)
        geoCorners[1] = -89.9999;
    if (geoCorners[7] < -89.9999)
        geoCorners[7] = -89.9999;
    if (geoCorners[3] > 89.9999)
        geoCorners[3] = 89.9999;
    if (geoCorners[5] > 89.9999)
        geoCorners[5] = 89.9999;
...
// in PCS coordinates of the *image*
//
bool GeoImageGeoTiff::_extractSubtexture(unsigned char *texture, size_t width, size_t height,
                                         const double pcsExtentsReq[4], string proj4StringReq,
                                         const double pcsExtentsImg[4],
                                         const double geoCornersImg[8], string proj4StringImg,
                                         string &subProj4StringImg, size_t &subWidth,
                                         size_t &subHeight, double subPCSExtentsImg[4],
    ...
    if (eqc) {
        if (geoCornersImg[1] > -89.5 || geoCornersImg[5] < 89.5)
            return (false);
        if (width != 2 * height)
            return (false);
    }
    if (mercator) {
        if (geoCornersImg[1] > -85.0 || geoCornersImg[5] < 85.0)
            return (false);
        if (width != height)
            return (false);
    }
    ...
    for (int i = 0; i < 4; i++)
        myGeoExtentsData[i] = pcsExtentsReq[i];
