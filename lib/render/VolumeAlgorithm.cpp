...
void VolumeAlgorithm::GetExtents(glm::vec3 *dataMin_, glm::vec3 *dataMax_, glm::vec3 *userMin_,
                                 glm::vec3 *userMax_) const {
    ...
    auto p = GetParams();
    _renderer->_dataMgr->GetVariableExtents(p->GetCurrentTimestep(), p->GetVariableName(),
                                            p->GetRefinementLevel(), p->GetCompressionLevel(),
                                            minDataExtents, maxDataExtents);

    glm::vec3 userMin =
        glm::vec3(minRendererExtents[0], minRendererExtents[1], minRendererExtents[2]);
    glm::vec3 userMax =
        glm::vec3(maxRendererExtents[0], maxRendererExtents[1], maxRendererExtents[2]);
    ...
    if (userMin_)
        *userMin_ = glm::max(userMin, dataMin);
    if (userMax_)
        *userMax_ = glm::min(userMax, dataMax);
}
