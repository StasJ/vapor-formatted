...
int TIFWriter::ConfigureWithFormat(ImageWriter::Format f) {
    ...
    default:
        MyBase::SetErrMsg("Unsupported format");
        return -1;
    }
}
...
int TIFWriter::Write(const unsigned char *buffer, const unsigned int width,
    ...
    int writeSuccess =
        TIFFWriteRawStrip(tif, 0, const_cast<unsigned char *>(buffer), width * height * 3);
    if (writeSuccess < 0) {
