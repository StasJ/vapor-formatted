...
int Framebuffer::Generate() {
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _colorBuffer._id,
                           0);

    if (_hasDepthBuffer) {
        _depthBuffer.Generate();
        _depthBuffer.TexImage(GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, _depthBuffer._id,
                               0);
    }
...
void Framebuffer::SetSize(int width, int height) {
    ...
    if (width != _width || height != _height) {
        ...
        if (_hasDepthBuffer)
            _depthBuffer.TexImage(GL_DEPTH_COMPONENT24, width, height, 0, GL_DEPTH_COMPONENT,
                                  GL_FLOAT, NULL);
    }
}
