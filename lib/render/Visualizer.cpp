...
int Visualizer::paintSetup(int timeStep) {
    ...
    // Lights are positioned relative to the view direction, do this before the modelView matrix is
    // set
    if (placeLights()) {
        return -1;
    }
...
int Visualizer::captureImage(string filename) {
    ...
    _imageCaptureEnabled = false;
    string suffix =
        filename.substr(filename.length() - 4, 4); // it assumes fixed length of all suffix...
    ...
    } else // write png files
    {
        ...
        if (!test) {
            SetErrMsg("Image Capture Error: Error opening output PNG file: %s",
                      (const char *)filename.c_str());
            return -1;
        }
        fclose(test);
