...
int Visualizer::paintEvent(bool fast) {
    ...
    for (int i = 0; i < _renderers.size(); i++) {
        ...
        if (_renderers[i]->IsGLInitialized() && !_renderers[i]->GetActiveParams()->GetValueLong(
                                                    RenderParams::OSPRayEnabledTag, false)) {

            //            void *t = _glManager->BeginTimer();
            int myrc = _renderers[i]->paintGL(fast);
            //            printf("%s: %f\n", _renderers[i]->GetMyName().c_str(),
            //            _glManager->EndTimer(t));
            GL_ERR_BREAK();
            if (myrc < 0)
                rc = -1;
        }
...
int Visualizer::_renderOSPRay() {
    ...
    OSPTexture depthTexture = OSPRayGetDepthTextureFromOpenGLPerspective(
        fov, aspect, viewpointParams->GetNearClip(), viewpointParams->GetFarClip(), cameraDir,
        cameraUp, glDepthBuffer, width, height);
...
int Visualizer::CreateRenderer(string dataSetName, string renderType, string renderName) {
    ...
    if (HasRenderer(renderType, renderName))
        return (0);
...
bool Visualizer::HasRenderer(string renderType, string renderName) const {
...
int Visualizer::_captureImage(std::string path) {
    ...
    if (geoTiffOutput) {
        string projString =
            _dataStatus->GetDataMgr(_dataStatus->GetDataMgrNames()[0])->GetMapProjection();

        vector<double> dataMinExtents, dataMaxExtents;
        _dataStatus->GetActiveExtents(_paramsMgr, _winName, _getCurrentTimestep(), dataMinExtents,
                                      dataMaxExtents);
        ...
        for (int y = 0; y < croppedHeight; y++)
            memcpy(&croppedFB[3 * y * croppedWidth],
                   &framebuffer[3 * ((y + cropMin[1]) * width + cropMin[0])], 3 * croppedWidth);
