...
int Visualizer::getCurrentTimestep() const {
    ...
    for (int i = 0; i < dataSetNames.size(); i++) {
        ...
        if (rParams.size()) {
            ...
            m_dataStatus->MapLocalToGlobalTimeRange(dataSetNames[i], local_ts, my_min_ts,
                                                    my_max_ts);
            if (first) {
                min_ts = my_min_ts;
                max_ts = my_max_ts;
...
int Visualizer::paintEvent() {
    ...
    if (paintSetup(timeStep))
        return -1;
    // make sure to capture whenever the time step or frame index changes (once we implement
    // capture!)
    ...
    // are sorted back-to-front.  Note: This only works if all the geometry of a renderer is ordered
    // by a simple depth test.
    ...
    int Visualizer::paintSetup(int timeStep) {
        ...
        // Lights are positioned relative to the view direction, do this before the modelView matrix
        // is set
        if (placeLights()) {
            return -1;
        }
    ...
    int Visualizer::initializeGL(ShaderMgr * shaderMgr) {
        ...
        SetDiagMsg("OpenGL Capabilities : GLEW_VERSION_2_0 %s",
                   GLEW_VERSION_2_0 ? "ok" : "missing");
        SetDiagMsg("OpenGL Capabilities : GLEW_EXT_framebuffer_object %s",
                   GLEW_EXT_framebuffer_object ? "ok" : "missing");
        SetDiagMsg("OpenGL Capabilities : GLEW_ARB_vertex_buffer_object %s",
                   GLEW_ARB_vertex_buffer_object ? "ok" : "missing");
        SetDiagMsg("OpenGL Capabilities : GLEW_ARB_multitexture %s",
                   GLEW_ARB_multitexture ? "ok" : "missing");
        SetDiagMsg("OpenGL Capabilities : GLEW_ARB_shader_objects %s",
    ...
    bool Visualizer::projectPointToWin(double cubeCoords[3], double winCoords[2]) {
        ...
        bool success = (0 != gluProject(cbCoords[0], cbCoords[1], cbCoords[2], mvMatrix, pMatrix,
                                        viewport, wCoords, (wCoords + 1), (GLdouble *)(&depth)));
        if (!success)
            return false;
    ...
    //
    bool Visualizer::pixelToVector(double winCoords[2], const vector<double> camPosStr,
        ...
        gluProject(strHandleMid[0], strHandleMid[1], strHandleMid[2], mvMatrix, pMatrix, viewport,
                   &screenx, &screeny, &screenz);
        // Obtain the coords of a point in view:
        bool success = (0 != gluUnProject((GLdouble)winCoords[0], (GLdouble)winCoords[1], screenz,
                                          mvMatrix, pMatrix, viewport, pt, pt + 1, pt + 2));
        if (success) {
    ...
    //
    bool Visualizer::pointIsOnQuad(double cor1[3], double cor2[3], double cor3[3], double cor4[3],
                                   double pickPt[2]) {
        double winCoord1[2];
        double winCoord2[2];
        double winCoord3[2];
        ...
        if (pointOnRight(winCoord3, winCoord4, pickPt))
            return false;
        if (pointOnRight(winCoord4, winCoord1, pickPt))
            return false;
        return true;
    }
    ...
    Visualizer::OGLVendorType Visualizer::GetVendor() {
        ...
        for (int i = 0; i < ven_str.size(); i++) {
            if (isupper(ven_str[i]))
                ven_str[i] = tolower(ven_str[i]);
        }
        for (int i = 0; i < ren_str.size(); i++) {
            if (isupper(ren_str[i]))
                ren_str[i] = tolower(ren_str[i]);
        }
    ...
    void Visualizer::renderManip() {
        ...
        else if ((MouseModeParams::GetCurrentMouseMode() != MouseModeParams::navigateMode) &&
                 windowIsActive()) {
    ...
    int Visualizer::captureImage(string filename) {
        ...
        if (suffix == ".tif") {
            ...
            if (!tiffFile) {
                SetErrMsg("Image Capture Error: Error opening output Tiff file: %s",
            ...
            if (!jpegFile) {
                SetErrMsg("Image Capture Error: Error opening output Jpeg file: %s",
                          (const char *)filename.c_str());
                return -1;
            }
        }
        ...
        if (suffix == ".jpg") {
            ...
            for (int row = 0; row < imagelength; row++) {
                ...
                if (rc != 1) {
                    SetErrMsg("Image Capture Error; Error writing tiff file %s",
                              (const char *)filename.c_str());
                    break;
                }
            }
    ...
    void Visualizer::renderColorbars(int timeStep) {
        ...
        //  First project all 8 box corners to the center line of the camera view, finding the
        //  furthest and nearest projection in front of the camera.  The furthest distance is used
        //  as the far distance. If some point projects behind the camera, then either the camera is
        //  inside the box, or a corner of the box is behind the camera.  This calculation is always
        //  performed in local coordinates since a translation won't affect the result
        void Visualizer::getFarNearDist(vector<double> posVec, vector<double> dirVec, float &boxFar,
                                        float &boxNear) const {
