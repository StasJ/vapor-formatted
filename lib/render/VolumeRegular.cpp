...
VolumeRegular::VolumeRegular(GLManager *gl, VolumeRenderer *renderer)
    : VolumeGLSL(gl, renderer), _hasSecondData(false) {
    _data.Generate();
    _missing.Generate();
}
...
int VolumeRegular::LoadSecondaryData(const Grid *grid) {
    ...
    if (_dataDimensions != grid->GetDimensions()) {
        Wasp::MyBase::SetErrMsg(
            "Secondary (color mapped) variable has different grid from primary variable");
        return -1;
    }
...
int VolumeRegular::_loadDataDirect(const Grid *grid, Texture3D *dataTexture,
                                   Texture3D *missingTexture, bool *hasMissingData) {
    ...
    *hasMissingData = grid->HasMissingData();
    if (*hasMissingData) {
        ...
        missingTexture->TexImage(GL_R8, dims[0], dims[1], dims[2], GL_RED, GL_UNSIGNED_BYTE,
                                 missingMask);
