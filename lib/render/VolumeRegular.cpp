...
VolumeRegular::VolumeRegular(GLManager *gl)
    : VolumeAlgorithm(gl), _dataTexture(NULL), _missingTexture(NULL), _hasSecondData(false),
      _dataTexture2(NULL), _missingTexture2(NULL) {
...
int VolumeRegular::_loadDataDirect(const Grid *grid, const unsigned int dataTexture,
    ...
    *hasMissingData = grid->HasMissingData();
    if (*hasMissingData) {
        ...
        glTexImage3D(GL_TEXTURE_3D, 0, GL_R32F, 0, 0, 0, 0, GL_RED, GL_UNSIGNED_BYTE,
                     NULL); // Fix driver bug with re-uploading large textures
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
        glTexImage3D(GL_TEXTURE_3D, 0, GL_R8, dims[0], dims[1], dims[2], 0, GL_RED,
                     GL_UNSIGNED_BYTE, missingMask);
