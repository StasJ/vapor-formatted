...
void VizFeatureRenderer::renderText(double text, double llx, double lly, double llz) {
    ...
    // float godHelpMe[] = {(float)axisColor[0], (float)axisColor[1], (float)axisColor[2],
    // (float)axisColor[3]}; float godHelpMe2[] = {(float)txtBackground[0], (float)txtBackground[1],
    // (float)txtBackground[2], (float)txtBackground[3]};

    _textObject = new TextObject();
    _textObject->Initialize(_fontFile, textString, fontSize, axisColor, txtBackground, vpParams,
                            TextObject::BILLBOARD,
