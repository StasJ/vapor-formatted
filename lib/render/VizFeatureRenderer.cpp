...
void VizFeatureRenderer::drawAxisTics() {
    ...
    if (numTics[0] > 1 && ticLength[0] > 0.f) {
        ...
        for (int i = 0; i < numTics[0]; i++) {
            pointOnAxis[0] =
                minTic[0] + (float)i * (maxTic[0] - minTic[0]) / (float)(numTics[0] - 1);
    ...
    if (numTics[1] > 1 && ticLength[1] > 0.f) {
        ...
        for (int i = 0; i < numTics[1]; i++) {
            pointOnAxis[1] =
                minTic[1] + (float)i * (maxTic[1] - minTic[1]) / (float)(numTics[1] - 1);
            vsub(pointOnAxis, ticVec, startPosn);
            vadd(pointOnAxis, ticVec, endPosn);
            _drawTic(startPosn, endPosn, width, axisColor);
    ...
    if (numTics[2] > 1 && ticLength[2] > 0.f) {
        ...
        for (int i = 0; i < numTics[2]; i++) {
            pointOnAxis[2] =
                minTic[2] + (float)i * (maxTic[2] - minTic[2]) / (float)(numTics[2] - 1);
            vsub(pointOnAxis, ticVec, startPosn);
            vadd(pointOnAxis, ticVec, endPosn);
            _drawTic(startPosn, endPosn, width, axisColor);
            renderText(pointOnAxis[1], startPosn[0], startPosn[1]);
...
void VizFeatureRenderer::_drawAxes(std::vector<double> min, std::vector<double> max,
...
void VizFeatureRenderer::_drawTic(double startPosn[], double endPosn[], double width,
...
void VizFeatureRenderer::convertPointToLonLat(double &xCoord, double &yCoord) {
    ...
    if (!rc) {
        MyBase::SetErrMsg("Could not convert point %f, %f to Lon/Lat", coords[0], coords[1]);
    }
...
void VizFeatureRenderer::renderText(double text, double llx, double lly, double llz) {
    ...
    //	cout << "A " <<  aa->GetAxisDataMgr() << " " << axisColor[0] << " " << axisColor[1] << " "
    //<< axisColor[2] << endl; 	cout << "B " << aa->GetAxisDataMgr() << " " << txtBackground[0] << "
    //" << txtBackground[1] << " " << txtBackground[2] << endl << endl; 	axisColor.push_back(1.f);
    //// alpha channel 	txtBackground.push_back(1.f);	// alpha channel
