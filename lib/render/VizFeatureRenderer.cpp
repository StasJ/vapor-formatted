...
//
//----------------------------------------------------------------------------
VizFeatureRenderer::VizFeatureRenderer(const ParamsMgr *pm, const DataStatus *dataStatus,
                                       string winName) {
    m_paramsMgr = pm;
...
void VizFeatureRenderer::DrawText(vector<billboard> billboards) {
    ...
    for (int i = 0; i < billboards.size(); i++) {
        ...
        _textObject->Initialize(_fontFile, text, size, coords, 0, txtColor, bgColor);
...
void VizFeatureRenderer::drawAxisTics() {
    ...
    //	cout << "GetAxisOrigin() " << axisOriginCoord[0] << " " << axisOriginCoord[1] << " " <<
    //axisOriginCoord[2] << endl;
    ...
    //	cout << "GetTicSize() " << ticLength[0] << " " << ticLength[1] << " " << ticLength[2] <<
    //endl;
    ...
    //	cout << "GetAxisColor() " << axisColor[0] << " " << axisColor[1] << " " << axisColor[2] <<
    //endl;
...
void VizFeatureRenderer::drawAxisTics2() {
...
#ifdef DEAD
        ConvertAxes(false, ticDir, minTic, maxTic, axisOriginCoord, ticLength, minTicA, maxTicA,
                    axisOriginCoordA, ticLengthA);
#endif
    } else {
    ...
    if (numTics[0] > 1 && ticLength[0] > 0.f) {
        ...
        for (int i = 0; i < numTics[0]; i++) {
            pointOnAxis[0] =
                sticMin[0] + (float)i * (sticMax[0] - sticMin[0]) / (float)(numTics[0] - 1);
    ...
    if (numTics[1] > 1 && sticLen[1] > 0.f) {
        ...
        for (int i = 0; i < numTics[1]; i++) {
            pointOnAxis[1] =
                sticMin[1] + (float)i * (sticMax[1] - sticMin[1]) / (float)(numTics[1] - 1);
            vsub(pointOnAxis, ticVec, drawPosn);
            glVertex3dv(drawPosn);
            vadd(pointOnAxis, ticVec, drawPosn);
            glVertex3dv(drawPosn);
    ...
    if (numTics[2] > 1 && sticLen[2] > 0.f) {
        ...
        for (int i = 0; i < numTics[2]; i++) {
            pointOnAxis[2] =
                sticMin[2] + (float)i * (sticMax[2] - sticMin[2]) / (float)(numTics[2] - 1);
            vsub(pointOnAxis, ticVec, drawPosn);
            glVertex3dv(drawPosn);
            vadd(pointOnAxis, ticVec, drawPosn);
            glVertex3dv(drawPosn);
    ...
    if (textSize > 0 && !_textObjectsValid) {
        ...
        if (numTics[0] > 1 && ticLengthA[0] > 0.f) {
            ...
            for (int i = 0; i < numTics[0]; i++) {
                pointOnAxis[0] =
                    minTicA[0] + (float)i * (maxTicA[0] - minTicA[0]) / (float)(numTics[0] - 1);
                doubleToString(pointOnAxis[0], labelText, digits);
                int objNum =
                    TextObject::addTextObject(this, GetAppPath("VAPOR", "share", fontpath).c_str(),
        ...
        if (numTics[1] > 1 && ticLengthA[1] > 0.f) {
            ...
            for (int i = 0; i < numTics[1]; i++) {
                pointOnAxis[1] =
                    minTicA[1] + (float)i * (maxTicA[1] - minTicA[1]) / (float)(numTics[1] - 1);
        ...
        if (numTics[2] > 1 && ticLengthA[2] > 0.f) {
            ...
            for (int i = 0; i < numTics[2]; i++) {
                pointOnAxis[2] =
                    minTicA[2] + (float)i * (maxTicA[2] - minTicA[2]) / (float)(numTics[2] - 1);
...
void VizFeatureRenderer::flatConvertFromLonLat(double x[2], double minLon, double maxLon,
...
void VizFeatureRenderer::ConvertAxes(bool toLatLon, const vector<long> ticDirs,
                                     const vector<double> fromMinTic,
                                     const vector<double> fromMaxTic,
                                     const vector<double> fromOrigin,
                                     const vector<double> fromTicLength, double toMinTic[3],
                                     double toMaxTic[3], double toOrigin[3],
    ...
    for (int j = 0; j < 3; j++) {
        ...
        // Determine ticLength as a fraction of maxTic-minTic along the direction in which the tic
        // is pointed Ignore tics that point in the z direction
        if (ticDirs[j] == 2)
            continue;
