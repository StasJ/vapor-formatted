...
VolumeRenderer::VolumeRenderer(const ParamsMgr *pm, std::string &winName, std::string &dataSetName,
                               std::string &instName, DataMgr *dataMgr)
    : VolumeRenderer(pm, winName, dataSetName, VolumeParams::GetClassType(),
                     VolumeRenderer::GetClassType(), instName, dataMgr) {}
...
VolumeRenderer::~VolumeRenderer() {
    ...
    if (_VAOChunked)
        glDeleteVertexArrays(1, &_VAOChunked);
    if (_VBOChunked)
        glDeleteBuffers(1, &_VBOChunked);
    if (_cache.tf)
        delete _cache.tf;
    if (_algorithm)
        delete _algorithm;
...
void VolumeRenderer::_setShaderUniforms(const ShaderProgram *shader, const bool fast) const {
    ...
    float smallestDimension =
        min(extLengthsScaled[0], min(extLengthsScaled[1], extLengthsScaled[2]));
    float largestDimension =
        max(extLengthsScaled[0], max(extLengthsScaled[1], extLengthsScaled[2]));
    ...
    shader->SetUniform("mapOrthoMode",
                       viewpointParams->GetProjectionType() == ViewpointParams::MapOrthographic);
...
int VolumeRenderer::_loadSecondaryData() {
    ...
    if (_cache.useColorMapVar) {
        Grid *grid = _dataMgr->GetVariable(_cache.ts, _cache.colorMapVar, _cache.refinement,
                                           _cache.compression, _cache.minExt, _cache.maxExt);
        if (!grid)
            return -1;
...
glm::vec3 VolumeRenderer::_getVolumeScales() const {
    ...
    return glm::vec3(datasetScales[0] * rendererScales[0], datasetScales[1] * rendererScales[1],
                     datasetScales[2] * rendererScales[2]);
}

void VolumeRenderer::_getExtents(glm::vec3 *dataMin, glm::vec3 *dataMax, glm::vec3 *userMin,
...
std::string VolumeRenderer::_getDefaultAlgorithmForGrid(const Grid *grid) const {
    ...
    if (dynamic_cast<const UnstructuredGrid *>(grid))
        return VolumeAlgorithmNull::GetName();
...
bool VolumeRenderer::_ospEnabled() { return GetActiveParams()->GetValueLong("osp_enable", false); }
...
int VolumeRenderer::_ospInit() {
    ...
    //    Grid *grid = _dataMgr->GetVariable(p->GetCurrentTimestep(), p->GetVariableName(),
    //    p->GetRefinementLevel(), p->GetCompressionLevel(), minExt, maxExt); assert(grid);
    //    _ospLoadData(grid);
    //    delete grid;
...
int VolumeRenderer::_ospRender(bool fast) {
    ...
    ospSetFloat(_ospVolumeModel, "densityScale",
                GetActiveParams()->GetValueDouble("osp_density", 1));
...
void VolumeRenderer::_ospSetupRenderer(bool fast) {
    ...
    ospSetFloat(_ospRenderer, "volumeSamplingRate",
                (fast ? 0.1 : 1) * GetActiveParams()->GetValueDouble("osp_volumeSamplingRate", 1));
...
void VolumeRenderer::_ospApplyTransform() {
    ...
    Transform *datasetTransform =
        _paramsMgr->GetViewpointParams(_winName)->GetTransform(_dataSetName);
...
int VolumeRenderer::_ospLoadData(const Grid *grid) {
    ...
    if (p->GetValueLong("osp_test_volume", 0))
        volume = _ospLoadVolumeTest(grid);
    else if (dynamic_cast<const RegularGrid *>(grid) || p->GetValueLong("osp_force_regular", false))
        volume = _ospLoadVolumeRegular(grid);
    else if (dynamic_cast<const StructuredGrid *>(grid))
        volume = _ospLoadVolumeStructured(grid);
    else if (dynamic_cast<const UnstructuredGrid *>(grid))
        volume = _ospLoadVolumeUnstructured(grid);
    else
...
const char *to_string(WindingOrder o) { return o == CCW ? "CCW" : "CW"; }
...
OSPVolume VolumeRenderer::_ospLoadVolumeUnstructured(const Grid *grid) {
    ...
    for (unsigned i = 0; i < cellStarts.size(); i++) {
        ...
        if (type == OSP_WEDGE) {
            ...
            //            if (CW == GetWindingOrderTetra(coords[cellIndices[start+0]],
            //            coords[cellIndices[start+1]], coords[cellIndices[start+2]],
            //            coords[cellIndices[start+3]]))
            //                swap(cellIndices[start+1], cellIndices[start+2]);
            //            assert(CCW == GetWindingOrderTetra(coords[cellIndices[start+0]],
            //            coords[cellIndices[start+1]], coords[cellIndices[start+2]],
            //            coords[cellIndices[start+3]]));
        }
    }
    ...
    for (auto i : cellStarts)
        assert(i < cellIndices.size());
    for (auto i : cellTypes)
        assert(i == OSP_WEDGE || i == OSP_TETRAHEDRON);
    assert(cellStarts[cellStarts.size() - 1] +
