...
int VolumeRenderer::OSPRayUpdate(OSPModel world) {
    ...
    ospSet1b(_volume, "gradientShadingEnabled",
             vp->GetValueLong(VolumeParams::OSPRayLightingEnabledTag, true));
...
OSPVolume VolumeRenderer::OSPRayCreateVolumeFromUnstructuredGrid(const Grid *grid,
    ...
    for (auto cellIt = grid->ConstCellBegin(); cellIt != cellEnd; ++cellIt) {
        ...
        if (numNodes == 4) {
            ...
            for (int i = 0; i < 4; i++) {
                vec3 originalCoord(coords[i * coordDim], coords[i * coordDim + 1],
                                   coords[i * coordDim + 2]);
