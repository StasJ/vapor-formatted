...
int VolumeRenderer::_paintGL(bool fast) {
    ...
    CheckCache(_cache.useOSPRay,
               GetActiveParams()->GetValueLong(RenderParams::OSPRayEnabledTag, false));
...
int VolumeRenderer::_loadData() {
    ...
    Grid *grid =
        _dataMgr->GetVariable(_cache.ts, _cache.var, _cache.refinement, _cache.compression);
    if (!grid)
        return -1;
...
int VolumeRenderer::OSPRayUpdate(OSPModel world) {
    ...
    if (OSPRayLoadData(world) < 0)
        return -1;
    if (OSPRayLoadSecondaryData(world) < 0)
        return -1;
    if (OSPRayLoadTF() < 0)
        return -1;
    ...
    ospSet1f(_volume, "adaptiveMaxSamplingRate",
             samplingRate * OSPRAY_ADAPTIVE_SAMPLING_MULTIPLIER);
...
OSPVolume VolumeRenderer::OSPRayCreateVolumeFromRegularGrid(const Grid *grid,
    ...
    ospSetRegion(volume, volumeData, {0, 0, 0},
...
OSPVolume VolumeRenderer::OSPRayCreateVolumeFromStructuredGrid(const Grid *grid,
                                                               const glm::mat4 &transform) {
...
OSPVolume VolumeRenderer::OSPRayCreateVolumeFromUnstructuredGrid(const Grid *grid,
                                                                 const glm::mat4 &transform) {
    ...
    for (size_t cellCounter = 0; cellCounter < nCells; ++cellIt, ++cellCounter) {
        ...
        if (hasMissing) {
            ...
            for (int i = 0; i < 8; i++)
                if (cells[cellId - 1].i[i] >= 0 &&
                    scalarData[cells[cellId - 1].i[i]] == missingValue)
                    cellHasMissing = true;
            if (cellHasMissing)
                cellId--;
        }
...
int VolumeRenderer::OSPRayLoadTF(OSPVolume volume, OSPTransferFunction *ospTF,
                                 MapperFunction *vTF) {
    VAssert(volume);
    float *LUT = new float[4 * 256];
    vTF->makeLut(LUT);
