...
int VolumeRenderer::_initializeGL() {
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, _framebufferTexture,
                           0);
...
int VolumeRenderer::_paintGL(bool fast) {
    ...
    glBindTexture(GL_TEXTURE_2D, _depthTexture);
    glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, viewport[0], viewport[1], viewport[2],
                     viewport[3], 0);
    ...
    if (useFramebuffer) {
        ...
        if (fbSize[0] != _framebufferSize[0] || fbSize[1] != _framebufferSize[1]) {
            ...
            glBindTexture(GL_TEXTURE_2D, _framebufferTexture);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, fbSize[0], fbSize[1], 0, GL_RGBA,
                         GL_UNSIGNED_BYTE, NULL);
            glBindTexture(GL_TEXTURE_2D, _framebufferDepthTexture);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, fbSize[0], fbSize[1], 0,
