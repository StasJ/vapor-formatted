...
bool DirectVolumeRenderer::UserCoordinates::Fill(const DVRParams *params, DataMgr *dataMgr) {
    ...
    VAPoR::StructuredGrid *grid = dynamic_cast<VAPoR::StructuredGrid *>(dataMgr->GetVariable(
...
DirectVolumeRenderer::~DirectVolumeRenderer() {
    /* A few useful files from VAPOR2:
         ...
         DVRShader.h/cpp
    */
    if (_volumeTextureUnit)
        glDeleteTextures(1, &_volumeTextureUnit);
    if (_colormapTextureUnit)
        glDeleteTextures(1, &_colormapTextureUnit);
...
int DirectVolumeRenderer::_paintGL() {
    if (!_userCoordinates.isFilled())
        _userCoordinates.Fill(dynamic_cast<DVRParams *>(GetActiveParams()), _dataMgr);
...
void DirectVolumeRenderer::_initializeTextures() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, viewport[2], viewport[3], 0, GL_RGB, GL_FLOAT,
                 nullptr);
