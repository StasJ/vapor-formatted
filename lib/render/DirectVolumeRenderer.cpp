...
void DirectVolumeRenderer::_initializeTextures() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, viewport[2], viewport[3], 0, GL_RGB, GL_UNSIGNED_BYTE,
                 0);
...
void DirectVolumeRenderer::_drawVolumeFaces(const float *frontFace, const float *backFace,
    ...
    if (frontFacing) {
        ...
        glCullFace(GL_FRONT);
        glDepthFunc(
            GL_GREATER); // Accept fragment if it is farther to the camera than the former one
    }
...
void DirectVolumeRenderer::_drawQuad() {
    ...
    const GLfloat quadVertices[] = {boxmin[0], boxmax[1], boxmin[2], boxmin[0],
                                    boxmin[1], boxmin[2], boxmax[0], boxmax[1],
                                    boxmin[2], boxmax[0], boxmin[1], boxmin[2]};
