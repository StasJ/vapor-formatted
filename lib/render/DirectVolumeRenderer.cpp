...
// Constructor
DirectVolumeRenderer::DirectVolumeRenderer(const ParamsMgr *pm, std::string &winName,
                                           std::string &dataSetName, std::string &instName,
                                           DataMgr *dataMgr)
    : Renderer(pm, winName, dataSetName, DVRParams::GetClassType(),
               DirectVolumeRenderer::GetClassType(), instName, dataMgr) {
...
bool DirectVolumeRenderer::UserCoordinates::isFilled() {
    if (frontFace == nullptr || backFace == nullptr || rightFace == nullptr ||
        leftFace == nullptr || topFace == nullptr || bottomFace == nullptr || field == nullptr ||
        missingValueMask == nullptr)
        return false;
    else
        return true;
...
bool DirectVolumeRenderer::UserCoordinates::Fill(const DVRParams *params, DataMgr *dataMgr) {
    ...
    StructuredGrid *grid = dynamic_cast<StructuredGrid *>(dataMgr->GetVariable(
        params->GetCurrentTimestep(), params->GetVariableName(), params->GetRefinementLevel(),
    ...
    if (grid == nullptr) {
        ...
        StructuredGrid::ConstCoordItr coordItr =
            grid->ConstCoordBegin(); // Iterator for coordinates

        float valueRange[2];
        grid->GetRange(valueRange);
        float valueRange1o = 1.0f / (valueRange[1] - valueRange[0]);
        float boxExtent1o[] = {1.0f / (boxMax[0] - boxMin[0]), 1.0f / (boxMax[1] - boxMin[1]),
        ...
        if (grid->HasMissingData()) {
            ...
            for (size_t i = 0; i < numOfVertices; i++) {
                field[i * 4] =
                    ((float)((*coordItr).at(0)) - boxMin[0]) * boxExtent1o[0]; // normalized X
                field[i * 4 + 1] =
                    ((float)((*coordItr).at(1)) - boxMin[1]) * boxExtent1o[1]; // normalized Y
                field[i * 4 + 2] =
                    ((float)((*coordItr).at(2)) - boxMin[2]) * boxExtent1o[2]; // normalized Z
                value = (float)(*valItr);
...
int DirectVolumeRenderer::_paintGL() {
    ...
    if (!_userCoordinates.isFilled()) {
        ...
        glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA32F, _userCoordinates.dims[0],
                     _userCoordinates.dims[1], _userCoordinates.dims[2], 0, GL_RGBA, GL_FLOAT,
                     _userCoordinates.field);
        glBindTexture(GL_TEXTURE_3D, _missingValueTextureId);
        glTexImage3D(GL_TEXTURE_3D, 0, GL_R8, _userCoordinates.dims[0], _userCoordinates.dims[1],
                     _userCoordinates.dims[2], 0, GL_RED, GL_UNSIGNED_BYTE,
    ...
    _drawVolumeFaces(_userCoordinates.frontFace, _userCoordinates.backFace,
                     _userCoordinates.rightFace, _userCoordinates.leftFace,
                     _userCoordinates.topFace, _userCoordinates.bottomFace, _userCoordinates.boxMin,
                     _userCoordinates.boxMax, _userCoordinates.dims, 1); // The 1st pass!!!

    /* 2nd pass, render front facing polygons to texture1 of the framebuffer */
    _drawVolumeFaces(_userCoordinates.frontFace, _userCoordinates.backFace,
                     _userCoordinates.rightFace, _userCoordinates.leftFace,
                     _userCoordinates.topFace, _userCoordinates.bottomFace, _userCoordinates.boxMin,
                     _userCoordinates.boxMax, _userCoordinates.dims, 2); // The 2nd pass!!!
    ...
    _drawVolumeFaces(_userCoordinates.frontFace, _userCoordinates.backFace,
                     _userCoordinates.rightFace, _userCoordinates.leftFace,
                     _userCoordinates.topFace, _userCoordinates.bottomFace, _userCoordinates.boxMin,
                     _userCoordinates.boxMax, _userCoordinates.dims, 3); // The 3rd pass!!!
...
void DirectVolumeRenderer::_initializeFramebufferTextures() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, viewport[2], viewport[3], 0, GL_RGBA, GL_FLOAT,
                 nullptr);
    ...
    /* Set "_backFaceTextureId" as colour attachement #0, and "_frontFaceTextureId" as attachement
     * #1 */
...
void DirectVolumeRenderer::_drawVolumeFaces(const float *frontFace, const float *backFace,
                                            ...
                                            const size_t *dims, int whichPass) {
    ...
    } else                                   // perform ray-casting
    {
        ...
        glBindTexture(GL_TEXTURE_3D, _missingValueTextureId);
        GLuint missingValueTextureLoc =
            glGetUniformLocation(_3rdPassShaderId, "missingValueMaskTexture");
...
void DirectVolumeRenderer::_drawQuad() {
    const GLfloat quadVertices[] = {
        _userCoordinates.boxMin[0], _userCoordinates.boxMax[1], _userCoordinates.boxMin[2],
        _userCoordinates.boxMin[0], _userCoordinates.boxMin[1], _userCoordinates.boxMin[2],
        _userCoordinates.boxMax[0], _userCoordinates.boxMax[1], _userCoordinates.boxMin[2],
        _userCoordinates.boxMax[0], _userCoordinates.boxMin[1], _userCoordinates.boxMin[2]};
