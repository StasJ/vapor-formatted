...
int ModelRenderer::Model::Load(const std::string &path) {
    ...
    _scene =
        _importer.ReadFile(path, aiProcessPreset_TargetRealtime_Quality | aiProcess_Triangulate);
...
std::vector<ModelRenderer::Scene::ModelInstance>
ModelRenderer::Scene::getInstances(const int ts) const {
    VAssert(ts >= 0);
...
int ModelRenderer::Scene::handleInstanceNode(XmlNode *node, ModelInstance *instance) {
    ...
    if (doesInstanceExist(instance->name)) {
        ...
        if (instance->file.empty()) {
            MyBase::SetErrMsg("Instance \"%s\" defined without an associated file",
                              node->Tag().c_str());
            return -1;
        }
...
int ModelRenderer::Scene::handleFloatAttribute(XmlNode *node, const std::string &name, float *f) {
    ...
    try {
        *f = std::stof(valueString, &charsRead);
        ...
        MyBase::SetErrMsg("Float attribute out of range %s=\"%s\"", name.c_str(),
                          valueString.c_str());
        return -1;
    }
...
ModelRenderer::Scene::ModelInstance
ModelRenderer::Scene::getInitInstance(const std::string &name) const {
