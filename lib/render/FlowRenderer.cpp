...
FlowRenderer::FlowRenderer(const ParamsMgr *pm, std::string &winName, std::string &dataSetName,
    ...
    : Renderer(pm, winName, dataSetName, FlowParams::GetClassType(), FlowRenderer::GetClassType(),
               instName, dataMgr),
      _velocityField(8), _colorField(2), _colorMapTexOffset(0) {}
...
int FlowRenderer::_paintGL(bool fast) {
    ...
    if (params->GetNeedFlowlineOutput()) {
        ...
        if (params->GetIsSteady()) {
            rv = _advection.OutputStreamsGnuplotMaxPart(params->GetFlowlineOutputFilename(),
                                                        params->GetSteadyNumOfSteps() + 1, false);
        } else {
            rv = _advection.OutputStreamsGnuplotMaxTime(
                params->GetFlowlineOutputFilename(), _timestamps.at(params->GetCurrentTimestep()),
                false);
        }
        if (rv != 0) {
        ...
        if (_2ndAdvection) // bi-directional advection
        {
            if (params->GetIsSteady()) {
                rv = _2ndAdvection->OutputStreamsGnuplotMaxPart(
                    params->GetFlowlineOutputFilename(), params->GetSteadyNumOfSteps() + 1, true);
            } else {
                rv = _2ndAdvection->OutputStreamsGnuplotMaxTime(
                    params->GetFlowlineOutputFilename(),
                    _timestamps.at(params->GetCurrentTimestep()), true);
            }
            if (rv != 0) {
                MyBase::SetErrMsg("Output flow lines wrong!");
                return flow::FILE_ERROR;
    ...
    if (!_advectionComplete) {
         ...
         * This scheme is used for unsteady flow */
            ...
            for (int i = 1; i <= _cache_currentTS; i++) {
                rv = _advection.AdvectTillTime(&_velocityField, _timestamps.at(i - 1), deltaT,
                                               _timestamps.at(i));
            }
        }
...
int FlowRenderer::_renderAdvection(const flow::Advection *adv) {
    ...
    if (_renderStatus != FlowStatus::UPTODATE) {
        ...
        double startingTime;
        if (!_cache_isSteady) {
            int pastNumOfTimeSteps =
                dynamic_cast<FlowParams *>(GetActiveParams())->GetPastNumOfTimeSteps();
...
int FlowRenderer::_renderAdvectionHelper(bool renderDirection) {
    ...
    FlowParams::RenderType renderType = (FlowParams::RenderType)rp->GetValueLong(
        FlowParams::RenderTypeTag, FlowParams::RenderTypeStream);
    FlowParams::GlpyhType glyphType = (FlowParams::GlpyhType)rp->GetValueLong(
        FlowParams::RenderGlyphTypeTag, FlowParams::GlpyhTypeSphere);
    ...
    if (radiusBase == -1) {
        ...
        _dataMgr->GetVariableExtents(rp->GetCurrentTimestep(), rp->GetColorMapVariableName(),
                                     rp->GetRefinementLevel(), rp->GetCompressionLevel(), mind,
                                     maxd);
    ...
    _paramsMgr->GetViewpointParams(_winName)->ReconstructCamera(m, cameraPosD, cameraUpD,
                                                                cameraDirD);
    ...
    shader->SetUniform("phongShininess",
                       (float)rp->GetValueDouble(FlowParams::PhongShininessTag, 0));
    ...
    shader->SetUniform("fade_length",
                       (int)rp->GetValueLong(FlowParams::RenderFadeTailLengthTag, 10));
    ...
    shader->SetUniform("falloff",
                       (float)rp->GetValueDouble(FlowParams::RenderDensityFalloffTag, 1));
    shader->SetUniform("tone",
                       (float)rp->GetValueDouble(FlowParams::RenderDensityToneMappingTag, 1));
...
int FlowRenderer::_renderFromAnAdvectionLegacy(const flow::Advection *adv, FlowParams *params,
                                               bool fast) {
...
int FlowRenderer::_updateFlowCacheAndStates(const FlowParams *params) {
    ...
    if (diff) {
        _cache_rake = rake;
        if (_cache_seedGenMode != FlowSeedMode::LIST) { // Mark out-of-date if we're currently using
                                                        // any mode that involves a rake
            _colorStatus = FlowStatus::SIMPLE_OUTOFDATE;
            _velocityStatus = FlowStatus::SIMPLE_OUTOFDATE;
        }
    }
...
void FlowRenderer::_dupSeedsNewTime(std::vector<flow::Particle> &seeds, size_t firstN,
                                    float newTime) const {
...
int FlowRenderer::_genSeedsRakeUniform(std::vector<flow::Particle> &seeds) const {
    ...
    for (int i = 0; i < dim; i++) // for each of the X, Y, Z dimensions
    {
        ...
        {
            start[i] = _cache_rake[i * 2] + 0.5f * (_cache_rake[i * 2 + 1] - _cache_rake[i * 2]);
            step[i] = 0.0f;
        } else // more than one seed in this dimension
        {
            start[i] = _cache_rake[i * 2];
            step[i] =
                (_cache_rake[i * 2 + 1] - _cache_rake[i * 2]) / float(_cache_gridNumOfSeeds[i] - 1);
        }
    }
...
int FlowRenderer::_genSeedsRakeRandomBiased(std::vector<flow::Particle> &seeds) const {
    ...
    Grid *grid = _dataMgr->GetVariable(params->GetCurrentTimestep(), _cache_rakeBiasVariable,
                                       params->GetRefinementLevel(), params->GetCompressionLevel(),
                                       rakeExtMin, rakeExtMax);
    if (grid == nullptr) {
