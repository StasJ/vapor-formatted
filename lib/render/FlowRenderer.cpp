...
int FlowRenderer::_paintGL(bool fast) {
    ...
    if (!_advectionComplete) {
        ...
        if (params->GetIsSteady()) {
            ...
            for (int i = _advection.GetMaxNumOfSteps();
            ...
            if (_2ndAdvection) {
                ...
                rv = flow::ADVECT_HAPPENED;
                for (int i = _2ndAdvection->GetMaxNumOfSteps();
                     i < numOfSteps && rv == flow::ADVECT_HAPPENED; i++) {
...
void FlowRenderer::_updateFlowCacheAndStates(const FlowParams *params) {
    ...
    if ((_cache_periodic[0] != peri.at(0)) || (_cache_periodic[1] != peri.at(1)) ||
...
int FlowRenderer::_genSeedsRakeUniform(std::vector<flow::Particle> &seeds, float timeVal) const {
    FlowParams *params = dynamic_cast<FlowParams *>(GetActiveParams());
    VAssert(params);
...
int FlowRenderer::_genSeedsRakeRandom(std::vector<flow::Particle> &seeds, float timeVal) const {
    FlowParams *params = dynamic_cast<FlowParams *>(GetActiveParams());
...
int FlowRenderer::_genSeedsRakeRandomBiased(std::vector<flow::Particle> &seeds,
                                            float timeVal) const {
    FlowParams *params = dynamic_cast<FlowParams *>(GetActiveParams());
    ...
    Grid *grid =
        _dataMgr->GetVariable(params->GetCurrentTimestep(), biasVar, params->GetRefinementLevel(),
                              params->GetCompressionLevel(), rakeExtMin, rakeExtMax);
    if (grid == nullptr) {
    ...
    while (seedIdx < totalNumOfSeeds) {
        const std::vector<double> cand{distX(gen), distY(gen),
                                       distZ(gen)}; // generate a random seed
        float seedVal = grid->GetValue(cand);
        float prob = std::abs(std::abs(seedVal) - rakeMin) *
                     rakeRange1o;           // prob is a value between 0.0 and 1.0;
...
void FlowRenderer::_updatePeriodicity(flow::Advection *advc) {
