...
//
//----------------------------------------------------------------------------
TwoDRenderer::TwoDRenderer(const ParamsMgr *pm, string winName, string paramsType, string classType,
                           string instName, DataStatus *ds)
    : Renderer(pm, winName, paramsType, classType, instName, ds) {
    _textureID = 0;
    _texture = NULL;
    _texCoords = NULL;
    _texWidth = 0;
...
int TwoDRenderer::_paintGL() {
    ...
    // Get the 2D texture
    //
    _texture = _GetTexture(dataMgr, _texWidth, _texHeight, _texInternalFormat, _texFormat, _texType,
                           _texelSize);
    if (!_texture) {
        return (-1);
    }
...
void TwoDRenderer::_renderMesh() {
    ...
    for (int j = 0; j < _meshHeight - 1; j++) {
        ...
        glDrawElements(GL_TRIANGLE_STRIP, 2 * _meshWidth, GL_UNSIGNED_INT, indeces);
    }
