...
int TwoDRenderer::_paintGL(bool) {
    ...
    // normals)
    //
    int rc = GetMesh(_dataMgr, &_verts, &_normals, _meshWidth, _meshHeight, &_indices, _nindices,
                     _structuredMesh);
    if (rc < 0) {
        return (-1);
    }
...
void TwoDRenderer::_renderMeshAligned() {
    ...
    if (_structuredMesh) {
        ...
        GL_ERR_BREAK();
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _EBO);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, _nindices * sizeof(GLuint), _indices,
                     GL_DYNAMIC_DRAW);
        GL_ERR_BREAK();
        glBindBuffer(GL_ARRAY_BUFFER, _VBO);
