...
int TwoDRenderer::_paintGL() {
    ...
    // Get the 2D texture
    //
    _texture = _getTexture(_dataMgr, _texWidth, _texHeight, _texInternalFormat, _texFormat,
                           _texType, _texelSize, _gridAligned);
    if (!_texture) {
        return (-1);
    }
    assert(_texWidth >= 1);
    ...
    // normals)
    //
    int rc = _getMesh(_dataMgr, &_verts, &_normals, _meshWidth, _meshHeight, &_indices, _nindices,
                      _structuredMesh);
    if (rc < 0) {
        return (-1);
    }
...
void TwoDRenderer::_renderMeshAligned() {
    ...
    if (_structuredMesh) {
        ...
        glVertexAttribPointer(attrindx, 2, GL_FLOAT, false, 0, data);
