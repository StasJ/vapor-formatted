...
int BarbRenderer::performRendering( // DataMgr* dataMgr,
    const RenderParams *params, int actualRefLevel, float vectorLengthScale,
...
float BarbRenderer::getHeightOffset(StructuredGrid *heightVar, float xCoord, float yCoord,
                                    bool &missing) {
    assert(heightVar);
...
void BarbRenderer::renderScottsGrid(int rakeGrid[3], double rakeExts[6],
                                    ...
                                    float vectorLengthScale, float rad,
...
void BarbRenderer::renderUnaligned(int rakeGrid[3], double rakeExts[6],
                                   StructuredGrid *variableData[5], int timestep,
    ...
    for (int k = 0; k < rakeGrid[2]; k++) {
        float pntz =
            (rakeExts[2] + (0.5 + (float)k) * ((rakeExts[5] - rakeExts[2]) / (float)rakeGrid[2]));

        for (int j = 0; j < rakeGrid[1]; j++) {
            point[1] = (rakeExts[1] +
                        (0.5 + (float)j) * ((rakeExts[4] - rakeExts[1]) / (float)rakeGrid[1]));

            for (int i = 0; i < rakeGrid[0]; i++) {
                point[0] = (rakeExts[0] +
                            (0.5 + (float)i) * ((rakeExts[3] - rakeExts[0]) / (float)rakeGrid[0]));
                bool missing = false;
                float offset = 0.;
                if (variableData[3]) {
                ...
                for (int dim = 0; dim < 3; dim++) {
                    ...
                    if (variableData[dim]) {
                        ...
                        dirVec[dim] =
                            variableData[dim]->GetValue(point[0], point[1], point[2] + offset);
                    ...
                    endPoint[dim] =
                        scales[dim] * (point[dim] - minExts[dim] + vectorLengthScale * dirVec[dim]);
...
void BarbRenderer::renderAligned(int rakeGrid[3], double rakeExts[6],
    ...
    for (int k = 0; k < rakeGrid[2]; k++) {
        ...
        for (int j = 0; j < rakeGrid[1]; j++) {
            point[1] =
                (rakeExts[1] + ((double)j) * ((rakeExts[4] - rakeExts[1]) / (double)rakeGrid[1]));

            for (int i = 0; i < rakeGrid[0]; i++) {
                bool missing = false;
                float offset = 0.;
                point[0] =
                    rakeExts[0] + ((double)i) * ((rakeExts[3] - rakeExts[0]) / (double)rakeGrid[0]);
                if (variableData[3]) {
                    offset = variableData[3]->GetValue(point[0], point[1], 0.);
                    if (offset == variableData[3]->GetMissingValue()) {
                        missing = true;
