...
int BarbRenderer::_paintGL() {
    ...
    //
    rc = DataMgrUtils::GetGrids(_dataMgr, ts, varnames, minExts, maxExts, true, &refLevel, &lod,
                                varData);
...
void BarbRenderer::drawBarb(const float startPoint[3], const float endPoint[3]) {
    ...
    // pointD[0] << "\t\t" << pointE[0] << "\t\t" << pointF[0] << endl; cout << "   " << pointA[1] <<
    // "\t\t" << pointB[1] << "\t\t" << pointC[1] << "\t" << pointD[1] << "\t" << pointE[1] << "\t\t"
    // << pointF[1] << endl; cout << "   " << pointA[2] << "\t\t" << pointB[2] << "\t\t" << pointC[2]
    // << "\t\t" << pointD[2] << "\t\t" << pointE[2] << "\t\t" << pointF[2] << endl;
...
int BarbRenderer::performRendering(BarbParams *bParams, int actualRefLevel,
...
void BarbRenderer::renderGrid(int rakeGrid[3], double rakeExts[6], vector<Grid *> variableData,
                              int timestep, BarbParams *bParams) {
    assert(variableData.size() == 5);
    ...
    for (int k = 1; k <= rakeGrid[Z]; k++) {
        ...
        for (int j = 1; j <= rakeGrid[Y]; j++) {
            ...
            for (int i = 1; i <= rakeGrid[X]; i++) {
                ...
                cout << "_getMaxAtBarbLocations " << xCoord << " " << yCoord << " " << zCoord
                     << endl;
...
void BarbRenderer::_setDefaultLengthAndThicknessScales(size_t ts, const vector<string> &varnames,
