...
void BarbRenderer::_saveCacheParams() {
    ...
    for (int i = 0; i < 10; i++) {
        float point = _cacheParams.minMapValue +
                      i / 10.f * (_cacheParams.maxMapValue - _cacheParams.minMapValue);
...
bool BarbRenderer::_isCacheDirty() const {
    BarbParams *p = dynamic_cast<BarbParams *>(GetActiveParams());
    assert(p);
    if (_cacheParams.fieldVarNames != p->GetFieldVariableNames())
        return true;
    if (_cacheParams.heightVarName != p->GetHeightVariableName())
        return true;
    if (_cacheParams.colorVarName != p->GetColorMapVariableName())
        return true;
    ...
    if (_cacheParams.lod != p->GetCompressionLevel())
        return true;
    if (_cacheParams.useSingleColor != p->UseSingleColor())
        return true;
    if (_cacheParams.lineThickness != p->GetLineThickness())
        return true;
    if (_cacheParams.lengthScale != p->GetLengthScale())
        return true;
    if (_cacheParams.grid != p->GetGrid())
        return true;
    ...
    if (memcmp(_cacheParams.constantColor, constantColor, sizeof(constantColor)))
        return true;
    ...
    MapperFunction *tf = p->GetMapperFunc(_cacheParams.colorVarName);
    if (_cacheParams.opacity != tf->getOpacityScale())
        return true;
    if (_cacheParams.minMapValue != tf->getMinMapValue())
        return true;
    if (_cacheParams.maxMapValue != tf->getMaxMapValue())
        return true;
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (_cacheParams.colorSamples[i][0] != color[0])
            return true;
        if (_cacheParams.colorSamples[i][1] != color[1])
            return true;
        if (_cacheParams.colorSamples[i][2] != color[2])
            return true;
        if (_cacheParams.alphaSamples[i] != tf->getOpacityValueData(point))
            return true;
    }
...
int BarbRenderer::_getVectorVarGrids(int ts, int refLevel, int lod, std::vector<double> minExts,
    ...
    //
    int rc = DataMgrUtils::GetGrids(_dataMgr, ts, varnames, minExts, maxExts, true, &refLevel, &lod,
                                    varData);

    return rc;
}

void BarbRenderer::_getGridRequirements(int &ts, int &refLevel, int &lod,
...
int BarbRenderer::_getHeightVarGrid(int ts, int refLevel, int lod, std::vector<double> minExts,
                                    std::vector<double> maxExts,
                                    std::vector<VAPoR::Grid *> &varData) {
    BarbParams *bParams = dynamic_cast<BarbParams *>(GetActiveParams());
    assert(bParams);
    ...
    if (!hname.empty()) {
        Grid *sg = NULL;
        int rc = DataMgrUtils::GetGrids(_dataMgr, ts, hname, minExts, maxExts, true, &refLevel,
                                        &lod, &sg);
        if (rc < 0) {
...
void BarbRenderer::_drawCylinderSides(const float nextNormal[3], const float nextVertex[3],
                                      const float startNormal[3],
                                      const float startVertex[3]) const {
...
void BarbRenderer::_drawBarbHead(const float dirVec[3], const float vertexPoint[3],
                                 const float startNormal[3], const float startVertex[3]) const {
...
//
void BarbRenderer::_drawBarb(const std::vector<Grid *> variableData, float startPoint[3],
                             bool doColorMapping, float clut[1024]) {
    ...
    const float sines[6] = {0.f, (float)(sqrt(3.) / 2.),  (float)(sqrt(3.) / 2.),
                            0.f, (float)(-sqrt(3.) / 2.), (float)(-sqrt(3.) / 2.)};
...
float BarbRenderer::_getHeightOffset(Grid *heightVar, float xCoord, float yCoord,
                                     bool &missing) const {
    assert(heightVar);
...
void BarbRenderer::_getDirection(float direction[3], vector<Grid *> variableData, float xCoord,
                                 float yCoord, float zCoord, bool &missing) const {
...
void BarbRenderer::_getStrides(vector<float> &strides, vector<int> &rakeGrid,
...
bool BarbRenderer::_defineBarb(std::vector<Grid *> variableData, float start[3], float end[3],
                               bool doColorMapping, float clut[1024]) {
    bool missing = false;

    Grid *heightVar = variableData[3];
    if (heightVar) {
        start[Z] += _getHeightOffset(heightVar, start[X], start[Y], missing);
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (grid == NULL)
            ...
            if (value > maxValue && value < std::numeric_limits<double>::max() &&
                value > std::numeric_limits<double>::lowest() && value != missingValue &&
                !isnan(value))
                maxValue = value;
        }
    }
...
double BarbRenderer::_getMaxAtBarbLocations(VAPoR::Grid *grid) const {
    ...
    for (int i = 0; i < 3; i++) {
        stride[i] = (maxExts[i] - minExts[i]) / (rakeGrid[i] + 1);
    }
...
vector<double> BarbRenderer::_getMaximumValues(size_t ts,
...
void BarbRenderer::_setDefaultLengthAndThicknessScales(size_t ts,
