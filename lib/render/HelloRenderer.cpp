...
//
static RendererRegistrar<HelloRenderer> registrar(HelloRenderer::GetClassType(),
                                                  HelloParams::GetClassType());

//----------------------------------------------------------------------------
//
//----------------------------------------------------------------------------
HelloRenderer::HelloRenderer(const ParamsMgr *pm, string winName, string instName, DataStatus *ds)
    : Renderer(pm, winName, HelloParams::GetClassType(), HelloRenderer::GetClassType(), instName,
               ds) {}
...
int HelloRenderer::_paintGL() {
    ...
    int rc = m_dataStatus->getGrids(timestep, varnames, regMin, regMax, &actualRefLevel, &lod,
                                    &helloGrid);
    if (rc) {
        return rc;
    }

    // Set the grid to use nearest-point interpolation, to calculate actual (uninterpolated) data
    // max and min
    helloGrid->SetInterpolationOrder(0);

    // In order to sample the data at the user-specified refinement level, need to determine the
    // number of voxels that the line crosses, which requires knowing the underlying grid.
    //
    ...
    // Determine the number of voxels traversed (in each dimension) as we go from the first point to
    // the second. Th maximum of these three counts is used to determine how frequently the data will
    // be sampled.
    int maxvox = 1;
    for (int i = 0; i < 3; i++) {
    ...
    // Divide the line into maxvox equal sections, sample the variable at each point along the line,
    // to find coordinates of min and max value
    ...
    // Set up lighting and color.  We will use the lighting settings from the viewpoint params for
    // rendering the lines, but lighting will be disabled for rendering the max and min points.
