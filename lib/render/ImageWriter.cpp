...
ImageWriter::ImageWriter(const std::string &path)
    : format(Format::RGB), path(path), opened(false) {}
...
ImageWriter *ImageWriter::CreateImageWriterForFile(const std::string &path) {
    ...
    for (auto factory = factories.begin(); factory != factories.end(); ++factory)
        for (auto writerExtension = (*factory)->Extensions.begin();
             writerExtension != (*factory)->Extensions.end(); ++writerExtension)
            if (*writerExtension == pathExtension)
