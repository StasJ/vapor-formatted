...
GeoTile::GeoTile(size_t tile_width, size_t tile_height, size_t pixel_size, double min_lon,
...
void GeoTile::PixelXYToTileXY(size_t pixelX, size_t pixelY, size_t &tileX, size_t &tileY,
                              size_t &tilePixelX, size_t &tilePixelY) const {
...
int GeoTile::GetMap(size_t pixelX0, size_t pixelY0, size_t pixelX1, size_t pixelY1, int lod,
                    unsigned char *map_image) const {
    ...
    for (int j = 0; j < nytiles; j++) {
        ...
        for (int i = 0; i < nxtiles; i++) {
            ...
            if (!tile)
                return (-1); // tile doesn't exist at this lod
            ...
            _CopyTileToMap(tile, tpx0, tpy0, tpx1, tpy1, map_image, map_px, map_py, map_width,
                           map_height);
...
int GeoTile::MapSize(size_t pixelX0, size_t pixelY0, size_t pixelX1, size_t pixelY1, int lod,
    ...
    if (pixelX0 > nx_global - 1)
        return (-1);
    if (pixelY0 > ny_global - 1)
        return (-1);
    if (pixelX1 > nx_global - 1)
        return (-1);
    if (pixelY1 > ny_global - 1)
        return (-1);
...
double GeoTile::_Clip(double n, double minValue, double maxValue) const {
    return std::min(std::max(n, minValue), maxValue);
}

void GeoTile::_CopyTileToMap(const unsigned char *tile, size_t tilePixelX0, size_t tilePixelY0,
                             size_t tilePixelX1, size_t tilePixelY1, unsigned char *map,
                             size_t pixelX0, size_t pixelY0, size_t nx, size_t ny) const {
