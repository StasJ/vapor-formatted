...
int MyPython::Initialize() {
    ...
    if (!m_pyHome.empty()) {
        ...
        if (STAT64((m_pyHome + "/lib/python3.6").c_str(), &statbuf) >= 0) {
            ...
            /*            std::wstring wStringPyHome = std::wstring( m_pyHome.begin(),
               m_pyHome.end() ); const wchar_t* wCharPyHome = wStringPyHome.c_str(); wchar_t*
               nonConstPyHome = const_cast<wchar_t*>(wCharPyHome);
                        //Py_SetPythonHome((char *) m_pyHome.c_str());
...
string MyPython::PyOut() {
    ...
    PyObject *catcher = NULL;
    if (pMain && PyObject_HasAttrString(pMain, "catchOut")) {
        catcher = PyObject_GetAttrString(pMain, "catchOut");
    }
