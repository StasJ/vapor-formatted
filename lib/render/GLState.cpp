...
void GLState::PushMatrix() {
    ...
    printf("%s Push mode=%s depth=%li glMode=%s glDepth=%i\n", _currentStateName.c_str(),
           GetMatrixModeStr(), _currentState->currentStack->size(), GetGLMatrixModeStr(),
           GetGLCurrentStackDepth());
...
void GLState::Test() {
    ...
    if (gl_modelview_stack_depth != _currentState->modelviewStack.size())
        printf("ModelView stack size not equal gl=%i, my=%i\n", gl_modelview_stack_depth,
               (int)_currentState->modelviewStack.size());
    if (gl_projection_stack_depth != _currentState->projectionStack.size())
        printf("Projection stack size not equal gl=%i, my=%i\n", gl_projection_stack_depth,
               (int)_currentState->projectionStack.size());

    bool equal = true;
    for (int i = 0; i < 16; i++)
        if (gl_modelview_matrix[i] != glm::value_ptr(_currentState->modelviewStack.top())[i])
            equal = false;
    if (!equal) {
        printf("ModelView matrix not equal\n");
        std::cout << "gl: " << glm::to_string(glm::make_mat4(gl_modelview_matrix)) << std::endl
                  << "my: " << glm::to_string(_currentState->modelviewStack.top()) << std::endl;
    }
    equal = true;
    for (int i = 0; i < 16; i++)
        if (gl_projection_matrix[i] != glm::value_ptr(_currentState->projectionStack.top())[i])
            equal = false;
    if (!equal) {
        printf("Projection matrix not equal\n");
        std::cout << "gl: " << glm::to_string(glm::make_mat4(gl_projection_matrix)) << std::endl
                  << "my: " << glm::to_string(_currentState->projectionStack.top()) << std::endl;
    }
}
...
const char *GLState::GetMatrixModeStr() {
    if (_currentState->mode == Mode::ModelView)
        return "ModelView";
    return "Projection";
}
