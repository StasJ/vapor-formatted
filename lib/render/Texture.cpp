...
int Texture::TexImage(int internalFormat, int width, int height, int depth, unsigned int format,
                      unsigned int type, const void *data, int level) {
    assert(Initialized());
    ...
    if (_nDims == 1)
        ...
        glTexImage3D(_type, level, internalFormat, 0, 0, 0, 0, format, type,
        ...
        for (int z = 0; z < depth; z++) {
            glTexSubImage3D(_type, level, 0, 0, z, width, height, 1, format, type,
                            &((char *)data)[width * height * z]);
        }
        //        glTexImage3D(_type, level, internalFormat, width, height, depth, 0, format, type,
        //        data);
    }
