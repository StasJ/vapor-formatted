...
//
static RendererRegistrar<TwoDDataRenderer> registrar(TwoDDataRenderer::GetClassType(),
                                                     TwoDDataParams::GetClassType());
...
//
void computeNormal(const Grid *hgtGrid, float x, float y, float dx, float dy, float mv, float &nx,
                   float &ny, float &nz) {
    nx = ny = 0.0;
    nz = 1.0;
    if (!hgtGrid)
...
TwoDDataRenderer::TwoDDataRenderer(const ParamsMgr *pm, string winName, string dataSetName,
                                   string instName, DataMgr *dataMgr)
    : TwoDRenderer(pm, winName, dataSetName, TwoDDataParams::GetClassType(),
...
int TwoDDataRenderer::_paintGL(bool fast) {
    ...
    // _shaderMgr->UploadEffectData( effect, "lightDirection", (float) 0.0, (float) 0.0,
    // (float) 1.0);
...
const GLvoid *TwoDDataRenderer::GetTexture(DataMgr *dataMgr, GLsizei &width, GLsizei &height,
...
int TwoDDataRenderer::GetMesh(DataMgr *dataMgr, GLfloat **verts, GLfloat **normals, GLsizei &nverts,
    ...
    Grid *g = NULL;
    int rc = DataMgrUtils::GetGrids(dataMgr, ts, varname, minBoxReq, maxBoxReq, true, &refLevel,
                                    &lod, &g);
    if (rc < 0)
        return (-1);
...
bool TwoDDataRenderer::_gridStateDirty() const {
    ...
    _grid_state_c current_state(_dataMgr->GetNumRefLevels(rParams->GetVariableName()),
                                rParams->GetRefinementLevel(), rParams->GetCompressionLevel(),
                                rParams->GetHeightVariableName(), dvar.GetMeshName(),
                                rParams->GetCurrentTimestep(), minExts, maxExts);
...
void TwoDDataRenderer::_gridStateSet() {
    ...
    _grid_state = _grid_state_c(_dataMgr->GetNumRefLevels(rParams->GetVariableName()),
                                rParams->GetRefinementLevel(), rParams->GetCompressionLevel(),
                                rParams->GetHeightVariableName(), dvar.GetMeshName(),
                                rParams->GetCurrentTimestep(), minExts, maxExts);
}
...
bool TwoDDataRenderer::_texStateDirty(DataMgr *dataMgr) const {
    ...
    _tex_state_c current_state(rParams->GetRefinementLevel(), rParams->GetCompressionLevel(),
                               rParams->GetVariableName(), rParams->GetCurrentTimestep(), minExts,
                               maxExts);
...
void TwoDDataRenderer::_texStateSet(DataMgr *dataMgr) {
    ...
    _tex_state =
        _tex_state_c(rParams->GetRefinementLevel(), rParams->GetCompressionLevel(),
                     rParams->GetVariableName(), rParams->GetCurrentTimestep(), minExts, maxExts);
}
...
//
int TwoDDataRenderer::_getMeshStructured(DataMgr *dataMgr, const StructuredGrid *g,
                                         double defaultZ) {
    TwoDDataParams *rParams = (TwoDDataParams *)GetActiveParams();
    ...
    for (GLuint i = 0; i < _vertsWidth; i++)
        indices[2 * i] = i;
    for (GLuint i = 0; i < _vertsWidth; i++)
        indices[2 * i + 1] = i + _vertsWidth;
...
int TwoDDataRenderer::_getMeshUnStructuredHelper(DataMgr *dataMgr, const Grid *g, double defaultZ) {
    ...
    if (!hgtvar.empty()) {
        int rc = DataMgrUtils::GetGrids(dataMgr, ts, hgtvar, minExts, maxExts, true, &refLevel,
                                        &lod, &hgtGrid);
    ...
    for (nitr = g->ConstNodeBegin(); nitr != endnitr; ++nitr) {
        ...
        //
        computeNormal(hgtGrid, coords[0], coords[1], dx, dy, mv, normals[voffset + 0],
                      normals[voffset + 1], normals[voffset + 2]);
...
//
int TwoDDataRenderer::_getMeshStructuredDisplaced(DataMgr *dataMgr, const StructuredGrid *g,
                                                  double defaultZ) {
    ...
    Grid *hgtGrid = NULL;
    int rc = DataMgrUtils::GetGrids(dataMgr, ts, hgtvar, minExtsReq, maxExtsReq, true, &refLevel,
                                    &lod, &hgtGrid);
    if (rc < 0)
        return (rc);
    assert(hgtGrid);
...
// I.e. it's planar.
//
int TwoDDataRenderer::_getMeshStructuredPlane(DataMgr *dataMgr, const StructuredGrid *g,
                                              double defaultZ) {
