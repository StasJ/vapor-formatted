...
//
void computeNormal(const StructuredGrid *hgtGrid, float x, float y, float dx, float dy, float mv,
...
bool TwoDDataRenderer::_gridStateDirty() const {
    ...
    return (refLevel != _currentRefLevel || lod != _currentLod || hgtVar != _currentHgtVar ||
            ts != _currentTimestep || boxMinExts != _currentBoxMinExts ||
...
bool TwoDDataRenderer::_texStateDirty(DataMgr *dataMgr) const {
    ...
    return (_currentTimestepTex != ts || _currentBoxMinExtsTex != boxMinExts ||
            _currentBoxMaxExtsTex != boxMaxExts || _currentVarname != varname);
}
...
//
int TwoDDataRenderer::_getMeshUnStructured(DataMgr *dataMgr, const StructuredGrid *g,
                                           double defaultZ) {
    TwoDDataParams *rParams = (TwoDDataParams *)GetActiveParams();
...
int TwoDDataRenderer::_getMeshUnStructuredHelper(DataMgr *dataMgr, const StructuredGrid *g,
                                                 double defaultZ) {
