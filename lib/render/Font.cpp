...
Font::Font(GLManager *glManager, const std::string &path, int size, FT_Library library)
...
Font::~Font() {
    ...
    for (auto it = _glyphMap.begin(); it != _glyphMap.end(); ++it)
...
glm::vec2 Font::TextDimensions(const std::string &text) {
    ...
    for (int i = 0; i < text.size(); i++) {
        ...
        if (ch.sizeY > maxHeightForLine)
            maxHeightForLine = ch.sizeY;
    }
    if (lineWidth > dimensions.x)
        dimensions.x = lineWidth;
