...
//
//----------------------------------------------------------------------------
AnnotationRenderer::AnnotationRenderer(const ParamsMgr *pm, const DataStatus *dataStatus,
                                       string winName) {
    m_paramsMgr = pm;
...
void AnnotationRenderer::_makeTransformMatrix(const Transform *transform,
    ...
    m = glm::translate(glm::mat4(1.f),
                       glm::vec3(translations[X], translations[Y], translations[Z]));
...
void AnnotationRenderer::_applyDataMgrCornerToDomain(std::vector<double> &domainExtents,
...
//  1 1 0       6
//  1 1 1       7
void AnnotationRenderer::_getDataMgrCorner(const int cornerNumber, glm::vec4 &dataMgrCorner,
...
void AnnotationRenderer::_applyDataMgrToDomainExtents(std::vector<double> &domainExtents,
...
void AnnotationRenderer::_calculateDomainExtents(std::vector<double> &domainExtents) const {
    ...
    for (int i = 0; i < names.size(); i++) {
        ...
        m_dataStatus->GetActiveExtents(m_paramsMgr, m_winName, names[i], _currentTimestep,
...
void AnnotationRenderer::scaleNormalizedCoordinatesToWorld(std::vector<double> &coords,
                                                           string dataMgrName) {
...
void AnnotationRenderer::drawAxisTics(AxisAnnotation *aa) {
    drawAxisTics(aa, std::vector<double>(), std::vector<double>());
}

void AnnotationRenderer::drawAxisTics(AxisAnnotation *aa, std::vector<double> minTic,
...
void AnnotationRenderer::_drawAxes(std::vector<double> min, std::vector<double> max,
...
void AnnotationRenderer::_drawTic(double startPosn[], double endPosn[], double width,
...
void AnnotationRenderer::renderText(double text, double coord[], AxisAnnotation *aa) {
    ...
    label.BackgroundColor =
        glm::vec4(backgroundColor[0], backgroundColor[1], backgroundColor[2], backgroundColor[3]);
