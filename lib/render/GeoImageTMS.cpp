...
unsigned char *GeoImageTMS::GetImage(size_t ts, const double pcsExtentsReq[4],
                                     string proj4StringReq, size_t maxWidthReq, size_t maxHeightReq,
    ...
    if (myGeoExtentsData[1] < minlat)
        myGeoExtentsData[1] = minlat;
    if (myGeoExtentsData[3] > maxlat)
        myGeoExtentsData[3] = maxlat;
    ...
    size_t pixelNE[2];
    size_t nx, ny;
    _geotile->LatLongToPixelXY(myGeoExtentsData[0], myGeoExtentsData[1], lod, pixelSW[0],
                               pixelSW[1]);
    _geotile->LatLongToPixelXY(myGeoExtentsData[2], myGeoExtentsData[3], lod, pixelNE[0],
                               pixelNE[1]);
...
//
string GeoImageTMS::_tilePath(string dir, size_t tileX, size_t tileY, int lod) const {
...
//
int GeoImageTMS::_getBestLOD(const double myGeoExtentsData[4], int maxWidthReq,
                             int maxHeightReq) const {
    size_t pixelSW[2];
    size_t pixelNE[2];
...
int GeoImageTMS::_getMap(const size_t pixelSW[2], const size_t pixelNE[2], int lod,
