...
SmartShaderProgram ShaderManager::GetSmartShader(const std::string name) {
    return SmartShaderProgram(GetResource(name));
}
...
bool ShaderManager::LoadResourceByKey(const std::string name) {
    ...
    shader->AddShaderFromFile(GL_VERTEX_SHADER,
                              _resourceDirectory + PATH_SEPARATOR + name + ".vert");
    shader->AddShaderFromFile(GL_FRAGMENT_SHADER,
                              _resourceDirectory + PATH_SEPARATOR + name + ".frag");
    shader->Link();
