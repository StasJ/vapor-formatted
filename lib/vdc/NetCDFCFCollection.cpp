...
int NetCDFCFCollection::Initialize(const vector<string> &files) {
    ...
    for (int i = 0; i < vars.size(); i++) {
        ...
        if (!GetTimeDimName(vars[i]).empty())
            timeDimName = GetTimeDimName(vars[i]);
        ...
        for (int j = 0; j < coordattr.size(); j++) {
            ...
            if (NetCDFCollection::VariableExists(coordattr[j]) &&
                (find(_coordinateVars.begin(), _coordinateVars.end(), coordattr[j]) ==
                 _coordinateVars.end())) {
    ...
    if (_timeCoordVars.empty() && !timeDimName.empty()) {
        DerivedVarTimeFromMem *derived_var =
            new DerivedVarTimeFromMem(timeDimName, "seconds", timeDimName, GetTimes());
...
int NetCDFCFCollection::Convert(const string from, const string to, const double *src, double *dst,
                                size_t n) const {
...
int NetCDFCFCollection::Convert(const string from, const string to, const float *src, float *dst,
                                size_t n) const {
    bool status = _udunit->Convert(from, to, src, dst, n);
...
int NetCDFCFCollection::_parse_formula(string formula_terms,
                                       map<string, string> &parsed_terms) const {
...
int NetCDFCFCollection::InstallStandardVerticalConverter(string cvar, string newvar, string units) {
    ...
    if (standard_name.compare("ocean_s_coordinate_g1") == 0) {
        derived_var = new DerivedVar_ocean_s_coordinate_g1(this, terms_map);
    } else if (standard_name.compare("ocean_s_coordinate_g2") == 0) {
        derived_var = new DerivedVar_ocean_s_coordinate_g2(this, terms_map);
    } else if (standard_name.compare("atmosphere_hybrid_sigma_pressure_coordinate") == 0) {
        derived_var = new DerivedVar_AHSPC(this, terms_map);
    } else if ((standard_name.compare("altitude") == 0) ||
...
int NetCDFCFCollection::InstallStandardTimeConverter(string cvar, string newvar, string units) {
    ...
    NetCDFCollection::DerivedVar *derived_var;
    derived_var = new DerivedVarTime(this, cvar, native_units, units);
...
void NetCDFCFCollection::InstallDerivedCoordVar(string varname, DerivedVar *derivedVar, int axis) {
...
bool NetCDFCFCollection::_IsCoordinateVar(const NetCDFSimple::Variable &varinfo) const {
    ...
    //
    if (dimnames.size() == 1 && varname == dimnames[0])
        return (true);

    if (dimnames.size() == 2 && IsTimeVarying(varname) && varname == dimnames[1])
        return (true);
...
bool NetCDFCFCollection::_IsLonCoordVar(const NetCDFSimple::Variable &varinfo) const {
    ...
    if (StrCmpNoCase(s, "grid_longitude") == 0)
        return (true);
...
bool NetCDFCFCollection::_IsLatCoordVar(const NetCDFSimple::Variable &varinfo) const {
    ...
    if (StrCmpNoCase(s, "grid_latitude") == 0)
        return (true);
...
bool NetCDFCFCollection::_IsVertCoordVar(const NetCDFSimple::Variable &varinfo) const {
    ...
    //
    if (_camZ3Exists(vector<string>(1, varinfo.GetName())))
        return (true);
...
void NetCDFCFCollection::_GetMissingValueMap(map<string, double> &missingValueMap,
                                             string &mv_attname) const {
...
NetCDFCFCollection::DerivedVar_ocean_s_coordinate_g1::DerivedVar_ocean_s_coordinate_g1(
    NetCDFCFCollection *ncdfcf, const std::map<string, string> &formula_map)
    : DerivedVar(ncdfcf) {
...
int NetCDFCFCollection::DerivedVar_ocean_s_coordinate_g1::Read(float *buf, int) {
    size_t nx = _dims[2];
    size_t ny = _dims[1];
    size_t nz = _dims[0];
    ...
    for (size_t z = 0; z < nz; z++) {
        for (size_t y = 0; y < ny; y++) {
            for (size_t x = 0; x < nx; x++) {
                buf[z * nx * ny + y * nx + x] =
                    _depth_c * _s[z] + (_depth[y * nx + x] - _depth_c) * _C[z] +
                    _eta[y * nx + x] *
                        (1 + (_depth_c * _s[z] + (_depth[y * nx + x] - _depth_c) * _C[z]) /
                                 _depth[y * nx + x]);
            }
        }
    }
...
int NetCDFCFCollection::DerivedVar_ocean_s_coordinate_g1::ReadSlice(float *slice, int) {
    size_t nx = _dims[2];
    size_t ny = _dims[1];
    size_t nz = _dims[0];
...
int NetCDFCFCollection::DerivedVar_ocean_s_coordinate_g1::SeekSlice(int offset, int whence, int) {
    size_t nz = _dims[0];
...
int NetCDFCFCollection::DerivedVar_ocean_s_coordinate_g2::Read(float *buf, int) {
    size_t nx = _dims[2];
    size_t ny = _dims[1];
    size_t nz = _dims[0];
    ...
    for (size_t z = 0; z < nz; z++) {
        for (size_t y = 0; y < ny; y++) {
            for (size_t x = 0; x < nx; x++) {
                buf[z * nx * ny + y * nx + x] =
                    _eta[y * nx + x] + (_eta[y * nx + x] + _depth[y * nx + x]) *
                                           ((_depth_c * _s[z] + _depth[y * nx + x] * _C[z]) /
...
int NetCDFCFCollection::DerivedVar_ocean_s_coordinate_g2::ReadSlice(float *slice, int fd) {
    size_t nx = _dims[2];
    size_t ny = _dims[1];
    size_t nz = _dims[0];
...
NetCDFCFCollection::DerivedVar_AHSPC::DerivedVar_AHSPC(NetCDFCFCollection *ncdfcf,
...
int NetCDFCFCollection::DerivedVar_AHSPC::DCZ2(const float *PS1, const float *PHIS1,
                                               float **TV2, // cannot make const...
                                               const int NL, const float P0,
    ...
    for (size_t K = 1; K < KMAX - 1; K++) {
        for (size_t I = 0; I < IMAX; I++) {
            float vpd =
                PMLN[I][K + 1] - PMLN[I][K - 1]; // vertical pressure difference between layers
    ...
    for (size_t K = 0; K < KMAX - 1; K++) {
        ...
        for (size_t I = 0; I < IMAX; I++) {
            float a = ZSLICE[I][K];
            float b =
                RBYG * TV2[I][L] * (log(PS1[I] * HYBB[0][0]) - .5 * (PMLN[I][L - 1] + PMLN[I][L]));
...
int NetCDFCFCollection::DerivedVar_AHSPC::ReadSlice(float *slice, int) {
    size_t nx = _dims[2];
    size_t ny = _dims[1];
    size_t nz = _dims[0];
    ...
    for (size_t y = 0; y < ny; y++) {
        for (size_t x = 0; x < nx; x++) {
            slice[y * nx + x] = _Z3[(z * nx * ny) + (nx * y) + (x)];
...
int NetCDFCFCollection::DerivedVar_AHSPC::SeekSlice(int offset, int whence, int) {
    size_t nz = _dims[0];
...
NetCDFCFCollection::DerivedVar_noop::DerivedVar_noop(NetCDFCFCollection *ncdfcf,
                                                     const std::map<string, string> &formula_map,
                                                     string units)
    : DerivedVar(ncdfcf) {
...
int NetCDFCFCollection::DerivedVar_noop::Read(float *buf, int fd) {
    ...
    rc = ncdfc->Convert(_native_units, _derived_units, buf, buf, n);
    if (rc < 0)
        return (-1);
    return (0);
}

int NetCDFCFCollection::DerivedVar_noop::ReadSlice(float *slice, int fd) {
    return (Read(slice, fd));
}

int NetCDFCFCollection::DerivedVar_noop::SeekSlice(int offset, int whence, int fd) { return (0); }

NetCDFCFCollection::DerivedVarTime::DerivedVarTime(NetCDFCFCollection *ncdfcf, string native_var,
                                                   string native_units, string derived_units)
    : DerivedVar(ncdfcf) {
...
int NetCDFCFCollection::DerivedVarTime::Open(size_t ts) {
    ...
    //
    NetCDFCFCollection *ncdfc = (NetCDFCFCollection *)_ncdfc;
    rc = ncdfc->Convert(_native_units, _derived_units, _timecoords, _timecoords, _timedim);
    if (rc < 0)
        return (-1);
...
int NetCDFCFCollection::DerivedVarTime::Read(float *buf, int) {
    if (_first)
        return (-1);
    *buf = _timecoords[_ts];
    return (0);
...
int NetCDFCFCollection::DerivedVarTime::ReadSlice(float *slice, int) {
    if (_first)
        return (-1);
...
NetCDFCFCollection::DerivedVarTimeFromMem::DerivedVarTimeFromMem(string name, string units,
                                                                 string timeDimName,
...
int NetCDFCFCollection::DerivedVarTimeFromMem::Read(float *buf, int) {
    *buf = _times[_ts];
    return (0);
}

int NetCDFCFCollection::DerivedVarTimeFromMem::ReadSlice(float *slice, int) {
    *slice = _times[_ts];
    return (1);
}

NetCDFCFCollection::DerivedVar_vertStag::DerivedVar_vertStag(NetCDFCFCollection *ncdfcf,
                                                             string unstagVar, string stagDimName)
    : DerivedVar(ncdfcf) {
