...
void _compute_bs(const vector<string> &dim_names, const vector<size_t> &default_bs,
                 vector<size_t> &bs) {
    bs.clear();
...
void _compute_periodic(const vector<string> &dim_names, const vector<bool> &default_periodic,
...
int VDC::_DefineImplicitCoordVars(vector<string> dim_names, vector<string> coord_vars_in,
    ...
    for (int i = 0; i < new_coord_vars.size(); i++) {
        ...
        _coordVars[name] = CoordVar(name, "", XType::FLOAT, "", vector<size_t>(), periodic,
                                    dim_names, bs, time_dim_name, axis, false);
...
int VDC::DefineCoordVar(string varname, vector<string> dim_names, string time_dim_name,
    ...
    if (axis == 3 && units.empty())
        units = "seconds";

    if (!_ValidDefineCoordVar(varname, dim_names, time_dim_name, units, axis, type, compressed)) {
        return (-1);
    }
    ...
    //
    _coordVars[varname] = CoordVar(varname, units, type, wname, cratios, periodic, dim_names, bs,
                                   time_dim_name, axis, false);

    return (0);
}

int VDC::DefineCoordVarUniform(string varname, vector<string> dim_names, string time_dim_name,
                               string units, int axis, XType type, bool compressed) {
...
bool VDC::GetCoordVarInfo(string varname, vector<string> &dim_names, bool &time_varying,
                          string &units, int &axis, XType &type, bool &compressed,
                          bool &uniform) const {
    dim_names.clear();
    units.erase();
...
int VDC::_DefineDataVar(string varname, vector<string> dim_names, vector<string> coord_vars,
    ...
    if (!_ValidDefineDataVar(varname, dim_names, coord_vars, units, type, compressed, maskvar)) {
    ...
    if (!maskvar.empty()) {
        _dataVars[varname] = DataVar(varname, units, type, wname, cratios, periodic, meshname, bs,
...
int VDC::DefineDataVar(string varname, vector<string> dim_names, vector<string> coord_vars,
...
bool VDC::GetDataVarInfo(string varname, vector<string> &dim_names, vector<string> &coord_vars,
                         bool &time_varying, string &units, XType &type, bool &compressed,
...
bool VDC::_ValidDefineCoordVar(string varname, vector<string> sdim_names, string time_dim_name,
                               string units, int axis, XType type, bool compressed) const {
    ...
    if (!time_dim_name.empty())
        dim_names.push_back(time_dim_name);
...
// size
//
bool VDC::_valid_dims(const vector<DC::Dimension> &dims0, const vector<size_t> &bs0,
                      const vector<DC::Dimension> &dims1, const vector<size_t> &bs1) const {
    ...
    for (int i = 0; i < dims0.size(); i++) {
        ...
        for (int j = 0; j < dims1.size(); j++) {
            ...
            if (dims0[i].GetName() == dims1[j].GetName()) {
                ...
                //
                if (!(bs0[i] == bs1[j] || bs0[i] == 1 || bs1[j] == 1))
                    return (false);
            }
        }
        if (!match)
...
bool VDC::_valid_blocking(const vector<DC::Dimension> &dimensions, const vector<size_t> &bs,
                          const vector<string> &coord_vars) const {
...
bool VDC::_valid_mask_var(string varname, vector<DC::Dimension> dimensions, vector<size_t> bs,
                          bool compressed, string maskvar) const {
    ...
    while (dimensions.size() > mdimensions.size())
        dimensions.pop_back();
    while (bs.size() > mdimensions.size())
        bs.pop_back();
...
bool VDC::_ValidDefineDataVar(string varname, vector<string> dim_names, vector<string> coord_vars,
                              string units, XType type, bool compressed, string maskvar) const {
