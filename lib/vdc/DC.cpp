...
DC::BaseVar::BaseVar(string name, string units, XType type, std::vector<size_t> bs,
                     std::vector<bool> periodic)
    : _name(name), _units(units), _type(type), _bs(bs), _periodic(periodic) {
    _wname = "";
...
int DC::GetHyperSliceInfo(string varname, int level, std::vector<size_t> &hslice_dims,
                          size_t &nslice) {
    hslice_dims.clear();
    nslice = 0;
...
bool DC::_getDataVarDimensions(string varname, bool spatial,
...
bool DC::_getCoordVarDimensions(string varname, bool spatial,
                                vector<DC::Dimension> &dimensions) const {
    dimensions.clear();
...
int DC::_openVariableRead(size_t ts, string varname, int level, int lod) {
    _ovrTS = ts;
    _ovrVarName = varname;
    _ovrLevel = level;
    _ovrLOD = lod;
...
template <class T> int DC::_readSliceTemplate(T *slice) {
    vector<size_t> dims_at_level;
    vector<size_t> bs_at_level;

    int rc = GetDimLensAtLevel(_ovrVarName, _ovrLevel, dims_at_level, bs_at_level);
    if (rc < 0)
        return (rc);
