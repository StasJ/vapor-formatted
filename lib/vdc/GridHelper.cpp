...
bool isUnstructured2D(const DC::Mesh &m, const vector<DC::CoordVar> &cvarsinfo,
...
bool isUnstructuredLayered(const DC::Mesh &m, const vector<DC::CoordVar> &cvarsinfo,
                           const vector<vector<string>> &cdimnames) {
    DC::Mesh::Type mtype = m.GetMeshType();
...
bool isCurvilinear(const DC::Mesh &m, const vector<DC::CoordVar> &cvarsinfo,
    ...
    if (cdimnames.size() == 3 && !((cdimnames[2].size() == 1) || (cdimnames[2].size() == 3))) {

        return (false);
    }

    if (!(cdimnames[0].size() == 2 && cdimnames[1].size() == 2))
        return (false);
...
string GridHelper::_getQuadTreeRectangleKey(size_t ts, int level, int lod,
...
RegularGrid *GridHelper::_make_grid_regular(const vector<size_t> &dims,
...
StretchedGrid *GridHelper::_make_grid_stretched(const vector<size_t> &dims,
...
LayeredGrid *GridHelper::_make_grid_layered(const vector<size_t> &dims,
                                            const vector<float *> &blkvec, const vector<size_t> &bs,
                                            const vector<size_t> &bmin,
                                            const vector<size_t> &bmax) const {
...
CurvilinearGrid *GridHelper::_make_grid_curvilinear(
    ...
    if (!qtr) {
        ...
        QuadTreeRectangle<float, size_t> *oldqtr =
            _qtrCache.put(qtr_key, (QuadTreeRectangle<float, size_t> *)qtr);
        if (oldqtr)
            delete oldqtr;
    }
...
UnstructuredGrid2D *GridHelper::_make_grid_unstructured2d(
    size_t ts, int level, int lod, const DC::DataVar &var, const vector<DC::CoordVar> &cvarsinfo,
    const vector<size_t> &dims, const vector<float *> &blkvec, const vector<size_t> &bs,
    ...
    UnstructuredGrid2D *g = new UnstructuredGrid2D(
        ...
        location, maxVertexPerFace, maxFacePerVertex, vertexOffset, faceOffset, xug, yug, zug, qtr);
...
UnstructuredGridLayered *GridHelper::_make_grid_unstructured_layered(
    ...
    UnstructuredGridCoordless xug(vertexDims1D, faceDims1D, edgeDims1D, bs1d, xcblkptrs, 2,
                                  vertexOnFace, faceOnVertex, faceOnFace, location,
                                  maxVertexPerFace, maxFacePerVertex, vertexOffset, faceOffset);

    UnstructuredGridCoordless yug(vertexDims1D, faceDims1D, edgeDims1D, bs1d, ycblkptrs, 2,
                                  vertexOnFace, faceOnVertex, faceOnFace, location,
                                  maxVertexPerFace, maxFacePerVertex, vertexOffset, faceOffset);

    UnstructuredGridCoordless zug(vertexDims, faceDims, edgeDims, bs, zcblkptrs, 3, vertexOnFace,
                                  faceOnVertex, faceOnFace, location, maxVertexPerFace,
                                  maxFacePerVertex, vertexOffset, faceOffset);
...
StructuredGrid *GridHelper::MakeGridStructured(
    string gridType, size_t ts, int level, int lod, const DC::DataVar &var,
    const vector<DC::CoordVar> &cvarsinfo, const vector<size_t> &roi_dims,
    const vector<size_t> &dims, const vector<float *> &blkvec, const vector<vector<size_t>> &bsvec,
    const vector<vector<size_t>> &bminvec, const vector<vector<size_t>> &bmaxvec) {
...
UnstructuredGrid *GridHelper::MakeGridUnstructured(
    ...
    const vector<size_t> &dims, const vector<float *> &blkvec, const vector<vector<size_t>> &bsvec,
    const vector<vector<size_t>> &bminvec, const vector<vector<size_t>> &bmaxvec,
    const vector<int *> &conn_blkvec, const vector<vector<size_t>> &conn_bsvec,
    ...
    if (gridType == UnstructuredGrid2D::GetClassType()) {
        rg = _make_grid_unstructured2d(
            ts, level, lod, var, cvarsinfo, roi_dims, blkvec, bsvec[0], bminvec[0], bmaxvec[0],
            conn_blkvec, conn_bsvec[0], conn_bminvec[0], conn_bmaxvec[0], vertexDims, faceDims,
...
GridHelper::~GridHelper() {
    ...
    while ((qtr = _qtrCache.remove_lru()) != NULL) {
        delete qtr;
    }
}

string GridHelper::GetGridType(const DC::Mesh &m, const vector<DC::CoordVar> &cvarsinfo,
                               const vector<vector<string>> &cdimnames) const {
...
bool GridHelper::IsUnstructured(std::string gridType) const {
    ...
    return (gridType == UnstructuredGrid2D::GetClassType() ||
...
bool GridHelper::IsStructured(std::string gridType) const {
    ...
    return (
        (gridType == RegularGrid::GetClassType()) || (gridType == StretchedGrid::GetClassType()) ||
        (gridType == LayeredGrid::GetClassType()) || (gridType == CurvilinearGrid::GetClassType()));
}
