...
SphericalGrid::SphericalGrid(const size_t bs[3], const size_t min[3], const size_t max[3],
                             const double extents[6], const size_t permutation[3],
                             const bool periodic[3], float **blks)
...
SphericalGrid::SphericalGrid(const size_t bs[3], const size_t min[3], const size_t max[3],
                             const double extents[6], const size_t permutation[3],
                             const bool periodic[3], float **blks, float missing_value)
...
void SphericalGrid::GetBoundingBox(const size_t min[3], const size_t max[3],
                                   double extents[6]) const {
    cerr << "SphericalGrid::GetBoundingBox() NOT SUPPORTED!!!\n";
}

int SphericalGrid::GetUserCoordinates(size_t i, size_t j, size_t k, double *x, double *y,
                                      double *z) const {
...
void SphericalGrid::GetIJKIndex(double x, double y, double z, size_t *i, size_t *j,
                                size_t *k) const {
...
void SphericalGrid::GetIJKIndexFloor(double x, double y, double z, size_t *i, size_t *j,
                                     size_t *k) const {
...
bool SphericalGrid::InsideGrid(double x, double y, double z) const {
    ...
    if (_extentsC[0] < _extentsC[3]) {
        if (x < _extentsC[0] || x > _extentsC[3])
            return (false);
    } else {
        if (x > _extentsC[0] || x < _extentsC[3])
            return (false);
    ...
    if (_extentsC[1] < _extentsC[4]) {
        if (y < _extentsC[1] || y > _extentsC[4])
            return (false);
    } else {
        if (y > _extentsC[1] || y < _extentsC[4])
            return (false);
    ...
    if (_extentsC[2] < _extentsC[5]) {
        if (z < _extentsC[2] || z > _extentsC[5])
            return (false);
    } else {
        if (z > _extentsC[2] || z < _extentsC[5])
            return (false);
    ...
    if (theta < lat0 || theta > lat1)
        return (false);

    double phi = atan2(y, x); // atan2 is in range [-pi, pi];
    if (phi < lon0 || theta > lon1)
        return (false);

    return (true);
}

inline void SphericalGrid::CartToSph(double x, double y, double z, double *phi, double *theta,
                                     double *r) {
    *r = sqrt(x * x + y * y + z * z);
...
inline void SphericalGrid::SphToCart(double phi, double theta, double r, double *x, double *y,
                                     double *z) {
...
void SphericalGrid::GetEnclosingRegion(const double minu[3], const double maxu[3], size_t min[3],
                                       size_t max[3]) const {
    //
...
//----------------------------------------------------------------------------
void SphericalGrid::_permute(const vector<long> &permutation, double result[3], double x, double y,
                             double z) const {
