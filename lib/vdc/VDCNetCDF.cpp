...
template <class T>
int VDCNetCDF::_copyVarHelper(DC &dc, int fdr, int fdw, vector<size_t> &buffer_dims,
...
int VDCNetCDF::CopyVar(DC &dc, size_t ts, string varname, int srclod, int dstlod) {
    ...
    if (varInfo.GetXType() == FLOAT || varInfo.GetXType() == DOUBLE) {
        ...
        rc = _copyVarHelper(dc, fdr, fdw, buffer_dims, src_hslice_dims, dst_hslice_dims, src_nslice,
                            dst_nslice, buffer);
        delete[] buffer;
    } else {
        size_t bufsize = vproduct(buffer_dims);
