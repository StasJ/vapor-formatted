...
void ws_info(const vector<size_t> &sdims, const vector<size_t> &bs, size_t &slice_size,
             size_t &slices_per_slab, size_t &num_slices) {
...
void vdc_2_ncdfcoords(size_t ts0, size_t ts1, bool time_varying, const vector<size_t> &min,
                      const vector<size_t> &max, vector<size_t> &start, vector<size_t> &count) {
    start.clear();
    count.clear();
...
int VDCNetCDF::GetHyperSliceInfo(string varname, int level, std::vector<size_t> &hslice_dims,
                                 size_t &nslice) {
    hslice_dims.clear();
    nslice = 0;
...
int VDCNetCDF::Initialize(const vector<string> &paths, const vector<string> &options,
...
// change.
//
int VDCNetCDF::GetPath(string varname, size_t ts, string &path, size_t &file_ts,
                       size_t &max_ts) const {
    path.clear();
    file_ts = 0;
    max_ts = 0;
...
int VDCNetCDF::getDimLensAtLevel(string varname, int level, vector<size_t> &dims_at_level,
...
WASP *VDCNetCDF::_OpenVariableRead(size_t ts, string varname, int clevel, int lod,
                                   size_t &file_ts) {
    file_ts = 0;
...
int VDCNetCDF::openVariableRead(size_t ts, string varname, int level, int lod) {
    ...
    vector<size_t> dims_at_level;
    vector<size_t> bs_at_level;
    int rc = VDCNetCDF::GetDimLensAtLevel(varname, clevel, dims_at_level, bs_at_level);
    if (rc < 0)
        return (-1);
    ...
    open_file_info ofi(wasp, false, dims_at_level, bs_at_level, 0, ts, file_ts, varname, clevel);
    ...
    open_file_info ofimask(wasp, false, dims_at_level, bs_at_level, 0, ts, file_ts, maskvar,
                           clevel);
...
int VDCNetCDF::OpenVariableWrite(size_t ts, string varname, int lod) {
    ...
    vector<size_t> dims_at_level;
    vector<size_t> bs_at_level;
    rc = VDCNetCDF::GetDimLensAtLevel(varname, nlevels - 1, dims_at_level, bs_at_level);
    if (rc < 0)
        return (-1);

    open_file_info ofi(wasp, true, dims_at_level, bs_at_level, 0, ts, file_ts, varname,
                       nlevels - 1);
...
unsigned char *VDCNetCDF::_read_mask_var(vector<size_t> start, vector<size_t> count) {
    // data variable may be time varying, while mask variable is not.
    // If so remove the time dimension from start and count
    //
    if (VDC::IsTimeVarying(_ofi._varname) && !VDC::IsTimeVarying(_ofimask._varname)) {
...
template <class T> int VDCNetCDF::_writeTemplate(const T *data) {
    ...
    vdc_2_ncdfcoords(_ofi._file_ts, _ofi._file_ts, time_varying, mins, maxs, start, count);
...
template <class T> int VDCNetCDF::_writeSliceTemplate(WASP *file, const T *slice) {
    ...
    vector<size_t> start;
    vector<size_t> count;
    vdc_2_ncdfcoords(_ofi._file_ts, _ofi._file_ts, IsTimeVarying(_ofi._varname), min, max, start,
                     count);
...
template <class T> int VDCNetCDF::_writeSliceTemplate(WASP *file, const T *slice) {
    ...
    if (((_ofi._slice_num % slices_per_slab) == 0) || _ofi._slice_num == num_slices) {
        //
        ...
        if (maxs.size() > 2) {
            mins[mins.size() - 1] = (_ofi._slice_num - 1) / _ofi._bs[_ofi._bs.size() - 1] *
                                    _ofi._bs[_ofi._bs.size() - 1];
...
template <class T> int VDCNetCDF::_ReadSlice(WASP *file, T *slice) {
    ...
    if (((_ofi._slice_num % slices_per_slab) == 0) ||
...
int VDCNetCDF::readRegionBlock(const vector<size_t> &min, const vector<size_t> &max,
                               float *region) {
    bool time_varying = VDC::IsTimeVarying(_ofi._varname);
...
template <class T>
int VDCNetCDF::_putVarTemplate(size_t ts, string varname, int lod, const T *data) {
    closeVariable();
...
template <class T>
int VDCNetCDF::_copyVarHelper(DC &dc, vector<size_t> &buffer_dims, vector<size_t> &src_hslice_dims,
                              vector<size_t> &dst_hslice_dims, size_t src_nslice, size_t dst_nslice,
                              T *buffer) {
...
int VDCNetCDF::CopyVar(DC &dc, size_t ts, string varname, int srclod, int dstlod) {
    ...
    if (varInfo.GetXType() == FLOAT || varInfo.GetXType() == DOUBLE) {
        ...
        rc = _copyVarHelper(dc, buffer_dims, src_hslice_dims, dst_hslice_dims, src_nslice,
                            dst_nslice, buffer);
        delete[] buffer;
    } else {
        size_t bufsize = vproduct(buffer_dims);
...
bool VDCNetCDF::CompressionInfo(std::vector<size_t> bs, string wname, size_t &nlevels,
                                size_t &maxcratio) const {
    nlevels = 1;
    maxcratio = 1;
    if (wname.empty())
        return (true);
...
int VDCNetCDF::_ReadMasterBaseVarDefs(string prefix, BaseVar &var) {
    ...
    for (int i = 0; i < iperiodic.size(); i++)
        periodic.push_back(iperiodic[i]);
    if (rc < 0)
        return (rc);
...
int VDCNetCDF::_WriteMasterBaseVarDefs(string prefix, const BaseVar &var) {
    ...
    vector<int> iperiodic;
    for (int i = 0; i < periodic.size(); i++)
        iperiodic.push_back(periodic[i]);
...
int VDCNetCDF::_DefBaseVar(WASP *wasp, const VDC::BaseVar &var, size_t max_ts) {
    ...
    int rc = wasp->DefVar(var.GetName(), vdc_xtype2ncdf_xtype(var.GetXType()), dimnames,
                          var.GetWName(), bs, var.GetCRatios());
    if (rc < 0)
        return (-1);
