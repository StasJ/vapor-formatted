...
int NetCDFCollection::Initialize(const vector<string> &files, const vector<string> &time_dimnames,
    ...
    int rc = NetCDFCollection::_InitializeTimesMap(files, time_dimnames, time_coordvars, _timesMap,
                                                   _times, file_org);
    if (rc < 0)
        return (-1);
    ...
    for (int i = 0; i < files.size(); i++) {
        ...
        for (int j = 0; j < dimnames.size(); j++) {
            ...
            //
            else if (find(time_dimnames.begin(), time_dimnames.end(), dimnames[j]) !=
                     time_dimnames.end()) {
        ...
        for (int j = 0; j < variables.size(); j++) {
            ...
            int rc =
                tvvref.Insert(netcdf, variables[j], files[i], time_dimnames, _timesMap, file_org);
...
bool NetCDFCollection::VariableExists(size_t ts, string varname) const {
    ...
    if (!tvvar.GetTimeVarying())
        return (true); // CV variables exist for all times
...
bool NetCDFCollection::IsStaggeredDim(string dimname) const {
    ...
    for (int i = 0; i < _staggeredDims.size(); i++) {
        if (dimname.compare(_staggeredDims[i]) == 0)
            return (true);
    }
    return (false);
}
...
int NetCDFCollection::_InitializeTimesMap(const vector<string> &files,
    ...
    if (time_coordvars.size() && (time_coordvars.size() != time_dimnames.size())) {
        SetErrMsg("NetCDFCollection::Initialize() : number of time coordinate variables and time "
                  "dimensions must match when time coordinate variables specified");
        return (-1);
    }
...
int NetCDFCollection::_InitializeTimesMapCase1(const vector<string> &files,
    ...
    for (int i = 0; i < files.size(); i++) {
        ...
        for (int j = 0; j < variables.size(); j++) {
            ...
            // Skip 0D variables
            //
            if (variables[j].GetDimNames().size() == 0)
                continue;
...
int NetCDFCollection::_InitializeTimesMapCase2(const vector<string> &files,
                                               const vector<string> &time_dimnames,
                                               map<string, vector<double>> &timesMap) const {
    timesMap.clear();
    map<string, double> currentTime; // current time for each variable
...
int NetCDFCollection::_InitializeTimesMapCase3(const vector<string> &files,
                                               const vector<string> &time_dimnames,
                                               const vector<string> &time_coordvars,
                                               map<string, vector<double>> &timesMap) const {
    timesMap.clear();
    ...
    for (int i = 0; i < files.size(); i++) {
        ...
        for (int j = 0; j < time_coordvars.size(); j++) {
            ...
            if (!buf) {
                SetErrMsg("Failed to read time coordinate variable \"%s\"",
...
void NetCDFCollection::_InterpolateLine(const float *src, size_t n, size_t stride, bool has_missing,
                                        float mv, float *dst) const {
    if (!has_missing) {
...
void NetCDFCollection::_InterpolateSlice(size_t nx, size_t ny, bool xstag, bool ystag,
                                         bool has_missing, float mv, float *slice) const {
...
float *NetCDFCollection::_Get1DVar(NetCDFSimple *netcdf,
                                   const NetCDFSimple::Variable &variable) const {
    if (variable.GetDimNames().size() != 1)
        return (NULL);
...
int NetCDFCollection::_get_var_index(const vector<NetCDFSimple::Variable> variables,
                                     string varname) const {
    for (int i = 0; i < variables.size(); i++) {
        if (varname.compare(variables[i].GetName()) == 0)
            return (i);
    }
    return (-1);
}
...
int NetCDFCollection::Read(float *data, int fd) {
    ...
    if (dims.size() > 3) {
        ...
        if (xstag) { // Deal with staggered data
            ...
            if (rc < 0)
                return (-1);
            _InterpolateLine((const float *)fh._linebuf, nx, 1, fh._has_missing, fh._missing_value,
                             data);
            return (0);
        } else {
            return (NetCDFCollection::ReadNative(start, count, data, fd));
...
int NetCDFCollection::TimeVaryingVar::Insert(const NetCDFSimple *netcdf,
    ...
    if (first) {
        ...
        //
        //		if (!( _spatial_dims == space_dims && _time_varying == time_varying && _time_name ==
        //time_name)) {
...
vector<double> NetCDFCollection::TimeVaryingVar::GetTimes() const {
    ...
    for (int i = 0; i < _tvmaps.size(); i++)
        times.push_back(_tvmaps[i]._time);
...
bool NetCDFCollection::TimeVaryingVar::tvmap_cmp(NetCDFCollection::TimeVaryingVar::tvmap_t &a,
