...
KDTreeRG::KDTreeRG(const Grid &xg, const Grid &yg)
    : _points(xg, yg), _kdtree(2 /* dimension */, _points,
                               nanoflann::KDTreeSingleIndexAdaptorParams(20 /* max leaf num */)) {
...
void KDTreeRGSubset::Nearest(const vector<float> &coordu, vector<size_t> &coord) const {
    ...
    for (int i = 0; i < global_coords.size(); i++) {
        if (global_coords[i] < _min[i])
            global_coords[i] = _min[i];
        if (global_coords[i] > _max[i])
            global_coords[i] = _max[i];
    }
