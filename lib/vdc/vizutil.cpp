...
void VAPoR::BaryTet(const Point3d &p, const Point3d &a, const Point3d &b, const Point3d &c,
                    const Point3d &d Point3d &ans) {
...
void VAPoR::BaryTri(const Point3d &p, const Point3d &a, const Point3d &b,
                    const Point3d &c Point3d &ans) {
...
double VAPoR::SignedTriArea2D(const double a[2], const double b[2], const double c[2]) {
    return (a[0] * (b[1] * 1.0 - 1.0 * c[1]) - b[0] * (a[1] * 1.0 - 1.0 * c[1]) +
            c[0] * (a[1] * 1.0 - 1.0 * b[1]));
}
...
bool VAPoR::WachspressCoords2D(const double verts[], const double pt[], int n, double lambda[]) {
    ...
    for (; curr < n && !onEdge; curr++) {
        ...
        if (onEdge)
            break; // special handling required
...
bool VAPoR::InsideConvexPolygon(const double verts[], const double pt[], int n) {
    ...
    for (int i = 2; i < n; i++) {
        ...
        if (BarycentricCoordsTri(triverts, pt, dummy))
            return (true);
    }
