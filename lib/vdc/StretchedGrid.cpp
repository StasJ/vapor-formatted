...
void StretchedGrid::GetBoundingBox(const std::vector<size_t> &min, const std::vector<size_t> &max,
    ...
    if (minu[0] > maxu[0])
        std::swap(minu[0], maxu[0]);

    minu[1] = _ycoords[cMin[1]];
    maxu[1] = _ycoords[cMax[1]];
    if (minu[1] > maxu[1])
        std::swap(minu[1], maxu[1]);
...
bool StretchedGrid::_insideGrid(double x, double y, double z, size_t &i, size_t &j, size_t &k,
    ...
    if (!Wasp::BinarySearchRange(_xcoords, x, i))
        return (false);

    xwgt[0] = 1.0 - (x - _xcoords[i]) / (_xcoords[i + 1] - _xcoords[i]);
    xwgt[1] = 1.0 - xwgt[0];

    if (!Wasp::BinarySearchRange(_ycoords, y, j))
        return (false);
    ...
    //
    if (!Wasp::BinarySearchRange(_zcoords, z, k))
        return (false);
