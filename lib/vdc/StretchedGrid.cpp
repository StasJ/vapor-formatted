...
void StretchedGrid::_stretchedGrid(const vector<double> &xcoords, const vector<double> &ycoords,
...
StretchedGrid::StretchedGrid(const vector<size_t> &dims, const vector<size_t> &bs,
                             const vector<float *> &blks, const vector<double> &xcoords,
...
void StretchedGrid::GetBoundingBox(const std::vector<size_t> &min, const std::vector<size_t> &max,
...
void StretchedGrid::GetEnclosingRegion(const std::vector<double> &minu,
...
void StretchedGrid::GetUserCoordinates(const std::vector<size_t> &indices,
...
void StretchedGrid::GetIndices(const std::vector<double> &coords,
...
StretchedGrid::ConstCoordItrSG::ConstCoordItrSG(const StretchedGrid *sg, bool begin)
...
StretchedGrid::ConstCoordItrSG::ConstCoordItrSG(const ConstCoordItrSG &rhs)
    : ConstCoordItrAbstract() {
    _sg = rhs._sg;
    _index = rhs._index;
    _coords = rhs._coords;
...
float StretchedGrid::GetValueLinear(const std::vector<double> &coords) const {
    ...
    float v0 = AccessIJK(i, j, k) * xwgt[0] + AccessIJK(i + 1, j, k) * xwgt[1] +
               AccessIJK(i + 1, j + 1, k) * ywgt[0] + AccessIJK(i, j + 1, k) * ywgt[1];

    if (GetGeometryDim() == 2)
        return (v0);

    float v1 = AccessIJK(i, j, k + 1) * xwgt[0] + AccessIJK(i + 1, j, k + 1) * xwgt[1] +
               AccessIJK(i + 1, j + 1, k + 1) * ywgt[0] + AccessIJK(i, j + 1, k + 1) * ywgt[1];
...
//
bool StretchedGrid::_insideGrid(double x, double y, double z, size_t &i, size_t &j, size_t &k,
