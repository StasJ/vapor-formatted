...
void CurvilinearGrid::_curvilinearGrid(
    const RegularGrid &xrg, const RegularGrid &yrg, const RegularGrid &zrg,
    const vector<double> &zcoords, std::shared_ptr<const QuadTreeRectangle<float, size_t>> qtr) {
    _zcoords.clear();
    _minu.clear();
    _maxu.clear();
    _xrg = xrg;
...
bool CurvilinearGrid::_insideGridHelperTerrain(double x, double y, double z, const size_t &i,
    ...
    double tverts0[] = {_xrg.AccessIJK(iv[0], jv[0], 0), _yrg.AccessIJK(iv[0], jv[0], 0),
                        _xrg.AccessIJK(iv[1], jv[1], 0), _yrg.AccessIJK(iv[1], jv[1], 0),
    ...
    if (!inside) {
        ...
        iv = {i, i + 1, i};
        jv = {j, j + 1, j + 1};
        double tverts1[] = {_xrg.AccessIJK(iv[0], jv[0], 0), _yrg.AccessIJK(iv[0], jv[0], 0),
                            _xrg.AccessIJK(iv[1], jv[1], 0), _yrg.AccessIJK(iv[1], jv[1], 0),
                            _xrg.AccessIJK(iv[2], jv[2], 0), _yrg.AccessIJK(iv[2], jv[2], 0)};
    ...
    for (int kk = 0; kk < nz; kk++) {
        ...
        //
        float zk = _zrg.AccessIJK(iv[0], jv[0], kk) * lambda[0] +
...
bool CurvilinearGrid::_insideFace(const vector<size_t> &face, double pt[2],
