...
void CurvilinearGrid::_curvilinearGrid(const RegularGrid &xrg, const RegularGrid &yrg,
                                       const RegularGrid &zrg, const vector<double> &zcoords,
...
CurvilinearGrid::CurvilinearGrid(const vector<size_t> &dims, const vector<size_t> &bs,
                                 const vector<float *> &blks, const RegularGrid &xrg,
                                 const RegularGrid &yrg, const vector<double> &zcoords,
                                 const KDTreeRG *kdtree)
...
CurvilinearGrid::CurvilinearGrid(const vector<size_t> &dims, const vector<size_t> &bs,
                                 const vector<float *> &blks, const RegularGrid &xrg,
                                 const RegularGrid &yrg, const RegularGrid &zrg,
                                 const KDTreeRG *kdtree)
    : StructuredGrid(dims, bs, blks) {
...
void CurvilinearGrid::GetBoundingBox(const std::vector<size_t> &min, const std::vector<size_t> &max,
...
void CurvilinearGrid::_getEnclosingRegionHelper(const std::vector<double> &minu,
...
void CurvilinearGrid::GetEnclosingRegion(const std::vector<double> &minu,
...
void CurvilinearGrid::GetUserCoordinates(const std::vector<size_t> &indices,
...
void CurvilinearGrid::GetIndices(const std::vector<double> &coords,
                                 std::vector<size_t> &indices) const {
    indices.clear();
...
CurvilinearGrid::ConstCoordItrCG::ConstCoordItrCG(const CurvilinearGrid *cg, bool begin)
...
CurvilinearGrid::ConstCoordItrCG::ConstCoordItrCG(const ConstCoordItrCG &rhs)
    : ConstCoordItrAbstract() {
    _cg = rhs._cg;
    _index = rhs._index;
    _coords = rhs._coords;
...
float CurvilinearGrid::GetValueLinear(const std::vector<double> &coords) const {
    ...
    float v1s[] = {AccessIJK(i, j, k + 1), AccessIJK(i + 1, j, k + 1),
                   AccessIJK(i + 1, j + 1, k + 1), AccessIJK(i, j + 1, k + 1)};
...
// 'sorted'
//
int CurvilinearGrid::_binarySearchRange(const vector<double> &sorted, double x, size_t &i) const {
    i = 0;
...
bool CurvilinearGrid::_insideGridHelperTerrain(double x, double y, double z, const size_t &i,
                                               const size_t &j, size_t &k, double zwgt[2]) const {
    ...
    double lambda[3];
    double pt[] = {x, y};
    double tverts0[] = {_xrg.AccessIJK(i, j, 0),         _yrg.AccessIJK(i, j, 0),
                        _xrg.AccessIJK(i + 1, j, 0),     _yrg.AccessIJK(i + 1, j, 0),
                        _xrg.AccessIJK(i + 1, j + 1, 0), _yrg.AccessIJK(i + 1, j + 1, 0)};

    bool inside = VAPoR::BarycentricCoordsTri(tverts0, pt, lambda);
    if (inside) {

        z0 = _zrg.AccessIJK(i, j, k) * lambda[0] + _zrg.AccessIJK(i + 1, j, k) * lambda[1] +
             _zrg.AccessIJK(i + 1, j + 1, k) * lambda[2];

        z1 = _zrg.AccessIJK(i, j, k + 1) * lambda[0] + _zrg.AccessIJK(i + 1, j, k + 1) * lambda[1] +
        ...
        double tverts1[] = {_xrg.AccessIJK(i, j, 0),         _yrg.AccessIJK(i, j, 0),
                            _xrg.AccessIJK(i + 1, j + 1, 0), _yrg.AccessIJK(i + 1, j + 1, 0),
                            _xrg.AccessIJK(i, j + 1, 0),     _yrg.AccessIJK(i, j + 1, 0)};

        inside = VAPoR::BarycentricCoordsTri(tverts1, pt, lambda);
        if (!inside)
            return (false);

        z0 = _zrg.AccessIJK(i, j, k) * lambda[0] + _zrg.AccessIJK(i + 1, j + 1, k) * lambda[1] +
...
//
bool CurvilinearGrid::_insideGrid(double x, double y, double z, size_t &i, size_t &j, size_t &k,
