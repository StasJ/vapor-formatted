...
CurvilinearGrid::CurvilinearGrid(const size_t bs[3], const size_t min[3], const size_t max[3],
                                 const bool periodic[3], const vector<float *> &blks,
...
CurvilinearGrid::CurvilinearGrid(const vector<size_t> &bs, const vector<size_t> &min,
                                 const vector<size_t> &max, const vector<bool> &periodic,
...
CurvilinearGrid::CurvilinearGrid(const vector<size_t> &bs, const vector<size_t> &min,
                                 ...
                                 const vector<float *> &blks, const RegularGrid &xrg,
                                 const RegularGrid &yrg, const vector<double> &zcoords,
                                 const KDTreeRGSubset &kdtree, float missing_value)
    : StructuredGrid(bs, min, max, periodic, blks, missing_value) {
...
int CurvilinearGrid::GetUserCoordinates(size_t i, size_t j, size_t k, double *x, double *y,
                                        double *z) const {
...
void CurvilinearGrid::GetIJKIndex(double x, double y, double z, size_t *i, size_t *j,
                                  size_t *k) const {
    *i = *j = *k = 0;
...
#ifdef DEAD
void CurvilinearGrid::GetIJKIndexFloor(double x, double y, double z, size_t *i, size_t *j,
                                       size_t *k) const {
    // Clamp coordinates on periodic boundaries to grid extents
    //
    _ClampCoord(x, y, z);
...
bool CurvilinearGrid::InsideGrid(double x, double y, double z) const {
    ...
    if (x < minu[0] || x > maxu[0])
        return (false);
    if (y < minu[1] || y > maxu[1])
        return (false);
    if (_ndim == 3) {
        if (z < minu[2] || z > maxu[2])
            return (false);
    }
...
float CurvilinearGrid::_GetValueLinear(double x, double y, double z) const {
    ...
    float v0 = AccessIJK(i, j, k) * lambda[0] + AccessIJK(i + 1, j, k) * lambda[1] +
               AccessIJK(i + 1, j + 1, k) * lambda[2] + AccessIJK(i, j + 1, k) * lambda[2];

    if (_ndim == 2)
        return (v0);

    float v1 = AccessIJK(i, j, k + 1) * lambda[0] + AccessIJK(i + 1, j, k + 1) * lambda[1] +
               AccessIJK(i + 1, j + 1, k + 1) * lambda[2] + AccessIJK(i, j + 1, k + 1) * lambda[2];
...
bool CurvilinearGrid::_binarySearchRange(const vector<double> &sorted, double x, int &i) const {
    i = 0;
    if (x < sorted[0] || x > sorted[sorted.size() - 1])
        return (false);
...
//
bool CurvilinearGrid::_insideGrid(double x, double y, double z, int &i, int &j, int &k,
