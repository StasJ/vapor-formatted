...
int DCMPAS::_InitMeshes(NetCDFCollection *ncdfc) {
    ...
    _meshMap[mesh2DTriName] =
        Mesh(mesh2DTriName, 3, nCellsDimName, "", coordvars, verticesOnCellVarName);

    coordvars = {lonVertexVarName, latVertexVarName, zGridM1VarName};
    _meshMap[mesh3DTriName] = Mesh(mesh3DTriName, 3, nCellsDimName, "", nVertLevelsDimName,
                                   coordvars, verticesOnCellVarName);

    coordvars = {lonVertexVarName, latVertexVarName, zGridVarName};
    _meshMap[mesh3DP1TriName] = Mesh(mesh3DP1TriName, 3, nCellsDimName, "", nVertLevelsP1DimName,
                                     coordvars, verticesOnCellVarName);
    ...
    _meshMap[mesh2DCellName] = Mesh(mesh2DCellName, dimension.GetLength(), nVerticesDimName, "",
                                    coordvars, cellsOnVertexVarName);

    coordvars = {lonCellVarName, latCellVarName, zGridM1VarName};
    _meshMap[mesh3DCellName] = Mesh(mesh3DCellName, dimension.GetLength(), nVerticesDimName, "",
                                    nVertLevelsDimName, coordvars, cellsOnVertexVarName);

    coordvars = {lonCellVarName, latCellVarName, zGridVarName};
    _meshMap[mesh3DP1CellName] = Mesh(mesh3DP1CellName, dimension.GetLength(), nVerticesDimName, "",
