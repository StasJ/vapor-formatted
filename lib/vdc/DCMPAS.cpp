...
const vector<string> requiredDimNames = {timeDimName,        nCellsDimName,   nVerticesDimName,
                                         nEdgesDimName,      maxEdgesDimName, maxEdges2DimName,
                                         vertexDegreeDimName};
...
const vector<string> requiredHorizCoordVarNames = {latCellVarName,   lonCellVarName,
                                                   latVertexVarName, lonVertexVarName,
                                                   latEdgeVarName,   lonEdgeVarName};

// Vertical coordinate variables
//
const string zGridP1VarName = "zgrid";

const vector<string> optionalVertCoordVarNames = {zGridP1VarName};
...
const vector<string> connectivityVarNames = {cellsOnVertexVarName, verticesOnCellVarName,
                                             verticesOnEdge, edgesOnCellVarName};
...
bool is_connectivity_var(string varname) {
    return (find(connectivityVarNames.begin(), connectivityVarNames.end(), varname) !=
            connectivityVarNames.end());
}

bool is_lat_or_lon(string varname) {
    return (find(requiredHorizCoordVarNames.begin(), requiredHorizCoordVarNames.end(), varname) !=
            requiredHorizCoordVarNames.end());
}
    ...
    if (ncdfc->VariableExists(uNormalVarName) && ncdfc->VariableExists(uTangentialVarName)) {
        ...
        for (auto varname : {"Uzonal", "Umeridional"}) {
            ...
            DerivedZonalMeridonal *derivedVar = new DerivedZonalMeridonal(
                varname, this, ncdfc, uNormalVarName, uTangentialVarName, zonalFlag);
...
int DCMPAS::getDimLensAtLevel(string varname, int, std::vector<size_t> &dims_at_level,
...
int DCMPAS::_readVarToSmartBuf(size_t ts, string varname, Wasp::SmartBuf &smartBuf) {
    ...
    if (is_lat_or_lon(varname)) {
        rad2degrees((float *)buf, n);
    }
...
int DCMPAS::_readRegionTransposed(MPASFileObject *w, const vector<size_t> &min,
...
int DCMPAS::_readRegionEdgeVariable(MPASFileObject *w, const vector<size_t> &min,
                                    const vector<size_t> &max, float *region) {
    VAssert(min.size() == 1 || min.size() == 2);
    VAssert(min.size() == max.size());
    ...
    for (size_t j = j0; j <= j1; j++) {
        for (size_t i = min[0], ii = 0; i <= max[0]; i++, ii++) {
            ...
            region[j * (max[0] - min[0] + 1) + ii] = edgeVariable[j * dims[0] + vidx0] * wgt +
...
template <class T>
int DCMPAS::_readRegionTemplate(int fd, const vector<size_t> &min, const vector<size_t> &max,
                                T *region) {
    MPASFileObject *w = (MPASFileObject *)_fileTable.GetEntry(fd);
    ...
    //
    if (is_lat_or_lon(varname)) {
        rad2degrees((float *)region, max[0] - min[0] + 1);
    }

    // Special handling for some auxiliary variables
    //
    if (varname == verticesOnCellVarName) {
        _addMissingFlag((int *)region);
    }

    if (is_connectivity_var(varname)) {
        _splitOnBoundary(varname, (int *)region);
    }
...
int DCMPAS::_InitDerivedVars(NetCDFCollection *ncdfc) {
    ...
    //
    DerivedCoordVar_WRFTime *derivedVar =
        new DerivedCoordVar_WRFTime(timeDimName, ncdfc, xTimeVarName, timeDimName);
...
int DCMPAS::_InitVerticalCoordinatesDerived(NetCDFCollection *ncdfc) {
    ...
    derivedVar = new DerivedCoordVar_UnStaggered(zGridVarName, nVertLevelsDimName, this,
                                                 zGridP1VarName, nVertLevelsP1DimName);
    ...
    derivedVar = new DerivedCoordVertFromCell(zGridVertP1VarName, nVerticesDimName, this,
                                              zGridP1VarName, cellsOnVertexVarName);
    ...
    derivedVar = new DerivedCoordVertFromCell(zGridVertVarName, nVerticesDimName, this,
                                              zGridVarName, cellsOnVertexVarName);
...
// is significant.
//
int DCMPAS::_GetVarCoordinates(NetCDFCollection *ncdfc, string varname, vector<string> &sdimnames,
...
int DCMPAS::_InitMeshes(NetCDFCollection *ncdfc) {
    ...
    _meshMap[mesh2DTriName] =
        Mesh(mesh2DTriName, 3, dimension.GetLength(), nCellsDimName, nVerticesDimName, coordvars,
             cellsOnVertexVarName, verticesOnCellVarName);

    if (_isAtmosphere(ncdfc) && _hasVertical) {
        coordvars = {lonCellVarName, latCellVarName, zGridVarName};
        _meshMap[mesh3DTriName] =
    ...
    _meshMap[mesh2DCellName] =
        Mesh(mesh2DCellName, dimension.GetLength(), 3, nVerticesDimName, nCellsDimName, coordvars,
             verticesOnCellVarName, cellsOnVertexVarName);

    if (_isAtmosphere(ncdfc) && _hasVertical) {

        coordvars = {lonVertexVarName, latVertexVarName, zGridVertVarName};
        _meshMap[mesh3DCellName] =
...
int DCMPAS::_InitAuxVars(NetCDFCollection *ncdfc) {
    ...
    for (int i = 0; i < vars.size(); i++) {
        ...
        //
        if (vars[i] == cellsOnVertexVarName || vars[i] == verticesOnCellVarName ||
            vars[i] == verticesOnEdge || vars[i] == edgesOnCellVarName) {
...
DCMPAS::DerivedCoordVertFromCell::DerivedCoordVertFromCell(string derivedVarName,
                                                           string derivedDimName, DC *dc,
                                                           string inName, string cellsOnVertexName
...
int DCMPAS::DerivedCoordVertFromCell::GetDimLensAtLevel(int, std::vector<size_t> &dims_at_level,
                                                        std::vector<size_t> &bs_at_level) const {
    dims_at_level.clear();
    bs_at_level.clear();
...
int DCMPAS::DerivedCoordVertFromCell::ReadRegion(int fd, const vector<size_t> &min,
                                                 const vector<size_t> &max, float *region) {
...
bool DCMPAS::DerivedCoordVertFromCell::VariableExists(size_t ts, int, int) const {
...
DCMPAS::DerivedZonalMeridonal::DerivedZonalMeridonal(string derivedVarName, DC *dc,
                                                     NetCDFCollection *ncdfc, string normalVarName,
                                                     string tangentialVarName, bool zonalFlag
...
int DCMPAS::DerivedZonalMeridonal::GetDimLensAtLevel(int, std::vector<size_t> &dims_at_level,
                                                     std::vector<size_t> &bs_at_level) const {
    dims_at_level.clear();
    bs_at_level.clear();
...
int DCMPAS::DerivedZonalMeridonal::ReadRegion(int fd, const vector<size_t> &min,
                                              const vector<size_t> &max, float *region) {
    VAssert(min.size() == 2);
    VAssert(min.size() == max.size());
...
bool DCMPAS::DerivedZonalMeridonal::VariableExists(size_t ts, int, int) const {

    return (_dc->VariableExists(ts, _normalVarName, -1, -1) &&
