...
int DCMPAS::_get_latlon_coordvars(NetCDFMPASCollection *ncdfc, string dvar, string &loncvar,
                                  string &latcvar) const {
    loncvar.clear();
    latcvar.clear();
...
int DCMPAS::_get_latlon_extents(NetCDFMPASCollection *ncdfc, string latlon, bool lonflag,
                                float &min, float &max) {
...
int DCMPAS::_get_coord_pair_extents(NetCDFMPASCollection *ncdfc, string lon, string lat,
                                    double &lonmin, double &lonmax, double &latmin,
                                    double &latmax) {
...
Proj4API *DCMPAS::_create_proj4api(double lonmin, double lonmax, double latmin, double latmax,
...
int DCMPAS::_AddCoordvars(NetCDFMPASCollection *ncdfc, const vector<string> &cvars) {
    ...
    for (int i = 0; i < cvars.size(); i++) {
        ...
        _coordVarsMap[cvars[i]] = CoordVar(cvars[i], units, DC::FLOAT, periodic, axis, false,
...
int DCMPAS::_InitHorizontalCoordinatesDerived(NetCDFMPASCollection *ncdfc,
    ...
    for (int i = 0; i < coordpairs.size(); i++) {
        ...
        int rc = _get_coord_pair_extents(ncdfc, coordpairs[i].first, coordpairs[i].second,
    ...
    for (int i = 0; i < coordpairs.size(); i++) {
        ...
        if (londimnames.size() > 1 && londimnames != latdimnames) {
            SetErrMsg("Invalid coordinate variable pair : %s, %s", coordpairs[i].first.c_str(),
                      coordpairs[i].second.c_str());
            return (-1);
        }
        ...
        derivedX = new DerivedVarHorizontal(ncdfc, coordpairs[i].first, coordpairs[i].second,
        ...
        _coordVarsMap[name] = CoordVar(name, "meters", DC::FLOAT, periodic, 0, false, londimnames,
                                       vector<size_t>(), lon_time_dim_name);

        name = coordpairs[i].second + "Y";

        DerivedVarHorizontal *derivedY;
        derivedY = new DerivedVarHorizontal(ncdfc, coordpairs[i].first, coordpairs[i].second,
                                            latdims, _proj4APIs[i], false);
        _derivedVars.push_back(derivedY);

        ncdfc->InstallDerivedCoordVar(name, derivedY, 1);

        _coordVarsMap[name] = CoordVar(name, "meters", DC::FLOAT, periodic, 1, false, latdimnames,
                                       vector<size_t>(), lat_time_dim_name
...
//
int DCMPAS::_InitHorizontalCoordinates(NetCDFMPASCollection *ncdfc) {
    _proj4APIs.clear();
    _proj4Strings.clear();
    _coordVarKeys.clear();
...
int DCMPAS::_get_vertical_coordvar(NetCDFMPASCollection *ncdfc, string dvar, string &cvar) {
    cvar.clear();
...
int DCMPAS::_InitVerticalCoordinatesDerived(NetCDFMPASCollection *ncdfc,
    ...
    for (int i = 0; i < cvars.size(); i++) {
        ...
        // variables later. Sigh
        //
        _coordVarsMap[name] = CoordVar(name, "meters", DC::FLOAT, periodic, 2, false, dimnames,
...
// ELEVATIONW.
//
int DCMPAS::_InitVerticalCoordinates(NetCDFMPASCollection *ncdfc) {
...
int DCMPAS::_get_time_coordvar(NetCDFMPASCollection *ncdfc, string dvar, string &cvar) {
    cvar.clear();
...
int DCMPAS::_InitTimeCoordinatesDerived(NetCDFMPASCollection *ncdfc, const vector<string> &cvars) {
    ...
    for (int i = 0; i < cvars.size(); i++) {
        ...
        // variables later. Sigh
        //
        _coordVarsMap[name] = CoordVar(name, "seconds", DC::FLOAT, periodic, 3, false,
...
//
int DCMPAS::_InitTimeCoordinates(NetCDFMPASCollection *ncdfc) {
    //
    // Get names of data variables  in the MPAS data set that have 1, 2 or 3
    // spatial dimensions
    //
...
// _dimsMap
//
int DCMPAS::_InitDimensions(NetCDFMPASCollection *ncdfc) {
    _dimsMap.clear();
...
// is significant.
//
int DCMPAS::_GetVarCoordinates(NetCDFMPASCollection *ncdfc, string varname,
                               vector<string> &sdimnames, vector<string> &scoordvars,
                               string &time_dim_name, string &time_coordvar) {
    sdimnames.clear();
    scoordvars.clear();
    time_dim_name.clear();
    time_coordvar.clear();
...
int DCMPAS::_InitVars(NetCDFMPASCollection *ncdfc) {
    ...
    for (int i = 0; i < vars.size(); i++) {
        ...
        if (!(NetCDFSimple::IsNCTypeFloat(type) || NetCDFSimple::IsNCTypeInt(type)))
            continue;
        ...
        if (!has_missing) {
            ...
            _dataVarsMap[vars[i]] = DataVar(vars[i], units, DC::FLOAT, periodic, mesh.GetName(),
                                            vector<size_t>(), time_coordvar, DC::Mesh::VOLUME, mv);
        }
    }
...
//
DCMPAS::DerivedVarHorizontal::DerivedVarHorizontal(NetCDFMPASCollection *ncdfc, string lonname,
                                                   string latname,
                                                   const vector<DC::Dimension> &dims,
                                                   Proj4API *proj4API, bool xflag)
    : DerivedVar(ncdfc) {
...
int DCMPAS::DerivedVarHorizontal::ReadSlice(float *slice, int) {
