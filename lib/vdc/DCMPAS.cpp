...
int DCMPAS::initialize(const vector<string> &files, const std::vector<string> &options) {
    ...
    //
    _proj4API = _create_proj4api(-180.0, 180.0, -90.0, 90.0, _proj4StringDefault);
...
template <class T>
int DCMPAS::_readRegionTemplate(const vector<size_t> &min, const vector<size_t> &max, T *region) {
...
DCMPAS::DerivedVarHorizontal::DerivedVarHorizontal(NetCDFCollection *ncdfc, string lonname,
                                                   string latname, Proj4API *proj4API, bool xflag,
                                                   bool uGridFlag, bool degrees)
    : DerivedVar(ncdfc) {
...
//
//
DCMPAS::DerivedVarVertical::DerivedVarVertical(NetCDFCollection *ncdfc, string staggeredVarName,
...
int DCMPAS::DerivedVarVertical::ReadSlice(float *slice, int) {
...
//
//
DCMPAS::DerivedVarWRFTime::DerivedVarWRFTime(NetCDFCollection *ncdfc, const UDUnits *udunits,
                                             string wrfTimeVar)
    : DerivedVar(ncdfc) {
...
int DCMPAS::DerivedVarWRFTime::ReadSlice(float *slice, int) {
