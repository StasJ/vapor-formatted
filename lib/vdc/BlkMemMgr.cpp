...
int BlkMemMgr::_Reinit(size_t n) {
    ...
    if (_mem_size_max_req == 0 || _blk_size_req == 0)
        return (false);
    ...
    size_t total_size = 0;
    int r;
    for (r = 0; r < _mem_regions.size(); r++)
        total_size += _mem_region_sizes[r];

    //
    // New region size is double preceding one
    //
    if (r > 0)
        mem_size = _mem_region_sizes[r - 1] << 1;

    // Make sure region size will be large enough, and not too large
    //
    if (mem_size < n)
        mem_size = n;
    if ((mem_size + total_size) > _mem_size_max)
        mem_size = _mem_size_max - total_size;
...
void *BlkMemMgr::Alloc(size_t n, bool fill) {
    ...
    if (fill) {
        ...
        for (size_t i = 0; i < n * _blk_size; i++)
            ptr[i] = 0;
    }
...
void BlkMemMgr::FreeMem(void *ptr) {
    ...
    if (!found)
        cerr << "Failed to free block " << ptr << endl;
