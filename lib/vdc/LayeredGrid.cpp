...
void LayeredGrid::_layeredGrid(const vector<bool> &periodic, const vector<double> &minu,
                               const vector<double> &maxu, const RegularGrid &rg) {
...
LayeredGrid::LayeredGrid(const vector<size_t> &bs, const vector<size_t> &min,
                         const vector<size_t> &max, const vector<double> &minu,
                         const vector<double> &maxu, const vector<bool> &periodic,
                         const vector<float *> &blks, const RegularGrid &rg)
...
#ifdef DEAD
LayeredGrid::LayeredGrid(const size_t bs[3], const size_t min[3], const size_t max[3],
                         const double extents[6], const bool periodic[3],
                         const vector<float *> &blks, const RegularGrid &rg)
    : RegularGrid(bs, min, max, periodic, blks) {
...
LayeredGrid::LayeredGrid(const vector<size_t> &bs, const vector<size_t> &min,
                         const vector<size_t> &max, const vector<double> &minu,
                         const vector<double> &maxu, const vector<bool> &periodic,
                         const vector<float *> &blks, const RegularGrid &rg, float missing_value)
...
void LayeredGrid::GetBoundingBox(const vector<size_t> &min, const vector<size_t> &max,
...
void LayeredGrid::GetEnclosingRegion(const vector<double> &minu, const vector<double> &maxu,
...
float LayeredGrid::_GetValueNearestNeighbor(double x, double y, double z) const {
...
void LayeredGrid::_GetUserExtents(vector<double> &minu, vector<double> &maxu) const {
    minu.clear();
    maxu.clear();
...
int LayeredGrid::GetUserCoordinates(size_t i, size_t j, size_t k, double *x, double *y,
                                    double *z) const {
...
void LayeredGrid::GetIJKIndex(double x, double y, double z, size_t *i, size_t *j, size_t *k) const {
...
void LayeredGrid::GetIJKIndexFloor(double x, double y, double z, size_t *i, size_t *j,
                                   size_t *k) const {
...
bool LayeredGrid::InsideGrid(double x, double y, double z) const {
    ...
    if (b00 < t00) {
        ...
        //
        if (b00 < vc && b01 < vc && b10 < vc && b11 < vc && t00 > vc && t01 > vc && t10 > vc &&
            t11 > vc) {
    ...
    double t =
        t00 + iwgt * (t01 - t00) + jwgt * ((t10 + iwgt * (t11 - t10)) - (t00 + iwgt * (t01 - t00)));
    double b =
        b00 + iwgt * (b01 - b00) + jwgt * ((b10 + iwgt * (b11 - b10)) - (b00 + iwgt * (b01 - b00)));
...
void LayeredGrid::_getBilinearWeights(double x, double y, double z, double &iwgt,
                                      double &jwgt) const {
    size_t dims[3];
    GetDimensions(dims);
...
double LayeredGrid::_bilinearInterpolation(size_t i0, size_t i1, size_t j0, size_t j1, size_t k0,
...
double LayeredGrid::_bilinearElevation(size_t i0, size_t i1, size_t j0, size_t j1, size_t k0,
                                       double iwgt, double jwgt) const {
    double xVal0, result;
    double xVal1 = 0.0;
...
double LayeredGrid::_interpolateVaryingCoord(size_t i0, size_t j0, size_t k0, double x, double y,
                                             double z) const {
    ...
    double c =
        c00 + iwgt * (c01 - c00) + jwgt * ((c10 + iwgt * (c11 - c10)) - (c00 + iwgt * (c01 - c00)));
    return (c);
}
