...
void LayeredGrid::_layeredGrid(const vector<double> &minu, const vector<double> &maxu,
...
LayeredGrid::LayeredGrid(const vector<size_t> &dims, const vector<size_t> &bs,
                         const vector<float *> &blks, const vector<double> &minu,
...
float LayeredGrid::_GetValueLinear(const std::vector<double> &coords) const {
    ...
    if (indices1[0] != dims[0] - 1) {
        indices1[0] += 1;
    }
    if (indices1[1] != dims[1] - 1) {
        indices1[1] += 1;
    }
    if (indices1[2] != dims[2] - 1) {
        indices1[2] += 1;
    }
...
void LayeredGrid::GetUserCoordinates(const std::vector<size_t> &indices,
...
void LayeredGrid::GetIndices(const std::vector<double> &coords,
    ...
    for (int i = 0; i < clampedCoords.size(); i++) {
        ...
        if (_delta[0] != 0.0) {
            wgt = ((clampedCoords[i] - _minu[i]) - (indices[i] * _delta[i])) / _delta[i];
        }
        if (wgt > 0.5)
            indices[i] += 1;
    }
    ...
    double z0 =
        _interpolateVaryingCoord(indices[0], indices[1], k0, clampedCoords[0], clampedCoords[1]);
    double z1 = _interpolateVaryingCoord(indices[0], indices[1], k0 + 1, clampedCoords[0],
                                         clampedCoords[1]);
...
void LayeredGrid::GetIndicesFloor(const std::vector<double> &coords,
                                  std::vector<size_t> &indices) const {
...
void LayeredGrid::_getBilinearWeights(const vector<double> &coords, double &iwgt,
                                      double &jwgt) const {
...
double LayeredGrid::_interpolateVaryingCoord(size_t i0, size_t j0, size_t k0, double x,
                                             double y) const {
...
namespace {
double pointDotPlane(const vector<double> &v1, const vector<double> &v2, const vector<double> &v3,
