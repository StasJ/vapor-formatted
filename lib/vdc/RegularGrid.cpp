...
void RegularGrid::_SetExtents(const vector<double> &minu, const vector<double> &maxu) {
    ...
    for (int i = 0; i < _minu.size(); i++) {
        _delta.push_back((_maxu[i] - _minu[i]) / (double)(_max[i] - _min[i]));
    }
}

RegularGrid::RegularGrid(const size_t bs[3], const size_t min[3], const size_t max[3],
...
RegularGrid::RegularGrid(const vector<size_t> &bs, const vector<size_t> &min,
...
float RegularGrid::_GetValueNearestNeighbor(double x, double y, double z) const {
    ...
    if (_delta[0] != 0.0)
        iwgt = ((x - _minu[0]) - (i * _delta[0])) / _delta[0];
    if (_delta[1] != 0.0)
        jwgt = ((y - _minu[1]) - (j * _delta[1])) / _delta[1];
    if (_ndim == 3)
        if (_delta[2] != 0.0)
            kwgt = ((z - _minu[2]) - (k * _delta[2])) / _delta[2];
...
void RegularGrid::GetBoundingBox(const vector<size_t> &min, const vector<size_t> &max,
    ...
    RegularGrid::GetUserCoordinates(a_min[0], a_min[1], a_min[2], &(a_minu[0]), &(a_minu[1]),
                                    &(a_minu[2]));
    RegularGrid::GetUserCoordinates(a_max[0], a_max[1], a_max[2], &(a_maxu[0]), &(a_maxu[1]),
                                    &(a_maxu[2]));
...
void RegularGrid::GetBoundingBox(const size_t min[3], const size_t max[3],
...
void RegularGrid::GetEnclosingRegion(const std::vector<double> &minu,
    ...
    RegularGrid::GetIJKIndex(a_minu[0], a_minu[1], a_minu[2], &temp_min[0], &temp_min[1],
                             &temp_min[2]);
    RegularGrid::GetIJKIndex(a_maxu[0], a_maxu[1], a_maxu[2], &temp_max[0], &temp_max[1],
                             &temp_max[2]);

    // Map the the closest voxels back to their actual user coordinates
    //
    double temp_minu[3], temp_maxu[3];
    RegularGrid::GetUserCoordinates(temp_min[0], temp_min[1], temp_min[2], &temp_minu[0],
                                    &temp_minu[1], &temp_minu[2]);
    RegularGrid::GetUserCoordinates(temp_max[0], temp_max[1], temp_max[2], &temp_maxu[0],
                                    &temp_maxu[1], &temp_maxu[2]);
...
int RegularGrid::GetUserCoordinates(size_t i, size_t j, size_t k, double *x, double *y,
                                    double *z) const {
...
void RegularGrid::GetIJKIndexFloor(double x, double y, double z, size_t *i, size_t *j,
                                   size_t *k) const {
