...
void downsample1d(const T *signalIn, size_t nIn, size_t strideIn, T *signalOut, size_t nOut,
    ...
    for (size_t i = 0; i < nOut; i++) {
        ...
        signalOut[i * strideOut] =
            (signalIn[i0 * strideIn] * (1.0 - w)) + (signalIn[(i0 + 1) * strideIn] * w);
    }
}
...
//
template <class T>
void copy_block(const T *src, T *dst, const vector<size_t> &min, const vector<size_t> &max,
        ...
        for (long j = min3[1], jj = 0; j <= (long)max3[1] && j < (long)dims3[1]; j++, jj++) {
            ...
            for (long i = min3[0], ii = 0; i <= (long)max3[0] && i < (long)dims3[0]; i++, ii++) {
                ...
                size_t dst_block_offset =
                    (dst_k_b * bdims3[0] * bdims3[1] + dst_j_b * bdims3[0] + dst_i_b) * block_size;
...
DataMgr::~DataMgr() {
    ...
    for (int i = 0; i < names.size(); i++) {
        if (_dvm.GetVar(names[i]))
            delete _dvm.GetVar(names[i]);
    }
...
vector<string> DataMgr::GetDataVarNames(int ndim) const {
    ...
    for (int i = 0; i < vars.size(); i++) {
        ...
        //
        if (_get_grid_type(vars[i]).empty())
            continue;
...
//
void DataMgr::_setupCoordVecsHelper(string data_varname, const vector<size_t> &data_dimlens,
    ...
    if (structured) {
        ...
        for (int j = 0; j < coord_dims.size(); j++) {
            while (data_dims[i].GetLength() != coord_dims[j].GetLength() && i < data_dims.size()) {
...
int DataMgr::_setupCoordVecs(size_t ts, string varname, int level, int lod,
                             ...
                             vector<vector<size_t>> &dimsvec, vector<vector<size_t>> &bsvec,
                             vector<vector<size_t>> &bminvec, vector<vector<size_t>> &bmaxvec,
                             bool structured) const {
    varnames.clear();
    roi_dims.clear();
    dimsvec.clear();
    ...
    for (int i = 0; i < cvarnames.size(); i++) {
        ...
        _setupCoordVecsHelper(varname, dims, bmin, bmax, cvarnames[i], i, coord_dims, coord_bmin,
                              coord_bmax, structured);
...
Grid *DataMgr::_getVariable(size_t ts, string varname, int level, int lod, vector<size_t> min,
    ...
    //
    int rc = _setupCoordVecs(ts, varname, level, lod, min, max, varnames, roi_dims, dimsvec, bsvec,
                             bminvec, bmaxvec, !_gridHelper.IsUnstructured(gridType));
    if (rc < 0)
        return (NULL);
    ...
    vector<float *> blkvec;
    rc = DataMgr::_get_regions<float>(ts, varnames, level, lod, true, dimsvec, bsvec, bminvec,
                                      bmaxvec, blkvec);
    if (rc < 0)
        return (NULL);
    ...
    if (_gridHelper.IsUnstructured(gridType)) {
        rc = _setupConnVecs(ts, varname, level, lod, conn_varnames, conn_dimsvec, conn_bsvec,
                            conn_bminvec, conn_bmaxvec);
        if (rc < 0)
            return (NULL);

        rc = DataMgr::_get_regions<int>(ts, conn_varnames, level, lod, true, conn_dimsvec,
    ...
    if (_gridHelper.IsUnstructured(gridType)) {
        ...
        _ugrid_setup(dvar, vertexDims, faceDims, edgeDims, location, maxVertexPerFace,
                     maxFacePerVertex, vertexOffset, faceOffset);

        rg = _gridHelper.MakeGridUnstructured(
            gridType, ts, level, lod, dvar, cvarsinfo, roi_dims, dimsvec[0], blkvec, bsvec, bminvec,
            bmaxvec, conn_blkvec, conn_bsvec, conn_bminvec, conn_bmaxvec, vertexDims, faceDims,
            edgeDims, location, maxVertexPerFace, maxFacePerVertex, vertexOffset, faceOffset);
    } else {
...
int DataMgr::GetVariableExtents(size_t ts, string varname, int level, int lod, vector<double> &min,
                                vector<double> &max) {
...
int DataMgr::GetDataRange(size_t ts, string varname, int level, int lod, vector<double> min,
                          vector<double> max, vector<double> &range) {
    SetDiagMsg("DataMgr::GetDataRange(%d,%s)", ts, varname.c_str());
...
int DataMgr::GetDimLensAtLevel(string varname, int level, std::vector<size_t> &dims_at_level,
    ...
    //
    if (std::all_of(bs_at_level.cbegin(), bs_at_level.cend(), [](size_t i) { return i == 1; })) {
...
vector<string> DataMgr::_get_var_dependencies_all(vector<string> varnames,
...
bool DataMgr::VariableExists(size_t ts, string varname, int level, int lod) const {
    ...
    //
    vector<string> varnames =
        _get_var_dependencies_all(vector<string>({varname}), vector<string>());
...
template <typename T>
int DataMgr::_get_unblocked_region_from_fs(size_t ts, string varname, int level, int lod,
...
int DataMgr::_get_blocked_region_from_fs(
    size_t ts, string varname, int level, int lod, const vector<size_t> &file_bs,
    const vector<size_t> &file_dims, const vector<size_t> &grid_dims, const vector<size_t> &grid_bs,
...
T *DataMgr::_get_region_from_fs(size_t ts, string varname, int level, int lod,
                                const vector<size_t> &grid_dims, const vector<size_t> &grid_bs,
                                const vector<size_t> &grid_bmin, const vector<size_t> &grid_bmax,
                                bool lock) {

    T *blks = (T *)_alloc_region(ts, varname, level, lod, grid_bmin, grid_bmax, grid_bs, sizeof(T),
                                 lock, false);
    if (!blks)
        return (NULL);
    ...
    if (!is_blocked(file_bs) || level < -nlevels) {
        rc = _get_unblocked_region_from_fs(ts, varname, level, lod, grid_dims, grid_bs, grid_min,
                                           grid_max, blks);
    } else {

        rc = _get_blocked_region_from_fs(ts, varname, level, lod, file_bs, file_dims, grid_dims,
                                         grid_bs, grid_min, grid_max, blks);
    }
    if (rc < 0) {
...
template <typename T>
T *DataMgr::_get_region(size_t ts, string varname, int level, int lod, int nlods,
                        const vector<size_t> &dims, const vector<size_t> &bs,
                        const vector<size_t> &bmin, const vector<size_t> &bmax, bool lock) {
    if (lod < -nlods)
        lod = -nlods;
    ...
    if (!blks) {
        ...
        blks = (T *)_get_region_from_fs<T>(ts, varname, level, lod, dims, bs, bmin, bmax, lock);
    }
    if (!blks) {
...
int DataMgr::_get_regions(size_t ts, const vector<string> &varnames, int level, int lod, bool lock,
    ...
    for (int i = 0; i < varnames.size(); i++) {
        ...
        T *blks = _get_region<T>(my_ts, varnames[i], level, lod, nlods, dimsvec[i], bsvec[i],
...
bool DataMgr::_hasHorizontalXForm() const {
    ...
    for (int i = 0; i < meshnames.size(); i++) {
        ...
        if (_hasHorizontalXForm(meshnames[i]))
            return (true);
    }
...
bool DataMgr::_hasVerticalXForm() const {
    ...
    for (int i = 0; i < meshnames.size(); i++) {
        ...
        if (_hasVerticalXForm(meshnames[i]))
            return (true);
    }
...
bool DataMgr::_hasVerticalXForm(string meshname, string &standard_name,
                                string &formula_terms) const {
    standard_name.clear();
    formula_terms.clear();
    ...
    if (!cvarInfo.GetAttribute("standard_name", attr_name))
        return (false);
    ...
    if (!cvarInfo.GetAttribute("formula_terms", attr_formula))
        return (false);
...
template <typename C>
string DataMgr::VarInfoCache<C>::_make_hash(string key, size_t ts, vector<string> varnames,
                                            int level, int lod) {
    ostringstream oss;
...
template <typename C>
void DataMgr::VarInfoCache<C>::_decode_hash(const string &hash, string &key, size_t &ts,
                                            vector<string> &varnames, int &level, int &lod) {
    varnames.clear();
...
template <typename C>
void DataMgr::VarInfoCache<C>::Set(size_t ts, vector<string> varnames, int level, int lod,
                                   string key, const vector<C> &values) {
    string hash = _make_hash(key, ts, varnames, level, lod);
    _cache[hash] = values;
}

template <typename C>
bool DataMgr::VarInfoCache<C>::Get(size_t ts, vector<string> varnames, int level, int lod,
                                   string key, vector<C> &values) const {
    values.clear();
...
template <typename C>
void DataMgr::VarInfoCache<C>::Purge(size_t ts, vector<string> varnames, int level, int lod,
                                     string key) {
    string hash = _make_hash(key, ts, varnames, level, lod);
...
bool DataMgr::_get_coord_vars(string varname, vector<DC::CoordVar> &scvarsinfo,
...
template <class T>
int DataMgr::_readRegion(int fd, const vector<size_t> &min, const vector<size_t> &max, T *region) {
    ...
    for (int i = 0; i < meshnames.size(); i++) {
        ...
        if (!_getDerivedCoordVar(derivedCoordvars[0])) {
            ...
            DerivedCoordVar_PCSFromLatLon *derivedVar =
                new DerivedCoordVar_PCSFromLatLon(derivedCoordvars[0], _dc, coordvars, _proj4String,
        ...
        if (!_getDerivedCoordVar(derivedCoordvars[1])) {
            ...
            DerivedCoordVar_PCSFromLatLon *derivedVar =
                new DerivedCoordVar_PCSFromLatLon(derivedCoordvars[1], _dc, coordvars, _proj4String,
...
int DataMgr::_initVerticalCoordVars() {
    ...
    if (!_doTransformVertical)
        return (0);

    if (!_hasVerticalXForm())
        return (0);
    ...
    for (int i = 0; i < meshnames.size(); i++) {
        ...
        if (DerivedCoordVarStandardWRF_Terrain::ValidFormula(formula_terms)) {
            derivedVar = new DerivedCoordVarStandardWRF_Terrain(_dc, meshnames[i], formula_terms);
        }
        if (DerivedCoordVarStandardOceanSCoordinateG2::ValidFormula(formula_terms)) {
            derivedVar =
                new DerivedCoordVarStandardOceanSCoordinateG2(_dc, meshnames[i], formula_terms);
        }
