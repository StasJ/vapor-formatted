...
//
void decimate1d(const vector<size_t> &src_bs, const float *src, float *dst) {
...
void decimate2d(const vector<size_t> &src_bs, const float *src, float *dst) {
...
void decimate3d(const vector<size_t> &src_bs, const float *src, float *dst) {
...
//
void grid_params(const DC::BaseVar &var, const vector<size_t> &min, const vector<size_t> &max,
                 const vector<size_t> &dims, vector<bool> &periodic, bool &has_missing,
                 float &missing) {
    periodic.clear();
    has_missing = false;
...
StructuredGrid *DataMgr::GetVariable(size_t ts, string varname, int level, int lod, bool lock) {
...
StructuredGrid *DataMgr::GetVariable(size_t ts, string varname, int level, int lod,
...
StructuredGrid *DataMgr::_getVariable(size_t ts, string varname, int level, int lod, bool lock,
                                      bool dataless) {
...
int DataMgr::_setupCoordVecs(size_t ts, string varname, int level, int lod,
                             ...
                             vector<string> &varnames, vector<vector<size_t>> &dimsvec,
...
StructuredGrid *DataMgr::_getVariable(size_t ts, string varname, int level, int lod,
                                      vector<size_t> min, vector<size_t> max, bool lock,
                                      bool dataless) {
    ...
    int rc = _setupCoordVecs(ts, varname, level, lod, min, max, varnames, dimsvec, dims_at_levelvec,
                             bsvec, bs_at_levelvec, bminvec, bmaxvec);
    if (rc < 0)
        return (NULL);
    ...
    vector<float *> blkvec;
    rc = DataMgr::_get_regions(ts, varnames, level, lod, true, bsvec, bminvec, bmaxvec, blkvec);
    if (rc < 0)
        return (NULL);
    ...
    if (DataMgr::IsVariableDerived(varname) && !blkvec[0]) {
        ...
        rg = _make_grid(dvar, min, max, dims_at_levelvec[0], blkvec, bs_at_levelvec, bminvec,
                        bmaxvec);
    }
...
StructuredGrid *DataMgr::GetVariable(size_t ts, string varname, int level, int lod,
    ...
    StructuredGrid *rg = _getVariable(ts, varname, level, lod, min, max, lock, false);
    if (!rg) {
        SetErrMsg("Failed to read variable \"%s\" at time step (%d), and\n"
                  "refinement level (%d) and level-of-detail (%d)",
                  varname.c_str(), ts, level, lod);
...
int DataMgr::GetDataRange(size_t ts, string varname, int level, int lod, vector<double> &range) {
    ...
    const StructuredGrid *sg = DataMgr::GetVariable(ts, varname, level, lod, false);
    if (!sg)
        return (-1);
...
void DataMgr::UnlockGrid(const StructuredGrid *rg) {
...
float *DataMgr::_get_region_from_cache(size_t ts, string varname, int level, int lod,
...
float *DataMgr::_get_region_from_fs(size_t ts, string varname, int level, int lod,
                                    const vector<size_t> &bs, const vector<size_t> &bmin,
                                    const vector<size_t> &bmax, bool lock) {

    float *blks =
        _alloc_region(ts, varname, level, lod, bmin, bmax, bs, sizeof(float), lock, false);
    if (!blks)
        return (NULL);
...
float *DataMgr::_get_region(size_t ts, string varname, int level, int nlevels, int lod, int nlods,
                            const vector<size_t> &bs, const vector<size_t> &bmin,
                            const vector<size_t> &bmax, bool lock) {
    if (lod < -nlods)
        lod = -nlods;
    ...
    // file system.
    //
    float *blks = _get_region_from_cache(ts, varname, level, lod, bmin, bmax, lock);
    if (!blks) {
        ...
        if (level < -nlevels) {
            ...
            blks = _get_region(ts, varname, level, nlevels, lod, nlods, bs, bmin, bmax, false);
            if (blks) {
                vector<size_t> bs_at_level = decimate_dims(bs, -level - 1);
                vector<size_t> bs_at_level_m1 = decimate_dims(bs, -level);

                float *newblks = _alloc_region(ts, varname, level - 1, lod, bmin, bmax,
                                               bs_at_level_m1, sizeof(float), false, false);
                if (!newblks)
                    return (NULL);
            ...
            blks = (float *)_get_region_from_fs(ts, varname, level, lod, bs_at_level, bmin, bmax,
                                                lock);
        }
        if (!blks) {
...
int DataMgr::_get_regions(size_t ts, const vector<string> &varnames, int level, int lod, bool lock,
                          ...
                          const vector<vector<size_t>> &bmaxvec, vector<float *> &blkvec) {
    ...
    for (int i = 0; i < varnames.size(); i++) {
        ...
        blks = _get_region(my_ts, varnames[i], level, nlevels, lod, nlods, bsvec[i], bminvec[i],
                           bmaxvec[i], true);
        if (!blks) {
            for (int i = 0; i < blkvec.size(); i++) {
                if (blkvec[i])
...
float *DataMgr::_alloc_region(size_t ts, string varname, int level, int lod, vector<size_t> bmin,
...
StructuredGrid *DataMgr::execute_pipeline(size_t ts, string varname, int level, int lod,
    ...
    for (int i = 0; i < input_varnames.size(); i++) {
        ...
        StructuredGrid *rg = GetGrid(ts, input_varnames[i], level, lod, min_in, max_in, true);
        if (!rg) {
    ...
    for (int i = 0; i < output_vars.size(); i++) {
        ...
        StructuredGrid *rg = _make_grid(ts, v, level, lod, min, max, blks, xcblks, ycblks, zcblks);
        if (!rg) {
            for (int j = 0; j < in_grids.size(); j++)
                UnlockGrid(in_grids[j]);
            for (int j = 0; j < out_grids.size(); j++)
...
#ifdef DEAD
float *DataMgr::_get_unblocked_region_from_fs(size_t ts, string varname, int level,
                                              const vector<size_t> &bs, const vector<size_t> &bmin,
                                              const vector<size_t> &bmax, bool lock) {
...
RegularGrid *DataMgr::_make_grid_regular(const DC::DataVar &var, const vector<size_t> &min,
                                         const vector<size_t> &max, const vector<size_t> &dims,
...
LayeredGrid *DataMgr::_make_grid_layered(const DC::DataVar &var, const vector<size_t> &min,
                                         const vector<size_t> &max, const vector<size_t> &dims,
                                         const vector<float *> &blkvec, const vector<size_t> &bs,
...
CurvilinearGrid *DataMgr::_make_grid_curvilinear(
    const DC::DataVar &var, const vector<DC::CoordVar> &cvarsinfo, const vector<size_t> &min,
    const vector<size_t> &max, const vector<size_t> &dims, const vector<float *> &blkvec,
...
StructuredGrid *DataMgr::_make_grid(const DC::DataVar &var, const vector<size_t> &min,
                                    const vector<size_t> &max, const vector<size_t> &dims,
    ...
    enum GridType { UNDEFINED = 0, REGULAR, LAYERED, CURVILINEAR };
    ...
    if (grid_type == REGULAR) {
        ...
        rg = _make_grid_curvilinear(var, cvarsinfo, min, max, dims, blkvec, bsvec[0], bminvec[0],
                                    bmaxvec[0]);
    }
...
int DataMgr::_find_bounding_grid(size_t ts, string varname, int level, int lod, vector<double> min,
    ...
    cout << "_find_bounding_grid lvl/lod: " << level << " " << lod << endl;
    ...
    for (int i = 0; i < max_ui.size(); i++) {
        if (max_ui[i] >= dims_at_level[i]) {
            cout << "CORRECTION " << max_ui[i] << " " << dims_at_level[i] << endl;
...
void DataMgr::_unlock_blocks(const float *blks) {
...
void DataMgr::_getKDSubtree2D(const vector<DC::CoordVar> &cvarsinfo, const RegularGrid &xrg,
