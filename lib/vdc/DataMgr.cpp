...
template <class T>
void copy_block(const T *src, const vector<size_t> &src_min, const vector<size_t> &src_max, T *dst,
                const vector<size_t> &dst_bs, const vector<size_t> &dst_min,
    ...
    for (size_t src_k = src_min_k, dst_k = dst_min_k; src_k <= src_max_k; src_k++, dst_k++) {
        if (dst_k % dst_bs_k == 0) {
            dst_kk = 0;
            dstptr = dst + block_size * ((dst_k / dst_bs_k) * dst_nb_k * dst_nb_i);
        }

        size_t dst_jj = dst_min_j % dst_bs_j;
        for (size_t src_j = src_min_j, dst_j = dst_min_j; src_j <= src_max_j; src_j++, dst_j++) {
            if (dst_j % dst_bs_j == 0) {
                dst_jj = 0;
                dstptr = dst + block_size * ((dst_k / dst_bs_k) * dst_nb_j * dst_nb_i +
                                             (dst_j / dst_bs_j) * dst_nb_i);
            }

            size_t dst_ii = dst_min_i % dst_bs_i;
            for (size_t src_i = src_min_i, dst_i = dst_min_i; src_i <= src_max_i;
                 src_i++, dst_i++) {
                if (dst_i % dst_bs_i == 0) {
                    dst_ii = 0;
                    dstptr =
                        dst + block_size * ((dst_k / dst_bs_k) * dst_nb_j * dst_nb_i +
                                            (dst_j / dst_bs_j) * dst_nb_i + (dst_i / dst_bs_i));
                }
...
template <typename T>
int DataMgr::_get_blocked_region_from_fs(size_t ts, string varname, int level, int lod,
...
T *DataMgr::_get_region_from_fs(size_t ts, string varname, int level, int lod,
    ...
    if (!is_blocked(file_bs) || level < -nlevels) {
        ...
        rc = _get_blocked_region_from_fs(ts, varname, level, lod, file_bs, grid_bs, grid_min,
                                         grid_max, blks);
