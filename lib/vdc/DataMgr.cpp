...
//
void DataMgr::_setupCoordVecsHelper(string data_varname, const vector<size_t> &data_bmin,
...
int DataMgr::_setupConnVecs(size_t ts, string varname, int level, int lod, vector<string> &varnames,
...
Grid *DataMgr::_getVariable(size_t ts, string varname, int level, int lod, vector<size_t> min,
    ...
    rc = _setupConnVecs(ts, varname, level, lod, conn_varnames, conn_dims_at_levelvec, conn_bsvec,
                        conn_bs_at_levelvec, conn_bminvec, conn_bmaxvec);
    if (rc < 0)
        return (NULL);
...
int DataMgr::GetVariableExtents(size_t ts, std::vector<std::string> varnames, int level,
...
int DataMgr::_initTimeCoord() {
    ...
    if (_udunits.IsTimeUnit(cvar.GetUnits())) {
        ...
        DerivedCoordVar_TimeInSeconds *derivedVar = new DerivedCoordVar_TimeInSeconds(
            derivedTimeCoordName, _dc, nativeTimeCoordName, cvar.GetTimeDimName());
...
CurvilinearGrid *
DataMgr::_make_grid_curvilinear(size_t ts, int level, int lod,
...
UnstructuredGrid2D *DataMgr::_make_grid_unstructured2d(
    ...
    const vector<DC::CoordVar> &cvarsinfo, const vector<size_t> &dims,
    const vector<float *> &blkvec, const vector<size_t> &bs, const vector<size_t> &bmin,
    const vector<size_t> &bmax, const vector<int *> &conn_blkvec, const vector<size_t> &conn_bs,
    ...
    UnstructuredGridCoordless xug(vertexDims, faceDims, edgeDims, bs, xcblkptrs, 2, vertexOnFace,
                                  faceOnVertex, faceOnFace, location, maxVertexPerFace,
                                  maxFacePerVertex);
    xug.SetNodeOffset(vertexOffset);
    xug.SetCellOffset(faceOffset);

    UnstructuredGridCoordless yug(vertexDims, faceDims, edgeDims, bs, ycblkptrs, 2, vertexOnFace,
                                  faceOnVertex, faceOnFace, location, maxVertexPerFace,
                                  maxFacePerVertex);
...
Grid *DataMgr::_make_grid(size_t ts, int level, int lod, const DC::DataVar &var,
    ...
    if (grid_type == REGULAR) {
        ...
        rg = _make_grid_curvilinear(ts, level, lod, cvarsinfo, roi_dims, blkvec, bsvec[0],
                                    bminvec[0], bmaxvec[0]);
    } else if (grid_type == UNSTRUC_2D) {
        rg = _make_grid_unstructured2d(ts, level, lod, var, cvarsinfo, roi_dims, blkvec, bsvec[0],
                                       bminvec[0], bmaxvec[0], conn_blkvec, conn_bsvec[0],
