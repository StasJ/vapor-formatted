...
int DataMgr::GetNumTimeSteps() const { return (_timeCoordinates.size() - 1); }
...
int DataMgr::_setupConnVecs(size_t ts, string varname, int level, int lod,
    ...
    bool ok = _dc->GetVarConnVars(varname, face_node_var, node_face_var, face_edge_var,
...
Grid *DataMgr::_getVariable(size_t ts, string varname, int level, int lod, vector<size_t> min,
    ...
    if (DataMgr::IsVariableDerived(varname) && !blkvec[0]) {
        ...
        rg = _make_grid(level, lod, dvar, roi_dims, dims_at_levelvec[0], blkvec, bs_at_levelvec,
...
bool DataMgr::VariableExists(size_t ts, string varname, int level, int lod) const {
    ...
    string edge_node_var;
    string edge_face_var;
    ok = _dc->GetVarConnVars(varname, face_node_var, node_face_var, face_edge_var, face_face_var,
...
CurvilinearGrid *
DataMgr::_make_grid_curvilinear(int level, int lod, const vector<DC::CoordVar> &cvarsinfo,
                                const vector<size_t> &dims, const vector<float *> &blkvec,
                                const vector<size_t> &bs, const vector<size_t> &bmin,
                                const vector<size_t> &bmax) {
    assert(dims.size() == bs.size());
    assert(dims.size() == bmin.size());
...
void DataMgr::_ugrid_setup(const DC::DataVar &var, std::vector<size_t> &vertexDims,
    ...
    bool ok = _dc->GetVarConnVars(var.GetName(), face_node_var, node_face_var, dummy, dummy, dummy,
                                  dummy);
    assert(ok);
...
UnstructuredGrid2D *DataMgr::_make_grid_unstructured2d(
    int level, int lod, const DC::DataVar &dvarinfo, const vector<DC::CoordVar> &cvarsinfo,
    const vector<size_t> &dims, const vector<float *> &blkvec, const vector<size_t> &bs,
    const vector<size_t> &bmin, const vector<size_t> &bmax, const vector<int *> &conn_blkvec,
    ...
    UnstructuredGridCoordless xug(vertexDims, faceDims, edgeDims, bs1d, xcblkptrs, 2, vertexOnFace,
                                  faceOnVertex, faceOnFace, location, maxVertexPerFace,
                                  maxFacePerVertex);
    xug.SetNodeOffset(vertexOffset);
    xug.SetCellOffset(faceOffset);

    UnstructuredGridCoordless yug(vertexDims, faceDims, edgeDims, bs1d, ycblkptrs, 2, vertexOnFace,
                                  faceOnVertex, faceOnFace, location, maxVertexPerFace,
                                  maxFacePerVertex);
    ...
    UnstructuredGrid2D *g = new UnstructuredGrid2D(
        vertexDims, faceDims, edgeDims, bs1d, blkptrs, vertexOnFace, faceOnVertex, faceOnFace,
...
Grid *DataMgr::_make_grid(int level, int lod, const DC::DataVar &var,
    ...
    if (grid_type == REGULAR) {
        ...
        rg = _make_grid_curvilinear(level, lod, cvarsinfo, roi_dims, blkvec, bsvec[0], bminvec[0],
                                    bmaxvec[0]);
    } else if (grid_type == UNSTRUC_2D) {
        rg = _make_grid_unstructured2d(level, lod, var, cvarsinfo, roi_dims, blkvec, bsvec[0],
...
const KDTreeRG *DataMgr::_getKDTree2D(int level, int lod, const vector<DC::CoordVar> &cvarsinfo,
                                      const Grid &xg, const Grid &yg) {
