...
Grid *DataMgr::_getVariable(size_t ts, string varname, int level, int lod, vector<size_t> min,
    ...
    if (_gridHelper.IsUnstructured(gridType)) {
        rc = _setupConnVecs(ts, varname, level, lod, conn_varnames, conn_dims_at_levelvec,
                            conn_bsvec, conn_bs_at_levelvec, conn_bminvec, conn_bmaxvec);
        if (rc < 0)
            return (NULL);

        rc = DataMgr::_get_regions<int>(ts, conn_varnames, level, lod, true, conn_bsvec,
                                        conn_bminvec, conn_bmaxvec, conn_blkvec);
        if (rc < 0)
            return (NULL);
    }
    ...
    if (DataMgr::IsVariableDerived(varname) && !blkvec[0]) {
        ...
        if (_gridHelper.IsUnstructured(gridType)) {
            ...
            rg = _gridHelper.MakeGridUnstructured(
                gridType, ts, level, lod, dvar, cvarsinfo, roi_dims, dims_at_levelvec[0], blkvec,
                bs_at_levelvec, bminvec, bmaxvec, conn_blkvec, conn_bs_at_levelvec, conn_bminvec,
                conn_bmaxvec, vertexDims, faceDims, edgeDims, location, maxVertexPerFace,
                maxFacePerVertex, vertexOffset, faceOffset);
        } else {
            rg = _gridHelper.MakeGridStructured(gridType, ts, level, lod, dvar, cvarsinfo, roi_dims,
                                                dims_at_levelvec[0], blkvec, bs_at_levelvec,
                                                bminvec, bmaxvec);
        }
        assert(rg);
    }
...
bool DataMgr::_hasVerticalConversion() const {
    ...
    for (int i = 0; i < varnames.size(); i++) {
        ...
        string dummy;
        if (_hasVerticalConversion(varnames[i], dummy, dummy))
            return (true);
    }
...
bool DataMgr::_hasVerticalConversion(string varname, string &standard_name,
    ...
    if (!dvarInfo.GetAttribute("standard_name", attr_name))
        return (false);
    ...
    if (!dvarInfo.GetAttribute("formula_terms", attr_formula))
        return (false);
...
int DataMgr::_initVerticalCoordVars() {
    ...
    for (int i = 0; i < varnames.size(); i++) {
        ...
        if (!_hasVerticalConversion(varnames[i], standard_name, formula_terms)) {
            continue;
        }
        ...
        DerivedCoordVarStandardWRF_Terrain *derivedVar =
            new DerivedCoordVarStandardWRF_Terrain(derivedCoordvars[0], _dc, formula_terms);
