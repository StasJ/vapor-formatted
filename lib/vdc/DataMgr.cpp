...
StructuredGrid *DataMgr::_getVariable(size_t ts, string varname, int level, int lod,
    ...
    int rc = _setupCoordVecs(ts, varname, level, lod, min, max, varnames, roi_dims, dimsvec,
...
StructuredGrid *DataMgr::_make_grid(int level, int lod, const DC::DataVar &var,
    ...
    if (grid_type == REGULAR) {
        ...
        rg = _make_grid_curvilinear(level, lod, cvarsinfo, dims, blkvec, bsvec[0], bminvec[0],
                                    bmaxvec[0]);
    }
...
const KDTreeRG *DataMgr::_getKDTree2D(int level, int lod, const vector<size_t> &bmin,
                                      const vector<size_t> &bmax,
                                      const vector<DC::CoordVar> &cvarsinfo, const RegularGrid &xrg,
