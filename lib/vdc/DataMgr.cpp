...
void DataMgr::_setupCoordVecsHelper(string data_varname, const vector<size_t> &data_bmin,
                                    ...
                                    int order, vector<size_t> &coord_bmin,
...
int DataMgr::_setupCoordVecs(size_t ts, string varname, int level, int lod,
                             ...
                             vector<size_t> &dims, vector<vector<size_t>> &bsvec,
    ...
    for (int i = 0; i < cvarnames.size(); i++) {
        ...
        _setupCoordVecsHelper(varname, bmin, bmax, cvarnames[i], i, coord_bmin, coord_bmax,
                              structured);
...
int DataMgr::_setupConnVecs(size_t ts, string varname, int level, int lod, vector<string> &varnames,
                            vector<vector<size_t>> &bsvec, vector<vector<size_t>> &bminvec,
...
Grid *DataMgr::_getVariable(size_t ts, string varname, int level, int lod, vector<size_t> min,
    ...
    if (_gridHelper.IsUnstructured(gridType)) {
        rc = _setupConnVecs(ts, varname, level, lod, conn_varnames, conn_bsvec, conn_bminvec,
                            conn_bmaxvec);
        if (rc < 0)
            return (NULL);
    ...
    if (_gridHelper.IsUnstructured(gridType)) {
        ...
        rg = _gridHelper.MakeGridUnstructured(
            gridType, ts, level, lod, dvar, cvarsinfo, roi_dims, dims, blkvec, bsvec, bminvec,
        ...
        rg = _gridHelper.MakeGridStructured(gridType, ts, level, lod, dvar, cvarsinfo, roi_dims,
                                            dims, blkvec, bsvec, bminvec, bmaxvec);
    }
    assert(rg);
...
int DataMgr::_get_region_from_fs_helper(
    size_t ts, string varname, int level, int lod, const vector<size_t> &file_bmin,
    const vector<size_t> &file_bmax, const vector<size_t> &file_bs,
    const vector<size_t> &downsample_bs, const vector<size_t> &grid_bs,
    ...
    // at once
    //
    if (std::all_of(file_bs.cbegin(), file_bs.cend(), [](size_t i) { return i == 1; })) {
        ...
        for (size_t i = 0; i < VProduct(Dims(file_bmin, file_bmax)); i++) {
            ...
            if (file_bs != downsample_bs) {
                ...
                copy_block(downsample_block, min, max, blks, grid_bs, grid_min, grid_max);
            } else {
                copy_block(file_block, min, max, blks, grid_bs, grid_min, grid_max);
            }
...
T *DataMgr::_get_region_from_fs(size_t ts, string varname, int level, int lod,
    ...
    rc = _get_region_from_fs_helper(ts, varname, level, lod, file_bmin, file_bmax, file_bs,
...
T *DataMgr::_get_region(size_t ts, string varname, int level, int nlevels, int lod, int nlods,
    ...
    if (!blks) {
        ...
        blks = (T *)_get_region_from_fs<T>(ts, varname, level, lod, bs, bmin, bmax, lock);
    }
    if (!blks) {
