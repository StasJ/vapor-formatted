...
//
void grid_params(const DC::DataVar &var, const vector<size_t> &roi_dims, const vector<size_t> &dims,
                 vector<bool> &periodic, bool &has_missing, float &missing) {
...
void coord_setup_helper(const vector<string> &dimnames, const vector<size_t> &dims,
                        ...
                        const vector<size_t> &bmax, const vector<string> &my_dimnames,
                        vector<size_t> &my_dims, vector<size_t> &my_dims_at_level,
                        vector<size_t> &my_bs, vector<size_t> &my_bs_at_level,
                        vector<size_t> &my_bmin, vector<size_t> &my_bmax) {
...
bool DataMgr::GetMesh(string meshname, DC::Mesh &m) const {
    ...
    //
    if (_doTransformHeight) {
        _assignVerticalCoords(coord_vars);
    }
...
bool DataMgr::IsTimeVarying(string varname) const {
    ...
    if (ok)
        return (!dvar.GetTimeCoordVar().empty());
    ...
    if (ok)
        return (!cvar.GetTimeDimName().empty());
...
Grid *DataMgr::_getVariable(size_t ts, string varname, int level, int lod, bool lock,
                            bool dataless) {
...
//
void DataMgr::_setupCoordVecsHelper(string data_varname, const vector<size_t> &data_min,
    ...
    for (int j = 0; j < coord_dimnames.size(); j++) {
        while (data_dimnames[i] != coord_dimnames[j] && i < data_dimnames.size()) {
            i++;
        }
...
int DataMgr::_setupCoordVecs(size_t ts, string varname, int level, int lod,
                             const vector<size_t> &min, const vector<size_t> &max,
                             vector<string> &varnames, vector<size_t> &roi_dims,
    ...
    for (int i = 0; i < varnames.size(); i++) {
        ...
        vector<size_t> dims_at_level;
        rc = DataMgr::GetDimLensAtLevel(varnames[i], level, dims_at_level, bs_at_level);
        assert(rc >= 0);
        ...
        _setupCoordVecsHelper(varnames[0], min, max, varnames[i], myMin, myMax);
    ...
    for (int i = 0; i < varnames.size(); i++) {
        ...
        coord_setup_helper(dimnames, dims, dims_at_level, bs, bs_at_level, bmin, bmax, my_dimnames,
...
int DataMgr::_setupConnVecs(size_t ts, string varname, int level, int lod,
                            const vector<size_t> &min, const vector<size_t> &max,
                            vector<string> &varnames, vector<vector<size_t>> &dims_at_levelvec,
                            vector<vector<size_t>> &bsvec, vector<vector<size_t>> &bs_at_levelvec,
                            vector<vector<size_t>> &bminvec,
                            vector<vector<size_t>> &bmaxvec) const {
    varnames.clear();
    ...
    bool ok = _getVarConnVars(varname, face_node_var, node_face_var, face_edge_var, face_face_var,
                              edge_node_var, edge_face_var);
    if (!ok) {
        SetErrMsg("Invalid variable reference : %s", varname.c_str());
        return (-1);
    }
    ...
    if (!face_node_var.empty())
        varnames.push_back(face_node_var);
    if (!node_face_var.empty())
        varnames.push_back(node_face_var);
    if (!face_edge_var.empty())
        varnames.push_back(face_edge_var);
    if (!face_face_var.empty())
        varnames.push_back(face_face_var);
    if (!edge_node_var.empty())
        varnames.push_back(edge_node_var);
    if (!edge_face_var.empty())
        varnames.push_back(edge_face_var);
...
Grid *DataMgr::_getVariable(size_t ts, string varname, int level, int lod, vector<size_t> min,
                            vector<size_t> max, bool lock, bool dataless) {
    Grid *rg = NULL;
    ...
    //
    int rc = _setupCoordVecs(ts, varname, level, lod, min, max, varnames, roi_dims,
    ...
    vector<float *> blkvec;
    rc = DataMgr::_get_regions<float>(ts, varnames, level, lod, true, bsvec, bminvec, bmaxvec,
                                      blkvec);
    if (rc < 0)
        return (NULL);
    ...
    rc = _setupConnVecs(ts, varname, level, lod, min, max, conn_varnames, conn_dims_at_levelvec,
                        conn_bsvec, conn_bs_at_levelvec, conn_bminvec, conn_bmaxvec);
    if (rc < 0)
        return (NULL);

    vector<int *> conn_blkvec;
    rc = DataMgr::_get_regions<int>(ts, conn_varnames, level, lod, true, conn_bsvec, conn_bminvec,
    ...
    if (DataMgr::IsVariableDerived(varname) && !blkvec[0]) {
        ...
        rg = _make_grid(ts, level, lod, dvar, roi_dims, dims_at_levelvec[0], blkvec, bs_at_levelvec,
                        bminvec, bmaxvec, conn_blkvec, conn_bs_at_levelvec, conn_bminvec,
                        conn_bmaxvec);
        assert(rg);
    }
...
int DataMgr::GetVariableExtents(size_t ts, string varname, int level, vector<double> &min,
                                vector<double> &max) {
    min.clear();
    max.clear();
...
int DataMgr::GetDimLensAtLevel(string varname, int level, std::vector<size_t> &dims_at_level,
    ...
    if (dvar) {
        return (dvar->GetDimLensAtLevel(varname, level, dims_at_level, bs_at_level));
    }
    ...
    for (int i = 0; i < native_vars.size(); i++) {
        for (int j = 0; j < my_outputs.size(); j++) {
            if (native_vars[i].compare(my_outputs[j].first) == 0) {
                SetErrMsg("Pipeline output %s matches native variable name",
...
vector<string> DataMgr::_get_var_dependencies(string varname) const {
    ...
    string edge_node_var;
    string edge_face_var;
    ok = _getVarConnVars(varname, face_node_var, node_face_var, face_edge_var, face_face_var,
                         edge_node_var, edge_face_var);
    if (ok) {
...
template <typename T>
T *DataMgr::_get_region_from_cache(size_t ts, string varname, int level, int lod,
...
template <typename T>
T *DataMgr::_get_region_from_fs(size_t ts, string varname, int level, int lod,
...
template <typename T>
T *DataMgr::_get_region(size_t ts, string varname, int level, int nlevels, int lod, int nlods,
                        const vector<size_t> &bs, const vector<size_t> &bmin,
                        const vector<size_t> &bmax, bool lock) {
    if (lod < -nlods)
        lod = -nlods;
    ...
    if (!blks) {
        ...
        if (level < -nlevels) {
            ...
            if (blks) {
                ...
                T *newblks = (T *)_alloc_region(ts, varname, level - 1, lod, bmin, bmax,
                                                bs_at_level_m1, sizeof(T), false, false);
                if (!newblks)
                    return (NULL);
            ...
            blks =
                (T *)_get_region_from_fs<T>(ts, varname, level, lod, bs_at_level, bmin, bmax, lock);
        }
        if (!blks) {
            SetErrMsg("Failed to read region from variable/timestep/level/lod (%s, %d, %d, %d)",
                      varname.c_str(), ts, level, lod);
            return (NULL);
        }
    }
...
int DataMgr::_get_regions(size_t ts, const vector<string> &varnames, int level, int lod, bool lock,
                          ...
                          const vector<vector<size_t>> &bmaxvec, vector<T *> &blkvec) {
    ...
    for (int i = 0; i < varnames.size(); i++) {
        ...
        T *blks = _get_region<T>(my_ts, varnames[i], level, nlevels, lod, nlods, bsvec[i],
...
Grid *DataMgr::execute_pipeline(size_t ts, string varname, int level, int lod, const size_t min[3],
                                const size_t max[3], bool lock, float *xcblks, float *ycblks,
                                float *zcblks) {
...
bool DataMgr::cycle_check(const map<string, vector<string>> &graph, const string &node,
...
vector<string> DataMgr::_get_derived_variables() const {
    ...
    for (itr1 = _derivedCoordVars.begin(); itr1 != _derivedCoordVars.end(); ++itr1) {
        svec.push_back(itr1->first);
    }

    map<string, DerivedDataVar *>::const_iterator itr2;
    for (itr2 = _derivedDataVars.begin(); itr2 != _derivedDataVars.end(); ++itr2) {
        svec.push_back(itr2->first);
    }
...
string DataMgr::VarInfoCache::_make_hash(string key, size_t ts, vector<string> varnames, int level,
                                         int lod) {
    ostringstream oss;
...
void DataMgr::VarInfoCache::Set(size_t ts, vector<string> varnames, int level, int lod, string key,
                                const vector<size_t> &values) {
    string hash = _make_hash(key, ts, varnames, level, lod);
    _cacheSize_t[hash] = values;
}

bool DataMgr::VarInfoCache::Get(size_t ts, vector<string> varnames, int level, int lod, string key,
...
void DataMgr::VarInfoCache::PurgeSize_t(size_t ts, vector<string> varnames, int level, int lod,
                                        string key) {
    string hash = _make_hash(key, ts, varnames, level, lod);
...
void DataMgr::VarInfoCache::PurgeDouble(size_t ts, vector<string> varnames, int level, int lod,
                                        string key) {
    string hash = _make_hash(key, ts, varnames, level, lod);
...
void DataMgr::VarInfoCache::PurgeVoidPtr(size_t ts, vector<string> varnames, int level, int lod,
                                         string key) {
    string hash = _make_hash(key, ts, varnames, level, lod);
...
void DataMgr::BlkExts::Insert(const std::vector<size_t> &bcoord, const std::vector<double> &min,
...
RegularGrid *DataMgr::_make_grid_regular(const vector<size_t> &dims, const vector<float *> &blkvec,
                                         const vector<size_t> &bs, const vector<size_t> &bmin,
...
StretchedGrid *DataMgr::_make_grid_stretched(const vector<size_t> &dims,
    ...
    for (int i = 0; i < dims[0]; i++)
        xcoords.push_back(blkvec[1][i]);

    vector<double> ycoords;
    for (int i = 0; i < dims[1]; i++)
        ycoords.push_back(blkvec[2][i]);
...
LayeredGrid *DataMgr::_make_grid_layered(const vector<size_t> &dims, const vector<float *> &blkvec,
    ...
    RegularGrid rg(dims, bs[3], zcblkptrs, vector<double>(3, 0.0), vector<double>(3, 1.0));
...
CurvilinearGrid *DataMgr::_make_grid_curvilinear(
    ...
    const vector<size_t> &dims, const vector<float *> &blkvec, const vector<vector<size_t>> &bs,
    const vector<vector<size_t>> &bmin, const vector<vector<size_t>> &bmax) {
    assert(bs.size() >= 3);
    assert(bs.size() == bmin.size());
    assert(bs.size() == bmax.size());
    assert(dims.size() == bs[0].size());
    ...
    CurvilinearGrid *g = new CurvilinearGrid(dims, bs[0], blkptrs, xrg, yrg, zcoords, kdtree);

    return (g);
}

void DataMgr::_ugrid_setup(const DC::DataVar &var, std::vector<size_t> &vertexDims,
                           std::vector<size_t> &faceDims, std::vector<size_t> &edgeDims,
                           UnstructuredGrid::Location &location, // node,face, edge
                           size_t &maxVertexPerFace, size_t &maxFacePerVertex, long &vertexOffset,
...
UnstructuredGrid2D *DataMgr::_make_grid_unstructured2d(
    size_t ts, int level, int lod, const DC::DataVar &dvarinfo,
    ...
    _ugrid_setup(dvarinfo, vertexDims, faceDims, edgeDims, location, maxVertexPerFace,
    ...
    UnstructuredGridCoordless xug(vertexDims, faceDims, edgeDims, bs[1], xcblkptrs, 2, vertexOnFace,
                                  faceOnVertex, faceOnFace, location, maxVertexPerFace,
                                  maxFacePerVertex);
    xug.SetNodeOffset(vertexOffset);
    xug.SetCellOffset(faceOffset);

    UnstructuredGridCoordless yug(vertexDims, faceDims, edgeDims, bs[2], ycblkptrs, 2, vertexOnFace,
                                  faceOnVertex, faceOnFace, location, maxVertexPerFace,
                                  maxFacePerVertex);
    ...
    UnstructuredGrid2D *g = new UnstructuredGrid2D(
        vertexDims, faceDims, edgeDims, bs[0], blkptrs, vertexOnFace, faceOnVertex, faceOnFace,
        location, maxVertexPerFace, maxFacePerVertex, xug, yug, zug, kdtree);
...
Grid *DataMgr::_make_grid(size_t ts, int level, int lod, const DC::DataVar &var,
                          const vector<size_t> &roi_dims, const vector<size_t> &dims,
                          const vector<float *> &blkvec, const vector<vector<size_t>> &bsvec,
                          const vector<vector<size_t>> &bminvec,
                          const vector<vector<size_t>> &bmaxvec, const vector<int *> &conn_blkvec,
    ...
    if (grid_type == REGULAR) {
        ...
        rg = _make_grid_layered(roi_dims, blkvec, bsvec, bminvec, bmaxvec);
    } else if (grid_type == CURVILINEAR) {
        rg = _make_grid_curvilinear(ts, level, lod, cvarsinfo, roi_dims, blkvec, bsvec, bminvec,
                                    bmaxvec);
    } else if (grid_type == UNSTRUC_2D) {
        rg = _make_grid_unstructured2d(ts, level, lod, var, cvarsinfo, roi_dims, blkvec, bsvec,
                                       bminvec, bmaxvec, conn_blkvec, conn_bsvec[0],
                                       conn_bminvec[0], conn_bmaxvec[0]);
    }
...
DataMgr::GridType DataMgr::_get_grid_type(const DC::DataVar &var,
    ...
    if (!grid_type) {
        if (cdimnames.size() == 3 && cvarsinfo[0].GetUniform() && cvarsinfo[1].GetUniform() &&
            cdimnames[2].size() == 3) {
            grid_type = LAYERED;
        }
    }
    if (!grid_type) {
        if ((cdimnames.size() == 2 || (cdimnames.size() == 3 && cdimnames[2].size() == 1)) &&
...
const KDTreeRG *DataMgr::_getKDTree2D(size_t ts, int level, int lod,
    ...
    vector<void *> values;
    bool found = _varInfoCache.Get(ts, varnames, level, lod, key, values);
    if (found) {
        ...
        _varInfoCache.Set(ts, varnames, level, lod, key, values);
    }
...
vector<string> DataMgr::_getCoordVarNamesDerived() const {
    ...
    for (itr = _derivedCoordVars.begin(); itr != _derivedCoordVars.end(); ++itr) {
...
bool DataMgr::_getBaseVarInfoDerived(string varname, VAPoR::DC::BaseVar &varInfo) const {
    ...
    if (itr1 != _derivedCoordVars.end()) {
        return (itr1->second->GetBaseVarInfo(varname, varInfo));
    }

    map<string, DerivedDataVar *>::const_iterator itr2;
    itr2 = _derivedDataVars.find(varname);

    if (itr2 != _derivedDataVars.end()) {
        return (itr2->second->GetBaseVarInfo(varname, varInfo));
    }
...
void DataMgr::_assignVerticalCoords(vector<string> &coord_vars) const {
    ...
    for (int i = 0; i < coord_vars.size(); i++) {
        ...
        if (varInfo.GetAxis() == 2 && !_udunits.IsLengthUnit(varInfo.GetUnits())) {
            coord_vars[i] = coord_vars[i] + "Z";
        }
    }
}
...
bool DataMgr::_getVarConnVars(string varname, string &face_node_var, string &node_face_var,
...
int DataMgr::_openVariableRead(size_t ts, string varname, int level, int lod) {
    ...
    if (derivedVar) {
        return (derivedVar->OpenVariableRead(ts, _openVarName, level, lod));
    }
...
template <class T>
int DataMgr::_readRegionBlock(int fd, const vector<size_t> &min, const vector<size_t> &max,
                              T *region) {
...
int DataMgr::_readRegion(int fd, const vector<size_t> &min, const vector<size_t> &max,
                         float *region) {
...
int DataMgr::_getCoordPairExtents(string lon, string lat, float &lonmin, float &lonmax,
...
int DataMgr::_initProj4String() {
    ...
    for (int i = 0; i < meshnames.size() && coordvars.size() < 2; i++) {
        ...
        if (!_is_geographicMesh(meshnames[i]))
            continue;
...
int DataMgr::_initHorizontalCoordVars() {
    ...
    for (int i = 0; i < meshnames.size(); i++) {
        ...
        DerivedCoordVar_PCSFromLatLon *derivedVar =
            new DerivedCoordVar_PCSFromLatLon(derivedCoordvars, _dc, coordvars, _proj4String,
                                              m.GetMeshType() != DC::Mesh::STRUCTURED);
