...
template <class T> void _ShiftLonTemplate(const T *srclon, int nx, int ny, T *dstlon) {
    ...
    for (int j = 0; j < ny; j++) {
        for (int i = 1; i < nx; i++) {
            ...
            if (dstlon[j * nx + i] < min)
                min = dstlon[j * nx + i];
            if (dstlon[j * nx + i] > max)
                max = dstlon[j * nx + i];
        }
    }
...
template <class T> void _LonExtentsTemplate(const T *lon, int nx, int ny, T &lonwest, T &loneast) {
    ...
    for (int i = 0; i < nx; i++)
        lonX0[i] = lon[i];
    for (int i = 0; i < nx; i++)
        lonX1[i] = lon[(nx * (ny - 1)) + i];
    for (int i = 0; i < ny; i++)
        lonY0[i] = lon[i * nx];
    for (int i = 0; i < ny; i++)
        lonY1[i] = lon[i * nx + nx - 1];
...
template <class T>
void _LatExtentsTemplate(const T *lat, int nx, int ny, T &latsouth, T &latnorth) {
...
void GeoUtil::LonExtents(const float *lon, int nx, int ny, float &lonwest, float &loneast) {
    _LonExtentsTemplate(lon, nx, ny, lonwest, loneast);
}

void GeoUtil::LonExtents(const double *lon, int nx, int ny, double &lonwest, double &loneast) {
    _LonExtentsTemplate(lon, nx, ny, lonwest, loneast);
}

void GeoUtil::LonExtents(const float *lon, int nx, float &lonwest, float &loneast) {
    _LonExtentsTemplate(lon, nx, lonwest, loneast);
}

void GeoUtil::LonExtents(const double *lon, int nx, double &lonwest, double &loneast) {
    _LonExtentsTemplate(lon, nx, lonwest, loneast);
}

void GeoUtil::LatExtents(const float *lat, int nx, int ny, float &latsouth, float &latnorth) {
    _LatExtentsTemplate(lat, nx, ny, latsouth, latnorth);
}

void GeoUtil::LatExtents(const double *lat, int nx, int ny, double &latsouth, double &latnorth) {
    _LatExtentsTemplate(lat, nx, ny, latsouth, latnorth);
}

void GeoUtil::LatExtents(const float *lat, int ny, float &latsouth, float &latnorth) {
    _LatExtentsTemplate(lat, ny, latsouth, latnorth);
}

void GeoUtil::LatExtents(const double *lat, int ny, double &latsouth, double &latnorth) {
    _LatExtentsTemplate(lat, ny, latsouth, latnorth);
}

void GeoUtil::ExtractBoundary(const float *a, int nx, int ny, float *bdry) {
    _ExtractBoundaryTemplate(a, nx, ny, bdry);
}

void GeoUtil::ExtractBoundary(const double *a, int nx, int ny, double *bdry) {
    _ExtractBoundaryTemplate(a, nx, ny, bdry);
}
