...
void VDCDimension_GetName(const VDCDimension *p, char **name) {
    _stringToCString(p->GetName(), name);
}
...
void VDCBaseVar_GetUnits(const VDCBaseVar *p, char **units) {
    _stringToCString(p->GetUnits(), units);
}
...
void VDCAuxVar_GetDimNames(const VDCAuxVar *p, char ***names, int *count) {
    _stringVectorToCStringArray(p->GetDimNames(), names, count);
}
...
void VDCCoordVar_GetTimeDimName(const VDCCoordVar *p, char **name) {
    _stringToCString(p->GetTimeDimName(), name);
}
...
int VDC_Initialize(VDC *p, const char *path, int mode, size_t *bs, int bsCount) {
    ...
    if (bs != NULL && bsCount > 0) {
        VDC_DEBUG_printff("(%s \"%s\", std::vector<std::string>(), %s, %s, 0);\n", _VDCPToStr(p),
                          path, _accessModeToStr(mode), bs ? "<bs[]>" : "NULL");
    } else {
        VDC_DEBUG_printff("(%s \"%s\", std::vector<std::string>(), %s);\n", _VDCPToStr(p), path,
                          _accessModeToStr(mode));
    }
#else
...
int VDC_GetAtt_long(const VDC *p, const char *varname, const char *attname, long **values,
                    int *count) {
...
int VDC_GetAtt_double(const VDC *p, const char *varname, const char *attname, double **values,
                      int *count) {
    VDC_DEBUG_called_readF();
...
int VDC_GetVarDimLensAtLevel(const VDC *p, const char *varname, int level, size_t **lens,
                             int *count) {
    VDC_DEBUG_called_readF();
...
int VDC_ReadRegion(VDC *p, int fd, const size_t *min, const size_t *max, const int dims,
                   float *region) {
...
int VDC_SetCompressionBlock(VDC *p, const char *wname, const size_t *cratios, int cratiosCount) {
    ...
    VDC_DEBUG_printff("(%s \"%s\", %s);\n", _VDCPToStr(p), wname,
                      _size_tArrayToString(cratios, cratiosCount).c_str());
#else
    VDC_DEBUG_printff("(%s \"%s\", %s, %i);\n", _VDCPToStr(p), wname,
                      _size_tArrayToString(cratios, cratiosCount).c_str(), cratiosCount);
#endif
...
int VDC_DefineDataVar(VDC *p, const char *varname, const char **dimnames, size_t dimnamesCount,
                      const char **coordvars, size_t coordvarsCount, const char *units,
                      VDC_XType xtype, int compressed) {
#ifdef VDC_DEBUG_CPP_RUN
    VDC_DEBUG_printff("(%s \"%s\", %s, %s, \"%s\", %s, %s);\n", _VDCPToStr(p), varname,
                      _strArrayToString(dimnames, dimnamesCount).c_str(),
                      _strArrayToString(coordvars, coordvarsCount).c_str(), units,
    ...
    VDC_DEBUG_printff("(%s \"%s\", %s, %li, %s, %li, \"%s\", %s, %s);\n", _VDCPToStr(p), varname,
                      ...
                      _strArrayToString(coordvars, coordvarsCount).c_str(), coordvarsCount, units,
                      _XTypeToIntCodeString(_IntToXType(xtype)), _boolToStr(compressed));
#endif
    ...
    VDC_DEBUG_printff_nRun(": calling VDC::DefineDataVar(\"%s\", %s, %s, \"%s\", %s, %s);\n",
                           varname, _stringVectorToString(dimnames_v).c_str(),
                           _stringVectorToString(coordvars_v).c_str(), units,
                           _XTypeToString(_IntToXType(xtype)), _boolToStr(compressed));
    int ret = p->DefineDataVar(string(varname), dimnames_v, coordvars_v, string(units),
                               _IntToXType(xtype), compressed);
...
int VDC_DefineCoordVar(VDC *p, const char *varname, const char **dimnames, size_t dimnamesCount,
                       const char *time_dim_name, const char *units, int axis, VDC_XType xtype,
                       int compressed) {
#ifdef VDC_DEBUG_CPP_RUN
    VDC_DEBUG_printff("(%s \"%s\", %s, \"%s\", \"%s\", %s, %s, %s);\n", _VDCPToStr(p), varname,
                      _strArrayToString(dimnames, dimnamesCount).c_str(), time_dim_name, units,
                      _AxisToStr(axis), _XTypeToIntCodeString(_IntToXType(xtype)),
                      _boolToStr(compressed));
#else
    VDC_DEBUG_printff("(%s \"%s\", %s, %li, \"%s\", \"%s\", %s, %s, %s);\n", _VDCPToStr(p), varname,
                      _strArrayToString(dimnames, dimnamesCount).c_str(), dimnamesCount,
                      time_dim_name, units, _AxisToStr(axis),
                      _XTypeToIntCodeString(_IntToXType(xtype)), _boolToStr(compressed));
#endif
    vector<string> dimnames_v;
    for (int i = 0; i < dimnamesCount; i++)
        dimnames_v.push_back(string(dimnames[i]));
    VDC_DEBUG_printff_nRun(
        ": calling VDC::DefineCoordVar(\"%s\", %s, \"%s\", \"%s\", %s, %s, %s);\n", varname,
        _stringVectorToString(dimnames_v).c_str(), time_dim_name, units, _AxisToStr(axis),
        _XTypeToString(_IntToXType(xtype)), _boolToStr(compressed));
    return p->DefineCoordVar(string(varname), dimnames_v, string(time_dim_name), string(units),
                             axis, _IntToXType(xtype), compressed);
}

int VDC_DefineCoordVarUniform(VDC *p, const char *varname, const char **dimnames,
                              size_t dimnamesCount, const char *time_dim_name, const char *units,
                              int axis, VDC_XType xtype, int compressed) {
    VDC_DEBUG_printff(
        "(%s \"%s\", %s, %li, \"%s\", \"%s\", %s, %s, %s);\n", _VDCPToStr(p), varname,
        _stringVectorToString(_strArrayToStringVector(dimnames, dimnamesCount)).c_str(),
        dimnamesCount, time_dim_name, units, _AxisToStr(axis), _XTypeToString(_IntToXType(xtype)),
        _boolToStr(compressed));
    vector<string> dimnames_v;
    for (int i = 0; i < dimnamesCount; i++)
        dimnames_v.push_back(string(dimnames[i]));
    VDC_DEBUG_printff_nRun(
        ": calling VDC::DefineCoordVarUniform(\"%s\", %s, \"%s\", \"%s\", %s, %s, %s);\n", varname,
        _stringVectorToString(dimnames_v).c_str(), time_dim_name, units, _AxisToStr(axis),
        _XTypeToString(_IntToXType(xtype)), _boolToStr(compressed));
    return p->DefineCoordVarUniform(string(varname), dimnames_v, string(time_dim_name),
                                    string(units), axis, _IntToXType(xtype), compressed);
}

int VDC_PutAtt(VDC *p, const char *varname, const char *attname, VDC_XType xtype,
               const void *values, size_t count) {
    VDC_DEBUG_printff_nRun("(%s \"%s\", \"%s\", %s, <data>, %li);\n", _VDCPToStr(p), varname,
                           attname, _XTypeToIntCodeString(_IntToXType(xtype)), count);
    switch (xtype) {
...
int VDC_PutAtt_double(VDC *p, const char *varname, const char *attname, VDC_XType xtype,
    ...
    VDC_DEBUG_printf("VDC_PutAtt(%s \"%s\", \"%s\", %s, %s);\n", _VDCPToStr(p), varname, attname,
                     _XTypeToIntCodeString(_IntToXType(xtype)),
                     valueCArrayToString(values, count, xtype).c_str());
#else
    VDC_DEBUG_printff("(%s \"%s\", \"%s\", %s, %s, %li);\n", _VDCPToStr(p), varname, attname,
                      _XTypeToIntCodeString(_IntToXType(xtype)),
                      valueCArrayToString(values, count, xtype).c_str(), count);
#endif
    vector<double> values_v;
...
int VDC_PutAtt_long(VDC *p, const char *varname, const char *attname, VDC_XType xtype,
...
int VDC_PutAtt_text(VDC *p, const char *varname, const char *attname, VDC_XType xtype,
                    const char *values) {
#ifdef VDC_DEBUG_CPP_RUN
    VDC_DEBUG_coloredLocation_standard();
    VDC_DEBUG_printf("VDC_PutAtt(%s \"%s\", \"%s\", %s, \"%s\");\n", _VDCPToStr(p), varname,
                     attname, _XTypeToIntCodeString(_IntToXType(xtype)), values);
#else
    VDC_DEBUG_printff("(%s \"%s\", \"%s\", %s, \"%s\");\n", _VDCPToStr(p), varname, attname,
                      _XTypeToIntCodeString(_IntToXType(xtype)), values);
#endif
...
const char *_AxisToStr(const int a) {
    switch (a) {
#ifdef VDC_DEBUG_CPP_RUN
    case VDCDimension_Axis_X:
        return xstr(VDCDimension_Axis_X);
    case VDCDimension_Axis_Y:
        return xstr(VDCDimension_Axis_Y);
    case VDCDimension_Axis_Z:
        return xstr(VDCDimension_Axis_Z);
    case VDCDimension_Axis_T:
        return xstr(VDCDimension_Axis_T);
#else
    case VDCDimension_Axis_X:
        return "VDCDimension_Axis_X";
    case VDCDimension_Axis_Y:
        return "VDCDimension_Axis_Y";
    case VDCDimension_Axis_Z:
        return "VDCDimension_Axis_Z";
    case VDCDimension_Axis_T:
        return "VDCDimension_Axis_T";
#endif
    default:
        return "INVALID AXIS";
    }
...
const char *_accessModeToStr(const int m) {
    switch (m) {
#ifdef VDC_DEBUG_CPP_RUN
    case VDC_AccessMode_R:
        return "VDC::AccessMode::R";
    case VDC_AccessMode_W:
        return "VDC::AccessMode::W";
    case VDC_AccessMode_A:
        return "VDC::AccessMode::A";
#else
    case VDC_AccessMode_R:
        return "VDC_AccessMode_R";
    case VDC_AccessMode_W:
        return "VDC_AccessMode_W";
    case VDC_AccessMode_A:
        return "VDC_AccessMode_A";
#endif
    default:
