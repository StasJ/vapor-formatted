...
template <class T>
int DCCF::_readRegionTemplate(int fd, const vector<size_t> &min, const vector<size_t> &max,
                              T *region) {
...
int DCCF::_AddCoordvars(NetCDFCFCollection *ncdfc, const vector<string> &cvars) {
    ...
    for (int i = 0; i < cvars.size(); i++) {
        ...
        _coordVarsMap[cvars[i]] =
            CoordVar(cvars[i], units, DC::FLOAT, periodic, axis, false, dimnames, time_dim_name);
