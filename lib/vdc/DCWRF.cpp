...
float read_scalar_float_attr(NetCDFCollection *ncdfc, string varname, string attrname,
                             float default_value) {
...
template <class T>
int DCWRF::_readRegionTemplate(int fd, const vector<size_t> &min, const vector<size_t> &max,
                               T *region) {
...
bool DCWRF::_isIdealized(NetCDFCollection *ncdfc) const {
    ...
    if (Wasp::StrCmpNoCase(s, "Cartesian") == 0)
        return (true);
    ...
    if (_isWRFSFIRE(ncdfc) && _cen_lat == 0.0 && _cen_lon == 0.0 && _true_lat1 == 0.0 &&
        _true_lat2 == 0.0) {
        return (true);
    }
...
DerivedCoordVar_CF2D *DCWRF::_makeDerivedHorizontalIdealized(NetCDFCollection *ncdfc, string name,
                                                             string &timeDimName,
    ...
    DerivedCoordVar_CF2D *derivedVar =
        new DerivedCoordVar_CF2D(name, spaceDimNames, dimLens, axis, units, data);
...
DerivedCoordVar_Staggered *DCWRF::_makeDerivedHorizontalStaggered(NetCDFCollection *ncdfc,
                                                                  string name, string &timeDimName,
                                                                  vector<string> &spaceDimNames) {
    timeDimName.clear();
    spaceDimNames.clear();
    ...
    DerivedCoordVar_Staggered *derivedVar =
        new DerivedCoordVar_Staggered(name, stagDimName, this, inName, dimName);
    int rc = derivedVar->Initialize();
    if (rc < 0)
        return (NULL);
...
int DCWRF::_InitHorizontalCoordinatesHelper(NetCDFCollection *ncdfc, string name, int axis) {
    ...
    _coordVarsMap[name] =
        CoordVar(name, units, DC::FLOAT, periodic, axis, false, spaceDimNames, timeDimName);
...
bool DCWRF::_GetVarCoordinates(NetCDFCollection *ncdfc, string varname, vector<string> &sdimnames,
    ...
    } else if (_isWRFSFIRE(ncdfc) && dimnames[0].compare("west_east_subgrid") == 0 &&
