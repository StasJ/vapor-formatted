...
int DCWRF::GetDimLensAtLevel(string varname, int, std::vector<size_t> &dims_at_level,
...
int DCWRF::Read(float *data) { return (_ncdfc->Read(data, _ovr_fd)); }

int DCWRF::ReadSlice(float *slice) { return (_ncdfc->ReadSlice(slice, _ovr_fd)); }

int DCWRF::ReadRegion(const vector<size_t> &min, const vector<size_t> &max, float *region) {
...
int DCWRF::ReadRegionBlock(const vector<size_t> &min, const vector<size_t> &max, float *region) {
...
int DCWRF::GetVar(size_t ts, string varname, float *data) {
    int rc = DCWRF::OpenVariableRead(ts, varname);
    if (rc < 0)
        return (-1);
...
//
int DCWRF::_GetProj4String(NetCDFCollection *ncdfc, float radius, int map_proj,
                           string &projstring) {
    projstring.clear();
    ostringstream oss;
...
DCWRF::DerivedVarHorizontal *
DCWRF::_InitHorizontalCoordinatesHelper(NetCDFCollection *ncdfc, Proj4API *proj4API, string name,
                                        string fastest_dim, int axis, vector<size_t> latlondims) {
    ...
    //
    derivedVar = new DerivedVarHorizontal(ncdfc, name, dims, latlondims, proj4API);
    ...
    _coordVarsMap[name] =
        CoordVar(name, "meters", DC::FLOAT, periodic, axis, true, sdimnames, bs, time_dim_name);
...
//
int DCWRF::_InitHorizontalCoordinates(NetCDFCollection *ncdfc, Proj4API *proj4API) {
    _coordVarsMap.clear();
    _derivedX = NULL;
    ...
    //
    _derivedX = _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "X", "west_east", 0, latlondims);

    // "Y" coordinate, unstaggered
    //
    _derivedY =
        _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "Y", "south_north", 1, latlondims);

    // "XU" coordinate, staggered
    //
    _derivedXU =
        _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "XU", "west_east_stag", 0, latlondims);

    // "YU" coordinate, unstaggered
    //
    _derivedYU =
        _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "YU", "south_north", 1, latlondims);

    // "XV" coordinate, unstaggered
    //
    _derivedXV =
        _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "XV", "west_east", 0, latlondims);

    // "YV" coordinate, staggered
    //
    _derivedYV =
        _InitHorizontalCoordinatesHelper(ncdfc, proj4API, "YV", "south_north_stag", 1, latlondims);

    return (0);
}

DCWRF::DerivedVarElevation *DCWRF::_InitVerticalCoordinatesHelper(NetCDFCollection *ncdfc,
                                                                  string name,
                                                                  vector<string> sdimnames) {
    ...
    _coordVarsMap[name] =
        CoordVar(name, "meters", DC::FLOAT, periodic, 2, true, sdimnames, bs, time_dim_name);
...
int DCWRF::_InitVerticalCoordinates(NetCDFCollection *ncdfc) {
    ...
    _derivedElev = _InitVerticalCoordinatesHelper(ncdfc, "ELEVATION", sdimnames);
    ...
    _derivedElevU = _InitVerticalCoordinatesHelper(ncdfc, "ELEVATIONU", sdimnames);
    ...
    _derivedElevV = _InitVerticalCoordinatesHelper(ncdfc, "ELEVATIONV", sdimnames);
    ...
    _derivedElevW = _InitVerticalCoordinatesHelper(ncdfc, "ELEVATIONW", sdimnames);
...
int DCWRF::_InitTime(NetCDFCollection *ncdfc) {
    ...
    for (size_t ts = 0; ts < ncdfc->GetTimeDim("Times"); ts++) {
        ...
        times.push_back(_udunits.EncodeTime(year, mon, mday, hour, min, sec) * _p2si);
    }
    delete[] buf;
    ...
    vector<bool> periodic(1, false);
    _coordVarsMap[name] = CoordVar(name, "seconds", DC::FLOAT, periodic, 3, false, vector<string>(),
                                   vector<size_t>(), "Time");
...
int DCWRF::_InitDimensions(NetCDFCollection *ncdfc) {
    ...
    if ((_dimsMap.find("west_east") == _dimsMap.end()) ||
...
// is significant.
//
bool DCWRF::_GetVarCoordinates(NetCDFCollection *ncdfc, string varname, vector<string> &sdimnames,
                               vector<string> &scoordvars, string &time_dim_name,
    ...
    } else if (dimnames[0].compare("west_east_stag") == 0 &&
    ...
    } else if (dimnames[0].compare("west_east") == 0 &&
...
int DCWRF::_InitVars(NetCDFCollection *ncdfc) {
    ...
    for (int i = 0; i < vars.size(); i++) {
        ...
        if (!(NetCDFSimple::IsNCTypeFloat(type) || NetCDFSimple::IsNCTypeFloat(type)))
            continue;
        ...
        bool ok =
            _GetVarCoordinates(ncdfc, vars[i], sdimnames, scoordvars, time_dim_name, time_coordvar);
        if (!ok)
            continue;
        // if (! ok) {
...
//
DCWRF::DerivedVarElevation::DerivedVarElevation(NetCDFCollection *ncdfc, string name,
                                                const vector<DC::Dimension> &dims, float grav)
    : DerivedVar(ncdfc) {

    assert((name.compare("ELEVATION") == 0) || (name.compare("ELEVATIONU") == 0) ||
           (name.compare("ELEVATIONV") == 0) || (name.compare("ELEVATIONW") == 0));
...
// required.
//
int DCWRF::DerivedVarElevation::_ReadSlice(float *slice) {
    // NetCDF dimension ordering
    //
...
int DCWRF::DerivedVarElevation::ReadSlice(float *slice, int) {
    if (!_is_open) {
    ...
    if (_xextrapolate) {
        ...
        for (size_t x = 0; x < nx; x++) { // bottom boundary
            slice[x] = _xysliceBuf[x] + (-0.5 * (_xysliceBuf[nx + x] - _zsliceBuf[x]));
        }
        for (size_t x = 0; x < nx; x++) { // top boundary
            slice[(nys - 1) * nx + x] =
                _xysliceBuf[(ny - 2) * nx + x] +
...
int DCWRF::DerivedVarElevation::SeekSlice(int offset, int whence, int) {
...
//
DCWRF::DerivedVarHorizontal::DerivedVarHorizontal(NetCDFCollection *ncdfc, string name,
                                                  const vector<DC::Dimension> &dims,
                                                  const vector<size_t> latlondims,
                                                  Proj4API *proj4API)
    : DerivedVar(ncdfc) {

    assert((name.compare("X") == 0) || (name.compare("Y") == 0) || (name.compare("XU") == 0) ||
           (name.compare("YU") == 0) || (name.compare("XV") == 0) || (name.compare("YV") == 0));
...
int DCWRF::DerivedVarHorizontal::_GetCartCoords(size_t ts) {
...
int DCWRF::DerivedVarHorizontal::Read(float *buf, int) {
    ...
    //
    bool staggered = ((_name.compare("XU") == 0) || (_name.compare("YU") == 0) ||
                      (_name.compare("XV") == 0) || (_name.compare("YV") == 0));
    ...
    if (!staggered || _hasStagCVars) {
        ...
        buf[_ncoords] =
            _coords[_ncoords - 1] + ((_coords[_ncoords - 1] - _coords[_ncoords - 2]) * 0.5);
    }
    return (0);
}

int DCWRF::DerivedVarHorizontal::ReadSlice(float *slice, int) {
    SetErrMsg("Invalid operation");
    return (-1);
}

int DCWRF::DerivedVarHorizontal::SeekSlice(int offset, int whence, int) {
    SetErrMsg("Invalid operation");
    return (-1);
}
...
//
DCWRF::DerivedVarTime::DerivedVarTime(NetCDFCollection *ncdfc, DC::Dimension dim,
...
int DCWRF::DerivedVarTime::ReadSlice(float *slice, int) {
    SetErrMsg("Invalid operation");
    return (-1);
}

int DCWRF::DerivedVarTime::SeekSlice(int offset, int whence, int) {
    SetErrMsg("Invalid operation");
    return (-1);
}

int DCWRF::DerivedVarTime::Close(int) { return (0); }
