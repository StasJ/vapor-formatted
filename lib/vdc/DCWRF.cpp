...
DerivedCoordVar_Staggered *DCWRF::_makeDerivedHorizontal(NetCDFCollection *ncdfc, string name,
                                                         string &timeDimName,
...
int DCWRF::_InitHorizontalCoordinatesHelper(NetCDFCollection *ncdfc, string name, int axis) {
    ...
    if (ncdfc->VariableExists(name)) {
        ...
        derivedVar = _makeDerivedHorizontal(ncdfc, name, timeDimName, spaceDimNames);
        if (!derivedVar)
            return (-1);
    }
    ...
    _coordVarsMap[name] =
        CoordVar(name, units, DC::FLOAT, periodic, axis, false, spaceDimNames, bs, timeDimName);
...
DerivedCoordVar_CF1D *DCWRF::_InitVerticalCoordinatesHelper(string varName, string dimName) {
    ...
    _coordVarsMap[varName] =
        CoordVar(varName, units, DC::FLOAT, periodic, axis, false, dimNames, bs, time_dim_name);
