...
bool DataMgrUtils::MaxXFormPresent(const DataMgr *dataMgr, size_t timestep, string varname,
                                   size_t &maxXForm) {
...
bool DataMgrUtils::MaxLODPresent(const DataMgr *dataMgr, size_t timestep, string varname,
                                 size_t &maxLOD) {
    maxLOD = 0;
...
int DataMgrUtils::GetGrids(DataMgr *dataMgr, size_t ts, const vector<string> &varnames,
    ...
    for (int i = 0; i < varnames.size(); i++) {
        ...
        Grid *rGrid =
            dataMgr->GetVariable(ts, varnames[i], *refLevel, *lod, minExtsReq, maxExtsReq, true);
...
template VDF_API int DataMgrUtils::GetGrids<size_t>(DataMgr *dataMgr, size_t ts,
                                                    const vector<string> &varnames,
                                                    const vector<size_t> &minExtsReq,
                                                    const vector<size_t> &maxExtsReq,
                                                    bool useLowerAccuracy, int *refLevel, int *lod,
                                                    vector<Grid *> &grids);

template VDF_API int DataMgrUtils::GetGrids<double>(DataMgr *dataMgr, size_t ts,
                                                    const vector<string> &varnames,
                                                    const vector<double> &minExtsReq,
                                                    const vector<double> &maxExtsReq,
                                                    bool useLowerAccuracy, int *refLevel, int *lod,
                                                    vector<Grid *> &grids);

int DataMgrUtils::GetGrids(DataMgr *dataMgr, size_t ts, string varname,
    ...
    *gridptr = NULL;
    ...
    vector<Grid *> grids;
    int rc = GetGrids(dataMgr, ts, varnames, minExtsReq, maxExtsReq, useLowerAccuracy, refLevel,
                      lod, grids);
    if (rc < 0)
        return (rc);
...
int DataMgrUtils::GetGrids(DataMgr *dataMgr, size_t ts, const vector<string> &varnames,
    ...
    return (DataMgrUtils::GetGrids(dataMgr, ts, varnames, minExtsReq, maxExtsReq, useLowerAccuracy,
                                   refLevel, lod, grids));
}

int DataMgrUtils::GetGrids(DataMgr *dataMgr, size_t ts, string varname, bool useLowerAccuracy,
...
bool DataMgrUtils::GetExtents(DataMgr *dataMgr, size_t timestep, string varname, int refLevel,
    ...
    if (varname.empty()) {
        for (int ndim = 3; ndim > 0; ndim--) {
            ...
            bool ok = DataMgrUtils::GetFirstExistingVariable(dataMgr, timestep, refLevel, lod, ndim,
                                                             varname);
...
int DataMgrUtils::GetDefaultMetaInfoStride(DataMgr *dataMgr, std::string varname,
                                           int refinementLevel) {
...
bool DataMgrUtils::GetFirstExistingVariable(DataMgr *dataMgr, int level, int lod, int ndim,
...
bool DataMgrUtils::GetFirstExistingVariable(DataMgr *dataMgr, size_t ts, int level, int lod,
                                            int ndim, string &varname) {
    varname.clear();
...
void DataMgrUtils::mapBoxToVox(Box *box, string varname, int refLevel, int lod, int timestep,
                               size_t voxExts[6]) {
    double userExts[6];
