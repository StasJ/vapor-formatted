...
int NetCDFSimple::GetAttType(string name) const {
    for (int i = 0; i < _flt_atts.size(); i++) {
        if (name.compare(_flt_atts[i].first) == 0)
            return (NC_DOUBLE);
    }
    for (int i = 0; i < _int_atts.size(); i++) {
        if (name.compare(_int_atts[i].first) == 0)
            return (NC_INT64);
    }
    for (int i = 0; i < _str_atts.size(); i++) {
        if (name.compare(_str_atts[i].first) == 0)
            return (NC_CHAR);
    }
    return (-1);
}
...
bool NetCDFSimple::IsNCTypeInt(int type) {
    if (type == NC_BYTE || type == NC_SHORT || type == NC_INT || type == NC_LONG ||
        type == NC_UBYTE || type == NC_USHORT || type == NC_UINT || type == NC_INT64 ||
        type == NC_UINT64) {
        return (true);
    }
...
int NetCDFSimple::_GetAtts(int ncid, int varid, vector<pair<string, vector<double>>> &flt_atts,
                           vector<pair<string, vector<long>>> &int_atts,
                           vector<pair<string, string>> &str_atts) {
    flt_atts.clear();
    int_atts.clear();
    ...
    for (int i = 0; i < natts; i++) {
        ...
        if (rc != 0) {
            ...
            if (rc != 0) {
                SetErrMsg("nc_get_att_long(%d, %d, %s) : %s", ncid, varid, namebuf,
                          nc_strerror(rc));
                return (-1);
            }
            ...
            if (rc != 0) {
                SetErrMsg("nc_get_att_double(%d, %d, %s) : %s", ncid, varid, namebuf,
                          nc_strerror(rc));
                return (-1);
            }
            ...
            if (rc != 0) {
                SetErrMsg("nc_get_att_text(%d, %d, %s) : %s", ncid, varid, namebuf,
                          nc_strerror(rc));
                return (-1);
            }
...
NetCDFSimple::Variable::Variable(string name, vector<string> dimnames, int varid, int type) {
    _name = name;
    _dimnames = dimnames;
    _varid = varid;
    _type = type;
