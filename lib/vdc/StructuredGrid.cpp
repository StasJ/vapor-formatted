...
bool StructuredGrid::GetCellNodes(const size_t cindices[], size_t nodes[], int &n) const {
    ...
    for (int j = 0; j < n; j++) {
        for (int i = 0; i < dims.size(); i++) {
            if (nodes[j * dims.size() + i] >= dims[i]) {
                nodes[j * dims.size() + i] -= 1;
            }
        }
    }
...
bool StructuredGrid::GetEnclosingRegion(const std::vector<double> &minu,
                                        const std::vector<double> &maxu, std::vector<size_t> &min,
                                        std::vector<size_t> &max) const {
    VAssert(minu.size() == maxu.size());

    if (!GetIndicesCell(minu, min))
        return (false);
    if (!GetIndicesCell(maxu, max))
        return (false);
...
void StructuredGrid::ClampCoord(const double coords[3], double cCoords[3]) const {
    ...
    for (int i = 0; i < GetGeometryDim(); i++) {
        ...
        if (cCoords[i] < minu[i] && periodic[i]) {
            while (cCoords[i] < minu[i])
                cCoords[i] += maxu[i] - minu[i];
        }
        if (cCoords[i] > maxu[i] && periodic[i]) {
            while (cCoords[i] > maxu[i])
                cCoords[i] -= maxu[i] - minu[i];
        }
    }
}
