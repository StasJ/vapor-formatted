...
void StructuredGrid::_StructuredGrid(const vector<size_t> &bs, const vector<size_t> &min,
...
StructuredGrid::StructuredGrid(const size_t bs[3], const size_t min[3], const size_t max[3],
                               const bool periodic[3], const std::vector<float *> &blks) {
    vector<size_t> bs_v;
    vector<size_t> min_v;
    vector<size_t> max_v;
...
StructuredGrid::StructuredGrid(const vector<size_t> &bs, const vector<size_t> &min,
                               const vector<size_t> &max, const vector<bool> &periodic,
                               const vector<float *> &blks) {
...
StructuredGrid::StructuredGrid(const std::vector<size_t> &bs, const std::vector<size_t> &min,
                               const std::vector<size_t> &max, const std::vector<bool> &periodic,
...
void StructuredGrid::GetEnclosingRegion(const double minu[3], const double maxu[3], size_t min[3],
                                        size_t max[3]) const {
    for (int i = 0; i < 3; i++) {
        min[i] = 0.0;
        max[i] = 0.0;
...
std::ostream &operator<<(std::ostream &o, const StructuredGrid &rg) {
    o << "StructuredGrid " << endl
      << " Dimensions " << rg._max[0] - rg._min[0] + 1 << " " << rg._max[1] - rg._min[1] + 1 << " ";
    if (rg._ndim == 3)
