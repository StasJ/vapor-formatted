...
DerivedCoordVar_PCSFromLatLon::DerivedCoordVar_PCSFromLatLon(const vector<string> &derivedVarNames,
                                                             DC *dc, vector<string> inNames,
...
int DerivedCoordVar_PCSFromLatLon::GetDimLensAtLevel(string varname, int,
...
DerivedCoordVar_CF1D::DerivedCoordVar_CF1D(const vector<string> &derivedVarNames, DC *dc,
    ...
    _coordVarInfo = DC::CoordVar(_coordName, units, DC::XType::FLOAT, vector<bool>(1, false), axis,
                                 false, vector<string>(1, dimName), "");
}
...
int DerivedCoordVar_CF1D::GetDimLensAtLevel(string varname, int level,
                                            std::vector<size_t> &dims_at_level,
                                            std::vector<size_t> &bs_at_level) const {
    dims_at_level.clear();
    bs_at_level.clear();
...
DerivedCoordVar_WRFTime::DerivedCoordVar_WRFTime(string derivedVarName, NetCDFCollection *ncdfc,
    ...
    int axis = 3;
    _coordVarInfo = DC::CoordVar(_coordName, units, DC::XType::FLOAT, vector<bool>(), axis, false,
...
int DerivedCoordVar_WRFTime::GetDimLensAtLevel(string varname, int level,
                                               std::vector<size_t> &dims_at_level,
                                               std::vector<size_t> &bs_at_level) const {
    dims_at_level.clear();
    bs_at_level.clear();
...
bool DerivedCoordVar_WRFTime::VariableExists(size_t ts, string varname, int reflevel,
                                             int lod) const {
    if (varname != _coordName)
        return (false);
...
int DerivedCoordVar_TimeInSeconds::GetDimLensAtLevel(string varname, int level,
                                                     std::vector<size_t> &dims_at_level,
                                                     std::vector<size_t> &bs_at_level) const {
    dims_at_level.clear();
    bs_at_level.clear();
...
bool DerivedCoordVar_TimeInSeconds::VariableExists(size_t ts, string varname, int reflevel,
                                                   int lod) const {
    if (varname != _coordName)
        return (false);
...
int DerivedCoordVar_Staggered::GetDimLensAtLevel(string varname, int level,
                                                 std::vector<size_t> &dims_at_level,
                                                 std::vector<size_t> &bs_at_level) const {
    dims_at_level.clear();
    bs_at_level.clear();
...
void DerivedCoordVar_Staggered::_transpose(const float *a, float *b, vector<size_t> inDims,
                                           int axis) const {
...
void DerivedCoordVar_Staggered::_transpose(vector<size_t> inDims, int axis,
                                           vector<size_t> &outDims) const {
    outDims = inDims;
...
int DerivedCoordVar_Staggered::ReadRegion(int fd, const vector<size_t> &min,
    ...
    if (max[_stagDim] == (dims[_stagDim] - 1)) {
        if (min[_stagDim] == (dims[_stagDim] - 2)) {
            inMin[_stagDim] -= 1;
        }
        inMax[_stagDim] -= 1;
    }
    ...
    for (size_t k = 0; k < nz; k++) {
        for (size_t j = 0; j < ny; j++) {
            for (size_t i = 1; i < nxs - 1; i++) {
                buf1[k * nxs * ny + j * nxs + k] =
                    0.5 * (buf2[k * nx * ny + j * nx + i - 1] + buf2[k * nx * ny + j * nx + i]);
            }
        }
    }
    ...
    for (size_t k = 0; k < nz; k++) {
        for (size_t j = 0; j < ny; j++) {
            ...
            buf1[k * nxs * ny + j * nxs] =
                buf2[k * nx * ny + j * nx + 0] +
                (-0.5 * (buf2[k * nx * ny + j * nx + 1] - buf2[k * nx * ny + j * nx + 0]));
        }
    }
    ...
    for (size_t k = 0; k < nz; k++) {
        for (size_t j = 0; j < ny; j++) {
            // right boundary
            //
            buf1[k * nxs * ny + j * nxs + nxs - 1] =
                buf2[k * nx * ny + j * nx + nx - 2] +
                (0.5 * (buf2[k * nx * ny + j * nx + nx - 1] - buf2[k * nx * ny + j * nx + nx - 2]));
        }
    }
...
bool DerivedCoordVar_Staggered::VariableExists(size_t ts, string varname, int reflevel,
                                               int lod) const {
    if (varname != _derivedVarName)
        return (false);
...
DerivedCoordVarStandardWRF_Terrain::DerivedCoordVarStandardWRF_Terrain(string derivedVarName,
                                                                       DC *dc, string formula)
    : DerivedCoordVar(derivedVarName) {
...
int DerivedCoordVarStandardWRF_Terrain::Initialize() {
    ...
    _coordVarInfo = DC::CoordVar(_derivedVarName, "m", DC::XType::FLOAT, vector<bool>(3, false), 2,
                                 false, m.GetDimNames(), timeDimName);
...
int DerivedCoordVarStandardWRF_Terrain::GetDimLensAtLevel(string varname, int level,
                                                          std::vector<size_t> &dims_at_level,
                                                          std::vector<size_t> &bs_at_level) const {
    dims_at_level.clear();
    bs_at_level.clear();
...
int DerivedCoordVarStandardWRF_Terrain::OpenVariableRead(size_t ts, string varname, int level,
                                                         int lod) {
    if (varname != _derivedVarName) {
        SetErrMsg("Invalid variable name: %s", varname.c_str());
        return (-1);
...
int DerivedCoordVarStandardWRF_Terrain::ReadRegionBlock(int fd, const vector<size_t> &min,
    ...
    for (size_t i = 0; i < nElements; i++) {
        region[i] = (phBuf[i] + phbBuf[i]) / _grav;
    }
...
bool DerivedCoordVarStandardWRF_Terrain::VariableExists(size_t ts, string varname, int reflevel,
                                                        int lod) const {
    if (varname != _derivedVarName)
        return (false);
