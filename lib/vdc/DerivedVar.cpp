...
bool parse_formula(string formula_terms, map<string, string> &parsed_terms) {
    ...
    for (int i = 0; i < tokens.size(); i += 2) {
        ...
        if (parsed_terms[tokens[i]].empty())
            return (false);
    }
    return (true);
}
...
int DerivedVar::_getVarDestagger(DC *dc, size_t ts, string varname, int level, int lod,
                                 const vector<size_t> &min, const vector<size_t> &max,
                                 float *region, int stagDim) const {
...
int DerivedCoordVar_PCSFromLatLon::_readRegionHelperCylindrical(DC::FileTable::FileObject *f,
...
int DerivedCoordVar_PCSFromLatLon::_readRegionHelper1D(DC::FileTable::FileObject *f,
                                                       const vector<size_t> &min,
...
int DerivedCoordVar_PCSFromLatLon::_readRegionHelper2D(DC::FileTable::FileObject *f,
                                                       const vector<size_t> &min,
                                                       const vector<size_t> &max, float *region) {
...
int DerivedCoordVar_PCSFromLatLon::ReadRegion(int fd, const vector<size_t> &min,
                                              const vector<size_t> &max, float *region) {
    DC::FileTable::FileObject *f = _fileTable.GetEntry(fd);
...
DerivedCoordVar_CF2D::DerivedCoordVar_CF2D(string derivedVarName, vector<string> dimNames,
    ...
    _coordVarInfo = DC::CoordVar(_derivedVarName, units, DC::XType::FLOAT, vector<bool>(2, false),
                                 axis, false, dimNames, "");
}
...
int DerivedCoordVar_CF2D::GetDimLensAtLevel(int level, std::vector<size_t> &dims_at_level,
                                            std::vector<size_t> &bs_at_level) const {
    dims_at_level.clear();
    bs_at_level.clear();
...
int DerivedCoordVar_CF2D::ReadRegion(int fd, const vector<size_t> &min, const vector<size_t> &max,
                                     float *region) {
    VAssert(min.size() == 2);
    VAssert(max.size() == 2);
...
int DerivedCoordVar_WRFTime::_encodeTime(UDUnits &udunits, const vector<string> &timeStrings,
...
DerivedCFVertCoordVar *DerivedCFVertCoordVarFactory::CreateInstance(string standard_name, DC *dc,
                                                                    string mesh, string formula) {
...
vector<string> DerivedCFVertCoordVarFactory::GetFactoryNames() const {
    ...
    for (itr = _factoryFunctionRegistry.begin(); itr != _factoryFunctionRegistry.end(); ++itr) {
...
static DerivedCFVertCoordVarFactoryRegistrar<DerivedCoordVarStandardWRF_Terrain>
...
int DerivedCoordVarStandardWRF_Terrain::Initialize() {
    ...
    _coordVarInfo = DC::CoordVar(_derivedVarName, "m", DC::XType::FLOAT, dvarInfo.GetWName(),
                                 dvarInfo.GetCRatios(), vector<bool>(3, false), dimnames,
                                 timeDimName, 2, false);
...
bool DerivedCoordVarStandardWRF_Terrain::ValidFormula(string formula) {
    ...
    return (DerivedCFVertCoordVar::ValidFormula(vector<string>{"PH", "PHB"}, formula));
}
...
//
static DerivedCFVertCoordVarFactoryRegistrar<DerivedCoordVarStandardOceanSCoordinateG2>
    registrar_ocean_s_coordinate_g1("ocean_s_coordinate_g1");

static DerivedCFVertCoordVarFactoryRegistrar<DerivedCoordVarStandardOceanSCoordinateG2>
    registrar_ocean_s_coordinate_g2("ocean_s_coordinate_g2");

DerivedCoordVarStandardOceanSCoordinateG2::DerivedCoordVarStandardOceanSCoordinateG2(DC *dc,
                                                                                     string mesh,
                                                                                     string formula)
    : DerivedCFVertCoordVar("", dc, mesh, formula) {
...
int DerivedCoordVarStandardOceanSCoordinateG2::initialize_stagger_flags() {
    ...
    if (nativeDims[0] == derivedDims[0] - 1)
        _destaggerEtaXDim = true;
    if (nativeDims[1] == derivedDims[1] - 1)
        _destaggerEtaYDim = true;
    ...
    if (nativeDims[0] == derivedDims[0] - 1)
        _destaggerDepthXDim = true;
    if (nativeDims[1] == derivedDims[1] - 1)
        _destaggerDepthYDim = true;
...
int DerivedCoordVarStandardOceanSCoordinateG2::Initialize() {
    ...
    if (initialize_missing_values() < 0)
        return (-1);

    if (initialize_stagger_flags() < 0)
        return (-1);
    ...
    //
    _coordVarInfo =
        DC::CoordVar(_derivedVarName, "m", DC::XType::FLOAT, etaInfo.GetWName(),
                     etaInfo.GetCRatios(), vector<bool>(3, false), dimnames, timeDimName, 2, false);

    return (0);
}

bool DerivedCoordVarStandardOceanSCoordinateG2::GetBaseVarInfo(DC::BaseVar &var) const {
    var = _coordVarInfo;
    return (true);
}

bool DerivedCoordVarStandardOceanSCoordinateG2::GetCoordVarInfo(DC::CoordVar &cvar) const {
    cvar = _coordVarInfo;
    return (true);
}
...
int DerivedCoordVarStandardOceanSCoordinateG2::GetDimLensAtLevel(
    int level, std::vector<size_t> &dims_at_level, std::vector<size_t> &bs_at_level) const {
    dims_at_level.clear();
    bs_at_level.clear();
...
int DerivedCoordVarStandardOceanSCoordinateG2::OpenVariableRead(size_t ts, int level, int lod) {
...
int DerivedCoordVarStandardOceanSCoordinateG2::ReadRegion(int fd, const vector<size_t> &min,
                                                          const vector<size_t> &max,
                                                          float *region) {
    ...
    if (_destaggerEtaXDim) {
        rc = _getVarDestagger(_dc, f->GetTS(), _etaVar, f->GetLevel(), f->GetLOD(), myMin, myMax,
                              eta.data(), 0);
        ...
        rc =
            _getVar(_dc, f->GetTS(), _etaVar, f->GetLevel(), f->GetLOD(), myMin, myMax, eta.data());
    }
    if (rc < 0)
        return (rc);
    ...
    if (_destaggerDepthXDim) {
        rc = _getVarDestagger(_dc, f->GetTS(), _depthVar, f->GetLevel(), f->GetLOD(), myMin, myMax,
                              depth.data(), 0);
    } else if (_destaggerDepthYDim) {
        rc = _getVarDestagger(_dc, f->GetTS(), _depthVar, f->GetLevel(), f->GetLOD(), myMin, myMax,
                              depth.data(), 1);
    } else {
        rc = _getVar(_dc, f->GetTS(), _depthVar, f->GetLevel(), f->GetLOD(), myMin, myMax,
                     depth.data());
    }
...
bool DerivedCoordVarStandardOceanSCoordinateG2::VariableExists(size_t ts, int reflevel,
                                                               int lod) const {

    return (_dc->VariableExists(ts, _sVar, reflevel, lod) &&
...
bool DerivedCoordVarStandardOceanSCoordinateG2::ValidFormula(string formula) {
    ...
    return (DerivedCFVertCoordVar::ValidFormula(vector<string>{"s", "C", "eta", "depth", "depth_c"},
                                                formula));
}

void DerivedCoordVarStandardOceanSCoordinateG2::compute_g1(const vector<size_t> &min,
                                                           const vector<size_t> &max,
                                                           const float *s, const float *C,
                                                           const float *eta, const float *depth,
                                                           float depth_c, float *region) const {
    ...
    for (size_t k = 0; k < max[2] - min[2] + 1; k++) {
        for (size_t j = 0; j < max[1] - min[1] + 1; j++) {
            for (size_t i = 0; i < max[0] - min[0] + 1; i++) {
                ...
                //
                if (C[k] == _CVarMV || eta[j * rDims[0] + i] == _etaVarMV ||
...
void DerivedCoordVarStandardOceanSCoordinateG2::compute_g2(const vector<size_t> &min,
                                                           const vector<size_t> &max,
                                                           const float *s, const float *C,
                                                           const float *eta, const float *depth,
                                                           float depth_c, float *region) const {
    ...
    for (size_t k = 0; k < max[2] - min[2] + 1; k++) {
        for (size_t j = 0; j < max[1] - min[1] + 1; j++) {
            for (size_t i = 0; i < max[0] - min[0] + 1; i++) {
                ...
                region[k * rDims[0] * rDims[1] + j * rDims[0] + i] =
                    eta[j * rDims[0] + i] + (eta[j * rDims[0] + i] + depth[j * rDims[0] + i]) * tmp;
            }
        }
    }
}
