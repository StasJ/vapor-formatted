...
class RENDER_API ImageWriter : public Wasp::MyBase {
  public:
    ...
    virtual int Write(const unsigned char *buffer, const unsigned int width,
...
#define REGISTER_IMAGEWRITER(name)                                                                 \
    class name##Factory : public ImageWriterFactory {                                              \
      public:                                                                                      \
        name##Factory() : ImageWriterFactory(name::GetFileExtensions()) {                          \
            ImageWriter::RegisterFactory(this);                                                    \
        }                                                                                          \
