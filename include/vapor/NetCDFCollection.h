...
class VDF_API NetCDFCollection : public Wasp::MyBase {
  public:
    ...
    class TimeVaryingVar {
      public:
        TimeVaryingVar();
        int Insert(const NetCDFSimple *netcdf, const NetCDFSimple::Variable &variable, string file,
                   const std::vector<string> &time_dimnames,
                   const std::map<string, std::vector<double>> &timesmap, int file_org);
  ...
  private:
    ...
    int _InitializeTimesMap(const std::vector<string> &files,
                            const std::vector<string> &time_dimnames,
    ...
    int _InitializeTimesMapCase1(const std::vector<string> &files,
                                 std::map<string, std::vector<double>> &timesMap) const;

    int _InitializeTimesMapCase2(const std::vector<string> &files,
                                 const std::vector<string> &time_dimnames,
                                 std::map<string, std::vector<double>> &timesMap) const;

    int _InitializeTimesMapCase3(const std::vector<string> &files,
                                 const std::vector<string> &time_dimnames,
                                 const std::vector<string> &time_coordvars,
                                 std::map<string, std::vector<double>> &timesMap) const;

    void _InterpolateLine(const float *src, size_t n, size_t stride, bool has_missing, float mv,
                          float *dst) const;

    void _InterpolateSlice(size_t nx, size_t ny, bool xstag, bool ystag, bool has_missing, float mv,
                           float *slice) const;

    int _GetTimesMap(NetCDFSimple *netcdf, const std::vector<string> &time_coordvars,
                     const std::vector<string> &time_dimnames,
                     std::map<string, std::vector<double>> &timesmap) const;
