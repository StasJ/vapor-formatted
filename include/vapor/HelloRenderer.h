...
class RENDER_API HelloRenderer : public Renderer {
  ...
  public:
    HelloRenderer(const ParamsMgr *pm, string winName, string instName, DataStatus *ds);
    ...
    //! with this renderer
    //
    static Renderer *CreateInstance(const ParamsMgr *pm, string winName, string instName,
                                    DataStatus *ds) {
        return new HelloRenderer(pm, winName, HelloParams::m_classType, ds);
    }
#endif
