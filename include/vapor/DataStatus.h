...
//! \date    January 2016
//!
//! The DataStatus class keeps track of available variables, timesteps, resolutions, and data
//! ranges. It is constructed by the Session whenever a new metadata is loaded. It keeps a lazily
//! evaluated value of min/max of each variable for each timestep. Variables can be referenced using
//! the variable name, the session variable num (a numbering all the variables in the session) or by
//! the active variable num.  Active variables are all those in the metadata plus all the derived
//! variables, and are a subset of the session variables. Session variables are those that were
//! specified in the session plus those that are derived, and these may not all be available in the
...
class PARAMS_API DataStatus {
  public:
    ...
    void GetExtents(size_t ts, const map<string, vector<string>> &varnames, vector<double> &minExt,
                    vector<double> &maxExt) const;
    ...
    //
    void GetActiveExtents(const ParamsMgr *paramsMgr, string winName, size_t ts,
                          vector<double> &minExts, vector<double> &maxExts) const;

    void GetActiveExtents(const ParamsMgr *paramsMgr, size_t ts, vector<double> &minExts,
                          vector<double> &maxExts) const;
    ...
    //! \sa GetTimeCoordinates().
    //
    void MapLocalToGlobalTimeRange(string dataSetName, size_t local_ts, size_t &min_ts,
                                   size_t &max_ts) const;
...
#ifdef DEAD
    //! Determine the number of active 3D variables.  This includes 3D variables in the VDC as well
    //! as any 3D derived variables \return number of active 3D variables
    int getNumActiveVariables3D() { return _dataMgr->GetDataVarNames(3, true).size(); }
    //! Determine the number of active 2D variables.  This includes 2D variables in the VDC as well
    //! as any 2D derived variables \return number of active 2D variables
    int getNumActiveVariables2D() { return _dataMgr->GetDataVarNames(2, true).size(); }
    //! Determine the number of active variables.  This includes variables in the VDC as well as any
    //! derived variables \return number of active variables
    ...
    void mapBoxToVox(Box *box, string varname, int refLevel, int lod, int timestep,
                     size_t voxExts[6]);
#endif
    ...
    const vector<double> getStretchFactors() {
        return ((VizFeatureParams *)_paramsMgr->GetDefaultParams(
                    Params::_visualizerFeaturesParamsTag))
            ->GetStretchFactors();
    }
#endif
    ...
    //! Method indicates if user allows using lower accuracy, when specified LOD or refinement is
    //! not available. \retval bool true if lower accuracy is requested.
    ...
    int mapVoxToUser(size_t timestep, string varname, const size_t vcoords[3], double uCoords[3],
                     int reflevel);
    ...
    //! \param[in] reflevel Refinement level used in the mapping
    void mapUserToVox(size_t timestep, string varname, const double uCoords[3], size_t vCoords[3],
                      int reflevel);
    ...
    int getGrids(size_t ts, const vector<string> &varnames, const vector<double> &minExtsReq,
                 const vector<double> &maxExtsReq, int *refLevel, int *lod,
                 StructuredGrid **grids) const;

    int getGrids(size_t ts, const vector<string> &varnames, int *refLevel, int *lod,
                 StructuredGrid **grids) const;

    //! \deprecated
    //
    int getGrids(size_t ts, const vector<string> &varnames, const double extents[6], int *refLevel,
                 int *lod, StructuredGrid **grids) const;
