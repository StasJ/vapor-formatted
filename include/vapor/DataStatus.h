...
class PARAMS_API DataStatus {
  public:
    ...
    int Open(const std::vector<string> &files, const std::vector<string> &options, string name,
             string format);
    ...
    //
    void GetActiveExtents(const ParamsMgr *paramsMgr, string winName, string datasetName, size_t ts,
  ...
  private:
    ...
    void _getExtents(size_t ts, const map<string, std::vector<var_info_t>> &variables,
                     vector<double> &minExt, vector<double> &maxExt) const;

    map<string, vector<var_info_t>> _getFirstVars(string dataSetName) const;
