...
class PARAMS_API ParamsMgr : public MyBase {
  ...
  public:
    ...
    //! \sa ParamsRegistrar()
    //
    ParamsMgr(std::vector<string> appParamNames = std::vector<string>(),
    ...
    //
    RenderParams *CreateRenderParamsInstance(string winName, string dataSetName, string classType,
                                             string instName);
    ...
    //
    RenderParams *CreateRenderParamsInstance(string winName, string dataSetName, string instName,
    ...
    //
    void RemoveRenderParamsInstance(string winName, string dataSetName, string classType,
                                    string instName);
    ...
    //
    RenderParams *GetRenderParams(string winName, string dataSetName, string classType,
    ...
    //!
    void GetRenderParamNames(string winName, string dataSetName, string classType,
    ...
    //! \sa CreateRenderParamsInstance
    //
    bool RenderParamsLookup(string instName, string &winName, string &dataSetName,
    ...
    //! \sa RenParamsRegistrar, RenParamsFactory
    //
    vector<string> GetRenderParamInstances(string winName, string dataSetName,
    ...
    //! Obtain the AnnotationsParams that are applicable in a particular Visualizer
    //! window.
    //!
    //! Return the AnnotationsParams instance associated with the
    //! window named by \p winName in the current session state
    //!
    //! \retval ptr AnnotationsParams instance that is applicable.
    //
    AnnotationsParams *GetAnnotationsParams(string winName) const {
        return ((AnnotationsParams *)_otherParams->GetParams(AnnotationsParams::GetClassType()));
    };
    ...
    //
    bool StateChanged() {
        if (*(_rootSeparator->GetNode()) == _prevState)
            return (false);
  ...
  private:
    ...
    const map<string, map<string, RenParamsContainer *>> *
    getWinMap3(const map<string, map<string, map<string, RenParamsContainer *>>> &m3,
               string key) const;

    const map<string, RenParamsContainer *> *
    getWinMap3(const map<string, map<string, map<string, RenParamsContainer *>>> &m3, string key1,
               string key2) const;

    const map<string, RenParamsContainer *> *
    getWinMap2(const map<string, map<string, RenParamsContainer *>> &m2, string key) const;

    RenParamsContainer *get_ren_container(string winName, string dataSetName,
                                          string renderName) const;

    map<string, map<string, RenParamsContainer *>> *
    getWinMap3(map<string, map<string, map<string, RenParamsContainer *>>> &m3, string key) const;

    map<string, RenParamsContainer *> *
    getWinMap3(map<string, map<string, map<string, RenParamsContainer *>>> &m3, string key1,
               string key2) const;

    map<string, RenParamsContainer *> *
    getWinMap2(map<string, map<string, RenParamsContainer *>> &m2, string key) const;
    ...
    RenParamsContainer *createRenderParamsHelper(string winName, string dataSetName,
