...
class VDF_API DataMgr : public Wasp::MyBase {
  public:
    ...
    //!
    //
    virtual bool GetVarCoordVars(string varname, bool spatial,
                                 std::vector<string> &coord_vars) const;
    ...
    //!
    VAPoR::Grid *GetVariable(size_t ts, string varname, int level, int lod, std::vector<double> min,
                             std::vector<double> max, bool lock = false);

    VAPoR::Grid *GetVariable(size_t ts, string varname, int level, int lod, std::vector<size_t> min,
                             std::vector<size_t> max, bool lock = false);
    ...
    //
    int GetDataRange(size_t ts, string varname, int level, int lod, vector<double> min,
                     vector<double> max, std::vector<double> &range);
    ...
    class BlkExts {
      public:
        ...
        void Insert(const std::vector<size_t> &bcoord, const std::vector<double> &min,
                    const std::vector<double> &max);

        bool Intersect(const std::vector<double> &min, const std::vector<double> &max,
  ...
  private:
    ...
    template <typename C> class VarInfoCache {
      public:
        //
        //
        void Set(size_t ts, std::vector<string> varnames, int level, int lod, string key,
                 const std::vector<C> &values);

        void Set(size_t ts, string varname, int level, int lod, string key,
                 const std::vector<C> &values) {
        ...
        bool Get(size_t ts, std::vector<string> varnames, int level, int lod, string key,
                 std::vector<C> &values) const;

        bool Get(size_t ts, string varname, int level, int lod, string key,
                 std::vector<C> &values) const {
            std::vector<string> varnames;
            varnames.push_back(varname);
        ...
        static string _make_hash(string key, size_t ts, std::vector<string> cvars, int level,
                                 int lod);

        void _decode_hash(const string &hash, string &key, size_t &ts, vector<string> &varnames,
                          int &level, int &lod);
    ...
    //
    std::vector<string> _get_var_dependencies_all(std::vector<string> varnames,
    ...
    _make_grid(size_t ts, int level, int lod, const VAPoR::DC::DataVar &var,
               const std::vector<size_t> &roi_dims, const std::vector<size_t> &dims,
               const std::vector<float *> &blkvec, const std::vector<std::vector<size_t>> &bsvec,
               const std::vector<std::vector<size_t>> &bminvec,
               const std::vector<std::vector<size_t>> &bmaxvec, const vector<int *> &conn_blkvec,
    ...
    void _setupCoordVecsHelper(string data_varname, const vector<size_t> &data_dimlens,
                               const vector<size_t> &data_bmin, const vector<size_t> &data_bmax,
                               string coord_varname, int order, vector<size_t> &coord_dimlens,
    ...
    VAPoR::Grid *_getVariable(size_t ts, string varname, int level, int lod, bool lock,
                              bool dataless);

    VAPoR::Grid *_getVariable(size_t ts, string varname, int level, int lod,
                              std::vector<size_t> min, std::vector<size_t> max, bool lock,
                              bool dataless);

    int _parseOptions(vector<string> &options);

    template <typename T>
    T *_get_region_from_cache(size_t ts, string varname, int level, int lod,
                              const std::vector<size_t> &bmin, const std::vector<size_t> &bmax,
                              bool lock);

    template <typename T>
    int _get_unblocked_region_from_fs(size_t ts, string varname, int level, int lod,
    ...
    int _get_blocked_region_from_fs(size_t ts, string varname, int level, int lod,
                                    const vector<size_t> &file_bs, const vector<size_t> &file_dims,
    ...
    T *_get_region_from_fs(size_t ts, string varname, int level, int lod,
                           const std::vector<size_t> &grid_dims, const std::vector<size_t> &grid_bs,
                           const std::vector<size_t> &grid_bmin,
                           const std::vector<size_t> &grid_bmax, bool lock);

    template <typename T>
    T *_get_region(size_t ts, string varname, int level, int lod, int nlods,
                   const std::vector<size_t> &dims, const std::vector<size_t> &bs,
                   const std::vector<size_t> &bmin, const std::vector<size_t> &bmax, bool lock);

    template <typename T>
    int _get_regions(size_t ts, const std::vector<string> &varnames, int level, int lod, bool lock,
    ...
    void *_alloc_region(size_t ts, string varname, int level, int lod, std::vector<size_t> bmin,
                        std::vector<size_t> bmax, std::vector<size_t> bs, int element_sz, bool lock,
                        bool fill);
    ...
    bool _getVarConnVars(string varname, string &face_node_var, string &node_face_var,
    ...
    template <class T>
    int _readRegionBlock(int fd, const vector<size_t> &min, const vector<size_t> &max, T *region);
    template <class T>
    int _readRegion(int fd, const vector<size_t> &min, const vector<size_t> &max, T *region);
