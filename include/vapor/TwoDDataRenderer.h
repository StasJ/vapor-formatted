  ...
  private:
    class _grid_state_c {
      public:
        ...
        _grid_state_c(int refLevel, int lod, string hgtVar, string meshName, size_t ts,
                      vector<double> minExts, vector<double> maxExts)
            : _refLevel(refLevel), _lod(lod), _hgtVar(hgtVar), _meshName(meshName), _ts(ts),
              _minExts(minExts), _maxExts(maxExts) {}
        ...
        bool operator==(const _grid_state_c &rhs) const {
            return (_refLevel == rhs._refLevel && _lod == rhs._lod && _hgtVar == rhs._hgtVar &&
                    _meshName == rhs._meshName && _ts == rhs._ts && _minExts == rhs._minExts &&
