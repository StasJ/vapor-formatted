...
class PARAMS_API MapperFunction : public ParamsBase {
  ...
  public:
    ...
    float getMinOfUpdate() const { return (getBoundsOfUpdate()[0]); };

    float getMaxOfUpdate() const { return (getBoundsOfUpdate()[1]); };
    ...
    void setTimestepOfUpdate(size_t ts) {
        SetValueDouble(_timestepOfUpdateTag, "Timestep of last histogram update", ts);
    }

    size_t getTimestepOfUpdate() const { return (size_t)GetValueDouble(_timestepOfUpdateTag, 0.0); }

    void setVariableNameOfUpdate(string varName) {
        SetValueString(_variableNameOfUpdateTag, "Variable name of last histogram update", varName);
    }

    string getVariableNameOfUpdate() const { return GetValueString(_variableNameOfUpdateTag, ""); }
    ...
    void setOpacityComposition(CompositionType t) {
        SetValueLong(_opacityCompositionTag, "Set Opacity Composition Type", (long)t);
    }
    ...
    float mapIndexToFloat(int indx) const {
        return (float)(getMinMapValue() + ((float)indx) *
                                              (float)(getMaxMapValue() - getMinMapValue()) /
                                              (float)(_numEntries - 1));
    }
    ...
    void SetAutoUpdateHisto(bool val) {
        SetValueLong(_autoUpdateHistoTag, "enable/disable auto update of histogram", val);
    }
    ...
    void SetSecondaryVarMapper(bool val) {
        SetValueLong(_secondaryVarMapperTag, "Apply color through a secondary color", val);
    }
