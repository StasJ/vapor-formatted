...
class VDF_API ImpExp : public Wasp::MyBase, public ParsedXml {
  public:
    ...
    //! \sa Import()
    //
    int Export(const string &path, size_t ts, const string &varname, const size_t min[3],
               const size_t max[3], const size_t timeseg[2]);
    ...
    //! \sa Export()
    //
    int Import(string *path, size_t *ts, string *varname, size_t min[3], size_t max[3],
               size_t timeseg[2]);
