...
class VDF_API WaveCodecIO : public VDFIOBase, protected Wasp::EasyThreads {
  public:
...
#ifdef DEAD
    WaveCodecIO(const size_t dim[3], const size_t bs[3], int numTransforms,
                const vector<size_t> cratios, const string &wname, const string &filebase);
    ...
    //!
    virtual int OpenVariableRead(size_t timestep, const char *varname, int reflevel = 0,
                                 int lod = 0);
    ...
    //
    virtual int BlockReadRegion(const size_t bmin[3], const size_t bmax[3], float *region,
                                bool unblock = true);
    ...
    //
    virtual int BlockWriteRegion(const float *region, const size_t bmin[3], const size_t bmax[3],
                                 bool block = true);
  ...
  private:
    ...
    class ReadWriteThreadObj {
      public:
        ReadWriteThreadObj(WaveCodecIO *wc, int id, float *region, const size_t bmin_p[3],
                           const size_t bmax_p[3], const size_t bdim_p[3], const size_t dim_p[3],
                           const size_t bs_p[3], bool reblock, bool pad);
