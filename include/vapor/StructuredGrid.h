...
class VDF_API StructuredGrid {
  public:
    ...
    //!
    StructuredGrid(const size_t bs[3], const size_t min[3], const size_t max[3],
    ...
    //!
    StructuredGrid(const std::vector<size_t> &bs, const std::vector<size_t> &min,
    ...
    //!
    StructuredGrid(const size_t bs[3], const size_t min[3], const size_t max[3],
                   const bool periodic[3], const std::vector<float *> &blks, float missing_value);
    ...
    StructuredGrid(const std::vector<size_t> &bs, const std::vector<size_t> &min,
                   const std::vector<size_t> &max, const std::vector<bool> &periodic,
                   const std::vector<float *> &blks, float missing_value);
    ...
    //! \deprecated  This method is deprecated
    //
    virtual void GetEnclosingRegion(const double minu[3], const double maxu[3], size_t min[3],
                                    size_t max[3]) const;
    ...
    //
    virtual void GetIJKOrigin(std::vector<size_t> &min) const { min = _minabs; }
    ...
    virtual int GetUserCoordinates(size_t i, size_t j, size_t k, double *x, double *y,
                                   double *z) const = 0;
    ...
    //!
    virtual void GetIJKIndex(double x, double y, double z, size_t *i, size_t *j,
                             size_t *k) const = 0;
    ...
    //
    virtual int GetRank() { return (_ndim); }
    ...
    template <class T> class VDF_API ForwardIterator {
      public:
        ...
        int GetUserCoordinates(double *x, double *y, double *z) const {
            return (_rg->GetUserCoordinates(_x - _rg->_min[0], _y - _rg->_min[1],
                                            _rg->_ndim == 3 ? _z - _rg->_min[2] : 0, x, y, z));
        };
  ...
  protected:
    float &_AccessIJK(const std::vector<float *> &blks, size_t i, size_t j, size_t k) const;
  ...
  private:
    ...
    void _StructuredGrid(const std::vector<size_t> &bs, const std::vector<size_t> &min,
                         const std::vector<size_t> &max, const std::vector<bool> &periodic,
                         const std::vector<float *> &blks);
};
}; // namespace VAPoR
