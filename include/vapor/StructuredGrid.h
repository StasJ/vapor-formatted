  ...
  public:
    ...
    class ConstNodeIteratorRG : public Grid::ConstNodeIteratorAbstract {
      public:
        ...
        ConstNodeIteratorRG(const StructuredGrid *rg, const std::vector<double> &minu,
        ...
        virtual void next();
        virtual ConstIndexType &deref() const { return (_nodeIndex); }
        virtual const void *address() const { return this; };

        virtual bool equal(const void *rhs) const {
            const ConstNodeIteratorRG *itrptr = static_cast<const ConstNodeIteratorRG *>(rhs);
    ...
    virtual ConstNodeIterator ConstNodeBegin() const override {
        return ConstNodeIterator(
            std::unique_ptr<ConstNodeIteratorAbstract>(new ConstNodeIteratorRG(this, true)));
    }

    virtual ConstNodeIterator ConstNodeBegin(const std::vector<double> &minu,
                                             const std::vector<double> &maxu) const override {
        return ConstNodeIterator(
            std::unique_ptr<ConstNodeIteratorAbstract>(new ConstNodeIteratorRG(this, minu, maxu)));
    }

    virtual ConstNodeIterator ConstNodeEnd() const override {
        return ConstNodeIterator(
            std::unique_ptr<ConstNodeIteratorAbstract>(new ConstNodeIteratorRG(this, false)));
    }
