...
class VDF_API DCCF : public VAPoR::DC {
  ...
  private:
    ...
    int _get_latlon_coordvars(NetCDFCFCollection *ncdfc, string dvar, string &loncvar,
                              string &latcvar) const;

    int _get_latlon_extents(NetCDFCFCollection *ncdfc, string latlon, bool lonflag, float &min,
                            float &max);

    int _get_coord_pair_extents(NetCDFCFCollection *ncdfc, string lon, string lat, double &lonmin,
                                double &lonmax, double &latmin, double &latmax);

    Proj4API *_create_proj4api(double lonmin, double lonmax, double latmin, double latmax,
    ...
    int _InitHorizontalCoordinatesDerived(NetCDFCFCollection *ncdfc,
    ...
    int _GetVarCoordinates(NetCDFCFCollection *ncdfc, string varname, vector<string> &sdimnames,
    ...
    template <class T>
    int _readRegionTemplate(const vector<size_t> &min, const vector<size_t> &max, T *region);
    ...
    class DerivedVarHorizontal : public NetCDFCollection::DerivedVar {
      public:
        DerivedVarHorizontal(NetCDFCFCollection *ncdfc, string lonname, string latname,
