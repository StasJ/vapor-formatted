...
class PARAMS_API ContourParams : public RenderParams {
  public:
    ...
    double GetContourMin() const { return (GetValueDouble(_contourMinTag, 0.f)); }

    void SetContourMin(double val) { SetValueDouble(_contourMinTag, "Contour minimum value", val); }

    double GetContourSpacing() const { return (GetValueDouble(_contourSpacingTag, 1.f)); }
    ...
    void SetNumDigits(int digits) {
        SetValueDouble(_numDigitsTag, "Number of digits in isovalue annotation", digits);
    }
    ...
    void SetTextDensity(int density) {
        SetValueDouble(_textDensityTag, "Density of isovalue annotations", density);
    }
