...
class VDF_API CurvilinearGrid : public StructuredGrid {
  public:
    ...
    //
    CurvilinearGrid(const size_t bs[3], const size_t min[3], const size_t max[3],
                    const bool periodic[3], const std::vector<float *> &blks,
                    const RegularGrid &xrg, const RegularGrid &yrg,
                    const std::vector<double> &zcoords, const KDTreeRGSubset &kdtree);
    ...
    //! \sa RegularGrid()
    //
    CurvilinearGrid(const std::vector<size_t> &bs, const std::vector<size_t> &min,
                    const std::vector<size_t> &max, const std::vector<bool> &periodic,
                    const std::vector<float *> &blks, const RegularGrid &xrg,
    ...
    //
    virtual void GetUserExtents(std::vector<double> &minext, std::vector<double> &maxext) const {
        minext = _minext;
        maxext = _maxext;
    }
  ...
  private:
    ...
    void _curvilinearGrid(const RegularGrid &xrg, const RegularGrid &yrg,
                          const std::vector<double> &zcoords, const KDTreeRGSubset &kdtree);

    void _GetUserExtents(std::vector<double> &minext, std::vector<double> &maxext) const;

    bool _binarySearchRange(const std::vector<double> &sorted, double x, int &i) const;

    bool _insideGrid(double x, double y, double z, int &i, int &j, int &k, double lambda[4],
                     double zwgt[2]) const;
