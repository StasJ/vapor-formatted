...
class VDF_API UnstructuredGrid2D : public UnstructuredGrid {
  public:
    ...
    //!
    //
    UnstructuredGrid2D(const std::vector<size_t> &vertexDims, const std::vector<size_t> &faceDims,
    ...
    class ConstCoordItrU2D : public Grid::ConstCoordItrAbstract {
      public:
        ...
        virtual bool equal(const void *rhs) const {
            ...
            return (_xCoordItr == itrptr->_xCoordItr && _yCoordItr == itrptr->_yCoordItr &&
  ...
  private:
    ...
    bool _insideGrid(const std::vector<double> &coords, std::vector<size_t> &face_indices,
                     double *lambda, int &nlambda, double zwgt[2]) const;

    bool _insideGridNodeCentered(const std::vector<double> &coords,
                                 std::vector<size_t> &face_indices, double *lambda, int &nlambda,
                                 double zwgt[2]) const;

    bool _insideGridFaceCentered(const std::vector<double> &coords,
                                 std::vector<size_t> &face_indices, double *lambda, int &nlambda,
                                 double zwgt[2]) const;
