...
class VDF_API Grid {
  public:
    ...
    //!
    virtual bool GetEnclosingRegion(const std::vector<double> &minu,
    ...
    //! by any cell.
    //!
    virtual bool GetIndicesCell(const double coords[3], size_t indices[3]) const = 0;
    ...
    static bool PointInsideBoundingRectangle(const double pt[], const double verts[], int n) {
        ...
        for (int i = 1; i < n; i++) {
            if (verts[i * 2 + 0] < left)
                left = verts[i * 2 + 0];
            if (verts[i * 2 + 0] > right)
                right = verts[i * 2 + 0];
            if (verts[i * 2 + 1] < top)
                top = verts[i * 2 + 1];
            if (verts[i * 2 + 1] > bottom)
                bottom = verts[i * 2 + 1];
        }
  ...
  protected:
    virtual float GetValueNearestNeighbor(const double coords[3]) const = 0;
