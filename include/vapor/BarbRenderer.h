...
class RENDER_API BarbRenderer : public Renderer {
  ...
  public:
    BarbRenderer(const ParamsMgr *pm, string winName, string dataSetName, string instName,
                 DataMgr *dataMgr);
  ...
  private:
    ...
    void _setDefaultLengthAndThicknessScales(size_t ts, const std::vector<VAPoR::Grid *> &varData,
                                             const BarbParams *bParams);

    void _getGridRequirements(int &ts, int &refLevel, int &lod, std::vector<double> &minExts,
    ...
    void _getDirection(float direction[3], std::vector<Grid *> varData, float xCoord, float yCoord,
                       float zCoord, bool &missing) const;
    ...
    bool _defineBarb(const std::vector<Grid *>, float start[3], float end[3], bool doColorMapping,
    ...
    void _drawCylinderSides(const float nextNormal[3], const float nextVertex[3],
                            const float startNormal[3], const float startVertex[3]) const;

    void _drawBarbHead(const float dirVec[3], const float vertexPoint[3],
                       const float startNormal[3], const float startVertex[3]) const;
    ...
    void _drawBarb(const std::vector<Grid *> variableData, const float startPoint[3],
