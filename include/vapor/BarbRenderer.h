...
class RENDER_API BarbRenderer : public Renderer {
  ...
  private:
    ...
    int performRendering(const RenderParams *rParams, int actualRefLevel, float vectorScale,
    ...
    //! \param[in] StructuredGrid* variableData[5] StructuredGrid containing field, color, and
    //! height variables \param[in] float vectorLengthScale:  Scale factor used in setting barb
    //! length \param[in] int timestep:  current time step. \param[in] float rad Radius of barbs in
    //! voxel diameters.
    void renderUnaligned(int rakeGrid[3], double rakeExts[6], StructuredGrid *variableData[5],
                         int timestep, float vectorLengthScale, float rad,
                         const RenderParams *params);

    float getHeightOffset(StructuredGrid *heightVar, float xCoord, float yCoord, bool &missing);

    void renderScottsGrid(int rakeGrid[3], double rakeExts[6], StructuredGrid *variableData[5],
                          int timestep, float vectorLengthScale, float rad,
                          const RenderParams *params);
    ...
    //! length \param[in] int timestep:  current time step. \param[in] float rad Radius of barbs in
    //! voxel diameters.
    void renderAligned(int rakeGrid[3], double rakeExts[6], StructuredGrid *variableData[5],
                       int timestep, float vectorLengthScale, float rad,
                       const RenderParams *params);
