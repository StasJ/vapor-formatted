...
class VDF_API DerivedVar : public Wasp::MyBase {
  public:
    ...
    virtual int ReadRegionBlock(int fd, const std::vector<size_t> &min,
                                const std::vector<size_t> &max, float *region) = 0;
  ...
  protected:
    ...
    int _getVar(DC *dc, size_t ts, string varname, int level, int lod,
                const std::vector<size_t> &min, const std::vector<size_t> &max,
                float *region) const;

    int _getVarDestagger(DC *dc, size_t ts, string varname, int level, int lod,
                         const std::vector<size_t> &min, const std::vector<size_t> &max,
                         float *region, int stagDim) const;

    int _getVarBlock(DC *dc, size_t ts, string varname, int level, int lod,
                     const std::vector<size_t> &min, const std::vector<size_t> &max,
                     float *region) const;
};
...
class VDF_API DerivedCFVertCoordVar : public DerivedCoordVar {
  public:
    DerivedCFVertCoordVar(string varName, DC *dc, string mesh, string formula)
        : DerivedCoordVar(varName), _dc(dc), _mesh(mesh), _formula(formula) {}
...
class PARAMS_API DerivedCFVertCoordVarFactory {
  public:
    ...
    void RegisterFactoryFunction(
        string name, function<DerivedCFVertCoordVar *(DC *, string, string)> classFactoryFunction) {
...
class VDF_API DerivedCoordVar_PCSFromLatLon : public DerivedCoordVar {
  public:
    DerivedCoordVar_PCSFromLatLon(string derivedVarName, DC *dc, std::vector<string> inNames,
                                  string proj4String, bool uGridFlag, bool lonFlag);
...
class VDF_API DerivedCoordVar_CF2D : public DerivedCoordVar {
  public:
    DerivedCoordVar_CF2D(string derivedVarName, std::vector<string> dimNames,
...
class VDF_API DerivedCoordVar_WRFTime : public DerivedCoordVar {
  public:
    DerivedCoordVar_WRFTime(string derivedVarName, NetCDFCollection *ncdfc, string wrfTimeVar,
                            string dimName, float p2si = 1.0);
  ...
  private:
    ...
    int _encodeTime(UDUnits &udunits, const vector<string> &timeStrings,
...
class VDF_API DerivedCoordVar_TimeInSeconds : public DerivedCoordVar {
  public:
    DerivedCoordVar_TimeInSeconds(string derivedVarName, DC *dc, string nativeTimeVar,
                                  string dimName);
...
class VDF_API DerivedCoordVar_Staggered : public DerivedCoordVar {
  public:
    DerivedCoordVar_Staggered(string derivedVarName, string stagDimName, DC *dc, string inName,
                              string dimName);
...
class VDF_API DerivedCoordVar_UnStaggered : public DerivedCoordVar {
  public:
    DerivedCoordVar_UnStaggered(string derivedVarName, string unstagDimName, DC *dc, string inName,
                                string dimName);
...
class VDF_API DerivedCoordVarStandardOceanSCoordinateG2 : public DerivedCFVertCoordVar {
  public:
    DerivedCoordVarStandardOceanSCoordinateG2(DC *dc, string mesh, string formula);
