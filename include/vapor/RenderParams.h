...
class PARAMS_API RenderParams : public ParamsBase {
  public:
    ...
    virtual void SetDistribVariableNames(vector<string> varNames) {
        SetValueStringVec(_distribVariableNamesTag, "Set Distrib Vars", varNames);
    }
    ...
    void SetStretchFactors(vector<double> factors) {
        vector<double> defaultvec(3, 1.);
        if (factors.size() != defaultvec.size())
            factors = defaultvec;
...
class PARAMS_API RenParamsFactory {
  public:
    ...
    void
    RegisterFactoryFunction(string name,
                            function<RenderParams *(DataMgr *, ParamsBase::StateSave *, XmlNode *)>
...
template <class T> class PARAMS_API RenParamsRegistrar {
  public:
    RenParamsRegistrar(string classType) {
        ...
        RenParamsFactory::Instance()->RegisterFactoryFunction(
            classType,
            [](DataMgr *dataMgr, ParamsBase::StateSave *ssave, XmlNode *node) -> RenderParams * {
                if (node)
