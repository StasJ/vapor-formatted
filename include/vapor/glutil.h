...
VDF_API void computeGradientData(int dim[3], int numChan, unsigned char *volume,
...
VDF_API void makeModelviewMatrixD(const std::vector<double> &vpos, const std::vector<double> &vdir,
...
VDF_API int rayBoxIntersect(const float rayStart[3], const float rayDir[3], const float boxExts[6],
                            float results[2]);
VDF_API int rayBoxIntersect(const double rayStart[3], const double rayDir[3],
                            const double boxExts[6], double results[2]);
// Determine the minimum and maximum projection of box corners to ray
VDF_API void rayBoxProject(std::vector<double> rayStart, std::vector<double> rayDir,
                           const double boxExts[6], double results[2]);
...
VDF_API void CalcRotation(double *q, double newX, double newY, double oldX, double oldY,
                          double ballsize);
...
VDF_API void squad(float quat1[4], float quat2[4], float s1[4], float s2[4], float t,
                   float result[4]);

VDF_API void imagQuat2View(const float q[3], float vdir[3], float upvec[3]);

VDF_API void views2ImagQuats(float vdir1[3], float upvec1[3], float vdir2[3], float upvec2[3],
