...
//! The RegionParams class also has several methods that are useful in setting up data requests from
//! the DataMgr.
//!
class PARAMS_API RegionParams : public ParamsBase {
  ...
  public:
    ...
    //! Insert a specific time in the list of time-varying extents.  Return false if it's already
    //! there \param[in] timestep to be inserted \retval true if the time was not already in the
    //! list.
    ...
    static const vector<string> GetDomainVariables() {
        ((RegionParams *)_paramsMgr->GetParamsInstance(_regionParamsTag, -1, -1))
            ->GetValueStringVec(_domainVariablesTag);
        return vec;
    }
    ...
    static int SetDomainVariables(vector<string> varnames) {
        if (varnames.size() == 0)
            return -1;
        return ((RegionParams *)_paramsMgr->GetParamsInstance(_regionParamsTag, -1, -1))
            ->SetValueStringVec(_domainVariablesTag, "Set Domain-defining variables", varnames);
    }
#endif
