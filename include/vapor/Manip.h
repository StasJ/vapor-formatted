...
//! [for built-in Manips] or VAPoR::InstallExtensionMouseModes() [for Manips on extension Params
//! classes]
//!
class RENDER_API Manip {
  ...
  public:
    ...
    virtual void Update(std::vector<double> llc, std::vector<double> urc,
                        ...
                        std::vector<int> windowSize, VAPoR::Transform *rpTransform,
    ...
    virtual bool MouseEvent(int buttonNum, std::vector<double> screenCoords,
                            double handleMidpoint[3], bool release = false) = 0;
    ...
    //! Pure virtual function, indicates that the mouse has been pressed over a handle, so is
    //! currently dragging the handle \return handle index
...
class RENDER_API TranslateStretchManip : public Manip {
  public:
    ...
    //! @copydoc Manip::Update(std::vector<double>, std::vector<double> std::vector<double>,
    //! std::vector<double>)
    virtual void Update(std::vector<double> llc, std::vector<double> urc,
                        std::vector<double> minExtents, std::vector<double> maxExtents,
                        std::vector<double> cameraPosition, std::vector<double> rotationCenter,
    ...
    virtual bool MouseEvent(int buttonNum, std::vector<double> screenCoords,
                            double handleMidpoint[3], bool release = false);
    ...
    //! Invoked during mouseMoveEvent, uses values of mouseDownPoint(), handleProjVec(),
  ...
  protected:
    ...
    virtual bool pointIsOnQuad(double cor1[3], double cor2[3], double cor3[3], double cor4[3],
                               double pickPt[2]);
    ...
    void drawHitBox(double winCoord1[2], double winCoord2[2], double winCoord3[2],
                    double winCoord4[2]);
    ...
    bool rayHandleIntersect(double ray[3], const std::vector<double> &cameraPos, int handleNum,
                            int faceNum, double intersect[3]);
    ...
    bool _mouseDownHere;
    double _initialSelectionRay[3]; // Vector from camera to handle, when mouse is initially
                                    // clicked.
