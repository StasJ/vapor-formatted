...
class VDF_API GridHelper : public Wasp::MyBase {
  ...
  public:
    ...
    string GetGridType(const DC::Mesh &m, const std::vector<DC::CoordVar> &cvarsinfo,
                       const std::vector<vector<string>> &cdimnames) const;
    ...
    //
    StructuredGrid *MakeGridStructured(string gridType, size_t ts, int level, int lod,
    ...
    UnstructuredGrid *MakeGridUnstructured(
        ...
        const std::vector<DC::CoordVar> &cvarsinfo, const std::vector<size_t> &roi_dims,
        const std::vector<size_t> &dims, const std::vector<float *> &blkvec,
        const std::vector<std::vector<size_t>> &bsvec,
        const std::vector<std::vector<size_t>> &bminvec,
        const std::vector<std::vector<size_t>> &bmaxvec, const std::vector<int *> &conn_blkvec,
        const std::vector<std::vector<size_t>> &conn_bsvec,
        const std::vector<std::vector<size_t>> &conn_bminvec,
        const std::vector<std::vector<size_t>> &conn_bmaxvec, const std::vector<size_t> &vertexDims,
        const std::vector<size_t> &faceDims, const std::vector<size_t> &edgeDims,
        UnstructuredGrid::Location location, size_t maxVertexPerFace, size_t maxFacePerVertex,
        long vertexOffset, long faceOffset);
  ...
  private:
    ...
    RegularGrid *_make_grid_regular(const std::vector<size_t> &dims,
                                    const std::vector<float *> &blkvec,
    ...
    StretchedGrid *_make_grid_stretched(const std::vector<size_t> &dims,
                                        const std::vector<float *> &blkvec,
                                        const std::vector<size_t> &bs,
                                        const std::vector<size_t> &bmin,
                                        const std::vector<size_t> &bmax) const;

    LayeredGrid *_make_grid_layered(const std::vector<size_t> &dims,
                                    const std::vector<float *> &blkvec,
                                    const std::vector<size_t> &bs, const std::vector<size_t> &bmin,
                                    const std::vector<size_t> &bmax) const;

    CurvilinearGrid *_make_grid_curvilinear(size_t ts, int level, int lod,
                                            const std::vector<DC::CoordVar> &cvarsinfo,
                                            const std::vector<size_t> &dims,
                                            const std::vector<float *> &blkvec,
                                            const std::vector<size_t> &bs,
    ...
    UnstructuredGrid2D *_make_grid_unstructured2d(
        size_t ts, int level, int lod, const DC::DataVar &var,
        const std::vector<DC::CoordVar> &cvarsinfo, const std::vector<size_t> &dims,
        const std::vector<float *> &blkvec, const std::vector<size_t> &bs,
        const std::vector<size_t> &bmin, const std::vector<size_t> &bmax,
        const std::vector<int *> &conn_blkvec, const std::vector<size_t> &conn_bs,
        const std::vector<size_t> &conn_bmin, const std::vector<size_t> &conn_bmax,
        const std::vector<size_t> &vertexDims, const std::vector<size_t> &faceDims,
        const std::vector<size_t> &edgeDims, UnstructuredGrid::Location location,
    ...
    UnstructuredGridLayered *_make_grid_unstructured_layered(
        ...
        const vector<size_t> &edgeDims, UnstructuredGrid::Location location,
        size_t maxVertexPerFace, size_t maxFacePerVertex, long vertexOffset, long faceOffset);

    void _makeGridHelper(const DC::DataVar &var, const std::vector<size_t> &roi_dims,
                         const std::vector<size_t> &dims, Grid *g) const;

    bool _isUnstructured2D(const DC::Mesh &m, const std::vector<DC::CoordVar> &cvarsinfo,
                           const std::vector<std::vector<string>> &cdimnames) const;

    bool _isUnstructuredLayered(const DC::Mesh &m, const std::vector<DC::CoordVar> &cvarsinfo,
                                const std::vector<std::vector<string>> &cdimnames) const;

    bool _isRegular(const DC::Mesh &m, const std::vector<DC::CoordVar> &cvarsinfo,
                    const std::vector<std::vector<string>> &cdimnames) const;

    bool _isStretched(const DC::Mesh &m, const std::vector<DC::CoordVar> &cvarsinfo,
                      const std::vector<std::vector<string>> &cdimnames) const;

    bool _isLayered(const DC::Mesh &m, const std::vector<DC::CoordVar> &cvarsinfo,
                    const std::vector<std::vector<string>> &cdimnames) const;

    bool _isCurvilinear(const DC::Mesh &m, const std::vector<DC::CoordVar> &cvarsinfo,
                        const std::vector<std::vector<string>> &cdimnames) const;

    string _getQuadTreeRectangleKey(size_t ts, int level, int lod,
                                    const vector<DC::CoordVar> &cvarsinfo,
                                    const vector<size_t> &bmin, const vector<size_t> &bmax) const;
};
