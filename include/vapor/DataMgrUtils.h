...
//
VDF_API bool MaxXFormPresent(const DataMgr *dataMgr, size_t timestep, string varname,
                             size_t &maxXForm);
...
//
template <typename T>
VDF_API int GetGrids(DataMgr *dataMgr, size_t ts, const vector<string> &varnames,
                     const vector<T> &minExtsReq, const vector<T> &maxExtsReq,
                     bool useLowerAccuracy, int *refLevel, int *lod, std::vector<Grid *> &grids);

VDF_API int GetGrids(DataMgr *dataMgr, size_t ts, string varname, const vector<double> &minExtsReq,
                     const vector<double> &maxExtsReq, bool useLowerAccuracy, int *refLevel,
                     int *lod, Grid **gridptr);

VDF_API int GetGrids(DataMgr *dataMgr, size_t ts, const vector<string> &varnames,
                     bool useLowerAccuracy, int *refLevel, int *lod, std::vector<Grid *> &grids);

VDF_API int GetGrids(DataMgr *dataMgr, size_t ts, string varname, bool useLowerAccuracy,
...
//! \sa GetAxes()
//
VDF_API bool GetExtents(DataMgr *dataMgr, size_t timestep, string varname, int refLevel, int lod,
...
//! \sa GetAxes()
//
VDF_API bool GetExtents(DataMgr *dataMgr, size_t timestep, const vector<string> &varnames,
                        int refLevel, int lod, vector<double> &minExts, vector<double> &maxExts,
                        vector<int> &axes);
...
//
VDF_API bool GetFirstExistingVariable(DataMgr *dataMgr, int level, int lod, int ndim,
                                      string &varname, size_t &ts);
...
//! \sa DataMgr::VariableExists()
//
VDF_API bool GetFirstExistingVariable(DataMgr *dataMgr, size_t ts, int level, int lod, int ndim,
                                      string &varname);
...
//! Determine the number of active 3D variables.  This includes 3D variables in the VDC as well as
//! any 3D derived variables \return number of active 3D variables
int getNumActiveVariables3D() { return _dataMgr->GetDataVarNames(3).size(); }
//! Determine the number of active 2D variables.  This includes 2D variables in the VDC as well as
//! any 2D derived variables \return number of active 2D variables
