...
class VDF_API UDUnits {
  public:
    ...
    //!
    bool Convert(const std::string from, const std::string to, const float *src, float *dst,
                 size_t n) const;

    bool Convert(const std::string from, const std::string to, const double *src, double *dst,
                 size_t n) const;
    ...
    //! time conversion
    //!
    void DecodeTime(double seconds, int *year, int *month, int *day, int *hour, int *minute,
                    int *second) const;
