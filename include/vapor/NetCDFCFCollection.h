...
class VDF_API NetCDFCFCollection : public NetCDFCollection {
  public:
    ...
    virtual bool IsCoordVarCF(string var) const {
        return (std::find(_coordinateVars.begin(), _coordinateVars.end(), var) !=
                _coordinateVars.end());
    }
    ...
    virtual bool IsAuxCoordVarCF(string var) const {
        return (std::find(_auxCoordinateVars.begin(), _auxCoordinateVars.end(), var) !=
                _auxCoordinateVars.end());
    }
    ...
    virtual bool IsTimeCoordVar(string var) const {
        return (std::find(_timeCoordVars.begin(), _timeCoordVars.end(), var) !=
                _timeCoordVars.end());
    }
    ...
    virtual bool IsVertCoordVar(string var) const {
        return (std::find(_vertCoordVars.begin(), _vertCoordVars.end(), var) !=
                _vertCoordVars.end());
    }
    ...
    //! \retval status a non-negative int is returned on success
    //!
    virtual int Convert(const string from, const string to, const double *src, double *dst,
                        size_t n) const;
    virtual int Convert(const string from, const string to, const float *src, float *dst,
                        size_t n) const;
    ...
    //!
    virtual int InstallStandardVerticalConverter(string cvar, string newvar,
                                                 string units = "meters");
  ...
  private:
    class DerivedVar_ocean_s_coordinate_g1 : public NetCDFCollection::DerivedVar {
      public:
        DerivedVar_ocean_s_coordinate_g1(NetCDFCFCollection *ncdfcf,
    ...
    class DerivedVar_ocean_s_coordinate_g2 : public NetCDFCollection::DerivedVar {
      ...
      public:
        DerivedVar_ocean_s_coordinate_g2(NetCDFCFCollection *ncdfcf,
                                         const std::map<string, string> &formula_map);
    ...
    class DerivedVar_AHSPC : public NetCDFCollection::DerivedVar {
      ...
      public:
        DerivedVar_AHSPC(NetCDFCFCollection *ncdfcf, const std::map<string, string> &formula_map);
      ...
      private:
        ...
        virtual int DCZ2(const float *PS1, const float *PHIS1, float **TV2, const int NL,
                         const float P0, float **HYBA, float **HYBB, const int KLEV, const int MLON,
                         const int MLON2, float **HYPDLN, float **HYALPH, float **PTERM,
                         float **ZSLICE);
        std::vector<size_t> _dims;
        std::vector<string> _dimnames;
        size_t _slice_num;
    ...
    class DerivedVar_noop : public NetCDFCollection::DerivedVar {
      ...
      public:
        DerivedVar_noop(NetCDFCFCollection *ncdfcf, const std::map<string, string> &formula_map,
                        string units);
        ...
        virtual size_t GetTimeDim() const { return (_ncdfc->GetTimeDim(_zvar)); };
        virtual string GetTimeDimName() const { return (_ncdfc->GetTimeDimName(_zvar)); };
    ...
    class DerivedVarTime : public NetCDFCollection::DerivedVar {
      public:
        DerivedVarTime(NetCDFCFCollection *ncdfcf, string native_var, string native_units,
    ...
    class DerivedVarTimeFromMem : public NetCDFCollection::DerivedVar {
      public:
        DerivedVarTimeFromMem(string name, string units, string timeDimName,
    ...
    class DerivedVar_vertStag : public NetCDFCollection::DerivedVar {
      ...
      public:
        ...
        virtual std::vector<string> GetAttNames() const {
            return (std::vector<string>(1, "units"));
        }
