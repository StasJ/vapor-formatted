...
COMMON_API void computeGradientData(int dim[3], int numChan, unsigned char *volume,
...
COMMON_API void makeModelviewMatrixD(const std::vector<double> &vpos,
                                     const std::vector<double> &vdir,
                                     const std::vector<double> &upvec, double *matrix);
...
COMMON_API int rayBoxIntersect(const float rayStart[3], const float rayDir[3],
                               const float boxExts[6], float results[2]);
COMMON_API int rayBoxIntersect(const double rayStart[3], const double rayDir[3],
                               const double boxExts[6], double results[2]);
// Determine the minimum and maximum projection of box corners to ray
COMMON_API void rayBoxProject(std::vector<double> rayStart, std::vector<double> rayDir,
                              const double boxExts[6], double results[2]);
...
COMMON_API void CalcRotation(float *q, float newX, float newY, float oldX, float oldY,
                             float ballsize);
COMMON_API void CalcRotation(double *q, double newX, double newY, double oldX, double oldY,
                             double ballsize);
...
COMMON_API void squad(float quat1[4], float quat2[4], float s1[4], float s2[4], float t,
                      float result[4]);

COMMON_API void imagQuat2View(const float q[3], float vdir[3], float upvec[3]);

COMMON_API void views2ImagQuats(float vdir1[3], float upvec1[3], float vdir2[3], float upvec2[3],
...
inline float vdot(const float *a, const double *b) {
    return (a[0] * b[0] + a[1] * b[1] + a[2] * b[2]);
}
...
inline double vlength(const std::vector<double> a) {
