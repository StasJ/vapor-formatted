...
//! a local VizFeatureParams for each window, that users can select whenever there are multiple
//! windows. When local settings are used, they only affect one currently active visualizer. The
//! VizFeatureParams class also has several methods that are useful in setting up data requests from
//! the DataMgr.
//!
class PARAMS_API VizFeatureParams : public ParamsBase {
  ...
  public:
    ...
    void SetUseDomainFrame(bool onOff) {
        SetValueLong(_domainFrameTag, "toggle domain frame", (long)onOff);
    }

    bool GetUseRegionFrame() const { return (0 != GetValueLong(_regionFrameTag, (long)false)); }
    void SetUseRegionFrame(bool onOff) {
        SetValueLong(_regionFrameTag, "toggle region frame", (long)onOff);
    }
    ...
    void SetAxisAnnotation(bool val) {
        SetValueLong(_axisAnnotationEnabledTag, "toggle axis annotation", (long)val);
    }

    //! query axis annotation enabled
    bool GetAxisAnnotation() const {
        return (0 != GetValueLong(_axisAnnotationEnabledTag, (long)false));
    }
    ...
    bool ShowAxisArrows() const { return (0 != GetValueLong(_showAxisArrowsTag, (long)false)); }
