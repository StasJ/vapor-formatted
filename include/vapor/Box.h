...
class PARAMS_API Box : public ParamsBase {
  public:
    ...
    void SetLocalExtents(const vector<double> &minExt, const vector<double> &maxExt,
                         int timestep = -1) {
    ...
    //! Specify the local extents as a double array.  If time step is -1, then set the generic
    //! extents. Otherwise set the extents for a specific timestep. \param[in] double extents[6] 6
    //! doubles that will be new extents \param[in] int timestep specified time step, or -1 for
    //! generic times \retval int zero if successful.
    void SetLocalExtents(const double extents[6], int timestep = -1);

    //! Specify the local extents as a float array.  If time step is -1, then set the generic
    //! extents. Otherwise set the extents for a specific timestep. \param[in] float extents[6]
...
#ifdef DEAD
    //! Specify the stretched local extents as a float array.  If time step is -1, then set the
    //! generic extents. Otherwise set the extents for a specific timestep. \param[in] float
    //! extents[6] \param[in] int timestep specified time step, or -1 for generic times \retval int
    //! zero if successful.
    ...
    const vector<long> GetTimes() { return (GetValueLongVec(Box::_timesTag)); }
    ...
    void buildLocalCoordTransform(double transformMatrix[12], double extraThickness, int timestep,
                                  double rotation = 0., int axis = -1) const;
  ...
  private:
    ...
    void calcContainingBoxExtents(double extents[6], bool rotated = false) const {

        if (!rotated)
            GetLocalExtents(extents, -1);
        else
            calcRotatedBoxExtents(extents);
    }
    ...
    //! \param[out] float[6] Extents of containing box
    //! \params[in] rotated indicates if the box is possibly rotated
    void calcContainingStretchedBoxExtents(double extents[6], bool rotated = false) const {

        assert(!rotated);
        if (!rotated)
            GetStretchedLocalExtents(extents, -1);
    ...
    //! \param[out] double extents[6] is smallest extents containing the box.
    void calcRotatedStretchedBoxExtents(vector<double> stretchFactors, double extents[6]) const;
    ...
    void convertThetaPhiPsi(double *newTheta, double *newPhi, double *newPsi, int axis,
                            double rotation) const;

    // Not part of public API
    void calcLocalBoxCorners(double corners[8][3], float extraThickness, int timestep,
                             double rotation = 0., int axis = -1) const;
