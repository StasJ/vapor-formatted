...
//! and the properties of light sources.  If the ViewpointParams is shared (i.e. not local), all
//! windows can use the same viewpoint and lights.  Local viewpoints are just applicable in one
//! visualizer.
class PARAMS_API ViewpointParams : public ParamsBase {
  ...
  public:
    ...
    //! \param[in] vector<double> scaleFac scale factors to be applied, relative to previous
    //! scaling.
    ...
    std::vector<double> GetRotationCenter() const {
        return (getCurrentViewpoint()->GetRotationCenter());
    }
    ...
    static const double *getDefaultLightDirection(int lightNum) {
