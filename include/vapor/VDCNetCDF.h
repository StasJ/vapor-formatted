...
class VDF_API VDCNetCDF : public VAPoR::VDC {
  public:
    ...
    //! for appending or reading.
    //
    VDCNetCDF(int numthreads = 0, size_t master_theshold = 10 * 1024 * 1024,
    ...
    //!
    virtual int GetPath(string varname, size_t ts, string &path, size_t &file_ts,
                        size_t &max_ts) const;
    ...
    //
    virtual int Initialize(const vector<string> &paths, const vector<string> &options,
                           AccessMode mode, size_t chunksizehint = 0);
    virtual int Initialize(string path, const vector<string> &options, AccessMode mode,
    ...
    //
    bool CompressionInfo(std::vector<size_t> bs, string wname, size_t &nlevels,
  ...
  private:
    ...
    class open_file_info {
      public:
        ...
        open_file_info(WASP *wasp, bool write, vector<size_t> dims, vector<size_t> bs,
                       size_t slice_num, size_t ts, size_t file_ts, string varname, int level)
            : _wasp(wasp), _write(write), _dims(dims), _bs(bs), _slice_num(slice_num), _ts(ts),
              _file_ts(file_ts), _varname(varname), _level(level){};
    ...
    template <class T>
    int _copyVarHelper(DC &dc, vector<size_t> &buffer_dims, vector<size_t> &src_hslice_dims,
