...
class RENDER_API WireFrameRenderer : public Renderer {
  public:
    ...
    //! this renderer
    WireFrameRenderer(const ParamsMgr *pm, string winName, string dataSetName, string instName,
                      DataMgr *dataMgr);
  ...
  private:
    ...
    void _drawCell(const GLuint *cellNodeIndices, int n, bool layered,
                   const std::vector<GLuint> &nodeMap, GLuint invalidIndex,
                   std::vector<unsigned int> &indices, DrawList &drawList) const;
