...
//! OpenGL rendering in the VAPOR visualizer window, using a RenderParams instance to describe the
//! rendering. All Renderer classes must derive from this class.
class RENDER_API Renderer : public RendererBase {
  ...
  public:
...
#ifdef DEAD
    //! Call setBypass to indicate that the renderer will not work until the state of the params is
    //! changed This will result in the renderer not being invoked for the specified timestep
    //! \param[in] int timestep The timestep when the renderer fails
    void setBypass(int timestep) {
        if (_currentRenderParams)
            _currentRenderParams->setBypass(timestep);
    }
    ...
    void setPartialBypass(int timestep) {
        if (_currentRenderParams)
            _currentRenderParams->setPartialBypass(timestep);
    }
    ...
    void setAllBypass(bool val) {
        if (_currentRenderParams)
            _currentRenderParams->setAllBypass(val);
    }
    ...
    //! \param[in] auxPrev is a copy of auxiliary InstanceParams at the start of the Command, not
    //! currently used \param[in] afterP is a copy of auxiliary InstanceParams at the end of the
    //! Command, not currently used
    static void UndoRedo(bool isUndo, int instance, Params *beforeP, Params *afterP,
                         Params *auxPrev = 0, Params *auxNext = 0);
    ...
    void buildLocal2DTransform(int dataOrientation, float a[2], float b[2], float *constVal,
                               int mappedDims[3]);
#endif
  ...
  protected:
    ...
    //! enableClippingPlanes(), enableFullClippingPlanes(), enableRegionClippingPlanes, or
    //! enable2DClippingPlanes()
...
class PARAMS_API RendererFactory {
  public:
    ...
    void RegisterFactoryFunction(
        ...
        function<Renderer *(const ParamsMgr *, string, string, string, string, DataStatus *)>
    ...
    RendererRegistrar(string classType, string paramsClassType) {
        ...
        RendererFactory::Instance()->RegisterFactoryFunction(
            ...
            [](const ParamsMgr *pm, string winName, string dataSetName, string classType,
               string instName, DataStatus *ds) -> Renderer * {
