...
class VDF_API UnstructuredGrid : public Grid {
  public:
    ...
    //! may have.
    //
    UnstructuredGrid(const std::vector<size_t> &node_dims, const std::vector<size_t> &cell_dims,
                     const std::vector<size_t> &edge_dims, const std::vector<float *> &blks,
                     const size_t *face_node_conn, const size_t *face_face_conn,
    ...
    std::vector<size_t> GetDimensionInBlks() const { return (_bdims); }
    ...
    //! to the constructor.
    //
    void GetBlockSize(std::vector<size_t> &bs) const { bs = _bs; }
    std::vector<size_t> GetBlockSize() const { return (_bs); }
    ...
    class ConstCoordItr {
      public:
        ...
        //
        ConstCoordItr(ConstCoordItr &&rhs) { _impl = std::move(rhs._impl); }
        ...
        bool operator!=(const ConstCoordItr &rhs) const { return (!(*this == rhs)); }
    ...
    template <class T> class VDF_API ForwardIterator {
      public:
        ForwardIterator(T *rg, const std::vector<double> &minu = {},
        ...
        friend void swap(UnstructuredGrid::ForwardIterator<T> &a,
    ...
    //!
    //
    template <class T> class VDF_API ForwardCellIterator {
      public:
        ForwardCellIterator(T *sg, const std::vector<double> &minu = {},
                            const std::vector<double> &maxu = {});
        ...
        bool operator!=(const ForwardCellIterator<T> &rhs) { return (!(*this == rhs)); }

        friend void swap(UnstructuredGrid::ForwardCellIterator<T> &a,
    ...
    //! inside or on the box defined by \p minu and \p maxu
    //
    CellIterator ConstCellBegin(const std::vector<double> &minu, const std::vector<double> &maxu) {
        return (CellIterator(this, minu, maxu));
    }
#endif

    ConstCellIterator ConstCellBegin() const { return (ConstCellIterator(this, true)); }

    ConstCellIterator ConstCellEnd() const { return (ConstCellIterator(this, false)); }
    ...
    //!
    //
    template <class T> class VDF_API ForwardNodeIterator {
      public:
        ForwardNodeIterator(T *sg, const std::vector<double> &minu = {},
                            const std::vector<double> &maxu = {});
        ...
        bool operator!=(const ForwardNodeIterator<T> &rhs) { return (!(*this == rhs)); }

        friend void swap(UnstructuredGrid::ForwardNodeIterator<T> &a,
    ...
    //! inside or on the box defined by \p minu and \p maxu
    //
    NodeIterator ConstNodeBegin(const std::vector<double> &minu, const std::vector<double> &maxu) {
        return (NodeIterator(this, minu, maxu));
    }
#endif

    ConstNodeIterator ConstNodeBegin() const { return (ConstNodeIterator(this, true)); }

    ConstNodeIterator ConstNodeEnd() const { return (ConstNodeIterator(this, false)); }
