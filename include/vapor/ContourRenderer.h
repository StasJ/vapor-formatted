...
class RENDER_API ContourRenderer : public Renderer {
  ...
  private:
    ...
    bool pointWithinBounds(double x, double y, double z, vector<double> varMin,
    ...
    //! Add an edge to the edge-edge mappings needed to traverse the contours, needed in order to
    //! place text annotation \param[in] segIndex Index associated with the edge being added
    ...
    //! Traverse the contour curves defined by the current edge->edge mappings, identifying the
    //! points where text annotation should be attached. \param[in] iso Index of isovalue associated
    //! with the curve
    ...
    float interp_j(int i, int j, float isoval, float *dataVals) {
        return (-1. + 2. * (double)(j) / ((double)_sampleSize - 1.) // y coordinate at (i,j)
                ...
                2. / (double)(_sampleSize - 1.) // y grid spacing
                    * (isoval - dataVals[i + _sampleSize * j]) /
                    (dataVals[i + _sampleSize * (j + 1)] -
                     dataVals[i + _sampleSize * j])); // ratio: 1 is iso at top, 0 if iso at bottom
    }
    ...
    float interp_i(int i, int j, float isoval, float *dataVals) {
        return (-1. + 2. * (double)(i) / ((double)_sampleSize - 1.) // x coordinate at (i,j)
                ...
                2. / (double)(_sampleSize - 1.) // x grid spacing
                    * (isoval - dataVals[i + _sampleSize * j]) /
                    (dataVals[i + 1 + _sampleSize * (j)] -
                     dataVals[i + _sampleSize * j])); // ratio: 1 is iso at right, 0 if iso at left
    }
    ...
    // Each edge is represented as a pair (int, int).  Horizontal edges have both ints positive;
    // vertical have both negative. Vertical edges connecting grid vertices (i,j) and (i,j+1) are
    // represented by the pair (-i, -j-1) (-i <=0; -j-1 < 0) Horizontal edges connecting grid
    // vertices (i,j) and (i+1,j) are represented by the pair (i+1,j) (i+1 > 0, j>=0)
    //

    // _lineCache maps pair<int,int>->vector<float[4]> maps a pair (timestep,isovalueIndex) to a
    // vector of segment endpoints, with one segment for each line in the contour
    // For each segment in the _lineCache, there are 5 temporary mappings:
    // prevEdge maps segments to edges.  Each segment is identified by (timestep, isoval,
    // segmentIndex), corresponding edge is pair(int,int) nextEdge is the same;
    // map(int,int,int)->pair(int,int) plus prevSegment maps prevEdge to segment; i.e. map
    // (timestep, isoval, (int,int))-> segmentIndex similarly nextSegment maps nextEdge to segment;
    // i.e. map (timestep, isoval, (int,int))-> segmentIndex plus forwardEdge maps
    // (timestep,isoval,edge)-> edge and backwardEdge maps (timestep,isoval,edge)-> edge

    std::map<pair<int, int>, int>
        _edgeSeg; // map an edge to a segment, to find 3d coords where edge is located
    ...
    std::map<pair<int, int>, bool>
        _markerBit; // indicate whether or not an edge has been visited during current traversal
