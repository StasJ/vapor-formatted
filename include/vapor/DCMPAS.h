...
class VDF_API DCMPAS : public VAPoR::DC {
  ...
  protected:
    ...
    virtual string getMapProjection() const {
        return ("+proj=eqc +ellps=WGS84 +lon_0=0.0 +lat_0=0.0");
    }
  ...
  private:
    ...
    class MPASFileObject : public DC::FileTable::FileObject {
      public:
        MPASFileObject(size_t ts, string varname, int level, int lod, int fd, bool derivedFlag)
    ...
    int _GetVarCoordinates(NetCDFCollection *ncdfc, string varname, vector<string> &sdimnames,
    ...
    class DerivedCoordVertFromCell : public DerivedCoordVar {
      public:
        DerivedCoordVertFromCell(string derivedVarName, string derivedDimName, DC *dc,
        ...
        int GetDimLensAtLevel(int, std::vector<size_t> &dims_at_level,
                              std::vector<size_t> &bs_at_level) const;
        ...
        int ReadRegionBlock(int fd, const vector<size_t> &min, const vector<size_t> &max,
                            float *region) {
    ...
    class DerivedZonalMeridonal : public DerivedDataVar {
      public:
        DerivedZonalMeridonal(string derivedVarName, DC *dc, NetCDFCollection *ncdfc,
