...
class RENDER_API Visualizer : public MyBase {
  public:
    ...
    bool pixelToVector(double winCoords[2], const vector<double> StrCameraPos, double dirVec[3],
    ...
    //! Turn on or off the image capture enablement.  If on, the next paintEvent will result in
    //! capture Also saves the capture file name
    ...
    //! Turn on or off the animation capture enablement.  If on, all paintEvents will result in
    //! capture until it is turned off
  ...
  private:
    ...
    bool pointIsOnQuad(double cor1[3], double cor2[3], double cor3[3], double cor4[3],
                       double pickPt[2]);
    ...
    void getFarNearDist(vector<double> posVec, vector<double> dirVec, float &boxFar,
                        float &boxNear) const;
