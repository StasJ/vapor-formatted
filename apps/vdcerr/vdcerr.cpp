...
void compute_error(const float *odata, const float *cdata, size_t nelements, bool has_missing,
                   float mv, float &lmax, float &rms, float &min, float &max) {
...
int main(int argc, char **argv) {
    ...
    compute_error(rawdata, vdcdata, dims[0] * dims[1] * dims[2], has_missing, mv, lmax, rms, min,
                  max);
    float lmaxrel = 0.0;
    if ((max - min) != 0.0)
        lmaxrel = lmax / (max - min);
    float nrms = 0.0;
    if ((max - min) != 0.0)
        nrms = rms / (max - min);
