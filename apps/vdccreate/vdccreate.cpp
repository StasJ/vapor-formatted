...
void set_coords_uniform(VDCNetCDF &vdc, const vector<float> &extents,
                        const vector<string> &dimnames, const vector<size_t> &dimlens) {
    if (!extents.size())
        return;
...
DC::XType parseXType(string xTypeStr) {
    if (xTypeStr == "uint8")
        return (DC::XType::UINT8);
    if (xTypeStr == "int8")
        return (DC::XType::INT8);
    //	if (xTypeStr == "int16") return (DC::XType::INT16);
    if (xTypeStr == "int32")
        return (DC::XType::INT32);
    if (xTypeStr == "int64")
        return (DC::XType::INT64);
    if (xTypeStr == "float")
        return (DC::XType::FLOAT);
    if (xTypeStr == "float32")
        return (DC::XType::FLOAT);
    if (xTypeStr == "float64")
...
int main(int argc, char **argv) {
    ...
    if (!opt.xcoords.empty()) {
        rc = vdc.DefineCoordVar(dimnames[0], vector<string>{dimnames[0]}, "", "", 0,
                                DC::XType::FLOAT, false);
    }
    if (!opt.ycoords.empty()) {
        rc = vdc.DefineCoordVar(dimnames[1], vector<string>{dimnames[1]}, "", "", 1,
                                DC::XType::FLOAT, false);
    }
    if (!opt.zcoords.empty()) {
        rc = vdc.DefineCoordVar(dimnames[2], vector<string>{dimnames[2]}, "", "", 2,
                                DC::XType::FLOAT, false);
    }
    if (!opt.tcoords.empty()) {
        rc = vdc.DefineCoordVar(dimnames[3], vector<string>(), "", dimnames[3], 3, DC::XType::FLOAT,
                                false);
    }
    ...
    for (int i = 0; i < opt.vars3d.size(); i++) {
        rc = vdc.DefineDataVar(opt.vars3d[i], dimnames, dimnames, "", xType, true);
    }
    for (int i = 0; i < opt.ncvars3d.size(); i++) {
        rc = vdc.DefineDataVar(opt.ncvars3d[i], dimnames, dimnames, "", xType, false);
    }
    ...
    for (int i = 0; i < opt.vars2dxy.size(); i++) {
        rc = vdc.DefineDataVar(opt.vars2dxy[i], dimnames2dxy, dimnames2dxy, "", xType, true);
    }
    for (int i = 0; i < opt.ncvars2dxy.size(); i++) {
        rc = vdc.DefineDataVar(opt.ncvars2dxy[i], dimnames2dxy, dimnames2dxy, "", xType, false);
    }
    ...
    for (int i = 0; i < opt.vars2dxz.size(); i++) {
        rc = vdc.DefineDataVar(opt.vars2dxz[i], dimnames2dxz, dimnames2dxz, "", xType, true);
    }
    for (int i = 0; i < opt.ncvars2dxz.size(); i++) {
        rc = vdc.DefineDataVar(opt.ncvars2dxz[i], dimnames2dxz, dimnames2dxz, "", xType, false);
    }
    ...
    for (int i = 0; i < opt.vars2dyz.size(); i++) {
        rc = vdc.DefineDataVar(opt.vars2dyz[i], dimnames2dyz, dimnames2dyz, "", xType, true);
    }
    for (int i = 0; i < opt.ncvars2dyz.size(); i++) {
        rc = vdc.DefineDataVar(opt.ncvars2dyz[i], dimnames2dyz, dimnames2dyz, "", xType, false);
    }
