...
void SettingsEventRouter::_chooseAutoSaveFile() {
    ...
    QString fileName =
        QFileDialog::getSaveFileName(_autoSaveFileButton, tr("Select auso-save VAPOR session file"),
                                     QString::fromStdString(sParams->GetAutoSaveSessionFile()),
                                     tr("Vapor 3 Session Files (*.vs3)"));

    if (!fileName.isEmpty() && FileOperationChecker::FileGoodToWrite(fileName)) {
...
void SettingsEventRouter::_setSessionPath() {
    ...
    _setFilePath(&SettingsParams::SetSessionDir, &SettingsParams::GetSessionDir, *sParams,
                 _sessionPathEdit);
}
...
void SettingsEventRouter::_setMetadataPath() {
    ...
    _setFilePath(&SettingsParams::SetMetadataDir, &SettingsParams::GetMetadataDir, *sParams,
                 _metadataPathEdit);
}
...
void SettingsEventRouter::_setImagePath() {
    ...
    _setFilePath(&SettingsParams::SetImageDir, &SettingsParams::GetImageDir, *sParams,
                 _imagePathEdit);
}
...
void SettingsEventRouter::_setPythonPath() {
    ...
    _setFilePath(&SettingsParams::SetPythonDir, &SettingsParams::GetPythonDir, *sParams,
                 _pythonPathEdit);
}
...
void SettingsEventRouter::_setFilePath(void (SettingsParams::*setFunc)(string),
                                       string (SettingsParams::*getFunc)() const,
                                       SettingsParams &sParams, QLineEdit *lineEdit) {
...
void SettingsEventRouter::_chooseTFPath() {
    ...
    string dir =
        _choosePathHelper(sParams->GetTFDir(), "Choose the Transfer Function file directory");
...
void SettingsEventRouter::_choosePythonPath() {
    ...
    string dir =
        _choosePathHelper(sParams->GetPythonDir(), "Choose the Python script file directory");
