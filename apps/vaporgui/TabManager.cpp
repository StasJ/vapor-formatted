...
RenderEventRouter *TabManager::GetRenderEventRouter(string winName, string renderType,
...
void TabManager::_createAllDefaultTabs() {
    ...
    connect((AnimationEventRouter *)er, SIGNAL(AnimationOnOffSignal(bool)), this,
            SLOT(_setAnimationOnOff(bool)));
    connect((AnimationEventRouter *)er, SIGNAL(AnimationDrawSignal()), this,
            SLOT(_setAnimationDraw()));
    ...
    connect((NavigationEventRouter *)er, SIGNAL(Proj4StringChanged(string)), this,
            SLOT(_setProj4String(string)));
    connect(this, SIGNAL(HomeViewpointSignal()), (NavigationEventRouter *)er,
            SLOT(UseHomeViewpoint()));
    connect(this, SIGNAL(ViewAllSignal()), (NavigationEventRouter *)er, SLOT(ViewAll()));
    connect(this, SIGNAL(SetHomeViewpointSignal()), (NavigationEventRouter *)er,
            SLOT(SetHomeViewpoint()));
    connect(this, SIGNAL(AlignViewSignal(int)), (NavigationEventRouter *)er, SLOT(AlignView(int)));
    connect(this, SIGNAL(CenterSubRegionSignal()), (NavigationEventRouter *)er,
            SLOT(CenterSubRegion()));
    ...
    for (int i = 0; i < renderNames.size(); i++) {
        RenderEventRouter *er = RenderEventRouterFactory::Instance()->CreateInstance(
            renderNames[i], parent, _controlExec);
...
void TabManager::_registerEventRouter(const std::string subTabName, EventRouter *router) {
...
void TabManager::_installWidgets() {
    ...
    for (int i = 0; i < _subTabWidgets[_renderersTabName].size(); i++) {
        ...
        RenderEventRouter *re =
            dynamic_cast<RenderEventRouter *>(_subTabWidgets[_renderersTabName][i]);
    ...
    for (int j = 0; j < _tabNames.size(); j++) {
        ...
        for (int subidx = 0; subidx < _subTabWidgets[tab].size(); subidx++) {
            EventRouter *ev = dynamic_cast<EventRouter *>(_subTabWidgets[tab][subidx]);
        }
    }
...
void TabManager::_initRenderHolder() {
    ...
    _renderHolder = new RenderHolder(this, _controlExec, widgets, widgetNames, descriptions,
