...
TabManager::TabManager(QWidget *parent, ControlExec *ce) : QTabWidget(parent) {
    ...
    connect(_renderHolder, SIGNAL(newRenderer(string, string, string)), this,
...
void TabManager::Update() { _renderHolder->Update(); }
...
void TabManager::newFrontTab(int topTab, int newSubPosn) {
    ...
    EventRouter *eRouter =
        VizWinMgr::getInstance()->GetEventRouter(_widgetTags[topTab][newSubPosn]);
    eRouter->updateTab();
...
void TabManager::setActive(string activeViz, string renderClass, string renderInst) {
    ...
    RenderEventRouter *eRouter =
        VizWinMgr::getInstance()->GetRenderEventRouter(activeViz, renderClass, renderInst);
