...
//! a local AppSettingsParams for each window, that users can select whenever there are multiple
...
class AppSettingsParams : public VAPoR::ParamsBase {
  ...
  public:
    ...
    void SetCurrentAutosaveEnabled(bool onOff) {
        SetValueLong(_currentAutosaveEnabledTag, "Toggle current autosave enabled", (long)onOff);
    }
    ...
    void SetCurrentShowWarning(bool onOff) {
        SetValueLong(_currentShowDataWarningTag, "toggle current show warning", (long)onOff);
    }
    ...
    void SetCurrentUseLessAccurate(bool onOff) {
        SetValueLong(_currentUseLessAccurateTag, "toggle current use less accuracy", (long)onOff);
    }
    ...
    string GetCurrentAutosaveName() const {
        return GetValueString(_currentAutosaveFileNameTag, string("/tmp/vapor_autosave.vs3"));
    }
