...
float TFWidget::getOpacity() {
    bool mainTF = true;
    if (_flags & COLORMAP_VAR_IS_IN_TF2 && !_rParams->UseSingleColor()) {
        mainTF = false;
    }
    string varName = getTFVariableName(mainTF);
...
void TFWidget::updateSecondaryMappingFrame() {
    ...
    bool histogramRecalculated =
        _secondaryMappingFrame->Update(_dataMgr, _paramsMgr, _rParams, buttonPress);
...
void TFWidget::connectWidgets() {
    connect(_rangeCombo, SIGNAL(valueChanged(double, double)), this,
            SLOT(setRange(double, double)));
    connect(_updateMainHistoButton, SIGNAL(pressed()), this, SLOT(updateMainMappingFrame()));
    connect(_autoUpdateMainHistoCheckbox, SIGNAL(stateChanged(int)), this,
            SLOT(autoUpdateMainHistoChecked(int)));
    ...
    connect(_autoUpdateSecondaryHistoCheckbox, SIGNAL(stateChanged(int)), this,
            SLOT(autoUpdateSecondaryHistoChecked(int)));
    connect(_secondaryVarInterpCombo, SIGNAL(activated(int)), this,
            SLOT(setColorInterpolation(int)));
    connect(_secondaryWhitespaceCheckbox, SIGNAL(stateChanged(int)), this,
            SLOT(setUseWhitespace(int)));
    connect(_secondaryMinSliderEdit, SIGNAL(valueChanged(double)), this,
            SLOT(setSecondaryMinRange(double)));
    connect(_secondaryMaxSliderEdit, SIGNAL(valueChanged(double)), this,
            SLOT(setSecondaryMaxRange(double)));
...
void TFWidget::opacitySliderChanged(int value) {
    ...
    bool mainTF = true;
    if (COLORMAP_VAR_IS_IN_TF2 && !_rParams->UseSingleColor()) {
        mainTF = false;
    }
    string varName = getTFVariableName(mainTF);
    MapperFunction *tf = _rParams->GetMapperFunc(varName);
...
LoadTFDialog::LoadTFDialog(QWidget *parent)
    : QDialog(parent), _fileDialog(nullptr), _checkboxFrame(nullptr), _fileDialogFrame(nullptr),
      ...
      _loadDataBoundsCheckbox(nullptr), _buttonGroup(nullptr), _myDir(""), _loadOpacityMap(false),
      _loadDataBounds(false), _selectedFile("") {
    setModal(true);
...
void LoadTFDialog::connectWidgets() {
    ...
    connect(_fileDialog, SIGNAL(currentChanged(const QString &)), this,
            SLOT(checkSelectedColorButton(const QString &)));
}
