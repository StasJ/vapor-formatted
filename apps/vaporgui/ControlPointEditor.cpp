...
//----------------------------------------------------------------------------
ControlPointEditor::ControlPointEditor(MappingFrame *parent, OpacityMap *map, int cp)
    : QDialog(parent), Ui_ControlPointEditorBase(), _controlPoint(cp),
      _mapper(parent->mapperFunction()), _omap(map), _cmap(NULL) {
    setupUi(this);
    initWidgets();
    initConnections();
}
...
//----------------------------------------------------------------------------
ControlPointEditor::ControlPointEditor(MappingFrame *parent, ColorMap *map, int cp)
    : QDialog(parent), Ui_ControlPointEditorBase(), _controlPoint(cp),
      _mapper(parent->mapperFunction()), _omap(NULL), _cmap(map) {
    setupUi(this);
    initWidgets();
    initConnections();
}
...
void ControlPointEditor::initWidgets() {
    ...
    if (_omap) {
        _opacityField->setText(QString("%1").arg(_omap->controlPointOpacity(_controlPoint)));
    ...
    if (_cmap) {
        ...
        _tempColor.setHsv((int)(359 * color.hue()), (int)(255 * color.sat()),
                          (int)(255 * color.val()));
...
void ControlPointEditor::initConnections() {
    connect(_dataValueField, SIGNAL(returnPressed()), this, SLOT(dataValueChanged()));

    connect(_quantValueField, SIGNAL(returnPressed()), this, SLOT(indexValueChanged()));

    connect(_colorButton, SIGNAL(clicked()), this, SLOT(pickColor()));

    connect(_okButton, SIGNAL(clicked()), this, SLOT(okHandler()));

    connect(_cancelButton, SIGNAL(clicked()), this, SLOT(cancelHandler()));
}
...
//----------------------------------------------------------------------------
void ControlPointEditor::cancelHandler() { close(); }

//----------------------------------------------------------------------------
// Returns the control point's data value.
//----------------------------------------------------------------------------
float ControlPointEditor::dataValue() {
    return _omap ? _omap->controlPointValue(_controlPoint)
...
float ControlPointEditor::toData(int index) {
    if (_mapper) {
        return _mapper->mapIndexToFloat(index);
    }
