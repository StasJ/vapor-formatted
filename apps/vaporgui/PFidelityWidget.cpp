...
PQuickFidelitySelector::PQuickFidelitySelector()
    : PLineItem("", "Fidelity", _vComboBox = new VComboBox({"Medium"})) {
    connect(_vComboBox, &VComboBox::ValueChanged, this,
            &PQuickFidelitySelector::dropdownTextChanged);
}
...
PLODSelector::PLODSelector()
    : PLineItem("", "Level of Detail", _vComboBox = new VComboBox({"0 (1000:1)"})) {
...
void PLODSelector::updateGUI() const {
    ...
    for (int i = 0; i < cr.size(); i++)
        items.push_back(to_string(i) + " (" + to_string(cr[i]) + ":1)");
...
PRefinementSelector::PRefinementSelector()
    : PLineItem("", "Refinement Level", _vComboBox = new VComboBox({"0 (100x100x100)"})) {
