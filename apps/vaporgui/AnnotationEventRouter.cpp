...
void AnnotationEventRouter::connectAnnotationWidgets() {
    connect(_axisAnnotationEnabledCheckbox, SIGNAL(toggled(bool)), this,
            SLOT(setAxisAnnotation(bool)));
    connect(_latLonAnnotationCheckbox, SIGNAL(toggled(bool)), this,
            SLOT(setLatLonAnnotation(bool)));
...
void AnnotationEventRouter::convertPCSToLonLat(double &xCoord, double &yCoord) {
    ...
    if (rc < 0) {
        char buff[100];
        sprintf(buff, "Could not convert point %f, %f to Lon/Lat", coordsForError[0],
                coordsForError[1]);
...
void AnnotationEventRouter::convertLonLatToPCS(double &xCoord, double &yCoord) {
    ...
    if (rc < 0) {
        char buff[100];
        sprintf(buff, "Could not convert point %f, %f to PCS", coordsForError[0],
                coordsForError[1]);
        MyBase::SetErrMsg(buff);
        MSG_ERR(buff);
    }
