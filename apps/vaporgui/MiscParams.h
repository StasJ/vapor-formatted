...
class MiscParams : public VAPoR::ParamsBase {
  ...
  public:
    MiscParams(ParamsBase::StateSave *ssave);

    MiscParams(VAPoR::ParamsBase::StateSave *ssave, VAPoR::XmlNode *node);
    ...
    //
    bool GetTimeStep() const { return (bool)GetValueLong(_timeStepTag, 0); }
    ...
    //! \retval bool indicating usage
    //
    bool GetTimeStamp() const { return (bool)GetValueLong(_timeStampTag, 0); }

    void SetTimeAnnotLLX(float llx) {
        SetValueLong(_timeAnnotLLXTag, "Set the lower-left coord of the time annotation",
                     (long)llx);
    }

    float GetTimeAnnotLLX() const { return (float)GetValueLong(_timeAnnotLLXTag, 10); }

    void SetTimeAnnotLLY(float lly) {
        SetValueLong(_timeAnnotLLYTag, "Set the lower-right coord of the time annotation",
                     (long)lly);
    }
    ...
    void SetTimeAnnotColor(const std::vector<double> &rgb) {
        ...
        SetValueDoubleVec(_timeAnnotColorTag,
                          "Specify time "
    ...
    void SetTimeAnnotBGColor(const std::vector<double> &rgb) {
        ...
        SetValueDoubleVec(_timeAnnotBGColorTag,
                          "Specify time "
                          "annotation color in RGB",
                          v);
    }
    ...
    //
    static string GetClassType() { return ("MiscParams"); }
