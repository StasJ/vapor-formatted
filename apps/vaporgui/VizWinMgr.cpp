...
void VizWinMgr::attachVisualizer(string vizName) {
    ...
    _vizWindow[vizName] =
        new VizWin(MainForm::getInstance(), qvizname, vizName, _controlExec, m_trackBall

        );

    connect(_vizWindow[vizName], SIGNAL(Closing(const string &)), this,
            SLOT(vizAboutToDisappear(const string &)));
    connect(_vizWindow[vizName], SIGNAL(HasFocus(const string &)), this,
            SLOT(setActiveViz(const string &)));
...
NavigationEventRouter *VizWinMgr::getViewpointRouter() {
    return ((NavigationEventRouter *)GetEventRouter(NavigationEventRouter::GetClassType()));
}
