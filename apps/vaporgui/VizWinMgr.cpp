...
VizWinMgr::VizWinMgr(ControlExec *ce) {
    ...
    connect(this, SIGNAL(activateViz(const QString &)), _tabManager,
...
void VizWinMgr::vizAboutToDisappear(string vizName) {
    ...
    //
    if (_vizWindow.size() == 1)
        emit enableMultiViz(false);
}

void VizWinMgr::attachVisualizer(string vizName) {

    if (_vizWindow.find(vizName) != _vizWindow.end())
        return;
    ...
    _vizWindow[vizName] = new VizWin(MainForm::getInstance(), qvizname, this, newRect, vizName,
...
int VizWinMgr::getNumVisualizers() { return _vizWindow.size(); }
...
void VizWinMgr::SetTrackBall(const double posvec[3], const double dirvec[3], const double upvec[3],
...
void VizWinMgr::alignView(int axis) {
    VizWin *vw = getActiveVizWin();
    if (!vw)
        return;
    if (axis < 1)
...
//
void VizWinMgr::resetViews(ViewpointParams *vp) {
    ...
    for (it = _vizWindow.begin(); it != _vizWindow.end(); it++) {
        int i = it->first;
        Params *vpParams =
            (ViewpointParams *)_paramsMgr->GetParamsInstance(Params::_viewpointParamsTag, i, -1);
...
//
void VizWinMgr::setAnimationLocalGlobal(int val) {
#ifdef DEAD
    ...
    if (val == 0) { // toGlobal.
        ...
        AnimationParams *ap = (AnimationParams *)_paramsMgr->GetParamsInstance(
            Params::_animationParamsTag, activeViz, -1);
        assert(ap);
 ...
 ******************************************************************************/
void VizWinMgr::setVpLocalGlobal(int val) {
#ifdef DEAD
    int activeViz = _controlExec->GetActiveVizIndex();
    // If changes  to global, revert to global panel.
    // If changes to local, may need to create a new local panel
    ...
    if (val == 0) { // toGlobal.
        ...
        // Note that updateDialog will trigger events changing values
        // on the current dialog
        ViewpointParams *vp = (ViewpointParams *)_paramsMgr->GetParamsInstance(
            Params::_viewpointParamsTag, activeViz, -1);
        assert(vp);
        ...
        ViewpointParams *vpParams = (ViewpointParams *)_paramsMgr->GetParamsInstance(
            Params::_viewpointParamsTag, activeViz, -1);
        if (!vpParams) {
            // create a new parameter panel, copied from global
            ViewpointParams *vp =
                (ViewpointParams *)_paramsMgr->GetDefaultParams(Params::_viewpointParamsTag);
            assert(vp);
 ...
 ******************************************************************************/
void VizWinMgr::setRgLocalGlobal(int val) {
#ifdef DEAD
    int activeViz = _controlExec->GetActiveVizIndex();
    // If changes  to global, just revert to global panel.
    // If changes to local, may need to create a new local panel
    ...
    if (val == 0) { // toGlobal.
        ...
        // on the current dialog
        RegionParams *rp =
            (RegionParams *)_paramsMgr->GetParamsInstance(Params::_regionParamsTag, activeViz, -1);
        assert(rp);
...
RenderEventRouter *VizWinMgr::GetRenderEventRouter(string winName, string renderType,
...
void VizWinMgr::installTab(const std::string tag, int tabType, EventRouter *eRouter) {
...
AnimationEventRouter *VizWinMgr::getAnimationRouter() {
    return ((AnimationEventRouter *)GetEventRouter(AnimationEventRouter::GetClassType()));
}

ViewpointEventRouter *VizWinMgr::getViewpointRouter() {
    return ((ViewpointEventRouter *)GetEventRouter(ViewpointEventRouter::GetClassType()));
}

RegionEventRouter *VizWinMgr::getRegionRouter() {
    return ((RegionEventRouter *)GetEventRouter(RegionEventRouter::GetClassType()));
}
