...
TFEditorVolume::TFEditorVolume() : TFEditor() {
    layout()->addWidget(_colormappedVariableCheckbox =
                            new ParamsWidgetCheckbox(VAPoR::VolumeParams::UseColormapVariableTag,
    ...
    layout()->addWidget(range2 =
                            new TFMappingRangeSelector(RenderParams::_colorMapVariableNameTag));
    connect(range2, SIGNAL(ValueChangedIntermediate(float, float)), _histogramMap2, SLOT(update()));
}

void TFEditorVolume::Update(VAPoR::DataMgr *dataMgr, VAPoR::ParamsMgr *paramsMgr,
    ...
    bool useColormappedVar =
        rParams->GetValueLong(VAPoR::VolumeParams::UseColormapVariableTag, false);
