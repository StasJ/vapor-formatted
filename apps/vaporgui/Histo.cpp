...
void Histo::reset(int newNumBins) {
    ...
    for (int i = 0; i < _numBins; i++)
        _binArray[i] = 0;
    if (_below)
        memset(_below, 0, _nBinsBelow * sizeof(*_below));
    if (_above)
        memset(_above, 0, _nBinsAbove * sizeof(*_above));
    _numSamplesBelow = 0;
    _numSamplesAbove = 0;
    _maxBinSize = -1;
    _populated = false;
...
int Histo::Populate(const std::string &varName, VAPoR::DataMgr *dm, VAPoR::RenderParams *rp) {
    ...
    if (_maxMapData - _minMapData > __FLT_EPSILON__) {
        _nBinsBelow =
            std::min(10000.0f, _numBins / (_maxMapData - _minMapData) * (_minMapData - _minData));
        _nBinsAbove =
            std::min(10000.0f, _numBins / (_maxMapData - _minMapData) * (_maxData - _maxMapData));
    } else {
        _nBinsAbove = 0;
        _nBinsBelow = 0;
    }
    ...
    Grid *grid;
    int rc =
        DataMgrUtils::GetGrids(dm, ts, varName, minExts, maxExts, true, &refLevel, &lod, &grid);
...
bool Histo::NeedsUpdate(const std::string &varName, VAPoR::DataMgr *dm, VAPoR::RenderParams *rp) {
    ...
    if (_minMapData != mf->getMinMapValue())
        return true;
    if (_maxMapData != mf->getMaxMapValue())
        return true;
    if (_refLevel != rp->GetRefinementLevel())
        return true;
    if (_lod != rp->GetCompressionLevel())
        return true;
    if (_varnameOfUpdate != varName)
        return true;
    if (_timestepOfUpdate != rp->GetCurrentTimestep())
        return true;
    if (_minExts != minExts)
        return true;
    if (_maxExts != maxExts)
...
int Histo::PopulateIfNeeded(const std::string &varName, VAPoR::DataMgr *dm,
...
int Histo::calculateStride(const std::string &varName, VAPoR::DataMgr *dm,
                           const VAPoR::RenderParams *rp) const {
    return dm->GetDefaultMetaInfoStride(varName, rp->GetRefinementLevel());
}

bool Histo::shouldUseSampling(const std::string &varName, VAPoR::DataMgr *dm,
                              const VAPoR::RenderParams *rp) const {
...
void Histo::_getDataRange(const std::string &varName, VAPoR::DataMgr *d, VAPoR::RenderParams *r,
    ...
    d->GetDataRange(r->GetCurrentTimestep(), varName, r->GetRefinementLevel(),
                    ...
                    d->GetDefaultMetaInfoStride(varName, r->GetRefinementLevel()), range);
    *min = range[0];
    *max = range[1];
}
