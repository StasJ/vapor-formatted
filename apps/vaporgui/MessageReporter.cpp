...
MessageReporter::MessageReporter() {
    ...
    AppSettingsParams *aParams =
        (AppSettingsParams *)_paramsMgr->GetParams(AppSettingsParams::GetClassType());
...
bool MessageReporter::doPopup(messagePriority t, const char *message) {
    ...
    QMessageBox *msgBox =
        new QMessageBox(msgIcon, title, message, QMessageBox::Ok, MainForm::getInstance());

    QPushButton *silenceButton = msgBox->addButton("Silence This Message", QMessageBox::AcceptRole);
    silenceButton->setToolTip(
        "Click to prevent the message(s) from being displayed again.\nMessages can be un-silenced "
