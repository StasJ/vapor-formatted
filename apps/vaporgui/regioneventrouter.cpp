...
RegionEventRouter::RegionEventRouter(QWidget *parent, ControlExec *ce)
    : QWidget(parent), Ui_RegionTab(), EventRouter(ce, RegionParams::GetClassType()) {
...
void RegionEventRouter::GetWebHelp(vector<pair<string, string>> &help) const {
    ...
    help.push_back(
        make_pair("Overview of the Region tab",
                  "http://www.vapor.ucar.edu/docs/vapor-gui-help/region-tab#RegionOverview"));

    help.push_back(
        make_pair("Controlling the region extents",
...
void RegionEventRouter::_updateTab() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        float maxcrd = mincrd;
        for (int j = 0; j < 8; j++) {
            if (mincrd > corners[j][i])
                mincrd = corners[j][i];
            if (maxcrd < corners[j][i])
                maxcrd = corners[j][i];
        }
        if (mincrd < 0.)
            mincrd = 0.;
        if (maxcrd > fullSizes[i])
            maxcrd = fullSizes[i];
        dboxmin[i] = mincrd;
        dboxmax[i] = maxcrd;
    }
...
void RegionEventRouter::loadRegionExtents() {
    ...
    QString filename = QFileDialog::getOpenFileName(
        this, "Specify file name for loading list of time-varying Region extents", ".",
        "Text files (*.txt)");
    ...
    while (1) {
        int ts;
        float exts[6];
        int numVals = fscanf(regionFile, "%d %g %g %g %g %g %g", &ts, exts, exts + 1, exts + 2,
                             exts + 3, exts + 4, exts + 5);
...
void RegionEventRouter::saveRegionExtents() {
    ...
    QString filename = QFileDialog::getSaveFileName(
        this, "Specify file name for saving list of current time-varying Local Region extents", ".",
        "Text files (*.txt)");
    // Check that user did specify a file:
    if (filename.isNull()) {
        return;
    ...
    for (int i = 1; i < times.size(); i++) {
        ...
        float exts[6];
        for (int j = 0; j < 6; j++)
            exts[j] = extents[6 * i + j];
        int rc = fprintf(regionFile, "%d %g %g %g %g %g %g\n", timestep, exts[0], exts[1], exts[2],
                         exts[3], exts[4], exts[5]);
        if (rc < 7) {
...
void RegionEventRouter::setCenter(const double newCenter[3]) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (maxSize > abs(domainExtents[i + 3] - newCenter[i]))
            maxSize = abs(domainExtents[i + 3] - newCenter[i]);
        if (newSize[i] > maxSize)
            newSize[i] = maxSize;
