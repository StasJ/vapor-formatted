...
SliceVariablesSubtab::SliceVariablesSubtab(QWidget *parent) {
    ...
    connect(_vs, SIGNAL(ValueChangedIntermediate(double)), this,
            SLOT(_vsChangedIntermediate(double)));
    ...
    connect(_vse, SIGNAL(ValueChangedIntermediate(double)), this,
            SLOT(_vseChangedIntermediate(double)));
    _vsei = new VSliderEdit(-10.0, 1500.0, 5.0);
    _vsei->SetIntType(true);
    layout()->addWidget(new VLineItem("IntSliderEdit", _vsei));
    ;
    connect(_vsei, SIGNAL(ValueChanged(double)), this, SLOT(_vseChanged(double)));
    connect(_vse, SIGNAL(ValueChangedIntermediate(double)), this,
            SLOT(_vseChangedIntermediate(double)));

    _pb = new VPushButton("myButton");
    layout()->addWidget(new VLineItem(" buuuutton", _pb));
    connect(_pb, SIGNAL(ButtonClicked()), this, SLOT(_bChanged()));
...
SliceGeometrySubtab::SliceGeometrySubtab(QWidget *parent) {
    ...
    connect(_geometryWidget->_planeComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(_orientationChanged(int)));
...
void SliceAnnotationSubtab::Update(VAPoR::ParamsMgr *paramsMgr, VAPoR::DataMgr *dataMgr,
                                   VAPoR::RenderParams *rParams) {
