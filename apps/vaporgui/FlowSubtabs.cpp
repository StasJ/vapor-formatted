...
FlowSeedingSubtab::FlowSeedingSubtab(QWidget *parent) : QVaporSubtab(parent) {
    ...
    connect(_geometryWriter, SIGNAL(ValueChanged(const std::string &)), this,
            SLOT(_geometryWriterFileChanged(const std::string &)));
...
void FlowSeedingSubtab::_createIntegrationSection() {
    ...
    connect(_pathlineSamplesSliderEdit, SIGNAL(ValueChanged(int)), this,
            SLOT(_pathlineSamplesChanged(int)));
    ...
    _streamlineFrame->addWidget(
        new VLineItem("Injection interval", _streamlineInjIntervalSliderEdit));
...
void FlowSeedingSubtab::_pathlineSamplesChanged(int newval) {
