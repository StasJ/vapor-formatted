...
FlowSeedingSubtab::FlowSeedingSubtab(QWidget *parent) : QVaporSubtab(parent) {
    ...
    connect(_steadyDirectionCombo, SIGNAL(ValueChanged(std::string)), this,
            SLOT(_flowDirectionChanged(std::string)));

    _steadyLengthSliderEdit = new VSliderEdit();
    _steadyLengthSliderEdit->SetIntType(true);
    connect(_steadyLengthSliderEdit, SIGNAL(ValueChanged(double)), this,
            SLOT(_steadyLengthChanged(int)));
    _integrationSection->AddWidget(new VLineItem("Pathline length", _steadyLengthSliderEdit));

    // Unsteady flow options
    //
    _unsteadyLengthSliderEdit = new VSliderEdit();
    _unsteadyLengthSliderEdit->SetIntType(true);
    _integrationSection->AddWidget(new VLineItem("Streamline length", _unsteadyLengthSliderEdit));
    connect(_unsteadyLengthSliderEdit, SIGNAL(ValueChanged(double)), this,
            SLOT(_unsteadyLengthChanged(int)));

    _injIntervalSliderEdit = new VSliderEdit();
    _injIntervalSliderEdit->SetIntType(true);
    _integrationSection->AddWidget(new VLineItem("Seeding interval", _injIntervalSliderEdit));
    connect(_injIntervalSliderEdit, SIGNAL(ValueChanged(double)), this,
...
void FlowSeedingSubtab::_unsteadyLengthChanged(int newVal) {
...
void FlowSeedingSubtab::_steadyLengthChanged(int newval) {
    int oldval;
