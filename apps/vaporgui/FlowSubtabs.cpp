...
FlowVariablesSubtab::FlowVariablesSubtab(QWidget *parent) : QVaporSubtab(parent) {
    ...
    /*    _velocityMltp = new VLineEdit( this, "Field Scale Factor", "1.0" );
        ...
        connect( _velocityMltp,     SIGNAL( _editingFinished() ), this, SLOT(
       _velocityMultiplierChanged() ) ); connect( _periodicX,        SIGNAL( _checkboxClicked() ),
       this, SLOT( _periodicClicked() ) ); connect( _periodicY,        SIGNAL( _checkboxClicked() ),
...
void FlowSeedingSubtab::_createSeedingSection() {
    ...
    connect(_seedTypeCombo, SIGNAL(ValueChanged(std::string)), this,
            SLOT(_configureSeedType(std::string)));
    ...
    connect(_xSeedSliderEdit, SIGNAL(ValueChanged(double)), this, SLOT(_rakeNumOfSeedsChanged()));
    ...
    connect(_ySeedSliderEdit, SIGNAL(ValueChanged(double)), this, SLOT(_rakeNumOfSeedsChanged()));
...
void FlowSeedingSubtab::_createIntegrationSection() {
    ...
    connect(_flowTypeCombo, SIGNAL(ValueChanged(std::string)), this,
            SLOT(_configureFlowType(std::string)));
    ...
    connect(_pathlineDirectionCombo, SIGNAL(ValueChanged(std::string)), this,
            SLOT(_flowDirectionChanged(std::string)));

    _pathlineLengthSliderEdit = new VSliderEdit();
    _pathlineLengthSliderEdit->SetIntType(true);
    connect(_pathlineLengthSliderEdit, SIGNAL(ValueChanged(double)), this,
            SLOT(_pathlineLengthChanged(int)));
    _integrationSection->AddWidget(new VLineItem("Pathline length", _pathlineLengthSliderEdit));

    // Unsteady flow options
    //
    _streamlineLengthSliderEdit = new VSliderEdit();
    _streamlineLengthSliderEdit->SetIntType(true);
    _integrationSection->AddWidget(new VLineItem("Streamline length", _streamlineLengthSliderEdit));
    connect(_streamlineLengthSliderEdit, SIGNAL(ValueChanged(double)), this,
            SLOT(_streamlineLengthChanged(int)));
    ...
    connect(_streamlineInjIntervalSliderEdit, SIGNAL(ValueChanged(double)), this,
            SLOT(_seedInjIntervalChanged(int)));
    ...
    connect(_periodicXCheckBox, SIGNAL(ValueChanged(bool)), this, SLOT(_periodicClicked()));
    _integrationSection->AddWidget(new VLineItem("X axis periodicity", _periodicXCheckBox));
    _periodicYCheckBox = new VCheckBox();
    connect(_periodicYCheckBox, SIGNAL(ValueChanged(bool)), this, SLOT(_periodicClicked()));
    _integrationSection->AddWidget(new VLineItem("Y axis periodicity", _periodicYCheckBox));
    _periodicZCheckBox = new VCheckBox();
    connect(_periodicZCheckBox, SIGNAL(ValueChanged(bool)), this, SLOT(_periodicClicked()));
    _integrationSection->AddWidget(new VLineItem("Z axis periodicity", _periodicZCheckBox));

    _velocityMultiplierLineEdit = new VLineEdit();
    _velocityMultiplierLineEdit->SetIsDouble(true);
    connect(_velocityMultiplierLineEdit, SIGNAL(ValueChanged(const std::string &)), this,
            SLOT(_velocityMultiplierChanged(const std::string &)));
    _integrationSection->AddWidget(
...
/*    _steady = new VCheckBox( this, "Use Steady Flow" );
    ...
    connect( _pastNumOfTimeSteps,SIGNAL(_valueChanged(int) ), this, SLOT(
_pastNumOfTimeStepsChanged(int) )); _seedInjInterval = new VIntSlider( this, "Seed Injection
Interval", 0, 1 ); _layout->addWidget( _seedInjInterval );

    connect( _seedInjInterval,  SIGNAL(_valueChanged(int)  ), this, SLOT(
_seedInjIntervalChanged(int) ));
    ...
    connect( _rakeBiasVariable, SIGNAL( _indexChanged(int) ), this, SLOT(
_rakeBiasVariableChanged(int) ) ); connect( _rakeBiasStrength, SIGNAL( _valueChanged() ),    this,
SLOT( _rakeBiasStrengthChanged()    ) );
...
void FlowSeedingSubtab::Update(VAPoR::DataMgr *dataMgr, VAPoR::ParamsMgr *paramsMgr,
    ...
    cout << "getting seeds " << seedVec[X] << " " << seedVec[Y] << " " << seedVec[Z] << " "
         << seedVec[RANDOM_INDEX] << endl;
...
void FlowSeedingSubtab::_velocityMultiplierChanged(const std::string &value) {
    ...
    if (newval >= 0.001 && newval <= 1000.0) // in the valid range
    {
        ...
        _velocityMultiplierLineEdit->SetValue(
            std::to_string(newval)); // QString::number( newval, 'f', 3 ) );
...
void FlowSeedingSubtab::_streamlineLengthChanged(int newVal) {
...
void FlowSeedingSubtab::_configureFlowType(const std::string &value) {
    ...
    if (verbose)
        std::cout << "Flow combo changed to " << value << endl;

    if (_params != nullptr) {
        _params->SetIsSteady((long)isSteady);
        if (verbose)
            std::cout << "Flow params changed to " << _params->GetIsSteady() << endl;
    }
...
void FlowSeedingSubtab::_rakeNumOfSeedsChanged() {
    ...
    std::cout << "_rakeNumOfSeedsChanged from " << seedsVector[0] << " " << seedsVector[1] << " "
              << seedsVector[2] << " " << seedsVector[3] << std::endl;
    _params->SetRakeNumOfSeeds(seedsVector);
    std::cout << "_rakeNumOfSeedsChanged to   " << _params->GetRakeNumOfSeeds()[0] << " "
              << _params->GetRakeNumOfSeeds()[1] << " " << _params->GetRakeNumOfSeeds()[2] << " "
              << _params->GetRakeNumOfSeeds()[3] << std::endl;
}
...
void FlowSeedingSubtab::_flowDirectionChanged() {
