...
void Plot::savePlotToFile() {
    ...
    QString f =
        fd->getSaveFileName(this, "Select Output File Name", defaultOpenLocation, tr("*.png"));
...
//
int Plot::findNyquist(const size_t minv[3], const size_t maxv[3], const double minu[3],
                      const double maxu[3], double &dX, double &dY, double &dZ) const {
...
int Plot::getTemporalVectors(const vector<string> vars, map<string, vector<float>> &data,
    ...
    for (int i = 0; i < vars.size(); i++) {
        ...
        for (size_t ts = ts0; ts <= ts1; ts++) {
            ...
            StructuredGrid *rg =
                _dm->GetVariable(ts, var, _refLevel, _cRatio, uPoint, uPoint, false);
            if (!rg) {
                // N.B. In VAPOR 2.x libvdc functions post their own
                // error popups!
                //
...
void Plot::initExtents(int ts) { _extents = _dm->GetExtents(0); }
