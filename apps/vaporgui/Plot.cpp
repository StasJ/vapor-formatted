...
Plot::Plot(QWidget *parent) : QDialog(parent), Ui_PlotWindow() {
    ...
    for (int i = 0; i < 4; i++) {
        connect(_spaceCheckBoxes[i], SIGNAL(stateChanged(int)), this,
                SLOT(constCheckboxChanged(int)));
        if (i < 3) {
            connect(_timeCheckBoxes[i], SIGNAL(stateChanged(int)), this,
                    SLOT(constCheckboxChanged(int)));
        }
    }
}
...
void Plot::savePlotToFile() {
    ...
    string file =
        fd->getSaveFileName(this, "Select Output File Name", "/", tr("(*.png)")).toStdString();
    ;
    delete fd;
...
//
int Plot::findNyquist(const vector<size_t> minv, const vector<size_t> maxv,
                      const vector<double> minu, const vector<double> maxu, double &dX, double &dY,
                      double &dZ) const {
...
void Plot::go() {
    ...
    PyObject *pFunc = MyPython::CreatePyFunc("plotModule", "plot1D", plotScript);
    if (!pFunc) {
        errReport(pyErr());
        return;
    }
...
//
void Plot::getSpatialExtents(vector<double> &minu, vector<double> &maxu, size_t &ts) const {
...
//
int Plot::getSpatialVectors(const vector<string> vars, map<string, vector<float>> &data,
...
//
void Plot::getTemporalExtents(vector<double> &xyz, size_t &ts0, size_t &ts1) const {
...
int Plot::getTemporalVectors(const vector<string> vars, map<string, vector<float>> &data,
                             map<string, vector<float>> &iData) const {
...
PyObject *Plot::buildNumpyArray(const vector<float> &vec) const {
    ...
    //
    NpyIter *iter =
        NpyIter_New((PyArrayObject *)myArray, NPY_ITER_READWRITE, NPY_CORDER, NPY_NO_CASTING, NULL);
    if (!iter)
        return (NULL);
...
int Plot::initExtents(int ts, vector<double> &extents) {
    ...
    int rc = _dm->GetVariableExtents(ts, _defaultVar, _refLevel, minExtents, maxExtents);
    if (rc < 0) {
        string myErr;
