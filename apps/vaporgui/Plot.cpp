...
Plot::Plot(VAPoR::DataStatus *status, VAPoR::ParamsMgr *manager, QWidget *parent) {
    ...
    connect(spaceTabTimeSelector, SIGNAL(valueChanged(double)), this,
            SLOT(_spaceModeTimeChanged(double)));
    ...
    connect(numOfSamplesSelector, SIGNAL(valueChanged(double)), this,
            SLOT(_numberOfSamplesChanged(double)));
...
VAPoR::PlotParams *Plot::_getCurrentPlotParams() const {
    ...
    return (dynamic_cast<PlotParams *>(
