...
Plot::Plot(VAPoR::DataStatus *status, VAPoR::ParamsMgr *manager, QWidget *parent)
    ...
    timeTabTimeRange->SetMainLabel(
        QString::fromAscii("Select the minimum and maximum time steps:"));
    ...
    connect(spaceTabTimeSelector, SIGNAL(valueChanged(int)), this,
            SLOT(_spaceModeTimeChanged(int)));
            ...
            for (int i = 0; i < numOfSamples; i++) {
                ...
                if (i == 0)
                    ...
                    for (int j = 0; j < point1.size(); j++)
                        sample.push_back((double)i / (double)(numOfSamples - 1) * p1p2span[j] +
                                         point1[j]);
                }
...
void Plot::_invokePython(const QString &outFile, const std::vector<std::string> &enabledVars,
...
void Plot::_updateExtents() {
    ...
    VAPoR::DataMgrUtils::GetExtents(currentDmgr, TSToExamine[0], enabledVars, refinementLevel,
                                    compressLevel, min, max, axes);

    // TSToExamine[1] and TSToExamine[2] are evaluated only when not duplicate
    if (TSToExamine[1] != TSToExamine[0]) {
        VAPoR::DataMgrUtils::GetExtents(currentDmgr, TSToExamine[1], enabledVars, refinementLevel,
    ...
    if ((TSToExamine[2] != TSToExamine[1]) && (TSToExamine[2] != TSToExamine[0])) {
        VAPoR::DataMgrUtils::GetExtents(currentDmgr, TSToExamine[2], enabledVars, refinementLevel,
