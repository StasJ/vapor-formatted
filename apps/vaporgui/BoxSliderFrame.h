...
//!	This QFrame should be embedded in various EventRouter .ui files to support controlling box
//!extents.
...
//!
//!		In EventRouter::updateTab(), invoke setFullDomain(), setBoxExtents(), setNumRefinements() and
//!setVariableName()
//! based on the current state of the Box.
//!
//!		Whenever the box extents are changed in the GUI (outside the BoxSliderFrame), invoke
//!setBoxExtents().
//!
//!		Whenever the refinement level is changed in the GUI, invoke setNumRefinements()
//!
//!		Implement the slot EventRouter::changeExtents() and connect it to the signal
//!BoxSliderFrame::changeExtents().
