...
void VolumeIsoAppearanceSubtab::Update(VAPoR::DataMgr *dataMgr, VAPoR::ParamsMgr *paramsMgr,
    ...
    _samplingRateComboBox->setCurrentIndex(_samplingRateComboBox->findText(
        VolumeAppearanceSubtab::GetQStringForSamplingRate(_params->GetSamplingMultiplier())));
    _samplingRateComboBox->blockSignals(false);
    ...
    int rc = dataMgr->GetDataRange(params->GetCurrentTimestep(), params->GetVariableName(),
                                   params->GetRefinementLevel(), params->GetCompressionLevel(),
                                   minExt, maxExt, valueRanged);
