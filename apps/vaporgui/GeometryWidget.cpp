...
void GeometryWidget::updateRangeLabels(vector<double> minExt, vector<double> maxExt) {
    ...
    QString yTitle = QString("Y Coordinates         Min:") + QString::number(minExt[1]) +
...
void GeometryWidget::GetVectorExtents(size_t ts, int level, vector<double> minFullExt,
...
void GeometryWidget::updateRenTypeCombo() {
    ...
    for (int i = 0; i < copyRenTypeCombo->count(); i++) {
        ...
        if ((std::find(typeNames.begin(), typeNames.end(), typeName) == typeNames.end()) &&
            (typeName != "Type")) {
...
void GeometryWidget::updateRenNameCombo() {
    ...
    // if they weren't in the comboBox already
    //
    vector<string> renNames = _paramsMgr->GetRenderParamInstances(visName, typeName);
    ...
    for (int i = 0; i < copyRenNameCombo->count(); i++) {
        ...
        if ((std::find(renNames.begin(), renNames.end(), renName) == renNames.end()) &&
            (renName != "Name")) {
...
void GeometryWidget::connectWidgets() {
    connect(_xRangeCombo, SIGNAL(valueChanged(double, double)), this,
            SLOT(setRange(double, double)));
    connect(_yRangeCombo, SIGNAL(valueChanged(double, double)), this,
            SLOT(setRange(double, double)));
