...
void GeometryWidget::Reinit(DimFlags dimFlags, VariableFlags varFlags,
...
void GeometryWidget::updateRangeLabels(std::vector<double> minExt, std::vector<double> maxExt) {
    ...
    if (minExt.size() < 3) {
        ...
        if (_zFrame->isHidden() && _dimFlags & THREED)
            _zFrame->show();
...
bool GeometryWidget::getVectorExtents(std::vector<double> &minFullExts,
                                      std::vector<double> &maxFullExts) {
...
bool GeometryWidget::getVariableExtents(std::vector<double> &minFullExts,
                                        std::vector<double> &maxFullExts) {
...
void GeometryWidget::_resetBoxIfNeeded(const std::vector<double> &minFullExt,
    ...
    if (_varFlags & AUXILIARY) {
        if (_rParams->GetAuxVariableNames().empty())
            return;
    } else if (_varFlags & VECTOR) {
        if (_rParams->GetFieldVariableNames().empty())
            return;
    } else {
        if (_rParams->GetVariableName().empty())
            return;
    }
