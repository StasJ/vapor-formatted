...
VolumeAppearanceSubtab::VolumeAppearanceSubtab(QWidget *parent) {
    ...
    _pg->Add(
        (new PTFEditor(RenderParams::_variableNameTag, {PTFEditor::Default}, "Transfer Function"))
            ->ShowColormapBasedOnParam(VolumeParams::UseColormapVariableTag, false));

    PSection *rtp = new PSection("Volume Parameters");
    _pg->Add(rtp);
    rtp->Add(new PStringDropdownHLI<VolumeParams>(
        "Raytracing Algorithm", VolumeParams::GetAlgorithmNames(VolumeParams::Type::DVR),
        &VolumeParams::GetAlgorithm, &VolumeParams::SetAlgorithmByUser));
    ...
    gpu->Add(rtp);
    rtp->Add(new PEnumDropdown(VolumeParams::SamplingRateMultiplierTag,
                               {"1x", "2x", "4x", "8x", "16x"}, {1, 2, 4, 8, 16},
                               "Sampling Rate Multiplier"));
    rtp->Add((new PDoubleSliderEdit(VolumeParams::VolumeDensityTag, "Volume Density"))
                 ->EnableDynamicUpdate()
                 ->SetTooltip("Changes the overall density or 'opacity' of the volume allowing for "
                              "finer tuning of the transfer function."));
    rtp->Add(new PCheckbox(VolumeParams::UseColormapVariableTag, "Color by other variable"));
    rtp->Add((new PVariableSelector3D(RenderParams::_colorMapVariableNameTag))
                 ->ShowBasedOnParam(VolumeParams::UseColormapVariableTag));

    gpu->Add(
        (new PTFEditor(RenderParams::_colorMapVariableNameTag,
                       {PTFEditor::Histogram, PTFEditor::Colormap}, "Colormap Transfer Function"))
            ->ShowBasedOnParam(VolumeParams::UseColormapVariableTag));
    ...
    lp->Add(
        (new PDoubleSliderEdit(VolumeParams::PhongAmbientTag, "Ambient"))->EnableDynamicUpdate());
    lp->Add(
        (new PDoubleSliderEdit(VolumeParams::PhongDiffuseTag, "Diffuse"))->EnableDynamicUpdate());
    lp->Add(
        (new PDoubleSliderEdit(VolumeParams::PhongSpecularTag, "Specular"))->EnableDynamicUpdate());
    lp->Add((new PDoubleSliderEdit(VolumeParams::PhongShininessTag, "Specular"))
                ->SetRange(1, 100)
                ->EnableDynamicUpdate());
    ...
    op->Add((new PDoubleSliderEdit(VolumeParams::OSPDensity, "Density"))
                ->SetRange(0, 3)
                ->EnableDynamicUpdate());
    op->Add((new PIntegerSliderEdit("osp_spp", "Samples Per Pixel"))->SetRange(1, 10));
    op->Add((new PDoubleInput(VolumeParams::OSPSampleRateScalar, "Volume Sample Rate Scalar"))
                ->EnableBasedOnParam("osp_usePT", false));
    ...
    op->Add((new PCheckbox("osp_enable_clipping", "Enable Clipping Planes"))
                ->SetTooltip("Breaks depth buffer due to a bug in OSPRay"));
    op->Add((new PCheckbox("osp_useBackplate", "Backplate"))->EnableBasedOnParam("NULL"));
    op->Add((new PDoubleSliderEdit("osp_light_intensity", "Light Intensity"))
                ->SetRange(0, 10)
                ->EnableDynamicUpdate()
                ->EnableBasedOnParam("osp_usePT"));
