...
AppSettingsEventRouter::AppSettingsEventRouter(QWidget *parent, ControlExec *ce)
    : QWidget(parent), Ui_appSettingsTab(), EventRouter(ce, AppSettingsParams::GetClassType()) {
    setupUi(this);
...
void AppSettingsEventRouter::hookUpTab() {
    connect(jpegQualityEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setAppSettingsTextChanged(const QString &)));
    connect(jpegQualityEdit, SIGNAL(returnPressed()), this, SLOT(appSettingsReturnPressed()));
    connect(autosaveIntervalEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setAppSettingsTextChanged(const QString &)));
    connect(autosaveIntervalEdit, SIGNAL(returnPressed()), this, SLOT(appSettingsReturnPressed()));
    connect(saveButton, SIGNAL(clicked()), this, SLOT(save()));
    connect(logFilePathEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setAppSettingsTextChanged(const QString &)));
    connect(logFilePathEdit, SIGNAL(returnPressed()), this, SLOT(appSettingsReturnPressed()));
    connect(autosaveFilenameEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setAppSettingsTextChanged(const QString &)));
...
void AppSettingsEventRouter::GetWebHelp(vector<pair<string, string>> &help) const {
    help.clear();
...
void AppSettingsEventRouter::setAppSettingsTextChanged(const QString &) { SetTextChanged(true); }
...
void AppSettingsEventRouter::_confirmText() {
    ...
    if (logFilePathEdit->text().toStdString() != aParams->GetCurrentLogFileName()) {
        ...
        if (!rc && aParams->GetCurrentLogfileEnabled())
...
void AppSettingsEventRouter::appSettingsReturnPressed(void) { confirmText(); }
...
void AppSettingsEventRouter::chooseAutoSaveFilename() {
    ...
    QString s = QFileDialog::getSaveFileName(this, "Choose a filename for autosaved session files",
...
void AppSettingsEventRouter::chooseLogFilePath() {
    ...
    // Launch a file-chooser dialog
    AppSettingsParams *sParams = (AppSettingsParams *)GetActiveParams();
    QString s = QFileDialog::getSaveFileName(this, "Choose a filename for log messages",
                                             sParams->GetLogFileName().c_str(), "Text (*.txt)");
    if (s != "") {
        ...
        if (!rc && sParams->GetCurrentLogfileEnabled())
            MessageReporter::setDiagMsgCB(true);
        else
            MessageReporter::setDiagMsgCB(false);
    }
