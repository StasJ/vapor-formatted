...
VSlider::VSlider(QWidget *parent, const std::string &label, double min, double max, double value)
    : VLabel(parent, label), _min(min), _max(max), _value(value) {
    VAssert(_max > _min);
...
VSlider::VSlider(const std::string &label, double min, double max, double value)
    : VSlider(nullptr, label, min, max, value) {}
