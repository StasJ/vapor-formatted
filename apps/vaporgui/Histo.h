...
class Histo {
  ...
  private:
    ...
    int calculateStride(const std::string &varName, VAPoR::DataMgr *dm,
                        const VAPoR::RenderParams *rp) const;
    bool shouldUseSampling(const std::string &varName, VAPoR::DataMgr *dm,
                           const VAPoR::RenderParams *rp) const;
    void setProperties(float mnData, float mxData, string var, int ts);
    void calculateMaxBinSize();
    void _getDataRange(const std::string &varName, VAPoR::DataMgr *d, VAPoR::RenderParams *r,
