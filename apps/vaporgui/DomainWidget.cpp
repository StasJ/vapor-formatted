...
DomainWidget::DomainWidget(QWidget *parent, Qt::Orientation orientation, float min, float max)
    : GLWidget(parent), _minValue(min), _maxValue(max), _handleRadius(0.06),
      _quadHandle(gluNewQuadric()), _orientation(orientation) {
    _minY = 1.14;
    _maxY = 1.3;
}
        ...
        if (_enabled) {
            ...
            if (_orientation == Qt::Horizontal) {
                ...
                glPushMatrix();
                {
                    ...
                    glRotatef(90, 0, 1, 0);
                    gluCylinder(_quadHandle, 0.4 * _handleRadius, 0.4 * _handleRadius, width(), 10,
                                2);
