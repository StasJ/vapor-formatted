...
VGeometry::VGeometry(const std::vector<double> &range, const std::vector<std::string> &label)
...
void VGeometry::SetGeometry(const std::vector<double> &vals) {
    if (vals.size() != _dim * 2)
        return;
...
void VGeometry::GetGeometry(std::vector<double> &vals) const {
...
void VGeometry::_respondChanges() { emit valueChanged(); }
