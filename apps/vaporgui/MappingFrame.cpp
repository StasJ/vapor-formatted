...
MappingFrame::MappingFrame(QWidget *parent)
    : QGLWidget(parent), _NUM_BINS(256), _mapper(NULL), _histogram(NULL), _isSampling(false),
      _histoNeedsUpdate(false), _opacityMappingEnabled(false), _colorMappingEnabled(false),
      _isoSliderEnabled(false), _isolineSlidersEnabled(false), _lastSelectedIndex(-1),
      navigateButton(NULL), _editButton(NULL), _variableName(""),
...
void MappingFrame::CopyHistogram(ParamsMgr *paramsMgr, string variableName, Histo *histo) {
...
Histo *MappingFrame::GetHistogram() { return _histogram; }
...
void MappingFrame::getGridAndExtents(VAPoR::Grid **grid, std::vector<double> &minExts,
    ...
    int rc = DataMgrUtils::GetGrids(_dataMgr, ts, _variableName, minExts, maxExts, true, &refLevel,
                                    &lod, grid);
    if (rc < 0)
...
std::vector<double> MappingFrame::calculateDeltas(std::vector<double> minExts,
                                                  std::vector<double> maxExts) const {
...
//----------------------------------------------------------------------------
bool MappingFrame::Update(DataMgr *dataMgr, ParamsMgr *paramsMgr, RenderParams *rParams,
                          bool buttonPress) {
    ...
    if (_initialized == false || variableName != _variableName || buttonPress ||
