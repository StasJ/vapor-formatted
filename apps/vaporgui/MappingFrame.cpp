...
ControlExec *MappingFrame::_controlExec = 0;
//----------------------------------------------------------------------------
// Constructor
//----------------------------------------------------------------------------
MappingFrame::MappingFrame(QWidget *parent, const char *)
    : QGLWidget(parent), _NUM_BINS(256), _mapper(NULL), _histogram(NULL),
      ...
      _colorbarWidget(new ColorbarWidget(this, NULL)), _lastSelected(NULL), _texid(0),
...
void MappingFrame::setMapperFunction(MapperFunction *mapper) {
    ...
    if (_mapper && _opacityMappingEnabled) {
        ...
        for (int i = 0; i < _mapper->getNumOpacityMaps(); i++) {
            ...
            connect((QObject *)widget, SIGNAL(startChange(QString)), this,
                    SIGNAL(startChange(QString)));
        ...
        _compTypeSubMenu->actions()[MapperFunction::ADDITION]->setChecked(type ==
                                                                          MapperFunction::ADDITION);
        _compTypeSubMenu->actions()[MapperFunction::MULTIPLICATION]->setChecked(
...
int MappingFrame::histoValue(const QPoint &p) {
    ...
    if (_histogram->getMaxData() <= _histogram->getMinData())
        return 0;
    float ind =
        (x - _histogram->getMinData()) / (_histogram->getMaxData() - _histogram->getMinData());
...
void MappingFrame::initConnections() {
    connect(_addOpacityControlPointAction, SIGNAL(triggered()), this,
            SLOT(addOpacityControlPoint()));
    ...
    connect(_addOpacityWidgetSubMenu, SIGNAL(triggered(QAction *)), this,
            SLOT(addOpacityWidget(QAction *)));

    connect(_histogramScalingSubMenu, SIGNAL(triggered(QAction *)), this,
            SLOT(setHistogramScale(QAction *)));

    connect(_compTypeSubMenu, SIGNAL(triggered(QAction *)), this,
            SLOT(setCompositionType(QAction *)));

    connect(_widgetEnabledSubMenu, SIGNAL(triggered(QAction *)), this,
            SLOT(setWidgetEnabled(QAction *)));
...
void MappingFrame::paintGL() {
    ...
    if (!_mapper)
        return;
#ifdef DEAD
    if (_controlExec->GuiRenderBlocked())
        return;
#endif
    resize();
    ...
    if (_variableName != "") {
        ...
        qglColor(Qt::red);
        renderText(wx, _domainLabelHeight + 15, QString::fromStdString(_variableName),
                   QFont("Arial", 10, 5, false));
    }
...
void MappingFrame::updateTexture() {
    ...
    for (int x = 0; x < _NUM_BINS; x++) {
        ...
        case LINEAR: {
            binValue =
                MIN(1.0, (stretch * _histogram->getBinSize(x) / _histogram->getMaxBinSize()));
            break;
        }

        case LOG: {
            binValue =
                logf(stretch * _histogram->getBinSize(x)) / logf(_histogram->getMaxBinSize());
            break;
        }
...
void MappingFrame::select(int hits, GLuint *selectionBuffer, Qt::KeyboardModifiers state) {
    ...
    } else if ((int)selectionBuffer[hitOffset + 3] >
    ...
    if (_editMode && (_button == Qt::LeftButton || _button == Qt::MidButton)) {
        if (_lastSelected) {
            if (_lastSelected != _domainSlider) {
                ...
                } else if (dynamic_cast<IsoSlider *>(
                               _lastSelected)) // check if an isolineSlider was picked...
                {
...
void MappingFrame::mouseReleaseEvent(QMouseEvent *event) {
    ...
    if (_editMode && (_button == Qt::LeftButton || _button == Qt::MidButton)) {
        if (_lastSelected == _domainSlider) {
            ...
            setIsoSlider();
        } else if (dynamic_cast<IsoSlider *>(_lastSelected) &&
                   _lastSelectedIndex >= 0) // check if an isolineSlider was picked...
        {
...
void MappingFrame::mouseMoveEvent(QMouseEvent *event) {
    ...
    if (_editMode && _selectedWidgets.size()) {
        ...
        float startXMapped =
            ((float)_clickedPos.x() / (float)(width())) * (_maxValueStart - _minValueStart) +
            _minValueStart;
...
void MappingFrame::contextMenuEvent(QContextMenuEvent * /*event*/) {
    ...
    for (iter = _opacityWidgets.begin(); iter != _opacityWidgets.end(); iter++) {
        ...
        if (omap->GetType() == OpacityMap::CONTROL_POINT && x >= omap->minValue() &&
            x <= omap->maxValue()) {
...
Histo *MappingFrame::getHistogram() {
    RenderParams *params = GetActiveParams();
    return (_eventRouter->GetHistogram(params->IsEnabled(), _isoSliderEnabled));
}
...
void MappingFrame::editControlPoint() {
    ...
    if (opacWidget && opacWidget->controlPointSelected()) {
        ControlPointEditor editor(this, opacWidget->opacityMap(),
    ...
    //
        ...
        ControlPointEditor editor(this, _colorbarWidget->colormap(),
...
void MappingFrame::hookup(RenderEventRouter *evRouter, QPushButton *editBtn, QPushButton *zoomPan,
                          QPushButton *fitToView, QPushButton *histoButton,
                          QPushButton *fitDataButton, QPushButton *bindColorToOpac,
                          QPushButton *bindOpacToColor, QSlider *opacityScaleSlider) {
    _eventRouter = evRouter;
    navigateButton = zoomPan;
    _editButton = editBtn;
    connect(_editButton, SIGNAL(toggled(bool)), this, SLOT(setEditMode(bool)));
    connect(navigateButton, SIGNAL(toggled(bool)), this, SLOT(setNavigateMode(bool)));
    connect(fitToView, SIGNAL(clicked()), this, SLOT(fitToView()));
    connect(fitDataButton, SIGNAL(clicked()), this, SLOT(fitToData()));
    connect(histoButton, SIGNAL(clicked()), this, SLOT(refreshHisto()));
}
...
#ifdef DEAD
    if (rParams->doBypass(_controlExec->GetActiveAnimationParams()->GetCurrentTimestep())) {
...
#ifdef DEAD
    DataMgr *dataManager = _controlExec->GetDataMgr();
    if (dataManager) {
#endif
