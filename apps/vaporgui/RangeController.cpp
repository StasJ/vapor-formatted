...
RangeLabel::RangeLabel(Range *range, QLabel *label, int type) {
    _range = range;
    _label = label;
    _type = type;
}

MinMaxLabel::MinMaxLabel(Range *range, QLabel *label, int type) : RangeLabel(range, label, type) {
    _range = range;
    _label = label;
    _type = type;
    notify();
...
SizeLabel::SizeLabel(Range *range, QLabel *label, int type) : RangeLabel(range, label, type) {
    _range = range;
    _label = label;
    _type = type;
    notify();
...
MinMaxSlider::MinMaxSlider(Range *range, QSlider *slider, int type) : Controller(range, type) {
    _slider = slider;
...
void MinMaxSlider::updateValue() {
    ...
    if (pos == _increments)
        ...
        val = pos * (_max - _min) / _increments + _min;
...
SinglePointSlider::SinglePointSlider(Range *range, QSlider *slider, int defaultPos)
    : Controller(range) {
    _slider = slider;
    _increments = _slider->maximum() - _slider->minimum();
    _val = (_min + _max) / 2.f;
    ...
    case 0:
        ...
        break;
    case 1:
        position = _increments;
        break;
    case 2:
        position = _increments / 2;
        break;
    default:
        assert(0 && "SinglePointSlider must have a default position of 0, 1, or 2");
    }
...
SinglePointLineEdit::SinglePointLineEdit(Range *range, QLineEdit *lineEdit, double defaultVal)
    : Controller(range) {
    _lineEdit = lineEdit;
...
MinMaxLineEdit::MinMaxLineEdit(Range *range, QLineEdit *lineEdit, int type)
...
CenterSizeSlider::CenterSizeSlider(Range *range, QSlider *slider, int type)
    : Controller(range, type) {
    _slider = slider;
    _increments = _slider->maximum() - _slider->minimum();
...
CenterSizeLineEdit::CenterSizeLineEdit(Range *range, QLineEdit *lineEdit, int type)
    : Controller(range, type) {
    _lineEdit = lineEdit;
...
MinMaxTableCell::MinMaxTableCell(Range *range, QTableWidget *table, int type, int row, int col)
    : Controller(range, type) {
    _table = table;
