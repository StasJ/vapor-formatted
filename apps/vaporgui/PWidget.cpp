...
PWidget *PWidget::ShowBasedOnParam(const std::string &tag, const std::string &whenEqualTo,
                                   bool invert) {
    _showBasedOnParam =
        std::unique_ptr<ParamTester>(new ParamsTesterString(tag, whenEqualTo, invert));
    return this;
}

PWidget *PWidget::EnableBasedOnParam(const std::string &tag, int whenEqualTo) {
    _enableBasedOnParam =
        std::unique_ptr<ParamTester>(new ParamsTesterLong(tag, whenEqualTo, false));
    return this;
}
...
bool PWidget::ParamsTesterLong::_isEqual(VAPoR::ParamsBase *p) const {
    return p->GetValueLong(_tag, 0) == _value;
}
bool PWidget::ParamsTesterString::_isEqual(VAPoR::ParamsBase *p) const {
    return p->GetValueString(_tag, "") == _value;
}
