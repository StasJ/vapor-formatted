...
BoxSliderFrame::BoxSliderFrame(QWidget *parent) : QFrame(parent), Ui_boxframe() {
    setupUi(this);
    connect(xCenterEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(boxTextChanged(const QString &)));
    connect(yCenterEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(boxTextChanged(const QString &)));
    connect(zCenterEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(boxTextChanged(const QString &)));
    connect(xSizeEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(boxTextChanged(const QString &)));
    connect(ySizeEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(boxTextChanged(const QString &)));
    connect(zSizeEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(boxTextChanged(const QString &)));
...
void BoxSliderFrame::setFullDomain(const double ext[6]) {
    for (int i = 0; i < 6; i++)
        _domainExtents[i] = ext[i];
}

void BoxSliderFrame::setBoxExtents(const std::vector<double> &minexts,
...
void BoxSliderFrame::confirmText() {
    ...
    for (int i = 0; i < 3; i++) {
        if (sizes[i] > _domainExtents[i + 3] - _domainExtents[i])
            sizes[i] = _domainExtents[i + 3] - _domainExtents[i];
        if (sizes[i] < 0.0)
            sizes[i] = 0.0;
        if (i < 2 && centers[i] < _domainExtents[i])
            centers[i] = _domainExtents[i];
        if (centers[i] > _domainExtents[i + 3])
            centers[i] = _domainExtents[i + 3];
        if ((centers[i] + 0.5 * sizes[i]) > _domainExtents[i + 3])
            centers[i] = _domainExtents[i + 3] - 0.5 * sizes[i];
        if (i < 2 && (centers[i] - 0.5 * sizes[i]) < _domainExtents[i])
            centers[i] = _domainExtents[i] + 0.5 * sizes[i];
...
void BoxSliderFrame::xSliderCtrChange() {
    ...
    if ((center + 0.5 * (_boxExtents[3] - _boxExtents[0])) > _domainExtents[3])
        center = _domainExtents[3] - 0.5 * (_boxExtents[3] - _boxExtents[0]);
    if ((center - 0.5 * (_boxExtents[3] - _boxExtents[0])) < _domainExtents[0])
        center = _domainExtents[0] + 0.5 * (_boxExtents[3] - _boxExtents[0]);
    ...
    xCenterSlider->setValue(
        (int)(0.5 + 256 * (center - _domainExtents[0]) / (_domainExtents[3] - _domainExtents[0])));
    emit extentsChanged();
    _silenceSignals = false;
}
void BoxSliderFrame::xSliderSizeChange() {
    ...
    if ((center + 0.5 * (newSize)) > _domainExtents[3])
        center = _domainExtents[3] - 0.5 * (newSize);
    if ((center - 0.5 * (newSize)) < _domainExtents[0])
        center = _domainExtents[0] + 0.5 * (newSize);
...
void BoxSliderFrame::ySliderCtrChange() {
    ...
    if ((center + 0.5 * (_boxExtents[4] - _boxExtents[1])) > _domainExtents[4])
        center = _domainExtents[4] - 0.5 * (_boxExtents[4] - _boxExtents[1]);
    if ((center - 0.5 * (_boxExtents[4] - _boxExtents[1])) < _domainExtents[1])
        center = _domainExtents[1] + 0.5 * (_boxExtents[4] - _boxExtents[1]);
    ...
    yCenterSlider->setValue(
        (int)(0.5 + 256 * (center - _domainExtents[1]) / (_domainExtents[4] - _domainExtents[1])));
    emit extentsChanged();
    _silenceSignals = false;
}
void BoxSliderFrame::ySliderSizeChange() {
    ...
    if ((center + 0.5 * (newSize)) > _domainExtents[4])
        center = _domainExtents[4] - 0.5 * (newSize);
    if ((center - 0.5 * (newSize)) < _domainExtents[1])
        center = _domainExtents[1] + 0.5 * (newSize);
...
void BoxSliderFrame::zSliderCtrChange() {
    ...
    if ((center + 0.5 * (_boxExtents[5] - _boxExtents[2])) > _domainExtents[5])
        center = _domainExtents[5] - 0.5 * (_boxExtents[5] - _boxExtents[2]);
    if ((center - 0.5 * (_boxExtents[5] - _boxExtents[2])) < _domainExtents[2])
        center = _domainExtents[2] + 0.5 * (_boxExtents[5] - _boxExtents[2]);
    ...
    zCenterSlider->setValue(
        (int)(0.5 + 256 * (center - _domainExtents[2]) / (_domainExtents[5] - _domainExtents[2])));
    emit extentsChanged();
    _silenceSignals = false;
}
void BoxSliderFrame::zSliderSizeChange() {
    ...
    if ((center + 0.5 * (newSize)) > _domainExtents[5])
        center = _domainExtents[5] - 0.5 * (newSize);
    if ((center - 0.5 * (newSize)) < _domainExtents[2])
        center = _domainExtents[2] + 0.5 * (newSize);
...
void BoxSliderFrame::updateGuiValues(const double centers[3], const double sizes[3]) {
    ...
    for (int i = 0; i < 3; i++)
        _lastCenterSlider[i] = (int)(0.5 + 256 * (centers[i] - _domainExtents[i]) /
                                               (_domainExtents[3 + i] - _domainExtents[i]));
...
void BoxSliderFrame::nudgeCenter(string varname, int val, int dir) {
    ...
    if (_lastCenterSlider[dir] != newSliderPos) {
        _lastCenterSlider[dir] = newSliderPos;
    }
    return;
}
// nudge size
...
void BoxSliderFrame::nudgeSize(int val, int dir) {
    ...
    if (_lastSizeSlider[dir] != newSliderPos) {
        _lastSizeSlider[dir] = newSliderPos;
    }
    return;
}
