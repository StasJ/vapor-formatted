...
void TranslateStretchManip::transformMatrix(VAPoR::Transform *transform,
                                            std::vector<double> parentScales) {
...
void TranslateStretchManip::Update(std::vector<double> llc, std::vector<double> urc,
...
bool TranslateStretchManip::MouseEvent(int buttonNum, std::vector<double> vscreenCoords,
                                       double handleMidpoint[3], bool release) {
    ...
    if (_selectedHandle >= 0) {
        ...
        if (success) {
            ...
            if (_buttonNum == 3) { // Don't restrict movement if using the manip to drag
                ...
                int oppositeHandle =
                    (_selectedHandle < 3) ? (2 - _selectedHandle) : (_selectedHandle - 3);
                if (_selectedHandle < 3) {
                    if (_dragDistance + _selection[oppositeHandle] >
                        _selection[oppositeHandle + 3]) {
                        _dragDistance = _selection[oppositeHandle + 3] - _selection[oppositeHandle];
                    }
                } else { // Moving "high" handle:
                    if (_dragDistance + _selection[oppositeHandle + 3] <
                        _selection[oppositeHandle]) {
...
void TranslateStretchManip::_mousePress(double screenCoords[2], double handleMidpoint[3],
                                        int buttonNum) {
    double dirVec[3];
...
int TranslateStretchManip::_mouseIsOverHandle(const double screenCoords[2],
...
bool TranslateStretchManip::pointIsOnQuad(double cor1[3], double cor2[3], double cor3[3],
                                          double cor4[3], const double pickPt[2]) const {
    double winCoord1[2];
    double winCoord2[2];
    double winCoord3[2];
    double winCoord4[2];
...
int TranslateStretchManip::makeHandleFaces(int sortPosition, double handle[8][3], int octant,
    ...
    for (int vertex = 0; vertex < 8; vertex++) {
        for (int coord = 0; coord < 3; coord++) {
            ...
            double fltCoord = (((double)((vertex >> coord) & 1) - 0.5f) *
                               worldHandleDiameter); //_handleSizeInScene);
...
bool TranslateStretchManip::startHandleSlide(const double mouseCoords[2], int handleNum) {
    ...
    float vecNorm =
        sqrt(_handleProjVec[0] * _handleProjVec[0] + _handleProjVec[1] * _handleProjVec[1]);
...
//
bool TranslateStretchManip::pixelToVector(double winCoords[2], double dirVec[3],
                                          const double strHandleMid[3], double mouseWorldPos[3]) {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    ...
    bool success =
        (0 != gluUnProject((GLdouble)winCoords[0], (GLdouble)winCoords[1], screenz,
                           _modelViewMatrix, _projectionMatrix, viewport, pt, pt + 1, pt + 2));
...
void TranslateStretchManip::makeHandleExtents(int sortPosition, double handleExtents[6], int octant,
                                              double boxExtents[6]) {
    // Identify the axis this handle is on:
    int axis = (sortPosition < 3) ? (2 - sortPosition) : (sortPosition - 3);
    int newPosition = sortPosition;
    ...
    for (int coord = 0; coord < 3; coord++) {
        ...
        handleExtents[coord] =
            .5f * (-worldHandleDiameter + (boxExtents[coord + 3] + boxExtents[coord]));
        handleExtents[coord + 3] =
            .5f * (worldHandleDiameter + (boxExtents[coord + 3] + boxExtents[coord]));
...
void TranslateStretchManip::_captureMouseDown(int handleNum, int buttonNum,
...
void TranslateStretchManip::drawHandleConnector(int handleNum, double *handleExtents,
                                                double *boxExtents) {
    LegacyGL *lgl = _glManager->legacy;
    ...
    if ((handleNum == _selectedHandle) || (handleNum == (5 - _selectedHandle)))
        ...
        lgl->Color4fv(_unselectedFaceColor);
    lgl->Begin(GL_LINES);
    lgl->Vertex3f(0.5f * (handleExtents[MAXX] + handleExtents[MINX]) + handleDisp[X],
                  0.5f * (handleExtents[MAXY] + handleExtents[MINY]) + handleDisp[Y],
                  0.5f * (handleExtents[MAXZ] + handleExtents[MINZ]) + handleDisp[Z]);
    lgl->Vertex3f(0.5f * (boxExtents[MAXX] + boxExtents[MINX]) + boxDisp[X],
...
#ifdef DEBUG
void TranslateStretchManip::drawHitBox(double winCoord1[2], double winCoord2[2],
                                       double winCoord4[2], double winCoord3[2]) const {
    glMatrixMode(GL_PROJECTION);
    glPushMatrix();
    glLoadIdentity();
    glMatrixMode(GL_MODELVIEW);
...
//
bool TranslateStretchManip::_projectPointToWin(const double cubeCoords[3],
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    bool success =
        (0 != gluProject(cbCoords[0], cbCoords[1], cbCoords[2], _modelViewMatrix, _projectionMatrix,
