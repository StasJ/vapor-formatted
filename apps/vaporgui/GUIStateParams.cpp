...
static ParamsRegistrar<GUIStateParams::DataSetParam>
    registrar_dsp(GUIStateParams::DataSetParam::GetClassType());
...
GUIStateParams::GUIStateParams(ParamsBase::StateSave *ssave, XmlNode *node)
    ...
    if (node->HasChild(MouseModeParams::GetClassType())) {
        m_mouseModeParams =
            new MouseModeParams(ssave, node->GetChild(MouseModeParams::GetClassType()));
    } else {
        m_mouseModeParams = new MouseModeParams(ssave);
        m_mouseModeParams->GetNode()->SetParent(GetNode());
    }
    ...
    if (node->HasChild(ActiveRenderer::GetClassType())) {
        m_activeRenderer =
            new ActiveRenderer(ssave, node->GetChild(ActiveRenderer::GetClassType()));
    } else {
        m_activeRenderer = new ActiveRenderer(ssave);
        m_activeRenderer->GetNode()->SetParent(GetNode());
    }
...
//
void GUIStateParams::GetActiveRenderer(string vizWin, string &renderType,
                                       string &renderInst) const {
...
void GUIStateParams::SetCurrentSessionFile(string path) {
    SetValueString(m_sessionFileTag, "Set current session file path", path);
}
...
void GUIStateParams::InsertOpenDateSet(string dataSetName, string format,
...
void GUIStateParams::ActiveRenderer::SetActiveRenderer(string vizWin, string renderType,
                                                       string renderInst) {
    std::vector<string> v;
...
void GUIStateParams::ActiveRenderer::GetActiveRenderer(string vizWin, string &renderType,
                                                       string &renderInst) const {
...
bool GUIStateParams::IsFlowSeedTabActive() const {
    return GetValueLong(_isFlowSeedTabActiveTag, false);
}

void GUIStateParams::SetFlowSeedTabActive(bool b) {
    SetValueLong(_isFlowSeedTabActiveTag, _isFlowSeedTabActiveTag, b);
}
