...
HelloEventRouter::HelloEventRouter(QWidget *parent, ControlExec *ce)
    : QTabWidget(parent), RenderEventRouter(ce, HelloParams::GetClassType()) {
    ...
    // Create and add the appearance sub-tab.  This is a customized internal class of the
    // HelloEventRouter
...
void HelloEventRouter::hookUpTab() {
    ...
    connect(_appearance->thicknessEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setHelloTextChanged(const QString &)));
    ...
    connect(_geometry->xCoord1Edit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setHelloTextChanged(const QString &)));
    connect(_geometry->xCoord1Edit, SIGNAL(returnPressed()), this, SLOT(helloReturnPressed()));
    connect(_geometry->xCoord2Edit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setHelloTextChanged(const QString &)));
    connect(_geometry->xCoord2Edit, SIGNAL(returnPressed()), this, SLOT(helloReturnPressed()));
    connect(_geometry->yCoord1Edit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setHelloTextChanged(const QString &)));
    connect(_geometry->yCoord1Edit, SIGNAL(returnPressed()), this, SLOT(helloReturnPressed()));
    connect(_geometry->yCoord2Edit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setHelloTextChanged(const QString &)));
    connect(_geometry->yCoord2Edit, SIGNAL(returnPressed()), this, SLOT(helloReturnPressed()));
    connect(_geometry->zCoord1Edit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setHelloTextChanged(const QString &)));
    connect(_geometry->zCoord1Edit, SIGNAL(returnPressed()), this, SLOT(helloReturnPressed()));
    connect(_geometry->zCoord2Edit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setHelloTextChanged(const QString &)));
...
void HelloEventRouter::GetWebHelp(vector<pair<string, string>> &help) const {
    ...
    help.push_back(make_pair("Hello Overview",
                             "http://www.vapor.ucar.edu/docs/vapor-gui-help/hello#HelloOverview"));
    ...
    help.push_back(make_pair("Hello geometry options",
                             "http://www.vapor.ucar.edu/docs/vapor-gui-help/hello#HelloGeometry"));

    help.push_back(
        make_pair("Hello Appearance settings",
                  "<>"
                  "http://www.vapor.ucar.edu/docs/vapor-gui-help/hello#HelloAppearance"));
}

// The updateTab() method is invoked whenever it is necessary to display the current values in the
// tab.
...
void HelloEventRouter::_updateTab() {
    ...
    _appearance->Update(_controlExec->GetDataMgr(), _controlExec->GetParamsMgr(),
                        GetActiveParams());
...
void HelloEventRouter::HelloLayoutGUI::Update(VAPoR::DataMgr *dataMgr, VAPoR::ParamsMgr *paramsMgr,
...
void HelloEventRouter::HelloAppearanceGUI::Update(VAPoR::DataMgr *dataMgr,
