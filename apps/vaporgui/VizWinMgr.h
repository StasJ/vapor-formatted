...
class VizWinMgr : public QObject {
  ...
  public:
    ...
    //! Reset the GUI to its default state, either due to New Session, or in preparation for loading
    //! a session file.
    void SetToDefaults();

    //! Static helper method invoked during Undo and Redo visualizer creation and destruction, as
    //! well setting the current viz window. This function must be passed in Command::CaptureStart()
    //! \sa UndoRedoHelpCB_T
    ...
    VizWin *getActiveVizWin() {
        GUIStateParams *p = (GUIStateParams *)_controlExec->GetParamsMgr()->GetParams(
