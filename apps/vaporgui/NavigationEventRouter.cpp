...
NavigationEventRouter::NavigationEventRouter(QWidget *parent, ControlExec *ce)
    ...
    framebufferSection->layout()->addWidget(
        _useCustomFramebufferCheckbox = new ParamsWidgetCheckbox(
            ViewpointParams::UseCustomFramebufferTag, "Use Custom Output Size"));
    framebufferSection->layout()->addWidget(
        _customFramebufferWidth = new ParamsWidgetNumber(ViewpointParams::CustomFramebufferWidthTag,
                                                         "Output Width (px)"));
    framebufferSection->layout()->addWidget(
...
void NavigationEventRouter::hookUpTab() {
    ...
    connect(_projectionCombo, SIGNAL(activated(const QString &)), this,
            SLOT(projectionComboBoxChanged(const QString &)));
}
...
void NavigationEventRouter::_performAutoStretching(string dataSetName) {
    ...
    for (int i = 0; i < winNames.size(); i++) {
        ...
        if (fabs(maxRange) <= FLT_EPSILON)
            maxRange = 1.0;

        vector<double> scale(range.size(), 1.0);
        for (int i = 0; i < range.size(); i++) {
            if (range[i] < (maxRange / 10.0) && fabs(range[i]) > FLT_EPSILON) {
    ...
    for (int i = 0; i < winNames.size(); i++) {
        ...
        for (int j = 0; j < names.size(); j++) {
            ...
            if (!t->IsOriginInitialized()) {
                ...
                dataStatus->GetActiveExtents(paramsMgr, winNames[i], names[j], ts, minExts,
                                             maxExts);
...
void NavigationEventRouter::_updateTab() {
    ...
    _customFramebufferWidth->setEnabled(
        vp->GetValueLong(ViewpointParams::UseCustomFramebufferTag, 0));
    _customFramebufferHeight->setEnabled(
        vp->GetValueLong(ViewpointParams::UseCustomFramebufferTag, 0));
}
...
void NavigationEventRouter::_setViewpointParams(const double center[3], const double posvec[3],
...
bool NavigationEventRouter::_getViewpointParams(double center[3], double posvec[3],
                                                double dirvec[3], double upvec[3]) const {
