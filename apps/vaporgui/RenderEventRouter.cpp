...
void RenderEventRouter::RefreshHistogram() {
    ...
    int rc = dataStatus->getGrids(timeStep, varnames, minExts, maxExts, &actualRefLevel, &lod,
                                  &histoGrid);
    ...
    for (itr = rg_const->begin(); itr != rg_const->end(); ++itr) {
        v = *itr;
        if (v == histoGrid->GetMissingValue())
            continue;
...
void RenderEventRouter::CalcSliceHistogram(int ts, Histo *histo) {
    ...
    // in grid coordinate along each edge of the probe box.  Map each corner of the box to grid
    // coordinates at current refinement level: First map them to user coordinates, then convert
    // these user coordinates to grid coordinates.
    ...
    for (int cornum = 0; cornum < 8; cornum++) {
        ...
        // icor[k][dir] indicates the integer (data grid) coordinate of corner k along data grid
        // axis dir
        for (int i = 0; i < 3; i++) {
    ...
    // for each probe axis direction,  find the difference of each of the integer coordinates of the
    // probe, across the probe in that direction. Because the data is layered, try all 4 edges for
    // each direction.  The various edges are identified by cornum increasing by 4,1,or 2 starting at
    //(0,1,2,3), (0,2,4,6), (0,1,4,5)
    // Once the fastest varying coordinate is known, subdivide that axis to match the resolution of
    // that coordinate. difference of a coordinate across a probe-axis direction is determined by
    // the change in data-grid coordinates going from one face to the opposite face of the probe.
    ...
    for (int iz = 0; iz < gridRes[2]; iz++) {
        ...
        for (int iy = 0; iy < gridRes[1]; iy++) {
            ...
            for (int ix = 0; ix < gridRes[0]; ix++) {
                ...
                bool dataOK = true;
                for (int i = 0; i < 3; i++) {
                    if (dataCoord[i] < extExtents[i] || dataCoord[i] > extExtents[i + 3])
                        dataOK = false;
                ...
                if (dataOK) { // find the coordinate in the data array
                    ...
                    if (varVal == probeGrid->GetMissingValue())
                        dataOK = false;
                }
                if (dataOK) {
...
#ifdef DEAD
    if (rParams->GetMapperFunc())
        p->GetMapperFunc()->setParams(p);
    getMappingFrame()->setMapperFunction(p->GetMapperFunc());
    if (getColorbarFrame())
        getColorbarFrame()->setParams(p);
...
float RenderEventRouter::CalcCurrentValue(const double point[3]) {
    ...
    DataStatus *dataStatus = _controlExec->getDataStatus();
    int rc =
        dataStatus->getGrids(timeStep, varnames, minExts, maxExts, &numRefinements, &lod, &grid);
...
void RenderEventRouter::fileSaveTF() {
    ...
    QString s = QFileDialog::getSaveFileName(0, "Choose a filename to save the transfer function",
...
void RenderEventRouter::fileLoadTF(string varname, const char *startPath, bool savePath) {
    ...
    QString s = QFileDialog::getOpenFileName(0, "Choose a transfer function file to open",
                                             startPath, "Vapor 3 Transfer Functions (*.tf3)");
    ...
    if (savePath)
        PathParams::SetCurrentTFPath(s.toStdString());
