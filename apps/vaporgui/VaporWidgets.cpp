...
VSpinBox::VSpinBox(QWidget *parent, const std::string &labelText, int min, int max,
                   int defaultValue)
    : VaporLine(parent, labelText), _min(min), _max(max), _value(defaultValue) {
...
VSlider::VSlider(QWidget *parent, const std::string &label, double min, double max, double value)
    : VaporLine(parent, label), _min(min), _max(max), _value(value) {
    VAssert(_max > _min);
...
void VSlider::Update(double val) {
    ...
    if (val >= _min && val <= _max) {
        ...
        std::cout << "VSlider value rejected " << val << std::endl;
}
