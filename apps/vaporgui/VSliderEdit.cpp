...
VSliderEdit::VSliderEdit(double min, double max, double value)
    ...
    connect(_lineEdit, SIGNAL(ValueChanged(const std::string &)), this,
            SLOT(emitLineEditValueChanged(const std::string &)));

    connect(_slider, SIGNAL(ValueChanged(double)), this, SLOT(emitSliderValueChanged(double)));
}
...
void VSliderEdit::emitSliderValueChanged(double value) {
    SetValue(value);
    emit ValueChanged(_value);
    std::cout << "void VSliderEdit::emitSliderValueChanged( double value ) {}" << value
              << std::endl;
}
