...
StartupEventRouter::StartupEventRouter(QWidget *parent, ControlExec *ce)
    : QWidget(parent), Ui_startupTab(), EventRouter(ce, StartupParams::GetClassType()) {
...
void StartupEventRouter::hookUpTab() {
    connect(cacheSizeEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setStartupTextChanged(const QString &)));
    ...
    connect(textureSizeEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setStartupTextChanged(const QString &)));
    connect(winWidthEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setStartupTextChanged(const QString &)));
    connect(winWidthEdit, SIGNAL(returnPressed()), this, SLOT(startupReturnPressed()));
    connect(winHeightEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setStartupTextChanged(const QString &)));
    connect(winHeightEdit, SIGNAL(returnPressed()), this, SLOT(startupReturnPressed()));
    connect(sessionPathEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setStartupTextChanged(const QString &)));
    connect(sessionPathEdit, SIGNAL(returnPressed()), this, SLOT(startupReturnPressed()));
    connect(metadataPathEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setStartupTextChanged(const QString &)));
    connect(metadataPathEdit, SIGNAL(returnPressed()), this, SLOT(startupReturnPressed()));
    connect(imagePathEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setStartupTextChanged(const QString &)));
    connect(imagePathEdit, SIGNAL(returnPressed()), this, SLOT(startupReturnPressed()));
    connect(tfPathEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setStartupTextChanged(const QString &)));
    connect(tfPathEdit, SIGNAL(returnPressed()), this, SLOT(startupReturnPressed()));
    connect(flowPathEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setStartupTextChanged(const QString &)));
    connect(flowPathEdit, SIGNAL(returnPressed()), this, SLOT(startupReturnPressed()));
    connect(pythonPathEdit, SIGNAL(textChanged(const QString &)), this,
            SLOT(setStartupTextChanged(const QString &)));
...
void StartupEventRouter::GetWebHelp(vector<pair<string, string>> &help) const {
    ...
    help.push_back(
        make_pair("Overview of the Startup tab",
...
void StartupEventRouter::setStartupTextChanged(const QString &) { SetTextChanged(true); }
...
void StartupEventRouter::chooseSessionPath() {
    ...
    QString dir;
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (sParams->GetSessionDir() == ".")
        dir = QDir::currentPath();
    else
        dir = sParams->GetSessionDir().c_str();
    QString s = QFileDialog::getExistingDirectory(this, "Choose the session file directory", dir);
    if (s != "") {
        setSettingsChanged(true);
...
void StartupEventRouter::chooseMetadataPath() {
    ...
    QString dir;
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (sParams->GetMetadataDir() == ".")
        dir = QDir::currentPath();
    else
        dir = sParams->GetMetadataDir().c_str();
    QString s = QFileDialog::getExistingDirectory(this, "Choose the metadata file directory", dir);
    if (s != "") {
        setSettingsChanged(true);
...
void StartupEventRouter::chooseImagePath() {
    ...
    QString dir;
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (sParams->GetImageDir() == ".")
        dir = QDir::currentPath();
    else
        dir = sParams->GetImageDir().c_str();
    QString s = QFileDialog::getExistingDirectory(this, "Choose the image file directory", dir);
    if (s != "") {
        setSettingsChanged(true);
...
void StartupEventRouter::chooseTFPath() {
    ...
    if (sParams->GetTFDir() == ".")
        ...
        dir = sParams->GetTFDir().c_str();
    QString s =
        QFileDialog::getExistingDirectory(this, "Choose the transfer function file directory", dir);
    if (s != "") {
        setSettingsChanged(true);
...
void StartupEventRouter::chooseFlowPath() {
    ...
    QString dir;
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (sParams->GetFlowDir() == ".")
        dir = QDir::currentPath();
    else
        dir = sParams->GetFlowDir().c_str();
    QString s = QFileDialog::getExistingDirectory(this, "Choose the flow file directory", dir);
    if (s != "") {
        setSettingsChanged(true);
...
void StartupEventRouter::choosePythonPath() {
    ...
    QString dir;
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (sParams->GetPythonDir() == ".")
        dir = QDir::currentPath();
    else
        dir = sParams->GetPythonDir().c_str();
    QString s = QFileDialog::getExistingDirectory(this, "Choose the python file directory", dir);
    if (s != "") {
        setSettingsChanged(true);
...
void StartupEventRouter::copyLatestSession() {
    ...
    if (pos != string::npos)
        latestPath = latestPath.substr(0, pos);
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (latestPath == sParams->GetSessionDir())
        return;
    setSettingsChanged(true);
...
void StartupEventRouter::copyLatestMetadata() {
    ...
    if (pos != string::npos)
        latestPath = latestPath.substr(0, pos);
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (latestPath == sParams->GetMetadataDir())
        return;
    setSettingsChanged(true);
...
void StartupEventRouter::copyLatestTF() {
    ...
    if (pos != string::npos)
        latestPath = latestPath.substr(0, pos);
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (latestPath == sParams->GetTFDir())
        return;
    setSettingsChanged(true);
...
void StartupEventRouter::copyLatestImage() {
    ...
    if (pos != string::npos)
        latestPath = latestPath.substr(0, pos);
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (latestPath == sParams->GetImageDir())
        return;
    setSettingsChanged(true);
...
void StartupEventRouter::copyLatestFlow() {
    ...
    if (pos != string::npos)
        latestPath = latestPath.substr(0, pos);
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (latestPath == sParams->GetFlowDir())
        return;
    setSettingsChanged(true);
...
void StartupEventRouter::copyLatestPython() {
    ...
    if (pos != string::npos)
        latestPath = latestPath.substr(0, pos);
    StartupParams *sParams = (StartupParams *)GetActiveParams();
    if (latestPath == sParams->GetPythonDir())
        return;
    setSettingsChanged(true);
...
void StartupEventRouter::apply() {
    ...
    if (!rc)
        setSettingsChanged(false); // settings were saved to file
#endif
...
#ifdef DEAD
    if (topIndex != _topTabIndex)
        return;
    if (subIndex != _subTabIndex)
        return;
#endif
    ...
    if (_settingsChanged || _textChangedFlag) {
        ...
        if (ret == QMessageBox::Save) {
            ...
            StartupParams *sParams =
                (StartupParams *)_paramsMgr->GetDefaultParams(StartupParams::_startupParamsTag);
            assert(sParams);
...
void StartupEventRouter::setSettingsChanged(bool didchange) {
    ...
    if (didchange && !_settingsChanged) {
        StartupParams *sParams = (StartupParams *)GetActiveParams();
        _savedStartupParams =
            (StartupParams *)sParams->deepCopy(sParams->GetRootNode()->deepCopy());
    }
