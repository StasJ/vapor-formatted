...
ColorbarSettings::ColorbarSettings(QWidget *parent) : QFrame(parent), Ui_ColorbarFrame() {
    setupUi(this);
    _eventRouter = NULL;
    _showSettings = false;
...
void ColorbarSettings::Update(ParamsMgr *paramsMgr, DataMgr *dataMgr, RenderParams *rParams) {
    assert(paramsMgr);
    assert(dataMgr);
    assert(rParams);
...
// slots:
void ColorbarSettings::colorbarTextChanged(const QString &) { _textChangedFlag = true; }

void ColorbarSettings::colorbarReturnPressed() { confirmText(); }
