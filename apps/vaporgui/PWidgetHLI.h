...
//! \class PWidgetHLI
//! Creates the framework for creating Params Widgets that use the Params Database high level
//! interface.
...
#define CreateInferredTemplateConstructor(className, T, constModifier)                             \
    template <class P>                                                                             \
    className##HLI<P> *new_##className##HLI(                                                       \
        const std::string &label, T (P::*getter)() constModifier, void (P::*setter)(T)) {          \
        return new className##HLI<P>(label, getter, setter);                                       \
    }

#define CreateHLI(className, T)                                                                    \
    template <class P>                                                                             \
    class className##HLI final : public className, public PWidgetHLIBase<P, T> {                   \
      public:                                                                                      \
        className##HLI(const std::string &label, typename PWidgetHLIBase<P, T>::GetterType getter, \
                       typename PWidgetHLIBase<P, T>::SetterType setter)                           \
...
//
// My attempt at implementing this using purely function templates and no defines. Below is pretty
// close. It works but requires too much specilization code so I stopped working on it for now.
//

#ifdef SAVED_FOR_LATER
template <class Base, class Derived> void AssertInheritance() {
    (void)static_cast<Base *>((Derived *)0);
}
...
template <class PW, typename T, class P, typename... Args> class PWidgetHLI : public PW {
  ...
  public:
    PWidgetHLI(std::string label, Args... args, GetterType getter, SetterType setter)
        : PW("", args..., label) {
...
template <class PW, typename T, class P, typename... Args>
class PWidgetHLI2 : public PWidgetHLI<PW, T, P, Args...> {
...
template <class PW, typename T, class P, typename... Args>
typename std::enable_if<std::is_same<T, bool>::value, T>::type class PWidgetHLI2
