...
//!	The MappingFrame is a QGLWidget that can be used to map colors and/or opacity from a 1D
//!interval.
//! The MappingFrame can be inserted in the VAPOR tabs using QDesigner.  Several Signal/Slot
//! connections must be implemented to support editing functionality.
//!
...
class MappingFrame : public QGLWidget {
  ...
  public:
    ...
    //! \param[in] bindColorToOpac QPushButton that binds the selected color control point to
    //! selected opacity control point. \param[in] bindOpacToColor QPushButton that binds the
    //! selected opacity control point to the selected color control point. \param[in]
    //! opacityScaleSlider QSlider that controls the opacity scale
    void hookup(RenderEventRouter *evrouter, QPushButton *editButton, QPushButton *ZoomPan,
                QPushButton *fitToView, QPushButton *histoButton, QPushButton *fitData,
                QPushButton *bindColorToOpac, QPushButton *bindOpacToColor,
                QSlider *opacityScaleSlider);
    ...
    //! Update the display of the Transfer Function or IsoControl based on the current RenderParams
  ...
  signals:
    ...
    //! This should be connected to a slot in the RenderEventRouter that completes and saves the
    //! Command that is used for saving the edit changes in the Command queue.
    void endChange();
    //
