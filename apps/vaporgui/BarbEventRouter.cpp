...
void BarbEventRouter::GetWebHelp(vector<pair<string, string>> &help) const {
    ...
    help.push_back(
        make_pair("Barb Overview", "http://www.vapor.ucar.edu/docs/vapor-gui-help/BarbOverview"));
    ...
    help.push_back(make_pair("Barb geometry options",
...
#ifdef DEAD
    if (rParams->IsEnabled() && !rParams->UseSingleColor())
