...
OpacityWidget::OpacityWidget(MappingFrame *parent, OpacityMap *omap)
    : GLWidget(), _parent(parent), _opacityMap(omap), _minValue(0.0), _maxValue(1.0),
      _handleRadius(0.045), _handle(NULL) {
    _minY = -0.05;
    _maxY = 1.05;
...
void OpacityWidget::move(float dx, float dy, float) {
    ...
    if ((_selected == BOTTOM_LEFT || _selected == TOP_LEFT || _selected == BOTTOM_RIGHT ||
         _selected == TOP_RIGHT || _selected == BORDER) &&
...
void OpacityWidget::drag(float dx, float dy, float) {
    ...
    if ((_selected == BOTTOM_LEFT || _selected == TOP_LEFT) && left() + dx <= right() &&
        left() + dx >= 0.0) {
        _opacityMap->setMinValue(_opacityMap->minValue() + dx * scaling);
    }

    else if ((_selected == BOTTOM_RIGHT || _selected == TOP_RIGHT) && right() + dx >= left() &&
             right() + dx <= 1.0) {
        _opacityMap->setMaxValue(_opacityMap->maxValue() + dx * scaling);
    }
        ...
        if (_selected != NONE) {
            ...
            if (hasLeftSlider()) {
                glPushMatrix();
                {
                    ...
                    glRotatef(90, 1, 0, 0);
                    gluCylinder(_handle, _handleRadius / 3.0, _handleRadius / 3.0, 3 * length, 10,
                                2);
                    glPopName();
                }
                glPopMatrix();
