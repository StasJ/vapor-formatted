...
PDoubleInput::PDoubleInput(const std::string &tag, const std::string &label)
    : PLineItem(tag, label, _doubleInput = new VLineEdit_Deprecated) {
    _doubleInput->SetIsDouble(true);
    connect(_doubleInput, &VLineEdit_Deprecated::ValueChanged, this,
            &PDoubleInput::doubleInputValueChanged);
}
