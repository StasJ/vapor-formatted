...
VRange::VRange(double min, double max, const std::string &minLabel, const std::string &maxLabel)
    : VRange(nullptr, min, max, minLabel, maxLabel) {}

VRange::~VRange() {}

void VRange::SetSelection(double min, double max) {
    VAssert(min <= max);
    _minSlider->SetValue(min);
    _maxSlider->SetValue(max);
}

void VRange::GetSelection(double &min, double &max) const {
    min = _minSlider->GetValue();
    max = _maxSlider->GetValue();
}

void VRange::SetExtents(double extentsLow, double extentsHigh) {
