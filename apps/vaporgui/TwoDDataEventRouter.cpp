...
//
static RenderEventRouterRegistrar<TwoDDataEventRouter>
    registrar(TwoDDataEventRouter::GetClassType());

TwoDDataEventRouter::TwoDDataEventRouter(QWidget *parent, ControlExec *ce)
    : QTabWidget(parent), RenderEventRouter(ce, TwoDDataParams::GetClassType()) {
...
void TwoDDataEventRouter::GetWebHelp(vector<pair<string, string>> &help) const {
    ...
    help.push_back(make_pair("TwoD Overview",
    ...
    help.push_back(make_pair("TwoD geometry options",
                             "http://www.vapor.ucar.edu/docs/vapor-gui-help/twoD#TwoDGeometry"));

    help.push_back(make_pair("TwoD Appearance settings",
