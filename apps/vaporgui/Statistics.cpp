...
Statistics::Statistics(QWidget *parent) : QDialog(parent), Ui_StatsWindow() {
    ...
    MyGeometryWidget->Reinit((DimFlags)THREED, (VariableFlags)AUXILIARY);
...
bool Statistics::Update() {
    ...
    StatisticsParams *statsParams =
        dynamic_cast<StatisticsParams *>(_controlExec->GetParamsMgr()->GetAppRenderParams(
            currentDatasetName, StatisticsParams::GetClassType()));
    ...
    if (_validStats.currentDataSourceName != currentDatasetName ||
        !_validStats.HaveSameParams(statsParams)) {
        _validStats.currentDataSourceName = currentDatasetName;
...
void Statistics::_dataSourceChanged(int index) {
    ...
    StatisticsParams *statsParams =
        dynamic_cast<StatisticsParams *>(_controlExec->GetParamsMgr()->GetAppRenderParams(
            newDataSourceName, StatisticsParams::GetClassType()));
...
void Statistics::_updateButtonClicked() {
    ...
    for (int i = 0; i < _validStats.GetVariableCount(); i++) {
        ...
        if ((statsParams->GetMinEnabled() || statsParams->GetMaxEnabled() ||
             statsParams->GetMeanEnabled()) &&
            std::isnan(m3[2])) {
            _calc3M(varname);
            _updateStatsTable();
        }
...
void Statistics::_newVarChanged(int index) {
    ...
    if (!currentDmgr->VariableExists(statsParams->GetCurrentTimestep(), varName,
...
bool Statistics::ValidStats::HaveSameParams(const VAPoR::StatisticsParams *rhs) const {
    ...
    return (_variables == rhs->GetAuxVariableNames() && currentExtentMin == paramsMin &&
            currentExtentMax == paramsMax && currentTimeStep[0] == rhs->GetCurrentTimestep() &&
            currentTimeStep[1] == rhs->GetCurrentMaxTS() &&
            currentLOD == rhs->GetCompressionLevel() && currentRefLev == rhs->GetRefinementLevel());
}
...
bool Statistics::ValidStats::SetCurrentExtents(const std::vector<float> &min,
...
bool Statistics::ValidStats::SetCurrentExtents(const std::vector<double> &min,
