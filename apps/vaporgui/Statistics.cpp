...
Statistics::~Statistics() {
    ...
    if (_zMaxSlider)
        delete _zMaxSlider;
    if (_zMinLineEdit)
        delete _zMinLineEdit;
    if (_zMaxLineEdit)
        delete _zMaxLineEdit;
    if (_zCenterSlider)
        delete _zCenterSlider;
    if (_zCenterLineEdit)
        delete _zCenterLineEdit;
    if (_zSizeSlider)
        delete _zSizeSlider;
    if (_zSizeLineEdit)
        delete _zSizeLineEdit;
    if (_zSinglePointSlider)
        delete _zSinglePointSlider;
    if (_zSinglePointLineEdit)
        delete _zSinglePointLineEdit;
    ...
    if (_yMaxSlider)
        delete _yMaxSlider;
    if (_yMinLineEdit)
        delete _yMinLineEdit;
    if (_yMaxLineEdit)
        delete _yMaxLineEdit;
    if (_yCenterSlider)
        delete _yCenterSlider;
    if (_yCenterLineEdit)
        delete _yCenterLineEdit;
    if (_ySizeSlider)
        delete _ySizeSlider;
    if (_ySizeLineEdit)
        delete _ySizeLineEdit;
    if (_ySinglePointSlider)
        delete _ySinglePointSlider;
    if (_ySinglePointLineEdit)
        delete _ySinglePointLineEdit;
    if (_xRange)
        delete _xRange;
    ...
    if (_xMaxSlider)
        delete _xMaxSlider;
    if (_xMinLineEdit)
        delete _xMinLineEdit;
    if (_xMaxLineEdit)
        delete _xMaxLineEdit;
    if (_xCenterSlider)
        delete _xCenterSlider;
    if (_xCenterLineEdit)
        delete _xCenterLineEdit;
    if (_xSizeSlider)
        delete _xSizeSlider;
    if (_xSizeLineEdit)
        delete _xSizeLineEdit;
    if (_xSinglePointSlider)
        delete _xSinglePointSlider;
    if (_xSinglePointLineEdit)
        delete _xSinglePointLineEdit;
#endif
}
...
int Statistics::initialize() {
    ...
    connect(regionSelectorCombo, SIGNAL(currentIndexChanged(int)), this,
...
void Statistics::initRegion() {
    ...
    void Statistics::update() {
        ...
        _dm->GetEnclosingRegion(_minTS, uCoordMin, uCoordMax, _vCoordMin, _vCoordMax, _refLevel,
                                _cRatio);
    ...
    void Statistics::addCalculationToTable(string varName) {
        ...
        VariablesTable->setVerticalHeaderItem(
            rowCount, new QTableWidgetItem(QString::fromStdString(varName)));
    ...
    void Statistics::generateTableColumns() {
        ...
        if (_calculations & _MIN) {
            ...
            VariablesTable->setHorizontalHeaderItem(
                colCount, new QTableWidgetItem(QString::fromStdString("Min")));
        }
        if (_calculations & _MAX) {
            colCount = VariablesTable->columnCount();
            VariablesTable->insertColumn(colCount);
            VariablesTable->setHorizontalHeaderItem(
                colCount, new QTableWidgetItem(QString::fromStdString("Max")));
        }
        if (_calculations & _MEAN) {
            colCount = VariablesTable->columnCount();
            VariablesTable->insertColumn(colCount);
            VariablesTable->setHorizontalHeaderItem(
                colCount, new QTableWidgetItem(QString::fromStdString("Mean")));
        }
        if (_calculations & _MEDIAN) {
            colCount = VariablesTable->columnCount();
            VariablesTable->insertColumn(colCount);
            VariablesTable->setHorizontalHeaderItem(
                colCount, new QTableWidgetItem(QString::fromStdString("Median")));
        }
        if (_calculations & _SIGMA) {
            colCount = VariablesTable->columnCount();
            VariablesTable->insertColumn(colCount);
            VariablesTable->setHorizontalHeaderItem(
                colCount, new QTableWidgetItem(QString::fromStdString("StdDev")));
        }
    }
    ...
    void Statistics::exportText() {
        ...
        QString fName = QFileDialog::getSaveFileName(
            this, "Select file to write statistics into:", "", "*.txt");
        if (!fName.isEmpty()) {
            ofstream file;
    ...
    bool Statistics::calcMinMax(string varname) {
        ...
        for (int ts = _minTS; ts <= _maxTS; ts++) {
            ...
            if (ts == _minTS) {
                ...
                if ((range[0] < _stats[varname].min) && (range[0] != mv)) {
                    _stats[varname].min = range[0];
                }
                if ((range[1] > _stats[varname].max) && (range[1] != mv)) {
                    _stats[varname].max = range[1];
                }
            }
        }
