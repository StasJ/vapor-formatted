...
void Statistics::initRegion(bool activeRegion) {
    ...
    //
    if (((oldExtents[0] == _fullExtents[0]) && (oldExtents[1] == _fullExtents[1]) &&
         (oldExtents[2] == _fullExtents[2]) && (oldExtents[3] == _fullExtents[3]) &&
         (oldExtents[4] == _fullExtents[4]) && (oldExtents[5] == _fullExtents[5]))) {
        return;
...
void Statistics::addCalculationToTable(string varName) {
    ...
    VariablesTable->setVerticalHeaderItem(rowCount,
                                          new QTableWidgetItem(QString::fromStdString(varName)));
