...
//! The parameters in VAPOR GUI are displayed using a hierarchy of tabs.  The TabManager is a
//! QTabWidget that handles the arrangement of these tabs.  Generally this layout is automatically
//! set up and maintained by the MainForm and VizWinMgr classes, however users may occasionally need
//! to query the status of a tab.
//!
//! At the top level there are three "Top" tabs:  The \p Renderer, the \p Navigation, and the \p
//! Settings.
//!
//! When the \p Renderer tab is selected, the RenderHolder is displayed.  This allows creation,
//! deletion, enablement, and selection of the renderer that is to be edited.  The currently
//! selected renderer has its parameters displayed in a QStackedWidget that is managed by the
//! RenderHolder class.  There is one EventRouter instance whose contents are displayed as tabs in
//! the QStackedWidget.
//!
//! When the \p Navigation tab is selected, A QTabWidget is displayed, with one tab for each of the
//! "navigation" EventRouters: Viewpoint/Lights, Region, and Animation.
//!
//! When the \p Settings tab is selected, another QTabWidget is displayed, with one tab for User
//! Preferences, and another tab for Visualizer Features.
//!
...
class TabManager : public QTabWidget {
  ...
  public:
    ...
    //! Method identifies the Lower level QWidget of specified widget type.  The lower level widget
    //! is either a QTabWidget (for widgetType 1 or 2) or (for widgetType 0) the RenderHolder
    //! instance that manages Renderer tabs. \param[in] widType is either 0, 1 or 2, for
    //! RenderHolder, Navigation, or Settings. \return QWidget* is either a QTabWidget* or a
    //! RenderHolder*
    QWidget *GetSubTabWidget(int widType) { return _topWidgets[widType]; }

    //! During initialization, after all the EventRouters have been created (and identified via
    //! addWidget()), the installWidgets method must be called to set up the various widgets in each
    //! tab.
    void InstallWidgets();

    //! In order to display the parameters for the selected renderer, QWidget::show() is invoked for
    //! the selected EventRouter, and QWidget::hide() is invoked for all other renderer
    //! EventRouters.  This is performed in the RenderHolder class. \param[in] tag is the tag
    ...
    //! This method identifies the EventRouter that is currently being displayed.  This can be
    //! invoked to determine if it is necessary to Refresh the specified EventRouter \return
    //! EventRouter* current displayed EventRouter
    EventRouter *GetFrontEventRouter() {

        return VizWinMgr::getInstance()->GetEventRouter(
            _widgetTags[_currentTopTab][_currentFrontPage[_currentTopTab]]);
    }
