...
QPointF TFMapWidget::NDCToQPixel(float x, float y) const { return NDCToQPixel(vec2(x, y)); }
...
glm::vec2 TFMapWidget::PixelToNDC(const QPointF &p) const { return PixelToNDC(vec2(p.x(), p.y())); }

QRectF TFMapWidget::PaddedRect() const {
    return QRectF(PADDING, PADDING, width() - PADDING * 2, height() - PADDING * 2);
}

int TFMapWidget::GetPadding() const { return PADDING; }

int TFMapWidget::GetControlPointRadius() const { return CONTROL_POINT_RADIUS; }
