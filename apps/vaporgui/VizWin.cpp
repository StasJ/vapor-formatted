 ...
 *
 */
VizWin::VizWin(QWidget *parent, const QString &name, string winName, ControlExec *ce,
               Trackball *trackBall)
    : QGLWidget(parent) {
    ...
    _manip->Update(minExts, maxExts, minExts, maxExts, vcameraPos, vrotCenter, mv, proj,
                   windowSize);
}
...
// the result
//
void VizWin::getNearFarDist(const double posVec[3], const double dirVec[3], double &boxNear,
                            double &boxFar) const {
...
void VizWin::mousePressEvent(QMouseEvent *e) {
    ...
    if ((_buttonNum == 1) && ((e->modifiers() & (Qt::ControlModifier | Qt::MetaModifier)))) {
        _buttonNum = 0;
    }
    ...
    if (handle >= 0) {
        ...
        533             manip->captureMouseDown(handleNum,  vParams->getCameraPosLocal(), dirVec,
        buttonNum, _strHandleMid); 534             EventRouter* rep =
        VizWinMgr::getInstance()->getEventRouter(tag); 535 rep->captureMouseDown(buttonNum); 536
    ...
    if (handleNum >= 0 && manip->startHandleSlide(_visualizer, screenCoords, handleNum, rParams)) {
        ...
        if (OK) {
            ...
            _visualizer->pixelToVector(screenCoords, vParams->getStretchedCamPosLocal(), dirVec,
                                       _strHandleMid);
            // Remember which handle we hit, highlight it, save the intersection point.
            manip->captureMouseDown(handleNum, vParams->getCameraPosLocal(), dirVec, buttonNum,
                                    _strHandleMid);
...
void VizWin::mouseMoveEvent(QMouseEvent *e) {
    ...
    if (handleNum >= 0) {
        if (manip->projectPointToLine(mouseCoords, projMouseCoords)) {
            double dirVec[3];
            _visualizer->pixelToVector(projMouseCoords, vParams->getStretchedCamPosLocal(), dirVec,
                                       _strHandleMid);
            // qWarning("Sliding handle %d, direction %f %f %f", handleNum,
