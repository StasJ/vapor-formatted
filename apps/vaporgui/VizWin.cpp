...
// the result
//
void VizWin::_getNearFarDist(const double posVec[3], const double dirVec[3], double &boxNear,
                             double &boxFar) const {
...
void VizWin::_setUpProjMatrix() {
    ...
    GLState::Perspective(fov, w, nearDist, farDist);
...
void VizWin::_setMatrixFromModeParams() {
    ...
    for (int i = 0; i < 3; i++) {
        if (center[i] != _center[i] || posvec[i] != _posvec[i] || dirvec[i] != _dirvec[i] ||
            upvec[i] != _upvec[i]) {
            updateFromMouseMode = true;
        }
    }
    if (!updateFromMouseMode)
        return;
...
void VizWin::mousePressEvent(QMouseEvent *e) {
    ...
    if (modeName == MouseModeParams::GetRegionModeName()) {
        ...
        glMatrixMode(GL_PROJECTION);
        GLState::MatrixModeProjection(); // Begin setup sequence
        ...
        GLState::MatrixModeModelView();
        glPopMatrix();
        GLState::PopMatrix(); // End cleanup sequence
...
void VizWin::_mouseReleaseEventNavigate(QMouseEvent *e) {
    ...
    double m[16];
    glGetDoublev(GL_MODELVIEW_MATRIX, m); // TODO GLState
...
VAPoR::RenderParams *VizWin::_getRenderParams(string &className) {
    ...
    VAPoR::RenderParams *rParams =
        paramsMgr->GetRenderParams(_winName, dataSetName, className, inst);
...
void VizWin::_getCenterAndCamPos(std::vector<double> &rotationCenter,
...
void VizWin::updateManip(bool initialize) {
    ...
    _manip->Update(llc, urc, minExts, maxExts, cameraPosition, mv, proj, windowSize, rpTransform,
                   dmTransform, constrain);
