 ...
 *
 */
VizWin::VizWin(const QGLFormat &format, QWidget *parent, const QString &name, string winName,
               ControlExec *ce, Trackball *trackBall)
...
void VizWin::_setUpProjMatrix() {
    ...
    if (vParams->GetValueLong(ViewpointParams::UseCustomFramebufferTag, 0)) {
        ...
        if (width > maxSize) {
            width = maxSize;
            vParams->SetValueLong(ViewpointParams::CustomFramebufferWidthTag,
                                  ViewpointParams::CustomFramebufferWidthTag, width);
            MSG_ERR("Selected width is larger than your OpenGL implementation supports");
        }
        if (height > maxSize) {
            height = maxSize;
            vParams->SetValueLong(ViewpointParams::CustomFramebufferHeightTag,
                                  ViewpointParams::CustomFramebufferHeightTag, height);
...
void VizWin::_setNewExtents() {
    ...
    if (_manipFlowSeedFlag) {
        ...
        if (fp) {
            ...
            ParamsMgr *paramsMgr = _controlExec->GetParamsMgr();
            GUIStateParams *gp = dynamic_cast<GUIStateParams *>(
...
void VizWin::updateManip(bool initialize) {
    ...
    if (initialize || rParams == NULL) {
        ...
        GUIStateParams *gp = (GUIStateParams *)_controlExec->GetParamsMgr()->GetParams(
        ...
        if (_manipFlowSeedFlag) {
            ...
            if (b.size() == 4) {
                ...
                int lod = rParams->GetCompressionLevel();
                double defaultZ =
                    VAPoR::DataMgrUtils::Get2DRendererDefaultZ(dataMgr, ts, refLevel, lod);
