...
void MainForm::hookupSignals() {
    ...
    //
    connect(_windowSelector, SIGNAL(winActivated(const QString &)),
...
void MainForm::setWinActive(const QString &visualizerName) {
    ...
    std::vector<string> rendererNames = _controlExec->GetRenderClassNames(stdVisualizerName);
    ...
    bool found =
        _controlExec->RenderLookup(rendererName, stdVisualizerName, dataSetName, rendererType);

    if (!found)
        return;

    cout << "1   visName, dsName, renType, renName " << stdVisualizerName << " " << dataSetName
         << " " << rendererType << " " << rendererName << endl;
    RenderParams *rParams =
        _controlExec->GetRenderParams(stdVisualizerName, dataSetName, rendererType, rendererName);
    cout << "visName, dsName, renType, renName " << stdVisualizerName << " " << dataSetName << " "
         << rendererType << " " << rendererName << endl;
    if (!rParams) {
        cout << "returning..." << endl;
        return;
    ...
    //    string rendererName =
    _controlExec->ActivateRender(stdVisualizerName, dataSetName, rendererType, rendererName, onOff);
