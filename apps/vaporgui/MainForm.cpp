...
void MainForm::hookupSignals() {
    ...
    connect(_homeAction, SIGNAL(triggered()), _vizWinMgr, SLOT(home()));
    connect(_sethomeAction, SIGNAL(triggered()), _vizWinMgr, SLOT(sethome()));
    connect(_viewAllAction, SIGNAL(triggered()), _vizWinMgr, SLOT(viewAll()));
    connect(_viewRegionAction, SIGNAL(triggered()), _vizWinMgr, SLOT(viewRegion()));
    connect(alignViewCombo, SIGNAL(activated(int)), _vizWinMgr, SLOT(alignView(int)));
...
void MainForm::sessionOpen(QString qfileName) {
    ...
    if (qfileName == "") {
        ...
        qfileName =
            QFileDialog::getOpenFileName(this, "Choose a VAPOR session file to restore a session",
...
void MainForm::fileSaveAs() {
    ...
    QString fileName =
        QFileDialog::getSaveFileName(this, "Choose the fileName to save the current session",
                                     defaultPath.c_str(), "Vapor 3 Session Files (*.vs3)");
...
void MainForm::loadData(QString fileName) {
    ...
    if (fileName == "") {
        ...
        QMessageBox::StandardButton btn = QMessageBox::question(
            this, "VAPOR Warning", messageText, QMessageBox::Ok | QMessageBox::Cancel);
        ...
        fileName = QFileDialog::getOpenFileName(this, "Choose the Master data File to load",
                                                path.c_str(), "Vapor WASP files (*.*)");
    }
    ...
    string dataSetName;
    bool newDataSet =
        make_dataset_name(currentPaths, currentDataSets, fileName.toStdString(), dataSetName);
...
void MainForm::importData(const string &modelType) {
    ...
    QMessageBox::StandardButton btn = QMessageBox::question(this, "VAPOR Warning", messageText,
...
void MainForm::setNavigate(bool on) {
    ...
    if (on) {
        ...
        _modeStatusWidget = new QLabel("Navigation Mode:  Use left mouse to rotate or "
                                       "spin-animate, right to zoom, middle to translate",
                                       this);
