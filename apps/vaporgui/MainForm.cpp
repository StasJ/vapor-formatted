...
void MainForm::_initMembers() {
    ...
    _captureStartJpegAction = NULL;
    _captureStartPngAction = NULL;
    _captureStartTiffAction = NULL;

    _captureEndJpegAction = NULL;
    _captureEndPngAction = NULL;
    _captureEndTiffAction = NULL;
...
void MainForm::_createCaptureMenu() {
    ...
    _captureStartJpegAction = new QAction(this);
    _captureStartJpegAction->setText(tr("Begin image capture sequence "));
    _captureStartJpegAction->setToolTip(
        "Begin saving jpeg image files rendered in current active visualizer");

    _captureEndJpegAction = new QAction(this);
    _captureEndJpegAction->setText(tr("End image capture"));
    _captureEndJpegAction->setToolTip("End capture of image files in current active visualizer");
    _captureEndJpegAction->setEnabled(false);
    ...
    _captureMenu->addAction(_captureEndJpegAction);
    ...
    connect(_captureStartJpegAction, SIGNAL(triggered()), this, SLOT(startAnimCapture()));

    connect(_captureEndJpegAction, SIGNAL(triggered()), this, SLOT(endAnimCapture()));
}
...
void MainForm::sessionOpen(QString qfileName) {
    ...
    if (qfileName == "") {
        ...
        vector<string> files =
            myGetOpenFileNames("Choose a VAPOR session file to restore a session", dir,
...
void MainForm::modeChange(int newmode) {
    ...
    _modeStatusWidget = new QLabel(QString::fromStdString(modeName) +
                                       " Mode: To modify box in scene, grab handle with left mouse "
                                       "to translate, right mouse to stretch",
                                   this);
...
void MainForm::buildWebHelpMenus() {
    ...
    currURL =
        "http://www.vapor.ucar.edu/docs/vapor-how-guide/user-preferences#MessageLoggingPopups";
    currAction = new QAction(QString(currText), this);
    myqurl.setUrl(currURL);
    qv.setValue(myqurl);
    currAction->setData(qv);
    ...
    currURL =
        "http://www.vapor.ucar.edu/docs/vapor-documentation/changing-default-tabbed-panel-settings";
    currAction = new QAction(QString(currText), this);
    myqurl.setUrl(currURL);
    qv.setValue(myqurl);
    currAction->setData(qv);
...
void MainForm::_setProj4String(string proj4String) {
    ...
    for (int i = 0; i < dataSets.size(); i++) {
        ...
        (void)openDataHelper(dataSets[i], formatsMap[i], filesMap[i], options);
    }
...
void MainForm::installCLITools() {
    ...
    if (error == WINDOWS_SUCCESS) {
        ...
        if (error == WINDOWS_SUCCESS) {
            ...
            if (path.find(";" + home + ";") != std::string::npos)
                alreadyExists = true;
            else if ((index = path.find(";" + home)) != std::string::npos &&
                     index + home.length() + 1 == path.length())
                alreadyExists = true;
    ...
    if (error == WINDOWS_SUCCESS && errorClose == WINDOWS_SUCCESS) {
        ...
        string errString = "";
        if (error != WINDOWS_SUCCESS)
            errString += Windows_GetErrorString(error) + "\n";
        if (errorClose != WINDOWS_SUCCESS)
            errString += "CloseRegistry: " + Windows_GetErrorString(errorClose);
...
void MainForm::startAnimCapture() {
    ...
    _captureEndJpegAction->setEnabled(true);
    _captureStartJpegAction->setEnabled(false);
...
void MainForm::endAnimCapture() {
    ...
    _captureEndJpegAction->setEnabled(false);
    _captureStartJpegAction->setEnabled(true);
