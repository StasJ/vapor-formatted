...
MainForm::MainForm(vector<QString> files, QApplication *app, QWidget *parent)
    ...
    _paramsMgr->RegisterIntermediateStateChangeCB(
        std::bind(&MainForm::_intermediateStateChangedCB, this));
...
int MainForm::RenderAndExit(int start, int end, const std::string &baseFile, int width,
                            int height) {
    if (start == 0 && end == 0)
        end = INT_MAX;
    ...
    auto baseFileWithTS = FileUtils::RemoveExtension(baseFile) + "-" + to_string(start) + "." +
                          FileUtils::Extension(baseFile);
...
void MainForm::_fileSaveHelper(string path) {
    if (path.empty()) {
        ...
        QFileDialog fileDialog(this, "Save VAPOR session file", QString::fromStdString(dir),
                               QString("Vapor 3 Session Save file (*.vs3)"));
...
void MainForm::startAnimCapture(string baseFile, string defaultSuffix) {
    ...
    QString suffix = fileInfo.suffix();
    if (suffix != "" || suffix != "jpg" || suffix != "jpeg" || suffix != "tif" ||
        suffix != "tiff" || suffix != "png") {
    }
    if (suffix == "") {
