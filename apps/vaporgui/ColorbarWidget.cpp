...
ColorbarWidget::ColorbarWidget(MappingFrame *parent, ColorMap *colormap)
    : GLWidget(parent), _NUM_BINS(256), _parent(parent), _colormap(colormap), _texid(0),
      _texture(new unsigned char[_NUM_BINS * 4]), _updateTexture(true), _minValue(0.0),
      _maxValue(1.0) {
    _minX = -0.03;
    _maxX = 1.03;
    _minY = -0.3;
...
//----------------------------------------------------------------------------
float ColorbarWidget::width() { return right() - left(); }
...
// Render the colorbar
//----------------------------------------------------------------------------
int ColorbarWidget::paintGL() {
    int rc = printOpenGLErrorMsg("ColorbarWidget");
    if (rc < 0)
        return -1;
    ...
    rc = printOpenGLErrorMsg("ColorbarWidget");
    if (rc < 0)
        return -1;
    return 0;
}
...
void ColorbarWidget::updateTexture() {
    ...
    glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA, 256, 0, GL_RGBA, GL_UNSIGNED_BYTE, _texture);
...
void ColorbarWidget::select(int handle, Qt::KeyboardModifiers state) {
    ...
    if (_colormap) {
        ...
        QColor qcolor;
        qcolor.setHsv((int)(360 * color.hue()), (int)(255 * color.sat()), (int)(255 * color.val()));
...
//----------------------------------------------------------------------------
float ColorbarWidget::right() { return dataToWorld(_parent->maxDataValue()); }

//----------------------------------------------------------------------------
// Leftmost edge of colorbar in world coordinates
//----------------------------------------------------------------------------
float ColorbarWidget::left() { return dataToWorld(_parent->minDataValue()); }
...
float ColorbarWidget::dataToWorld(float x) {
    return (x - _parent->minDataValue()) / (_parent->maxDataValue() - _parent->minDataValue());
}
...
float ColorbarWidget::worldToData(float x) {
    return _parent->minDataValue() + (x * (_parent->maxDataValue() - _parent->minDataValue()));
}
