...
//	Messages are either Info (diagnostic), Warning (immediate information to user), Error (cannot
//perform) or Fatal (app must end).
//  MyBase::SetErrMsg() results in an Error message, MyBase::SetDiagMsg() results in an Info
//  message. MessageReporter::fatalMsg(), errorMsg(), warningMsg(), and infoMsg() can be invoked by
//  the GUI and produce the appropriate type of message.  All types of messages are written to the
//  Logfile (if it is enabled).  Fatal messages always result in a popup. warnings and errors may
//  result in a popup depending on whether or not they have been silenced. Whenever a message
//  (Error, or Warning) is posted it is compared with the silencedMessages list.  If it is found in
//  the list
//	then it is not displayed in a popup.  When a popup is displayed, it will include a button
//"silence message(s)".  Clicking that button
...
class MessageReporter : public QObject {
  ...
  public:
    ...
    enum messagePriority { Fatal = 3, Error = 2, Warning = 1, Info = 0 };
  ...
  protected:
    ...
    static void releaseMessageLock() { messageListMutex.unlock(); }
