...
QSinglePoint::QSinglePoint(QWidget *parent) : QWidget(parent), _ui(new Ui::QSinglePoint) {
    ...
    connect(_ui->_xSliderEdit, SIGNAL(valueChanged(double)), this,
            SLOT(_coordinateChanged(double)));
    connect(_ui->_ySliderEdit, SIGNAL(valueChanged(double)), this,
            SLOT(_coordinateChanged(double)));
    connect(_ui->_zSliderEdit, SIGNAL(valueChanged(double)), this,
            SLOT(_coordinateChanged(double)));
    connect(_ui->_tSliderEdit, SIGNAL(valueChanged(double)), this,
            SLOT(_coordinateChanged(double)));
}
...
void QSinglePoint::SetDimensionality(int dim) {
    ...
    if (dim >= 3)
        _ui->_zSliderEdit->setVisible(true);
    if (dim >= 4)
        _ui->_tSliderEdit->setVisible(true);
}
...
void QSinglePoint::GetCurrentPoint(std::vector<double> &point) {
    ...
    if (_dimensionality >= 3)
        point.push_back(_ui->_zSliderEdit->GetCurrentValue());
    if (_dimensionality >= 4)
        point.push_back(_ui->_tSliderEdit->GetCurrentValue());
}
...
void QSinglePoint::SetExtents(const std::vector<double> &min, const std::vector<double> &max) {
    ...
    if (_dimensionality >= 3)
        _ui->_zSliderEdit->SetExtents(min[2], max[2]);
    if (_dimensionality >= 4)
        _ui->_tSliderEdit->SetExtents(min[3], max[3]);
}
