...
class VolumeVariablesSubtab : public QWidget, public Ui_VolumeVariablesGUI {
  ...
  public:
    VolumeVariablesSubtab(QWidget *parent) {
        ...
        _ps->Add((new PDoubleSliderEdit("osp_density", "Density"))
                     ->SetRange(0, 100)
                     ->EnableDynamicUpdate());
        ...
        _ps->Add((new PDoubleInput("osp_volumeSamplingRate", "Volume Sample Rate Scalar"))
                     ->EnableBasedOnParam("osp_usePT", false));
        _ps->Add((new PDoubleSliderEdit("osp_light_intensity", "Light Intensity"))
                     ->SetRange(0, 10000)
                     ->EnableDynamicUpdate()
                     ->EnableBasedOnParam("osp_usePT"));
        _ps->Add((new PDoubleSliderEdit("osp_light_brightness", "Light Brightness"))
                     ->SetRange(0, 10)
                     ->EnableDynamicUpdate()
                     ->EnableBasedOnParam("osp_usePT"));
