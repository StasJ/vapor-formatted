...
string ErrorReporter::GetSystemInformation() {
    ...
    // struct utsname info;
    // uname(&info);
    // ret += "OS: " + string(info.sysname) + " " + string(info.release) + " " +
    // string(info.version) "\n"; ret += "Distro:\n"; char buffer[128]; std::shared_ptr<FILE>
    // pipe(popen("lsb_release", "r"), pclose); if (pipe) { 	while (!feof(pipe.get())) { 		if
    // (fgets(buffer, 128, pipe.get()) != std::nullptr) 			ret += buffer;
    // 	}
    // } else  {
