...
int SeedMe::initPython() { return 0; }
/*
...
void SeedMe::showFirstTimeDialog() {
    if (!_firstTime)
        _firstTime = new FirstTimeDialog(this);
...
void SeedMe::connectWidgets() {
    ...
    connect(QualityComboBox, SIGNAL(currentIndexChanged(int)), this,
            SLOT(qualityComboChanged(int)));
...
void SeedMe::selectOutFile() {
    ...
    QString file =
        fd->getSaveFileName(this, "Select Output File Name", defaultOpenLocation, tr("(*.mp4)"));
    delete fd;
...
void SeedMe::go() {
    ...
    rc = PyTuple_SetItem(pArgs, 4,
                         (!_apiKey.empty()) ? PyUnicode_FromString(_apiKey.c_str()) : Py_None);
    if (rc < 0) {
        errReport(pyErr());
        Py_DECREF(pArgs);
        return;
...
void SeedMe::selectImages() {
    ...
    QStringList files = QFileDialog::getOpenFileNames(this, "Select images", defaultOpenLocation,
...
void SeedMe::removeImages() {
    ...
    for (int i = 0; i < size; i++) {
        _images.erase(
            std::remove(_images.begin(), _images.end(), selection[i]->text().toStdString()),
            _images.end());
    }
