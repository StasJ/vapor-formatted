...
RenderHolder::RenderHolder(QWidget *parent, ControlExec *ce)
    ...
    connect(tableWidget, SIGNAL(itemChanged(QTableWidgetItem *)), this,
            SLOT(itemTextChange(QTableWidgetItem *)));

    connect(tableWidget, SIGNAL(itemClicked(QTableWidgetItem *)), this,
            SLOT(itemChangeHack(QTableWidgetItem *)));
...
void RenderHolder::newRenderer() {
    ...
    for (int i = 0; i < dataSetNames.size(); i++) {
        rDialog.dataMgrCombo->addItem(QString::fromStdString(dataSetNames[i]));
    }
...
void RenderHolder::checkboxChanged(int state) {
    ...
    //	cout << widget->parentWidget()->parentWidget()->parentWidget()->metaObject()->className() <<
    //"!!!" << endl;
...
void RenderHolder::getRow(int row, string &renderInst, string &renderClass, string &dataSetName,
                          bool &enabled) const {
    renderInst.clear();
    renderClass.clear();
    enabled = false;
    ...
    //	QCheckBox* checkbox =
    //(QCheckBox*)widget->findChild<QCheckBox*>();;//tableWidget->cellWidget(row,3);
    /*	if (checkbox != NULL){
            enabled = checkbox->checkState() == Qt::Checked;
        }
        else {
...
void RenderHolder::getRow(string &renderInst, string &renderClass, string &dataSetName,
                          bool &enabled) const {
