...
NewRendererDialog::NewRendererDialog(QWidget *parent, std::vector<string> rendererNames,
...
void NewRendererDialog::InitializeDataSources(VAPoR::DataStatus *dataStatus) {
    ...
    for (int i = 0; i < datasets.size(); i++) {
        dataMgrCombo->addItem(QString::fromStdString(datasets[i]));
    }
...
void NewRendererDialog::_showRelevantRenderers() {
    ...
    for (int i = 0; i < _buttons.size(); i++) {
        if ((has2D && _dim2DSupport[i]) || (has3D && _dim3DSupport[i])) {
            ...
            _buttons[i]->setToolTip(QString::fromStdString(
                "Dataset \"" + datasetName + "\" has no " + (has2D ? "3D" : "2D") + " data"));
        }
    }
...
RenderHolder::RenderHolder(QWidget *parent, ControlExec *ce, const vector<QWidget *> &widgets,
                           ...
                           const vector<bool> &dim2DSupport, const vector<bool> &dim3DSupport)
    ...
    _newRendererDialog = new NewRendererDialog(this, widgetNames, descriptions, iconPaths,
                                               smallIconPaths, dim2DSupport, dim3DSupport);
...
void RenderHolder::_makeConnections() {
    connect(_vaporTable, SIGNAL(cellClicked(int, int)), this,
            SLOT(_activeRendererChanged(int, int)));
...
void RenderHolder::_tableValueChanged(int row, int col) {
    ...
    int rc =
        _controlExec->ActivateRender(activeViz, dataSetName, rendererType, rendererName, state);
    if (rc < 0) {
...
void RenderHolder::_getRowInfo(int row, string &rendererName, string &rendererType,
