...
StatisticsParams::StatisticsParams(ParamsBase::StateSave *ssave)
    : ParamsBase(ssave, StatisticsParams::GetClassType()) {}

StatisticsParams::StatisticsParams(ParamsBase::StateSave *ssave, XmlNode *node)
...
void StatisticsParams::SetRegionSelection(int state) {
    SetValueDouble(_regionSelectTag, "State of statistics region selector", (double)state);
}
...
void StatisticsParams::SetMinExtents(vector<double> minExts) {
    SetValueDoubleVec(_minExtentsTag, "Minimum extents for statistics", minExts);
}
...
void StatisticsParams::SetMaxExtents(vector<double> maxExts) {
    SetValueDoubleVec(_maxExtentsTag, "Maximum extents for statistics", maxExts);
}
...
void StatisticsParams::SetCRatio(int cRatio) {
    SetValueDouble(_cRatioTag, "Compression ratio for statistics", cRatio);
}
...
void StatisticsParams::SetRefinement(int ref) {
    SetValueDouble(_refinementTag, "Refinement level for statistics", ref);
}
...
void StatisticsParams::SetVarNames(vector<string> varNames) {
    SetValueStringVec(_varsTag, "Variable names selected for statistics", varNames);
}
