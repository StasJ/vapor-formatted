...
void computeLMax(const float *buf1, const float *buf2, size_t nelements, double &lmax, double &min,
                 double &max, bool hasMissing, float mv) {
    lmax = 0.0;
    min = 0.0;
    max = 0.0;
    if (nelements < 1)
    ...
    size_t index = 0;
    while (hasMissing && buf1[index] == mv && index < nelements)
        index++;
...
int get_var(DC *dc, size_t ts, string varname, float *Buffer) {
...
template <class S, class T>
int process(S *dc1, const vector<string> &files1, T *dc2, const vector<string> &files2) {
...
int main(int argc, char **argv) {
    ...
    if (argc < 6 || opt.help) {
        cerr << "Usage: " << ProgName
             << " source_ftype secondary_ftype source_files... -- secondary_files... " << endl;
