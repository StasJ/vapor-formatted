...
/*
   This version differs from the previous ones in that this one does not actually calculate voronoi
   cells, nor use a kdtree. Instead, it find the nearest particle for each grid point brute force.
*/
...
{
    float dist2 = (p[0] - q[0]) * (p[0] - q[0]) + (p[1] - q[1]) * (p[1] - q[1]) +
                  (p[2] - q[2]) * (p[2] - q[2]);
...
#pragma omp parallel for
        for (long y = 0; y < GRIDY; y++) {
            ...
            if (y % 10 == 0) {
                ...
                std::cerr << "brute force retrieval line " << y << " takes "
                          << GetElapsedSeconds(&lineStart, &lineEnd) << " seconds." << std::endl;
            }
        }
        gettimeofday(&planeEnd, NULL);
        std::cerr << "brute force retrieval plane " << z << " takes "
                  << GetElapsedSeconds(&planeStart, &planeEnd) << " seconds." << std::endl;
    }
    gettimeofday(&end, NULL);
    std::cerr << "total brute force retrieval takes " << GetElapsedSeconds(&start, &end)
              << " seconds." << std::endl;
...
#pragma omp parallel for
    ...
    for (long int i = 0; i < nPtcToUse; i++)
        if (counter[i] == 0) {
            ...
            // for( int j = 0; j < 3; j++ )
            //    std::cerr << "Particle coord = " << particle[j] << ";  Grid coord = " << grid[j]
            //    << std::endl;
        }
