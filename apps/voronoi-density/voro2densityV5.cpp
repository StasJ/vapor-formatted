...
double GetElapsedSeconds(const struct timeval *begin, const struct timeval *end) {
...
int main(int argc, char **argv) {
    ...
    std::cerr << "kdtree construction takes " << GetElapsedSeconds(&start, &end) << " seconds."
              << std::endl;
...
#pragma omp parallel for
    for (INT z = 0; z < GRIDZ; z++) {
        ...
        std::cerr << "kdtree retrieval plane " << z << " takes "
                  << GetElapsedSeconds(&planeStart, &planeEnd) << " seconds." << std::endl;
#endif
    }
    gettimeofday(&end, NULL);
    std::cerr << "total kdtree retrieval takes " << GetElapsedSeconds(&start, &end) << " seconds."
              << std::endl;
...
#pragma omp parallel for
    ...
    for (INT i = 0; i < nPtcToUse; i++)
        if (counter[i] == 0) {
            emptyCellCount++;
            /*  This block of code distributes the entire weight of a particle to its closest grid
            point. long  grid[3]; grid[0] = std::lround( ptcBuf[i*3  ] / EXTX * (GRIDX-1) ); grid[1]
            = std::lround( ptcBuf[i*3+1] / EXTY * (GRIDY-1) ); grid[2] = std::lround( ptcBuf[i*3+2]
            / EXTZ * (GRIDZ-1) ); density[ grid[2] * GRIDX * GRIDY + grid[1] * GRIDY + grid[0] ]
            += 1.0f;
            */
            ...
            float dist[8] = {CalcDist2(ptc, g0), CalcDist2(ptc, g1), CalcDist2(ptc, g2),
                             CalcDist2(ptc, g3), CalcDist2(ptc, g4), CalcDist2(ptc, g5),
    ...
    fclose(f);
    gettimeofday(&end, NULL);
    std::cerr << "writing density fields takes " << GetElapsedSeconds(&start, &end) << " seconds."
              << std::endl;
