...
int DefFile(const WASP &wasp, NetCDFCpp &ncdf) {
    ...
    for (int i = 0; i < attnames.size(); i++) {
        ...
        if (attnames[i].find("WASP") != std::string::npos)
            continue;
...
int GetVarNames(const WASP &wasp, vector<string> &vars, vector<string> &copy_vars,
...
int DefVars(const WASP &wasp, const vector<string> &vars, NetCDFCpp &ncdf) {
    ...
    for (int i = 0; i < vars.size(); i++) {
        ...
        for (int j = 0; j < attnames.size(); j++) {
            ...
            if (attnames[j].find("WASP") != std::string::npos)
                continue;
