...
//
void get_compressed_dims(const vector<string> &dimnames, const vector<size_t> &dimlens,
                         vector<string> &cdimnames, vector<size_t> &cdimlens) {
...
vector<size_t> get_cratios(const vector<string> &dimnames, const vector<size_t> &dims) {
    ...
    if (cdims.size() == 2 && opt.cratios2d.size())
        return (opt.cratios2d);
...
int GetVarNames(const NetCDFCpp &ncdf, vector<string> &vars, vector<string> &copy_vars,
                vector<string> &compress_vars) {
    vars.clear();
    copy_vars.clear();
    compress_vars.clear();
...
// in 'ncdf'
//
int DefVars(const NetCDFCpp &ncdf, const vector<string> &vars, const vector<string> &copy_vars,
            const vector<string> &comp_vars, WASP &wasp) {
    int rc;
    for (int i = 0; i < vars.size(); i++) {
