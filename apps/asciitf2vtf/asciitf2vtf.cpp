...
void Usage(OptionParser &op, const char *msg) {
    ...
    cerr << "Usage: " << ProgName << " [options] (-cmap cmap.txt | -omap omap.txt) file.tf3"
         << endl;
...
int ProcessNCLCMAP(TransferFunctionLite *transFunct) {
    ...
    for (int i = 0; ((i < numColors) && !nclFile.eof()); i++) {
        ...
        if (strlen(readLine) > 0) {
            ...
            if ((abs((comp_vals[2] - hsv[2]) / (i - last_i) - (last_hsv[2] - hsv[2])) >=
                 SLOPE_THRESHOLD) ||
                (abs((comp_vals[1] - hsv[1]) / (i - last_i) - (last_hsv[1] - hsv[1])) >=
                 SLOPE_THRESHOLD)) {
                slope_flag = true;
            }
