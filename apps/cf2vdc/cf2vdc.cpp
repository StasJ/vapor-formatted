...
OptionParser::Option_T get_options[] = {
    ...
    {"xvars", Wasp::CvtToStrVec, &opt.xvars, sizeof(opt.xvars)},
...
int copyVarHelper(DC &dc, VDC &vdc, int fdr, int fdw, vector<size_t> &buffer_dims,
...
int CopyVar2d3dMask(DC &dc, VDC &vdc, size_t ts, string varname, int lod) {
    ...
    //
    if (maskvar.empty() || vdc.VariableExists(ts, maskvar, 0, lod))
        return (0);
    ...
    rc = copyVarHelper(dc, vdc, fdr, fdw, buffer_dims, src_hslice_dims, dst_hslice_dims, src_nslice,
                       dst_nslice, mv, buffer);
