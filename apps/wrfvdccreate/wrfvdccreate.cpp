...
int main(int argc, char **argv) {
    ...
    for (int d = 0; d < 4; d++) {
        ...
        for (int i = 0; i < coordnames.size(); i++) {
            ...
            if (cvar.GetUniform()) {
                rc = vdc.DefineCoordVarUniform(cvar.GetName(), dimnames, time_dim_name,
                                               cvar.GetUnits(), cvar.GetAxis(), cvar.GetXType(),
                                               false);
            } else {
                rc = vdc.DefineCoordVar(cvar.GetName(), dimnames, time_dim_name, cvar.GetUnits(),
    ...
    for (int d = 0; d < 4; d++) {
        ...
        for (int i = 0; i < datanames.size(); i++) {
            ...
            rc = vdc.DefineDataVar(dvar.GetName(), dimnames, coordvars, dvar.GetUnits(),
