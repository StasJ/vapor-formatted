...
void process_volume(size_t ts, string varname, int level, int lod, VDCNetCDF &vdc, FILE *fp,
                    vector<size_t> dims, string type) {
...
void process_region(size_t ts, string varname, int level, int lod, VDCNetCDF &vdc, FILE *fp,
                    vector<size_t> dims, string type, const vector<int> &xregion,
                    const vector<int> &yregion, const vector<int> &zregion) {
...
int main(int argc, char **argv) {
    ...
    if (opt.xregion[0] == -1 && opt.xregion[1] == -1 && opt.yregion[0] == -1 &&
        opt.yregion[1] == -1 && opt.zregion[0] == -1 && opt.zregion[1] == -1) {
        ...
        process_region(opt.ts, opt.varname, opt.level, opt.lod, vdc, fp, dims, opt.type,
                       opt.xregion, opt.yregion, opt.zregion);
