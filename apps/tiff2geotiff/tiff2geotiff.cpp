...
int main(int argc, char *argv[]) {
    ...
    while ((c = getopt(argc, argv, "c:f:l:m:M:n:o:p:r:w:e:g:4:aistd")) != -1)
        ...
        {
            int retval = sscanf(myoptarg, "%f %f %f %f", lonLatExts, lonLatExts + 1, lonLatExts + 2,
                                lonLatExts + 3);
            if (retval != 4) {
...
static void InstallGeoTIFF(TIFF *out) {
    ...
    if (geofile) {
        ...
        if (timeLonLatFile) {
            ...
            int rc = fscanf(timeLonLatFile, "%19s %f %f %f %f %f %f %f %f", timestamp, lonlat,
                            lonlat + 1, lonlat + 2, lonlat + 3, relPos, relPos + 1, relPos + 2,
                            relPos + 3);
            dirnum++;
            if (rc != 9) {
                ...
                if (rc == 0)
                    fprintf(stderr,
                            "time-lon-lat file has fewer entries than images in tiff file\n");
                exit(-3);
        ...
        if (rc != 1) {
            ...
            if (rc == 0)
                fprintf(stderr, "timestamp file has fewer entries than images in tiff file\n");
            exit(-3);
...
static int applyCorners(float lonlat[4], float relPos[4], TIFF *out) {
    ...
    if (p) {
        ...
        if (pj_is_latlong(static_cast<projPJ>(p))) {
            ...
            // convert to radians...
            for (int j = 0; j < 4; j++)
                dbextents[j] = lonlat[j] * DEG2RAD;
            // convert the latlons to coordinates in the projection.
            double dummy[1] = {0.0};
            int rc = pj_transform(latlon_p, static_cast<projPJ>(p), 2, 2, dbextents, dbextents + 1,
                                  dummy);
            if (rc && !ignore) {
                int *pjerrnum = pj_get_errno_ref();
        ...
        newDBExtents[0] =
            (dbextents[0] * relPos[2] - dbextents[2] * relPos[0]) / (relPos[2] - relPos[0]);
        newDBExtents[2] = newDBExtents[0] + (dbextents[2] - dbextents[0]) / (relPos[2] - relPos[0]);
        newDBExtents[1] =
            (dbextents[1] * relPos[3] - dbextents[3] * relPos[1]) / (relPos[3] - relPos[1]);
        ...
        modelPixelScale[1] =
            (newDBExtents[3] - newDBExtents[1]) / ((double)currentImageHeight - 1.);
...
#define DECLAREcpFunc(x)                                                                           \
    static int x(TIFF *in, TIFF *out, uint32 imagelength, uint32 imagewidth, tsample_t spp)

#define DECLAREreadFunc(x)                                                                         \
    static void x(TIFF *in, unsigned char *buf, uint32 imagelength, uint32 imagewidth,             \
                  tsample_t spp)
typedef void (*readFunc)(TIFF *, unsigned char *, uint32, uint32, tsample_t);

#define DECLAREwriteFunc(x)                                                                        \
    static int x(TIFF *out, unsigned char *buf, uint32 imagelength, uint32 imagewidth,             \
                 tsample_t spp)
...
DECLAREcpFunc(cpContig2ContigByRow_8_to_4) {
    ...
    for (row = 0; row < imagelength; row++) {
        ...
        for (i_in = 0, i_out_byte = 0; i_in < (int)imagewidth; i_in += 2, i_out_byte++) {
            buf_out[i_out_byte] = (buf_in[i_in] & 0xf) * 16 + (buf_in[i_in + 1] & 0xf);
        }
...
DECLAREcpFunc(cpDecodedStrips) {
    ...
    if (buf) {
        ...
        uint32 row = 0;
        for (s = 0; s < ns; s++) {
            tsize_t cc = (row + rowsperstrip > imagelength) ? TIFFVStripSize(in, imagelength - row)
                                                            : stripsize;
...
static void cpStripToTile(unsigned char *out, unsigned char *in, uint32 rows, uint32 cols,
...
static void cpContigBufToSeparateBuf(unsigned char *out, unsigned char *in, uint32 rows,
                                     uint32 cols, int outskew, int inskew, tsample_t spp) {
    while (rows-- > 0) {
        uint32 j = cols;
        while (j-- > 0)
            *out++ = *in, in += spp;
...
static void cpSeparateBufToContigBuf(unsigned char *out, unsigned char *in, uint32 rows,
                                     uint32 cols, int outskew, int inskew, tsample_t spp) {
    while (rows-- > 0) {
        uint32 j = cols;
        while (j-- > 0)
...
static int cpImage(TIFF *in, TIFF *out, readFunc fin, writeFunc fout, uint32 imagelength,
        ...
        for (col = 0; col < imagewidth; col += tw) {
            ...
            for (s = 0; s < spp; s++) {
                ...
                if (colb + tilew > imagew) {
                    ...
                    cpSeparateBufToContigBuf(bufp + colb + s, tilebuf, nrow, width, oskew + iskew,
                                             oskew, spp);
                } else
        ...
        for (col = 0; col < imagewidth; col += tw) {
            ...
            for (s = 0; s < spp; s++) {
                ...
                if (colb + tilew > imagew) {
                    ...
                    cpContigBufToSeparateBuf(obuf, bufp + colb + s, nrow, width, oskew / spp,
                                             oskew + imagew, spp);
                } else
...
DECLAREcpFunc(cpContigStrips2ContigTiles) {
    return cpImage(in, out, readContigStripsIntoBuffer, writeBufferToContigTiles, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpContigStrips2SeparateTiles) {
    return cpImage(in, out, readContigStripsIntoBuffer, writeBufferToSeparateTiles, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpSeparateStrips2ContigTiles) {
    return cpImage(in, out, readSeparateStripsIntoBuffer, writeBufferToContigTiles, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpSeparateStrips2SeparateTiles) {
    return cpImage(in, out, readSeparateStripsIntoBuffer, writeBufferToSeparateTiles, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpContigTiles2ContigTiles) {
    return cpImage(in, out, readContigTilesIntoBuffer, writeBufferToContigTiles, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpContigTiles2SeparateTiles) {
    return cpImage(in, out, readContigTilesIntoBuffer, writeBufferToSeparateTiles, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpSeparateTiles2ContigTiles) {
    return cpImage(in, out, readSeparateTilesIntoBuffer, writeBufferToContigTiles, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpSeparateTiles2SeparateTiles) {
    return cpImage(in, out, readSeparateTilesIntoBuffer, writeBufferToSeparateTiles, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpContigTiles2ContigStrips) {
    return cpImage(in, out, readContigTilesIntoBuffer, writeBufferToContigStrips, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpContigTiles2SeparateStrips) {
    return cpImage(in, out, readContigTilesIntoBuffer, writeBufferToSeparateStrips, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpSeparateTiles2ContigStrips) {
    return cpImage(in, out, readSeparateTilesIntoBuffer, writeBufferToContigStrips, imagelength,
                   imagewidth, spp);
}
...
DECLAREcpFunc(cpSeparateTiles2SeparateStrips) {
    return cpImage(in, out, readSeparateTilesIntoBuffer, writeBufferToSeparateStrips, imagelength,
                   imagewidth, spp);
}
 ...
 */
static copyFunc pickCopyFunc(TIFF *in, TIFF *out, uint16 bitspersample, uint16 samplesperpixel) {
    uint16 shortv;
    uint32 w, l, tw, tl;
    int bychunk;
