...
class OutFormat {
  ...
  public:
    ...
    template <typename T>
    void vectorProperty(string owner, string name, vector<T> array, bool reversed) {
        pre();
    ...
    static string GetAxisString(int axis) {
        ...
        case 3:
            return "Time";
        default:
            return std::to_string((long long int)axis);
        }
    }
...
struct CoordVariable : public Variable {
    ...
    void printChild(OutFormat *out, Verbosity V, bool reversedCoordOrder) {
        out->stringProperty(_name, "Axis",
                            OutFormat::GetAxisString(((DC::CoordVar *)_data)->GetAxis()));
...
int main(int argc, char **argv) {
    ...
    VDCNetCDF vdc(1);
    int rc = vdc.Initialize(string(argv[1]), vector<string>(), VDC::R, vector<size_t>(),
                            4 * 1024 * 1024);
    if (rc < 0)
        exit(1);
    ...
    for (int i = 0; i < variables.size(); i++)
        variables[i]->print(out, opt.verbose, opt.ncOrder);

    out->header("Coordinate Variables");
    for (int i = 0; i < coordinates.size(); i++)
        coordinates[i]->print(out, opt.verbose, opt.ncOrder);
