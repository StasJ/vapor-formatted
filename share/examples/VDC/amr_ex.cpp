...
//
const char *ProgName;
void process_variable(AMRIO *amrio, AMRTree *tree, AMRData *amrdata, float *grid,
                      const char *varname) {
            ...
            for (int j = 0; j < cell_dim[1]; j++) {
                for (int i = 0; i < cell_dim[0]; i++) {
                    ...
                    block[k * cell_dim[0] * cell_dim[1] + j * cell_dim[0] + i] =
                        resample_grid_point(grid, startx + i * deltax, starty + j * deltay,
                                            startz + k * deltaz);
                }
            }
        }
...
int main(int argc, char **argv) {
    ...
    if (opt.debug) {
        ...
        while ((cellid = tree->GetNextCell(first)) >= 0) {
            ...
            xyz_b[1] = xyz[1] >> level;
            xyz_b[2] = xyz[2] >> level;
            cerr << "level, id, location base_location " << level << " (" << baseblockidx << " "
                 << nodeidx << ") (" << xyz[0] << " " << xyz[1] << " " << xyz[2] << ") ("
                 << xyz_b[0] << " " << xyz_b[1] << " " << xyz_b[2] << ")" << endl;
        }
    }
